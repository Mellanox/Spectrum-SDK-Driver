...
#define SXD_KERNEL_PREFIX "sxd_kernel: "
#define sxd_log(verbosity, prefix, fmt, arg ...) printk(verbosity SXD_KERNEL_PREFIX prefix fmt, ## arg)
#define sxd_log_ratelimit(log)    \
    do {                          \
        if (printk_ratelimit()) { \
            log;                  \
        }                         \
...
#define sxd_log_err(fmt, arg ...)     sxd_log(KERN_ERR, "[error] ", fmt, ## arg) /*KERN_EMRG KERN_ALERT and KERN_CRIT should use sxd_log_err  */
#define sxd_log_warning(fmt, arg ...) sxd_log(KERN_WARNING, "", fmt, ## arg)
#define sxd_log_notice(fmt, arg ...)  sxd_log(KERN_NOTICE, "", fmt, ## arg)
#define sxd_log_info(fmt, arg ...)    sxd_log(KERN_INFO, "", fmt, ## arg)
#define sxd_log_debug(fmt, arg ...)   pr_debug(SXD_KERNEL_PREFIX fmt, ## arg)

#define sxd_log_rl_err(format, arg ...)     sxd_log_ratelimit(sxd_log_err(format, ## arg))
#define sxd_log_rl_warning(format, arg ...) sxd_log_ratelimit(sxd_log_warning(format, ## arg))
#define sxd_log_rl_notice(format, arg ...)  sxd_log_ratelimit(sxd_log_notice(format, ## arg))
#define sxd_log_rl_info(format, arg ...)    sxd_log_ratelimit(sxd_log_info(format, ## arg))
#define sxd_log_rl_debug(format, arg ...)   sxd_log_ratelimit(sxd_log_debug(format, ## arg))

#define sxd_log_pci_err(pdev, fmt, arg ...)     sxd_log_err("[%s] " fmt, dev_name(&(pdev)->dev), ## arg)
#define sxd_log_pci_warning(pdev, fmt, arg ...) sxd_log_warning("[%s] " fmt, dev_name(&(pdev)->dev), ## arg)
#define sxd_log_pci_notice(pdev, fmt, arg ...)  sxd_log_notice("[%s] " fmt, dev_name(&(pdev)->dev), ## arg)
#define sxd_log_pci_info(pdev, fmt, arg ...)    sxd_log_info("[%s] " fmt, dev_name(&(pdev)->dev), ## arg)
#define sxd_log_pci_debug(pdev, fmt, arg ...)   sxd_log_debug("[%s] " fmt, dev_name(&(pdev)->dev), ## arg)
...
struct sxd_emad_mcion_reg {
    ...
    u8                      slot_index;
    __be16                  reserved2;
    __be16                  module_status_bits;
    __be32                  reserved3;
    __be16                  module_status_bits_valid;
    u8                      reserved4;
    u8                      module_inputs;
    __be16                  reserved5;
    u8                      reserved6;
...
/** Rare case: for PRM register fields with little-endian format. */
#define SX_CORE_BITMAP_LE_GET(BITMAP, LEN, WID, BIT) \
    ((BITMAP[((LEN)-1) - (((int)(BIT)) / ((int)(WID)))]) & (1 << (((int)(BIT)) % ((int)(WID)))))
