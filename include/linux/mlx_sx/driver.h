...
enum sx_dev_event {
    ...
    SX_DEV_EVENT_MODULE_SYSFS_CREATE,
    SX_DEV_EVENT_POST_SET_PROFILE,
};
...
#define NETDEV_PREFIX            "sx_netdev"
...
/* Increase it to 130 to be compatible with some customized boards with 128 label ports + 2 service ports, i.e., IEI board. */
#define MAX_MODULE_NUM          130
#define MAX_LANE_NUM_PER_MODULE 8

#define PMPE_MODULE_PLUGGED(status)                          \
    ((status == SXD_PMPE_MODULE_STATUS_PLUGGED_ENABLED_E) || \
     (status == SXD_PMPE_MODULE_STATUS_PLUGGED_DISABLED_E))

#define PMPE_MODULE_PLUGGED_ERROR(status) \
    (status == SXD_PMPE_MODULE_STATUS_MODULE_PLUGGED_ERROR_E)
...
enum sx_module_power_mode_policy {
    ...
    SX_MODULE_POWER_MODE_POLICY_LOW,
};
...
struct sx_module_status_params {
    u8 status;
    u8 error_type;
};

/**
 * enum sx_module_present_state - module present state
 * @SX_MODULE_PRESENT_INVALID:   Module default base state.
 * @SX_MODULE_PRESENT_PLUGGED:       Module plugged.
 * @SX_MODULE_PRESENT_UNPLUGGED:     Module unplugged.
 * @SX_MODULE_PRESENT_PLUGGED_ERROR: Module plugged with error.
 */
enum sx_module_present_state {
    SX_MODULE_PRESENT_INVALID       = 0,
    SX_MODULE_PRESENT_PLUGGED       = 1,
    SX_MODULE_PRESENT_UNPLUGGED     = 2,
    SX_MODULE_PRESENT_PLUGGED_ERROR = 3
};

/**
 * enum sx_oper_module_present_state - module present state
 * @SX_OPER_MODULE_NOT_PRESENT:      Module not present.
 * @SX_OPER_MODULE_PRESENT:          Module present.
 * @SX_OPER_MODULE_PRESENT_ERROR:    Module present with error.
 */
enum sx_oper_module_present_state {
    SX_OPER_MODULE_NOT_PRESENT   = 0,
    SX_OPER_MODULE_PRESENT       = 1,
    SX_OPER_MODULE_PRESENT_ERROR = 2
};

/**
 * enum sx_module_hw_present_state - module hw_present state
 * @SX_MODULE_HW_PRESENT_INVALID:   Module default base state.
 * @SX_MODULE_HW_PRESENT_ON:        Module is present.
 * @SX_MODULE_HW_PRESENT_OFF:       Module is not present.
 */
enum sx_module_hw_present_state {
    SX_MODULE_HW_PRESENT_INVALID = 0,
    SX_MODULE_HW_PRESENT_ON      = 1,
    SX_MODULE_HW_PRESENT_OFF     = 2,
};


/**
 * enum sx_module_interrupt_state - module interrupt state
 * @SX_MODULE_INTERRUPT_INVALID:   Module interrupt base state.
 * @SX_MODULE_INTERRUPT_ON:        Module interrupt is set.
 * @SX_MODULE_INTERRUPT_OFF:       Module interrupt is cleared.
 */
enum sx_module_interrupt_state {
    SX_MODULE_INTERRUPT_INVALID = 0,
    SX_MODULE_INTERRUPT_ON      = 1,
    SX_MODULE_INTERRUPT_OFF     = 2,
};

/**
 * enum sx_module_power_good_state - module power_good state
 * @SX_MODULE_POWER_GOOD_INVALID:   Module power_good base state.
 * @SX_MODULE_POWER_GOOD_ON:        Module power_good is set.
 * @SX_MODULE_POWER_GOOD_OFF:       Module power_good is cleared.
 */
enum sx_module_power_good_state {
    SX_MODULE_POWER_GOOD_INVALID = 0,
    SX_MODULE_POWER_GOOD_ON      = 1,
    SX_MODULE_POWER_GOOD_OFF     = 2,
};
 ...
 * @threshold_hi: High threshold of warning event.
 * @threshold_lo: Low threshold of warning event.
 */
...
struct module_sysfs_node {
    ...
    struct kobject                 *temperature;
    struct module_sysfs_eeprom_tree eeprom_tree;
};

typedef struct pmpe_dwdata {
    struct kobject     *module;
    struct delayed_work dwork;
} pmpe_dwdata_t;

typedef struct mcion_dwdata {
    struct kobject     *module;
    bool                hw_present;
    bool                interrupt;
    bool                power_good;
...
/* this enum defines reset types supported by the driver */
enum chip_reset_type {
    CHIP_RESET_TYPE_SW_RESET = 1,
    CHIP_RESET_TYPE_RESET_BY_PCI_DISABLE,
    CHIP_RESET_TYPE_INVALID
};


/* this enum defines sysfs sniffer event source type */
enum sysfs_sniffer_event_type {
    SYSFS_SX_CORE = 1,
    SYSFS_NETDEV,
};

static inline bool sx_validate_emad_format(const struct sx_emad          *emad_hdr,
                                           const struct sxd_emad_tlv_reg *reg_tlv,
                                           u16                            expected_reg_id,
                                           u16                            expected_reg_len_dwords_min,
                                           u16                            expected_reg_len_dwords_max)
{
    u16 ethertype = be16_to_cpu(emad_hdr->eth_hdr.ethertype);
    u16 reg_id = be16_to_cpu(emad_hdr->emad_op.register_id);
    u16 reg_tlv_type = sxd_emad_tlv_type(reg_tlv);
    u16 reg_tlv_len = sxd_emad_tlv_len(reg_tlv);

    if (ethertype != ETHTYPE_EMAD) {
        goto out_bad;
    }
    ...
    if (reg_tlv_type != TLV_TYPE_REG_E) {
        goto out_bad;
    }

    if ((expected_reg_len_dwords_min != 0) && (reg_tlv_len < expected_reg_len_dwords_min)) {
        goto out_bad;
    }

    if ((expected_reg_len_dwords_max != 0) && (reg_tlv_len > expected_reg_len_dwords_max)) {
        goto out_bad;
    }
...
out_bad:
    printk(KERN_ERR "EMAD validation failed: "
           "ethtype=0x%04x, "
           "reg_id=0x%04x (expected: 0x%04x), "
           "reg_tlv_type=%u, "
           "reg_tlv_len=%u (min=%u, max=%u)\n",
           ethertype, reg_id, expected_reg_id, reg_tlv_type,
           reg_tlv_len, expected_reg_len_dwords_min, expected_reg_len_dwords_max);
    print_hex_dump(KERN_NOTICE, "emad_hdr ", DUMP_PREFIX_OFFSET, 16, 1, emad_hdr, sizeof(*emad_hdr), 0);
    print_hex_dump(KERN_NOTICE, "reg_tlv ", DUMP_PREFIX_OFFSET, 16, 1, reg_tlv, sizeof(*reg_tlv), 0);
...
#define SX_LISTENER_HANDLER_STR_MAX_LEN (64)
...
int sx_core_get_port_module_status(struct sx_dev                  *dev,
                                   uint16_t                        local_port,
...
void sx_core_delete_module_sysfs_eeprom_tree(struct module_sysfs_eeprom_tree *root,
                                             struct bin_attribute            *page0_attr,
                                             struct bin_attribute            *pagex_attr);
int sx_core_create_module_sysfs_default_eeprom_tree(struct kobject                  *parent,
                                                    struct module_sysfs_eeprom_tree *root);
...
int sx_core_get_port_module_power_on(struct sx_dev *dev,
                                     uint16_t       local_port,
                                     bool          *power_on);
int sx_core_power_on_off_port_module(struct sx_dev *dev,
                                     uint16_t       local_port,
                                     bool           power_on);
...
int sx_core_get_port_module_present(struct sx_dev                     *dev,
                                    uint16_t                           local_port,
...
#define PERF_CNTR_MAX_HW_UNIT_ID 132
#define PERF_CNTR_MAX_GROUP_ID   256

struct mopct_node {
    struct list_head    hw_unit_list;
    struct list_head    group_id_list;
    struct ku_mopct_reg mopct;
};

struct hw_unit_counters_list {
    struct list_head mopct_list;        /*ll of ku_mopct_reg */
    int              mopct_in_hw_unit;         /* to calculate the next group_id for the hw_unit */
};

struct group_id_counters_list {
    struct list_head mopct_list;       /*ll of ku_mopct_reg */
    int              mopct_in_group;       /* to calculate if we can add the group to the MOCS session */
};

struct sx_perf_cntr {
    int                           interval;
    struct hw_unit_counters_list  hw_units_arr[PERF_CNTR_MAX_HW_UNIT_ID];
    struct group_id_counters_list counter_group_arr[PERF_CNTR_MAX_GROUP_ID];
};
...
/* hold a device while handling external tools (such as ethtool) or sysfs entries,
 * to prevent device reset/ISSU during the operation. */
int sx_core_sysfs_dev_hold(struct sx_dev *dev);

/* release a device after external tool (such as ethtool) or sysfs entry handling.
 * A released device may start reset or ISSU flow */
