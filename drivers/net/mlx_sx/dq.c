...
int __sx_core_post_send(struct sx_dev *dev, struct sk_buff *skb, struct isx_meta *meta, u8 sdq_id)
{
    ...
    if (!dev || !priv->profile.pci_profile_set) {
        sxd_log_warning("__sx_core_post_send() cannot "
    ...
    if (priv->global_flushing) {
        sxd_log_rl_warning("__sx_core_post_send: Cannot send packet "
    ...
    if (NUMBER_OF_SWIDS <= meta->swid) {
        sxd_log_warning("__sx_core_post_send: Cannot send packet on swid %u\n",
                        meta->swid);
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return -EFAULT;
    }
    ...
    if (err) {
        sxd_log_rl_warning("sx_core_post_send: error in callback structure sx_get_sdq for "
                           "selecting SDQ \n");
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return -EFAULT;
    }
    ...
    if (!sdq) {
        sxd_log_rl_warning("sx_core_post_send: ERR: " \
    ...
    if (sdq->is_flushing == 1) {
        sxd_log_rl_warning("__sx_core_post_send: Cannot send packet on dqn [%u] "
    ...
    if (skb->len > cap_max_mtu) {
        sxd_log_rl_warning("cannot send packet of size %u, "
    ...
    if (err) {
        sxd_log_rl_warning("sx_core_post_send: "
    ...
    if ((skb->len > 2048) && (priv->profile.pci_profile.cpu_egress_tclass[sdqn] > max_cpu_etclass_for_unlimited_mtu)) {
        sxd_log_err("sx_core_post_send: cannot send packet of size %u "
    ...
    if (sx_dq_overflow(sdq)) {
        if ((sdq->last_full_queue != sdq->last_completion) &&
            ...
            sxd_log_err("%s: sdq[%d] buffer is full for at least %d seconds\n",
                        __func__,
                        sdq->dqn,
