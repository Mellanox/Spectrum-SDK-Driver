...
int __sx_core_post_send(struct sx_dev *dev, struct sk_buff *skb, struct isx_meta *meta, u8 sdq_id)
{
    ...
    if (priv->global_flushing) {
        sxd_log_rl_debug("Cannot send packet while in global_flushing mode\n");
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return 0; /* this is not an error */
    }

    if (NUMBER_OF_SWIDS <= meta->swid) {
        sxd_log_rl_err("Cannot send packet on swid %u\n", meta->swid);
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return -EINVAL;
    }
    ...
    if (sdq->is_flushing == 1) {
        sxd_log_rl_debug("Cannot send packet on dqn [%u] while in flushing mode\n", sdqn);
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return 0; /* this is not an error */
    }

    if (skb->len > cap_max_mtu) {
        sxd_log_rl_err("Cannot send packet of size %u, larger than max MTU %u\n",
                       skb->len, cap_max_mtu);
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return -EINVAL;
    }
    ...
    if (err) {
        sxd_log_rl_err("Error in build header/stub\n");
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return err;
    }
