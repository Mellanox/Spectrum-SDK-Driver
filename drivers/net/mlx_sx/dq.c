...
static int sx_dq_overflow(struct sx_dq *sdq)
{
    ...
    if (sdq->send_watermark < q_size) {
        sdq->send_watermark = q_size;
    }
...
void sx_dq_update_occupancy(struct sx_dq *sdq)
{
    int q_size;
...
int sx_flush_dq(struct sx_dev *dev, struct sx_dq *dq, bool update_flushing_state)
{
    ...
    if (err) {
        /* failed to mark DQ as erroneous (that will eventually send completions for all buffers).
         * don't worry, sx_free_dq_sges() will do that in another way */
        sx_warn(dev, "Failed to modify DQ (dqn=%d) to error\n", dq->dqn);
        goto out_mod;
    }

    end = jiffies + 5 * HZ;
    spin_lock_irqsave(&dq->lock, flags);
    while ((dq->head != dq->tail) || sx_cq_active_bitmap_get(dev, dq->cq->cqn) != 0) {
        ...
        if (time_after(jiffies, end)) {
            ...
            sx_err(dev,
                   "sx_core: Error: DQ flush timeout [dqn=%d, head=%u, tail=%u, is_monitor=%s, cq_active_bitmap=0x%x]\n",
                   dq->dqn,
                   dq->head,
                   dq->tail,
                   (dq->is_monitor ? "true" : "false"),
                   sx_cq_active_bitmap_get(dev, dq->cq->cqn));
            break;
        }
...
int __sx_core_post_send(struct sx_dev *dev, struct sk_buff *skb, struct isx_meta *meta, u8 sdq_id)
{
    ...
    if (!dev || !priv->profile.pci_profile_set) {
        sxd_log_rl_err("Cannot send because the profile is not set\n");
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return -EFAULT;
    }

    if (priv->global_flushing) {
        sxd_log_rl_warning("Cannot send packet while in global_flushing mode\n");
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return -EFAULT;
    }

    if (NUMBER_OF_SWIDS <= meta->swid) {
        sxd_log_rl_warning("Cannot send packet on swid %u\n", meta->swid);
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return -EFAULT;
    }
    ...
    if (err) {
        sxd_log_rl_err("Error in callback structure sx_get_sdq for selecting SDQ\n");
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return -EFAULT;
    }
    ...
    if (!sdq) {
        sxd_log_rl_err("SDQ was not allocated\n");
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return -EFAULT;
    }

    if (sdq->is_flushing == 1) {
        sxd_log_rl_warning("Cannot send packet on dqn [%u] while in flushing mode\n", sdqn);
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return -EFAULT;
    }

    if (skb->len > cap_max_mtu) {
        sxd_log_rl_warning("Cannot send packet of size %u, larger than max MTU %u\n",
                           skb->len, cap_max_mtu);
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return -EFAULT;
    }
    ...
    if (err) {
        sxd_log_rl_warning("Error in build header/stub\n");
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return -EFAULT;
    }

    if ((skb->len > 2048) && (priv->profile.pci_profile.cpu_egress_tclass[sdqn] > max_cpu_etclass_for_unlimited_mtu)) {
        sxd_log_rl_err("Cannot send packet of size %u from SDQ %u since it's bounded to cpu_tclass %u\n",
    ...
    if (!new_pkt) {
        sxd_log_rl_err("Cannot send because packet allocation has failed\n");
        sx_skb_free(skb); /* drop packet flow, use kfree_skb */
        return -ENOMEM;
    }
    ...
    if (sdq->pkts_in_sw_queue < SX_MAX_SDQ_SW_QUEUE_LEN) {
        ...
        sxd_log_rl_notice("SW queue for SDQ %u is full. dropping the packet!\n", sdq->dqn);
        err = -EFAULT;
        goto out;
    }
    ...
    if (sx_dq_overflow(sdq)) {
        if ((sdq->last_full_queue != sdq->last_completion) &&
            ...
            sxd_log_rl_err("sdq[%d] buffer is full for at least %d seconds\n",
                           sdq->dqn,
...
static void __print_tx_packet(struct sk_buff *skb, const struct isx_meta *meta)
{
    u16 reg_id = 0xffff; /* invalid register-id */
    int i;

    /* if packet type is not interesting, go out */
    if ((tx_debug_pkt_type != SX_DBG_PACKET_TYPE_ANY) && (tx_debug_pkt_type != meta->type)) {
        return;
    }

    /* if packet's register-id (packet is EMAD) is not interesting, go out */
    if (meta->type == SX_PKT_TYPE_EMAD_CTL) {
        reg_id = be16_to_cpu(((struct sx_emad *)skb->data)->emad_op.register_id);

        if ((tx_debug_emad_type != SX_DBG_EMAD_TYPE_ANY) && (tx_debug_emad_type != reg_id)) {
            return;
        }
    }

    if (tx_debug_metadata) {
        sxd_log_debug("TX_META: et=%d, swid=%d, sysport=0x%x, rdq=%d, to_cpu=%d, lp=%d, type=%d, "
                      "dev_id=%d, rx_is_router=%d, fid_valid=%d, fid=%d, skb_headlen(skb)=%d, "
    ...
    if (tx_dump) {
        ...
        for (i = 0; i < 2 && i < skb_shinfo(skb)->nr_frags; i++) {
            skb_frag_t  *frag = &skb_shinfo(skb)->frags[i];
            void        *buf;
            unsigned int cnt;
            ...
            print_hex_dump_debug("TX-SKB (Frag)   ",
                                 DUMP_PREFIX_OFFSET,
                                 16,
                                 1,
                                 buf,
...
void sx_core_post_recv(struct sx_dq *rdq, struct sk_buff *skb)
{
    ...
    if (!priv->profile.pci_profile_set) {
        sxd_log_rl_warning("sx_core_post_recv() cannot execute because the profile is not set\n");
        return;
    }
    ...
    if (sx_is_overflow(rdq->head, rdq->tail, rdq->wqe_cnt)) {
        sxd_log_rl_warning("ERR: overflow in rdq, " \
                           "rdq->tail = %u, "
                           "rdq->head = %u\n", rdq->tail, rdq->head);
        goto out;
    }
    ...
    if (dma_mapping_error(&rdq->dev->pdev->dev,
                          rdq->sge[idx].hdr_pld_sg.dma_addr)) {
        sxd_log_rl_warning("sx_core_post_recv: Err: "
                           "got dma_mapping error\n");
        goto out;
    }
...
static int sx_create_dq(struct sx_dev *dev, int nent, u8 dqn, struct sx_dq **dq, u8 send)
{
    ...
    if (!tdq) {
        sxd_log_err("failed to allocate DQ (dqn=%d, is_send=%u)\n", dqn, send);
        return -ENOMEM;
    }
    ...
    ret = sx_core_create_cq(dev, (1 << priv->dev_cap.log_max_cq_sz),
                            &cq, cqn);
    if (ret < 0) {
        sxd_log_err("failed to allocate CQ for DQ (dqn=%d, is_send=%u, err=%d)\n", dqn, send, ret);
        goto free_dq;
    }
    ...
    ret = sx_buf_alloc(dev, nent * sizeof(struct sx_wqe),
                       PAGE_SIZE, &tdq->buf, 0);
    if (ret) {
        sxd_log_err("failed to allocate buffers for DQ (dqn=%d, is_send=%u, err=%d)\n", dqn, send, ret);
        ret = -ENOMEM;
        goto free_cq;
    }
    ...
    if (ret) {
        sxd_log_err("failed to allocate DQ elements (dqn=%d, is_send=%u, err=%d)\n", dqn, send, ret);
        goto free_buf;
    }
    ...
    if (ret) {
        sxd_log_err("failed to create DQ in HW (err=%d)\n", ret);
        goto free_buf;
    }
