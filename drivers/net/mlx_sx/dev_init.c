...
after_reset:
    ...
    if (__probe_params.do_reset && !SX_GET_FW_CAP(priv, SX_FW_CAP_MOD_SUPPORT_MASK, SX_FW_CAP_MOD_SUPPORT_OFFSET)) {
        /* udev event for system management purpose:
         * - only if chip was reset or warm_boot
         * - it is raised here only if FW does not support independent/standalone-module
         */
                                 ...
                                 u32                driver_flag)
{
    ...
    if (__probe_params.total_probes == 0) {
        sxd_log_err("pci_register_driver() returned successfully but probe function not called\n");
        unreg_driver = true;
        goto out;
    }
...
int sx_restart_one_pci(struct sx_dev *dev)
{
    ...
    sxd_log_info("RESTART PCI DEVICE %u\n", dev->device_id);
