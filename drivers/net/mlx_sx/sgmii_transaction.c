                                ...
                                struct sk_buff               **rx_skb)
{
    ...
    /* context.rx_skb holds a copy of the original RX skb (if it is not NULL).
     * so either it is given to the user (and he is the responsible to free it) or
     * we free it here.
     */
    free_context_skb = (context.rx_skb != NULL);

    switch (context.status) {
    case SGMII_TR_COMP_ST_COMPLETED:
        err = 0;
        if (rx_skb) {
            *rx_skb = context.rx_skb; /* caller of this function is responsible to free skb! */
            free_context_skb = 0;
        }
        break;
    ...
    if (free_context_skb) {
        consume_skb(context.rx_skb);    /* free unused skb, use consume_skb */
    }
                              ...
                              sgmii_transaction_completion_cb_t  transaction_completion_cb)
{
    ...
    if (!tr_db || !transaction_completion_cb) {
        sxd_log_err("failed to initialize SGMII transaction database - invalid argument\n");
        return -EINVAL;
    }

    ret = sx_core_map_init(&tr_db->db_map, __sgmii_transaction_compare_cb, sizeof(sgmii_transaction_id_t));
    if (ret) {
        sxd_log_err("failed to initialize SGMII transaction database\n");
        return ret;
    }
