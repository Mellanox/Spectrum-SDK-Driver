...
#define SX_CORE_PORTS_PER_LAG_NUM_SPECTRUM5_MAX 128
...
static void __get_specific_data(enum ku_pkt_type type, u8 to_cpu, u8 etclass, u8 lp, struct isx_specific_data *data)
{
    ...
    if (type >= SX_PKT_TYPE_IB_RAW_CTL) {
        if (type == SX_PKT_TYPE_IB_CTL_2) {
            data->protocol = PROTOCOL_IBGen2;
        } else {
...
static int __sx_build_isx_header_v0(struct isx_meta *meta, struct sk_buff *skb, u8 stclass, u8 hw_etclass)
{
    ...
    if (p_hdr == NULL) {
        sxd_log_rl_warning("sx_build_isx_header: "
                           "Error in skb_push\n");
        return -ENOMEM;
    }
    ...
    if (tx_debug && tx_debug_metadata) {
        sxd_log_debug("TX_BASE_HEADER_v0: sysport=0x%x, ctl=%d, protocol=%d, type=%d\n",
                      meta->system_port_mid,
                      specific_meta.ctl,
                      specific_meta.protocol,
                      specific_meta.type);
...
static int __sx_build_isx_header_v1(struct isx_meta *meta, struct sk_buff *skb, u8 stclass,  u8 hw_etclass)
{
    ...
    if (meta->type == SX_PKT_TYPE_ETH_DATA) {
        if ((!(meta->rx_is_router)) && (meta->fid_valid)) {
            ...
            if (sx_priv(dev_p)->fid_to_hwfid[meta->fid] == INVALID_HW_FID_ID) {
                sxd_log_rl_warning("Failed to update meta of outgoing packet:"
    ...
    if (tx_debug && tx_debug_metadata) {
        sxd_log_debug("TX_BASE_HEADER_v1: swid=%d, sysport=0x%x, ctl=%d, protocol=%d, "
                      "rx_is_router=%d, type=%d, fid_valid=%d, use_control_tclass=%d, etclass=%d, "
                      "fid=%d, rx_is_tunnel=%d\n",
                      meta->swid,
                      meta->system_port_mid,
                      specific_meta.ctl,
                      specific_meta.protocol,
...
{
    if (meta->span_oob_data_valid) {
        sxd_log_err("SPAN OOB message send supported on Spectrum4+ only.\n");
        return -EINVAL;
    }
...
{
    if (sdq == NULL) {
        sxd_log_err("Error: null ptr sdq\n");
        return -EINVAL;
    }

    if (etclass >= NUMBER_OF_ETCLASSES) {
        sxd_log_err("Error: etclass %d is out of range [0..%d], force to 0\n",
                                     ...
                                     u8              *sdq)
{
    ...
    if (swid >= NUMBER_OF_SWIDS) {
        sxd_log_notice("swid %d is out of range [0..%d], force to 0\n",
                       swid, NUMBER_OF_SWIDS);
        swid = 0;
    }

    if (etclass >= NUMBER_OF_ETCLASSES) {
        sxd_log_notice("etclass %d is out of range [0..%d], force to 0\n",
                       etclass, NUMBER_OF_ETCLASSES);
        etclass = 0;
    }
...
static int __sx_get_sdq_num_from_profile(struct sx_dev *dev, u8 swid, u8 etclass, u8 *sdq)
{
    ...
    if (swid >= NUMBER_OF_SWIDS) {
        sxd_log_err("swid %d is out of range [0..%d], force to 0\n",
                    swid, NUMBER_OF_SWIDS);
        return -EINVAL;
    }

    if (etclass >= NUMBER_OF_ETCLASSES) {
        sxd_log_err("etclass %d is out of range [0..%d], force to 0\n",
                    etclass, NUMBER_OF_ETCLASSES);
        return -EINVAL;
    }

    if (dev == NULL) {
        sxd_log_err("Error: null ptr dev\n");
        return -EINVAL;
    }
...
static int __get_swid_from_db(struct sx_dev *dev, struct completion_info *comp_info, u8 *swid)
{
    ...
    default:
        sxd_log_rl_warning("Received packet type is FC, "
...
static int __sx_get_lag_max(uint16_t *lags, uint16_t *ports_per_lag)
{
    ...
    if (ports_per_lag) {
        *ports_per_lag = SX_CORE_PORTS_PER_LAG_NUM_MAX;  /* maximum number of ports per LAG per PRM */
        ...
        sxd_log_err("ports_per_lag is NULL\n");
        return -EINVAL;
    }
...
{
    if (!is_cmd_ifc_only) {
        sxd_log_err("is_cmd_ifc_only is NULL\n");
        return -EINVAL;
    }
...
static void __sx_disconnect_all_trap_groups_spectrum(struct sx_dev *dev)
{
    ...
    for (i = 0; i < NUM_OF_TRAP_GROUPS; i++) {
        ...
        if (err) {
            sxd_log_err("sx_disconnect_all_trap_groups: failed to disconnect trap group %d \n", i);
            return;
        }
    }
    sxd_log_debug("sx_disconnect_all_trap_groups: succeeded \n");
}
...
static int __sx_get_phy_port_max_spectrum5(uint16_t *port)
{
    if (port) {
        *port = SX_CORE_PHY_PORT_NUM_SPECTRUM5_MAX;  /* maximum number of physical ports for Spectrum5 per PRM */
    } else {
        sxd_log_err("port is NULL\n");
        return -EINVAL;
    }
...
static int __sx_get_lag_max_spectrum5(uint16_t *lags, uint16_t *ports_per_lag)
{
    if (lags) {
        *lags = SX_CORE_LAG_NUM_SPECTRUM5_MAX;  /* maximum number of LAGs per PRM */
    } else {
        sxd_log_err("lags is NULL\n");
        return -EINVAL;
    }
    ...
    if (ports_per_lag) {
        *ports_per_lag = SX_CORE_PORTS_PER_LAG_NUM_SPECTRUM5_MAX;  /* maximum number of ports per LAG per PRM */
    } else {
        sxd_log_err("ports_per_lag is NULL\n");
        return -EINVAL;
    }
...
static u32 __system_status_mem_offset_default(void)
{
    return 0xa1844; /* PRM: System_Status_Mem */
}

static u32 __system_status_mem_offset_qtm3(void)
{
    return 0x152080; /* PRM: System_Status_Mem */
}
...
static int __prepare_dev_for_secured_dump(struct sx_priv *priv)
{
    int err = 0;

    err = sx_core_secured_cr_init(&priv->dev);
    if (err) {
        sxd_log_err("failed to initialize secured CR access (err=%d)\n", err);
        goto out;
    }

    err = sx_core_cr_dump_init(priv);
    if (err) {
        sxd_log_err("failed to initialize CR-dump (err=%d)\n", err);
        goto out;
    }
...
static u64 __get_base_info_pointer_mem_spectrum(void)
{
    return 0x100000 /* scratchpad fixed memory */ + 0x40 /* base_info_mem offset */;
}

static u64 __get_base_info_pointer_mem_quantum(void)
{
    return 0x200000 /* scratchpad fixed memory */ + 0x40 /* base_info_mem offset */;
}
...
static struct dev_specific_cb __spec_cb_sx_a2 = {
    ...
    .sx_clock_dump = NULL,
    .sx_clock_ptp_pps_init = NULL,
    .cap_max_mtu_get_cb = __cap_max_mtu_get_spectrum,
    .synce_init_cb = NULL,
    .synce_deinit_cb = NULL,
    .chip_reset_cb = NULL,
    .chip_emergency_reset_cb = NULL,
    .chip_post_reset_cb = NULL,
    .get_system_status_mem_offset_cb = NULL,
    .is_mirror_header_v2_ext_cb = NULL,
    .multi_asic_sysfs_env_init = NULL,
    .prepare_dev_for_secured_dump_cb = NULL,
    .get_base_info_pointer_mem_cb = NULL,
};
...
static struct dev_specific_cb __spec_cb_sx_ib = {
    ...
    .synce_init_cb = NULL,
    .synce_deinit_cb = NULL,
    .chip_reset_cb = sx_reset_with_mrsr_1,
    .chip_emergency_reset_cb = sx_emergency_reset_with_cr_space,
    .chip_post_reset_cb = NULL,
    .get_system_status_mem_offset_cb = __system_status_mem_offset_default,
    .is_mirror_header_v2_ext_cb = NULL,
    .multi_asic_sysfs_env_init = NULL,
    .prepare_dev_for_secured_dump_cb = NULL,
    .get_base_info_pointer_mem_cb = NULL,
...
static struct dev_specific_cb __spec_cb_quantum2 = {
    ...
    .synce_init_cb = NULL,
    .synce_deinit_cb = NULL,
    .chip_reset_cb = sx_reset_by_capability,
    .chip_emergency_reset_cb = sx_emergency_reset_by_capability,
    .chip_post_reset_cb = NULL,
    .get_system_status_mem_offset_cb = __system_status_mem_offset_default,
    .is_mirror_header_v2_ext_cb = NULL,
    .multi_asic_sysfs_env_init = sx_multi_asic_module_sysfs_env_init,
    .prepare_dev_for_secured_dump_cb = NULL,
    .get_base_info_pointer_mem_cb = NULL,
};
...
static struct dev_specific_cb __spec_cb_quantum3 = {
    ...
    .chip_reset_cb = sx_reset_by_capability,
    .chip_emergency_reset_cb = sx_emergency_reset_by_capability,
    .chip_post_reset_cb = sx_post_reset_mrsr_6,
    .get_system_status_mem_offset_cb = __system_status_mem_offset_qtm3,
    .is_mirror_header_v2_ext_cb = NULL,
    .multi_asic_sysfs_env_init = sx_multi_asic_module_sysfs_env_init,
    .prepare_dev_for_secured_dump_cb = __prepare_dev_for_secured_dump,
    .get_base_info_pointer_mem_cb = __get_base_info_pointer_mem_quantum,
};
...
static struct dev_specific_cb __spec_cb_spectrum2 = {
    ...
    .sx_clock_ptp_pps_init = sx_clock_ptp_pps_init_spc2,
    .cap_max_mtu_get_cb = __cap_max_mtu_get_spectrum,
...
static struct dev_specific_cb __spec_cb_spectrum4 = {
    ...
    .multi_asic_sysfs_env_init = NULL,
    .prepare_dev_for_secured_dump_cb = __prepare_dev_for_secured_dump,
    .get_base_info_pointer_mem_cb = __get_base_info_pointer_mem_spectrum,
};

static struct dev_specific_cb __spec_cb_spectrum5 = {
    .get_hw_etclass_cb = __sx_core_get_hw_etclass_impl_spectrum,
    .sx_build_isx_header_cb = __sx_build_isx_header_v1_spc4,
    .max_cpu_etclass_for_unlimited_mtu = __max_cpu_etclass_for_unlimited_mtu_spectrum,
    .sx_get_sdq_cb = __sx_get_sdq_per_traffic_type,
    ...
    .sx_fill_params_from_cqe_cb = sx_fill_params_from_cqe_v2,
    .sx_disconnect_all_trap_groups_cb = __sx_disconnect_all_trap_groups_spectrum,
    .sx_get_phy_port_max_cb = __sx_get_phy_port_max_spectrum5,
    .sx_get_lag_max_cb = __sx_get_lag_max_spectrum5,
    .sx_get_rdq_max_cb = __sx_get_rdq_max_spectrum2,
    .is_eqn_cmd_ifc_only_cb = NULL,
    .sx_clock_dev_init = sx_clock_dev_init_spc2,
    .sx_clock_dev_cleanup = sx_clock_dev_cleanup_spc2,
...
/* priv->db_lock must be held by the caller */
int sx_core_dev_init_cb(struct sx_priv *priv, enum sxd_chip_types chip_type)
{
    int err = 0;

    sxd_log_info("initialize device callbacks for chip type %d\n", chip_type);
    ...
    case SXD_CHIP_TYPE_SPECTRUM5:
        /* for spectrum4 add specific cb */
        priv->dev_specific_cb = __spec_cb_spectrum5;
        break;

    default:
        sxd_log_err("attempt to initialize callbacks of an unrecognized chip type: %d\n", chip_type);
        err = -EINVAL;
        break;
    }

    if (!err) {
        /* we take the reference in init and release in deinit to get event in case refcnt is 0 */
        atomic_set(&priv->dev_specific_cb_refcnt, 1);
...
int sx_core_dev_init_switchx_cb(struct sx_dev *dev, enum sxd_chip_types chip_type, bool force)
{
    ...
    sxd_log_info("initialize device callbacks: force=%s, cb_init_done=%s\n",
                 ((force) ? "yes" : "no"),
                 ((priv->dev_specific_cb_init_done) ? "yes" : "no"));

    if (force || !priv->dev_specific_cb_init_done) {
        err = sx_core_dev_init_cb(priv, chip_type);
        if (err) {
            sxd_log_err("initialization of device callbacks failed (err=%d)\n", err);
            goto out;
        }
    }
    ...
    sxd_log_debug("sx_core_dev_init_switchx_cb chip_type [%d]\n", chip_type);
...
int sx_core_dev_deinit_switchx_cb(struct sx_dev *dev)
{
    ...
    if (!priv->dev_specific_cb_init_done) {
        sxd_log_err("deinit callbacks called before was initialized. \n");
        spin_unlock_irqrestore(&priv->db_lock, flags);
        goto out;
    }
    ...
    if (!wait_event_timeout(priv->dev_specific_cb_not_in_use,
        ...
        sxd_log_err("sx_core_dev_specific_cb_deinit_timeout.\n");
        err = -EBUSY;
        goto out;
    }
    ...
    sxd_log_debug("sx_core_dev_deinit_cb done successfully.\n");
