...
void __enqueue_hft_mocs_done_event(struct sx_dev* dev);
                                   ...
                                   void                     *context)
{
    ...
    if (((reg_id != MLXSW_MAFTI_ID) && (reg_id != MLXSW_MAFRI_ID) &&
        ...
        err = bulk_cntr_db_event_id_to_buffer(dev, &event_id, &layout_common);
        if (err) {
 ...
 */
int sx_bulk_cntr_handle_ack(struct sx_dev *dev, const sxd_bulk_cntr_event_id_t *ev_id, unsigned long buffer_id)
{
    enum sxd_bulk_cntr_done_status_e status;
    uint32_t                         cookie;
    ...
    if (type == SXD_BULK_CNTR_KEY_TYPE_HFT_E) {
        __enqueue_hft_mocs_done_event(dev);
        goto out;
    } else {
        err = bulk_cntr_db_complete(dev, ev_id, NULL, &status, &cookie);
    }
    ...
    err = __send_notification_to_user(ev_id, buffer_id, status, cookie, GFP_KERNEL, dev->device_id);
    if (err) {
...
int sx_bulk_cntr_handle_continue_ack(struct sx_dev *dev, const sxd_bulk_cntr_event_id_t *ev_id,
                                     unsigned long buffer_id)
{
    enum sxd_bulk_cntr_done_status_e status;
    uint32_t                         cookie;
                               ...
                               void                                      *context)
{
    ...
    uint32_t                         cookie;
    int                              err;
    u64                              emad_tid = ((u64*)context)[0];
    struct sx_dev                   *dev = (struct sx_dev*)((u64*)context)[1];
    ...
    /* complete the transaction: get status and delete from DB */
    err = bulk_cntr_db_complete(dev, &event_id, &buffer_id, &status, &cookie);
    if (err) {
    ...
    /* send notification to user upon transaction completion */
    err = __send_notification_to_user(&event_id, buffer_id, status, cookie, GFP_ATOMIC, dev->device_id);
    if (err) {
...
{
    u64 dev_id_emad_tid[2];

    dev_id_emad_tid[0] = EVENT_EMAD_HDR_TID(ci->skb);
    dev_id_emad_tid[1] = (u64)ci->dev;
                           ...
                           void                                      *context)
{
    ...
    case SXD_PPCNT_GRP_PHYSICAL_LAYER_STATISTICAL_COUNTERS_E:
        ...
        GET_PPCNT_COUNTER_32(phy_layer_stats, fc_zero_hist, phys_layer_stat_cntrs, fc_zero_hist);
        GET_PPCNT_COUNTER_32(phy_layer_stats, symbol_ber_magnitude, phys_layer_stat_cntrs, symbol_ber_magnitude);
        GET_PPCNT_COUNTER_32(phy_layer_stats, symbol_ber_coef, phys_layer_stat_cntrs, symbol_ber_coef);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_magnitude_lane0, phys_layer_stat_cntrs, raw_ber_magnitude_lane0);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_coef_lane0, phys_layer_stat_cntrs, raw_ber_coef_lane0);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_magnitude_lane1, phys_layer_stat_cntrs, raw_ber_magnitude_lane1);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_coef_lane1, phys_layer_stat_cntrs, raw_ber_coef_lane1);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_magnitude_lane2, phys_layer_stat_cntrs, raw_ber_magnitude_lane2);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_coef_lane2, phys_layer_stat_cntrs, raw_ber_coef_lane2);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_magnitude_lane3, phys_layer_stat_cntrs, raw_ber_magnitude_lane3);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_coef_lane3, phys_layer_stat_cntrs, raw_ber_coef_lane3);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_magnitude_lane4, phys_layer_stat_cntrs, raw_ber_magnitude_lane4);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_coef_lane4, phys_layer_stat_cntrs, raw_ber_coef_lane4);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_magnitude_lane5, phys_layer_stat_cntrs, raw_ber_magnitude_lane5);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_coef_lane5, phys_layer_stat_cntrs, raw_ber_coef_lane5);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_magnitude_lane6, phys_layer_stat_cntrs, raw_ber_magnitude_lane6);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_coef_lane6, phys_layer_stat_cntrs, raw_ber_coef_lane6);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_magnitude_lane7, phys_layer_stat_cntrs, raw_ber_magnitude_lane7);
        GET_PPCNT_COUNTER_32(phy_layer_stats, raw_ber_coef_lane7, phys_layer_stat_cntrs, raw_ber_coef_lane7);
        GET_PPCNT_COUNTER_64(phy_layer_stats, phy_effective_errors, phys_layer_stat_cntrs, phy_effective_errors);
        break;

    case SXD_PPCNT_GRP_INFINIBAND_PKTS_COUNTERS_E:
        CHECK_PPCNT_GRP_AND_COUNT(SXD_BULK_CNTR_PORT_GRP_IB_PACKETS_E);
        GET_PPCNT_COUNTER_64(infiniband_packets,
                             time_since_last_clear,
                             ppcnt_infiniband_packets_counter,
                             time_since_last_clear);
        GET_PPCNT_COUNTER_64(infiniband_packets,
        ...
        GET_PPCNT_COUNTER_64(infiniband_packets,
                             ...
                             port_neighbor_mtu_discards);
        GET_PPCNT_COUNTER_64(infiniband_packets,
                             port_llu_rx_icrc_error,
                             ppcnt_infiniband_packets_counter,
                             port_llu_rx_icrc_error);
        GET_PPCNT_COUNTER_64(infiniband_packets,
                             port_tx_parity_error,
                             ppcnt_infiniband_packets_counter,
                             port_tx_parity_error);
        break;

    case SXD_PPCNT_GRP_PLR_COUNTERS_GROUP_E:
        CHECK_PPCNT_GRP_AND_COUNT(SXD_BULK_CNTR_PORT_GRP_PLR_E);
        GET_PPCNT_COUNTER_64(plr, plr_rcv_codes, ppcnt_plr_counters, plr_rcv_codes);
        GET_PPCNT_COUNTER_64(plr, plr_rcv_code_err, ppcnt_plr_counters, plr_rcv_code_err);
        GET_PPCNT_COUNTER_64(plr, plr_rcv_uncorrectable_code, ppcnt_plr_counters, plr_rcv_uncorrectable_code);
        GET_PPCNT_COUNTER_64(plr, plr_xmit_codes, ppcnt_plr_counters, plr_xmit_codes);
        GET_PPCNT_COUNTER_64(plr, plr_xmit_retry_codes, ppcnt_plr_counters, plr_xmit_retry_codes);
        GET_PPCNT_COUNTER_64(plr, plr_xmit_retry_events, ppcnt_plr_counters, plr_xmit_retry_events);
        GET_PPCNT_COUNTER_64(plr, plr_sync_events, ppcnt_plr_counters, plr_sync_events);
        GET_PPCNT_COUNTER_64(plr, plr_codes_loss, ppcnt_plr_counters, plr_codes_loss);
        GET_PPCNT_COUNTER_64(plr,
                             plr_xmit_retry_events_within_t_sec_max,
                             ppcnt_plr_counters,
                             plr_xmit_retry_events_within_t_sec_max);
        GET_PPCNT_COUNTER_64(plr, time_since_last_clear, ppcnt_plr_counters, time_since_last_clear);
        break;

    case SXD_PPCNT_GRP_RS_FEC_HISTOGRAM_GROUP_E:
        CHECK_PPCNT_GRP_AND_COUNT(SXD_BULK_CNTR_PORT_GRP_RS_FEC_E);
        for (hist_idx = 0; hist_idx < SXD_PPCNT_PPCNT_RS_FEC_HISTOGRAMS_COUNTERS_HIST_NUM; hist_idx++) {
            layout_port->counters[port_cntr_index].rs_fec.hist[hist_idx] =
                ppcnt.counter_set.ppcnt_rs_fec_histograms_counters.hist[hist_idx];
        }
        break;

    case SXD_PPCNT_GRP_INFINIBAND_EXTENDED_PORT_COUNTERS_E:
        CHECK_PPCNT_GRP_AND_COUNT(SXD_BULK_CNTR_PORT_GRP_IB_EXT_E);
        GET_PPCNT_COUNTER_64(ib_ext, symbol_error_counter, ib_long_portcntrs_attribute_grp_data, symbol_error_counter);
        GET_PPCNT_COUNTER_64(ib_ext,
                             link_error_recovery_counter,
                             ib_long_portcntrs_attribute_grp_data,
                             link_error_recovery_counter);
        GET_PPCNT_COUNTER_64(ib_ext, link_downed_counter, ib_long_portcntrs_attribute_grp_data, link_downed_counter);
        GET_PPCNT_COUNTER_64(ib_ext, port_rcv_errors, ib_long_portcntrs_attribute_grp_data, port_rcv_errors);
        GET_PPCNT_COUNTER_64(ib_ext,
        ...
        GET_PPCNT_COUNTER_64(ib_ext,
                             ...
                             ib_long_portcntrs_attribute_grp_data,
                             port_rcv_switch_relay_errors);
        GET_PPCNT_COUNTER_64(ib_ext, port_xmit_discards, ib_long_portcntrs_attribute_grp_data, port_xmit_discards);
        GET_PPCNT_COUNTER_64(ib_ext,
        ...
        GET_PPCNT_COUNTER_64(ib_ext,
                             ...
                             ib_long_portcntrs_attribute_grp_data,
                             excessive_buffer_overrun_errors);
        GET_PPCNT_COUNTER_64(ib_ext, vl_15_dropped, ib_long_portcntrs_attribute_grp_data, vl_15_dropped);
        GET_PPCNT_COUNTER_64(ib_ext, port_xmit_data, ib_long_portcntrs_attribute_grp_data, port_xmit_data);
        GET_PPCNT_COUNTER_64(ib_ext, port_rcv_data, ib_long_portcntrs_attribute_grp_data, port_rcv_data);
        GET_PPCNT_COUNTER_64(ib_ext, port_xmit_pkts, ib_long_portcntrs_attribute_grp_data, port_xmit_pkts);
        GET_PPCNT_COUNTER_64(ib_ext, port_rcv_pkts, ib_long_portcntrs_attribute_grp_data, port_rcv_pkts);
        GET_PPCNT_COUNTER_64(ib_ext, port_xmit_wait, ib_long_portcntrs_attribute_grp_data, port_xmit_wait);
        GET_PPCNT_COUNTER_64(ib_ext, qp1_dropped, ib_long_portcntrs_attribute_grp_data, qp1_dropped);
        GET_PPCNT_COUNTER_64(ib_ext,
                             port_unicast_xmit_pkts,
                             ib_long_portcntrs_attribute_grp_data,
                             port_unicast_xmit_pkts);
        GET_PPCNT_COUNTER_64(ib_ext,
                             ...
                             ib_long_portcntrs_attribute_grp_data,
                             port_multicast_xmit_pkts);
        GET_PPCNT_COUNTER_64(ib_ext, port_unicast_rcv_pkts, ib_long_portcntrs_attribute_grp_data,
                             port_unicast_rcv_pkts);
        GET_PPCNT_COUNTER_64(ib_ext,
    ...
    case SXD_PPCNT_GRP_INFINIBAND_GENERAL_COUNTERS_E:
        CHECK_PPCNT_GRP_AND_COUNT(SXD_BULK_CNTR_PORT_GRP_IB_GENERAL_E);
        GET_PPCNT_COUNTER_64(ib_general, port_rcv_data_qword, ppcnt_infiniband_general_counter, port_rcv_data_qword);
        GET_PPCNT_COUNTER_64(ib_general, port_xmit_data_qword, ppcnt_infiniband_general_counter, port_xmit_data_qword);
        GET_PPCNT_COUNTER_64(ib_general, rq_general_error, ppcnt_infiniband_general_counter, rq_general_error);
        GET_PPCNT_COUNTER_64(ib_general, port_xmit_ebp, ppcnt_infiniband_general_counter, port_xmit_ebp);
        GET_PPCNT_COUNTER_64(ib_general,
                             port_sw_hoq_lifetime_limit_discards,
                             ppcnt_infiniband_general_counter,
                             port_sw_hoq_lifetime_limit_discards);
        GET_PPCNT_COUNTER_64(ib_general,
                             dqs2llu_xmit_wait_arb_global,
                             ppcnt_infiniband_general_counter,
                             dqs2llu_xmit_wait_arb_global);
        GET_PPCNT_COUNTER_64(ib_general, grxb_fecn_mark, ppcnt_infiniband_general_counter, grxb_fecn_mark);
        GET_PPCNT_COUNTER_64(ib_general,
        ...
        GET_PPCNT_COUNTER_64(ib_general,
                             ...
                             ppcnt_infiniband_general_counter,
                             general_receive_discard_external_contain);
        GET_PPCNT_COUNTER_64(ib_general, port_receive_poison, ppcnt_infiniband_general_counter, port_receive_poison);
        break;
...
{
    sx_bulk_cntr_multi_emad_parse(ci->skb->data, ci->skb->len, __ppcnt_parser, ci->dev, NULL);
}
...
{
    sx_bulk_cntr_multi_emad_parse(ci->skb->data, ci->skb->len, __mgpcb_parser, ci->dev, NULL);
}
...
{
    sx_bulk_cntr_multi_emad_parse(ci->skb->data, ci->skb->len, __mofrb_parser, ci->dev, NULL);
}
...
{
    sx_bulk_cntr_multi_emad_parse(ci->skb->data, ci->skb->len, __pbsr_parser, ci->dev, NULL);
}
...
{
    sx_bulk_cntr_multi_emad_parse(ci->skb->data, ci->skb->len, __sbsrd_parser, ci->dev, NULL);
}
...
{
    sx_bulk_cntr_multi_emad_parse(ci->skb->data, ci->skb->len, __ceer_parser, ci->dev, NULL);
}
...
{
    sx_bulk_cntr_multi_emad_parse(ci->skb->data, ci->skb->len, __fsed_parser, ci->dev, NULL);
}
...
{
    sx_bulk_cntr_multi_emad_parse(ci->skb->data, ci->skb->len, __upcnt_parser, ci->dev, NULL);
}
...
{
    sx_bulk_cntr_multi_emad_parse(ci->skb->data, ci->skb->len, __utcc_parser, ci->dev, NULL);
}
...
{
    sx_bulk_cntr_multi_emad_parse(ci->skb->data, ci->skb->len, __usacn_parser, ci->dev, NULL);
}
...
static void __sx_hft_mocs_done_handler_cb(struct work_struct *work_p)
{
    ...
    uint32_t                         cookie;
    sxd_bulk_cntr_event_id_t         event_id;
    struct sx_dev                   *dev = hft_work_p->dev;
    ...
    /* send notification to user upon transaction completion */
    err = __send_notification_to_user(&event_id, buffer_id, status, cookie, GFP_KERNEL, dev->device_id);
    if (err) {
        sxd_log_err("Bulk-Cntr SDK ack: failed to send notification to user upon transaction completion\n");
        goto out;
    }
...
void __enqueue_hft_moftd_event(struct completion_info *comp_info_p)
{
    ...
    hft_work_p->dev = comp_info_p->dev;
