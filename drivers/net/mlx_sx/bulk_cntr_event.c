...
int sx_bulk_cntr_handle_ack(const sxd_bulk_cntr_event_id_t *ev_id, unsigned long buffer_id)
{
    ...
    if (type == SXD_BULK_CNTR_KEY_TYPE_HFT_E) {
        err = bulk_cntr_db_complete(ev_id, &buffer_id, &status, &cookie);
    } else {
...
#endif
#define SXD_MOFTD_ENTRY_OFFSET     32
#define MOFTD_ENTRY_SIZE           4
                           ...
                           void                                      *context)
{
    ...
    memset(&moftd, 0, sizeof(moftd));
    ret = __MOFTD_decode((u8*)moftd_reg, &moftd, context);
    if (ret != 0) {
    ...
    if (rx_debug_hft_event) {
        sxd_log_debug("RX_MOFTD: number_records=%d, first_port=%d, entries_per_port=%d, metadata_entries_expected=%d, port_expected=%d, flow0_expected=%d, flow1_expected=%d, "
                      ...
                      moftd.num_rec,
                      moftd.first_port_in_chunk,
                      moftd.num_entries_per_port,
    ...
    if (layout_hft->samples_received != moftd.itr_index) {
    ...
    ptr = &(((u8*)moftd_reg)[SXD_MOFTD_ENTRY_OFFSET]);
    moftd_entries = (u32*)ptr;
    entry_index = 0;
    for (entry_index = 0; entry_index < moftd.num_rec; entry_index++) {
        moftd_entries[entry_index] = cpu_to_be32(moftd_entries[entry_index]);
    }
    ...
    if ((metadata_total_expected != 0) && (entry_index < moftd.num_rec) &&
        ...
        remaining_received_entry_32 = moftd.num_rec - entry_index;
        ...
        memcpy((u8*)&layout_hft->buffer[layout_hft->buffer_idx], (u8*)&moftd_entries[entry_index],
               (remaining_min_entries_32 * MOFTD_ENTRY_SIZE));
        ...
        layout_hft->buffer_idx += (remaining_min_entries_32 * MOFTD_ENTRY_SIZE);
    ...
    if (moftd.num_entries_per_port != 0) {
        if ((layout_hft->port_map.port_counter_buffer_size / 4) != moftd.num_entries_per_port) {
            sxd_log_err(
    ...
    if ((moftd.num_entries_per_port != 0) && (port_total_expected != 0) &&
        ...
        idx = layout_hft->port_map.sample_port_index_map[moftd.first_port_in_chunk];
        ...
        while (entry_index < moftd.num_rec && port_total_received < port_total_expected) {
            if (rx_debug_hft_event) {
            ...
            entry_index += moftd.num_entries_per_port;
    ...
    for (idx = 0; idx < layout_hft->global_map.flow_counter_type_count; idx++) {
        ...
        if ((flow_total_expected != 0) && (flow_total_received < flow_total_expected)) {
            ...
            __process_flow_counter_buffer(layout_hft, &layout_hft->buffer[layout_hft->buffer_idx],
...
record_end:
    if (rx_debug_hft_event) {
        sxd_log_debug(
            ...
            moftd.num_entries_in_itr,
            entry_index,
            moftd.entry_index);
    }
    if (moftd.num_entries_in_itr == entry_index + moftd.entry_index) {
...
{
    sx_bulk_cntr_multi_emad_parse(ci->skb->data, ci->skb->len, __moftd_parser, NULL);
}
