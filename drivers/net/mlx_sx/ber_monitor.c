...
static struct ber_monitor_info * __dev_id_to_bm_info(sxd_dev_id_t dev_id, bool create_new)
{
    ...
    if (dev) {
        ...
        if (!__ber_monitor_db[dev_id]) {
            ...
            if (!dev) {
                sxd_log_err("Ber-Monitor: could not find default device\n");
                bm_info = ERR_PTR(-ENODEV);
                goto out;
            }
...
void sx_core_ber_monitor_deinit(void)
{
    ...
    sxd_log_info("Ber-Monitor (deinit)\n");
...
int sx_core_ber_monitor_set_state(sxd_dev_id_t dev_id, u16 local_port, u8 state)
{
    ...
    if (!DEV_ID_IS_VALID(dev_id)) {
        sxd_log_err("Ber-Monitor (set-state): device ID %u is invalid\n", dev_id);
        err = -EINVAL;
        goto out;
    }

    if (local_port > MAX_PHYPORT_NUM) {
        sxd_log_err("Ber-Monitor (set-state): local port %u is invalid (max. %u)\n",
                    local_port,
                    MAX_PHYPORT_NUM);
        err = -EINVAL;
        goto out;
...
int sx_core_ber_monitor_set_bitmask(sxd_dev_id_t dev_id, u16 local_port, u8 bitmask)
{
    ...
    /* coverity[result_independent_of_operands] */
    if (!DEV_ID_IS_VALID(dev_id)) {
        sxd_log_err("Ber-Monitor (set-bitmask): device ID %u is invalid\n", dev_id);
        err = -EINVAL;
        goto out;
    }

    if (local_port > MAX_PHYPORT_NUM) {
        sxd_log_err("Ber-Monitor (set-bitmask): local_port %u is invalid (max. %u)\n",
                    local_port,
                    MAX_PHYPORT_NUM);
        err = -EINVAL;
        goto out;
    ...
    if (err) {
        sxd_log_err("Ber-Monitor (set-bitmask): failed to set bitmask (dev=%u, port=%u, bitmask=%u)\n",
...
static void __ber_monitor_work_handler(struct work_struct *work)
{
    ...
    if (!to_rearm) {
        sxd_log_rl_notice("Ber-Monitor (event-handler): "
    ...
    if (err) {
        sxd_log_err("Ber-Monitor (event-handler) [dev_id=%u, local_port=%u]: "
    ...
    if (ppbmc_reg_data.ppbmc_reg.e != 0) {
        sxd_log_rl_notice("Ber-Monitor (event-handler): Already armed, skip re-arm "
...
void sx_core_ber_monitor_handle_ppbme_event(struct completion_info *ci)
{
    ...
    sxd_log_rl_notice("Ber-Monitor (event-handler): [dev=%u, port=%u]\n",
