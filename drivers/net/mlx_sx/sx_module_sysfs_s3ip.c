                              ...
                              struct sx_module_status_params *params)
{
    ...
    pmpe_reg_data.pmpe_reg.module = local_module_id;
 ...
 *   For multi-asic system, return specific sx_dev and global module id.
 */
...
static int _access_reg_pddr_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_pddr_reg * reg_data)
{
    ...
    case SXD_PDDR_PAGE_SELECT_MODULE_INFO_PAGE_E:
        ...
        memset(str_buf, 0, sizeof(str_buf));
        vendor_rev = be32_to_cpu(reg_data->page_data.pddr_module_info.vendor_rev);
        DISCARD_LSB_ZEROS(vendor_rev);
        strncpy(str_buf, (char *)&vendor_rev,
                sizeof(str_buf));
        snp_res = snprintf(buffer,
                           buffer_length - buffer_size,
                           "vendor_rev: %s\n", str_buf);
...
static void __sx_handle_pmpe_event(struct completion_info *comp_info, void *context)
{
    ...
    if (sx_core_has_predefined_devices() && MULTI_ASIC_MODULE_SYSFS_IS_SUPPORT(priv)) {
        ...
        if (ret) {
            sxd_log_err("%s: failed to check global and local module map: global module %u, local module %u\n",
                        __func__, global_module_id, module_id);
            return;
        }
    ...
    if (present != priv->module_data[pmpe_reg.slot_index][module_id].present) {
        priv->module_data[pmpe_reg.slot_index][module_id].present = present;
...
static void __sx_mcion_dwork_handler(struct work_struct *dwork_p)
{
    ...
    uint8_t         last_page = 0;
    struct sx_priv *priv = sx_priv(dwdata_p->dev);

    if (dwdata_p->hw_present) {
        mutex_lock(&priv->module_access_mutex);
        if (dwdata_p->plug_in_event) {
            /* Plug in event: Read last page value from EEPROM. */
            err = sx_core_get_module_page_from_eeprom(dwdata_p->dev, dwdata_p->module_id, dwdata_p->slot_id,
                                                      &last_page);
            if (err) {
                sxd_log_err("Failed to get module %u page from EEPROM, err: %d.\n", dwdata_p->module_id, err);
            } else {
                priv->module_data[dwdata_p->slot_id][dwdata_p->module_id].independent_params.last_page = last_page;
                priv->module_data[dwdata_p->slot_id][dwdata_p->module_id].independent_params.last_page_valid = true;
            }
        } else {
            /* Plug out event: Reset last page value to invalid. */
            priv->module_data[dwdata_p->slot_id][dwdata_p->module_id].independent_params.last_page = 0;
            priv->module_data[dwdata_p->slot_id][dwdata_p->module_id].independent_params.last_page_valid = false;
        }
        mutex_unlock(&priv->module_access_mutex);
...
static void __sx_handle_mcion_event(struct completion_info *comp_info, void *context)
{
    ...
    if (!(mcion_reg.module_status_bits_valid & MCION_STATUS_MASK_HW_PRESENT)) {
        ...
        if (hw_present != priv->module_data[slot_id][module_id].independent_params.hw_present) {
            ...
            dwdata_p->plug_in_event = (hw_present == SX_MODULE_HW_PRESENT_ON);
            dwdata_p->module_id = module_id;
            dwdata_p->slot_id = slot_id;
            dwdata_p->dev = comp_info->dev;
...
static int __module_control_set(struct sx_dev *dev, uint8_t slot_id, uint8_t module_id, bool enable_sw_control)
{
    ...
    err = sx_core_get_possible_local_module(module_id, &local_module_id);
    if (err) {
        sxd_log_err("Failed to get possible local module ID for module ID=%d, err=%d\n", module_id, err);
        goto out;
    }
...
int sx_core_set_module_control(struct sx_dev *dev, uint8_t slot_id, uint8_t module_id)
{
    ...
    priv->module_data[slot_id][module_id].independent_params.last_page = 0;
    priv->module_data[slot_id][module_id].independent_params.last_page_valid = false;
