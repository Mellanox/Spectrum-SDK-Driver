               ...
               sgmii_fill_tx_base_header_cb_t  fill_tx_base_header_cb)
{
    ...
    if (tx_debug && tx_debug_metadata && meta) {
        sxd_log_debug(
...
out:
    kfree_skb(skb_send);    /* drop packet flow, use kfree_skb */
...
static rx_handler_result_t __sgmii_rx_handler(struct sk_buff **pskb)
{
    ...
    if (skb_is_nonlinear(skb)) {
        ...
        if (rx_debug) {
            sxd_log_info("skb is fragmented: nr_frags=%d, data_len=%u\n",
    ...
    if (rx_dump) {
        sxd_log_err("RX buffer [data=%p, size=%d]\n", skb->data, skb->len);
...
static int __netdev_notify_cb(struct notifier_block *this, unsigned long event, void *context)
{
    ...
    if ((event == NETDEV_UNREGISTER) && (netdev_notif == sgmii_netdev)) {
        sxd_log_notice("SGMII network link is being removed!\n");
                         ...
                         uint8_t           cqe_ver)
{
    ...
    if (!netdev_name || (netdev_name[0] == '\0')) {
        sxd_log_err("SGMII network's netdev name is empty\n");
        return -EINVAL;
    }
    ...
    default:
        sxd_log_err("cannot initialize transport with invalid chassis type!\n");
        return -EINVAL;
    }
    ...
    if (!sx_glb.fake_dev) {
        sxd_log_err("OOB driver has no device to work with\n");
        ret = -ENODEV;
        goto error;
    }
    ...
    if (ret) {
        sxd_log_err("failed to initialize per-chip-type callbacks (ret=%d)\n", ret);
        goto error;
    }

    netdev = dev_get_by_name(&init_net, netdev_name);
    if (!netdev) {
        sxd_log_err("SGMII network's netdev '%s' was not found\n", netdev_name);
        ret = -ENODEV;
        goto error;
    }
...
#endif
    if (ret) {
        sxd_log_err("Failed to set MAC address of SGMII netdev (err=%d)\n", ret);
    } else {
        ret = netdev_rx_handler_register(netdev, __sgmii_rx_handler, NULL);
        if (ret) {
            sxd_log_err("Failed to enslave SGMII netdev to sx_core (err=%d)\n", ret);
        }
    }
...
void sgmii_transport_deinit(void)
{
    ...
    sxd_log_notice("Deinitializing SGMII transport!\n");
