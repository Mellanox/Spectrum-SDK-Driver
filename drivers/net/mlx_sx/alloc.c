...
void sx_set_u64_to_bitmap(u64 obj, struct sx_bitmap *bitmap)
{
    u32 bit_id;

    sx_bitmap_clear_all(bitmap);
    for (bit_id = 0; bit_id < BITS_PER_LONG; bit_id++) {
        if (test_bit(bit_id, (long unsigned int *)&obj)) {
            sx_bitmap_set(bitmap, bit_id);
        }
    }
}
