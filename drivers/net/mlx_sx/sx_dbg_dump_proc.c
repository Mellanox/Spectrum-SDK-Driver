...
static DEFINE_MUTEX(__proc_list_lock);
...
static int __sx_dbg_dump_proc_open(struct inode *inode, struct file *file)
{
    ...
    if (iter) {
        ...
        sxd_log_err("could not find proc file '%s' information for device %u\n",
                                      ...
                                      void                      *context)
{
    ...
    if (!name || !show_cb) {
        sxd_log_err("proc file name is empty or no show handler\n");
        return -EINVAL;
    }

    if (dev != NULL) {
        if ((dev->device_id > SX_MAX_DEVICES) || (dev->device_id == 0)) {
            sxd_log_err("create info for proc %s failed - invalid device_id %u\n", name, dev->device_id);
            err = -EINVAL;
            goto error;
        }
    ...
    if (__name_to_info(&read_handler_struct_key)) {
        sxd_log_err("proc file %s already exists\n", name);
        err = -EEXIST;
        goto error;
    }
    ...
    if (read_handler_info == NULL) {
        sxd_log_warning("create info for proc %s failed\n", name);
        goto error;
    }
    ...
    if (read_handler_info->read_handler_struct_key.name == NULL) {
        sxd_log_warning("allocate name of info for proc %s failed\n", name);
        goto error;
    }
...
void sx_dbg_dump_read_handler_unregister(const char *name, struct sx_dev *dev)
{
    ...
    if (read_handler_info == NULL) {
        sxd_log_err("proc file %s does not exist\n", name);
    } else {
                                       ...
                                       void                     *context)
{
    ...
    if (!cmd || !usage || !description || !cmd_cb) {
        sxd_log_err("write handler invalid arguments\n");
        return -EINVAL;
    }
    ...
    if (__cmd_to_info(cmd)) {
        if (sx_core_has_predefined_devices()) {
            /* On multi device systems - the first device to init will register the write handler */
            err = 0;
            goto out;
        }
        sxd_log_err("command handler '%s' already exists\n", cmd);
        err = -EEXIST;
        goto error;
    }
...
void sx_dbg_dump_write_handler_unregister(const char *cmd)
{
    ...
    if (write_handler_info == NULL) {
        if (sx_core_has_predefined_devices()) {
            /* On multi device systems - the first device to de-init will unregister the write handler */
            goto out;
        }
        sxd_log_err("cmd handler '%s' does not exist\n", cmd);
    } else {
...
#define SX_WRITE_PROC_MAX_ARGS (10)
    ...
    if (tmp) {
        sxd_log_err("too many arguments for a single command!\n");
        goto out;
    }
    ...
    if (info) {
        ...
        if (rc) {
            sxd_log_err("command '%s' failed (err=%d)\n", argv[0], rc);
        }
    } else {
        sxd_log_err("unknown command '%s'\n", argv[0]);
    }
    mutex_unlock(&__proc_list_lock);
...
static int __help_dump(struct seq_file *m, void *v, void *context)
{
    ...
    if (sx_core_has_predefined_devices()) {
        seq_printf(m, "*** All commands should be written to /proc/dbg_dump/dev_X/dbg_cmd ***\n\n");
    } else {
...
bool __init sx_dbg_dump_proc_fs_init(struct sx_dev *dev, void *context)
{
    ...
    if (dev != NULL) {
        ...
        if ((dev->device_id > SX_MAX_DEVICES) || (dev->device_id == 0)) {
            sxd_log_warning("device %u exceeds MAX_DEVICE - creating dbg_dump folder failed\n", dev->device_id);
            return 1;
        }
        ...
        if (__root_dir[device_id] == NULL) {
            sxd_log_warning("create proc dir for device %u - %s failed\n", device_id, device_proc_folder_path);
            return 1;
        }
