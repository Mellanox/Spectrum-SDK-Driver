...
{
    static const char* vendors[] = { /* PRM values (MSPI.vendor_id) */
        [0] = "Renesas",
        [1] = "SiTime-148",
        [2] = "SiTime-348",
        [3] = "TI",
    ...
    if ((id >= 0) && (id < sizeof(vendors) / sizeof(vendors[0]))) {
        return vendors[id];
    }
...
static int __get_mspi_and_sx_dev(struct ku_access_mspi_reg *mspi_reg, struct sx_dev **dev_out)
{
    ...
    dev = __get_sx_dev();
    if (!dev) {
        sxd_log_err("%s: Could not find a valid device\n", __func__);
        err = -ENODEV;
        goto out;
    }
...
static ssize_t __show_synce_enabled(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    err = sx_core_send_sniffer_event(kobj, __FUNCTION__, attr->attr.name, NULL, 0);
    if (err) {
...
int sx_core_synce_init(void)
{
    ...
    if (err) {
        sxd_log_err("%s: Could not get device specific callback\n", __func__);
        return err;
    }
