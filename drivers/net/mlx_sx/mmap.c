...
struct sx_mmap_metadata {
    ...
    uint64_t                   usr_size; /* size of allocation from user point of view*/
    ...
    struct sx_rsc             *file; /* the file that created the shared memory */
    mmap_pfn_dtor              pfn_dtor;        /* Destructor for mapped memory */
};
...
static void __sx_vm_close(struct vm_area_struct *vma)
{
    ...
    if (--metadata->refcnt == 0) {
        atomic_dec(&metadata->file->mmap_refcnt);
        hash_del(&metadata->h_entry);
        spin_unlock_bh(&__mmap_entries_lock);

        if (metadata->pfn_dtor != NULL) {
            metadata->pfn_dtor(metadata->user_ptr, metadata->pid, metadata->usr_size);
        }
...
void sx_mmap_ref_dec(pid_t pid, unsigned long user_ptr)
{
    ...
    if (del_entry) {
        if (iter->pfn_dtor != NULL) {
            iter->pfn_dtor(iter->user_ptr, iter->pid, iter->usr_size);
        }
        kfree(iter);
    }
}
...
void sx_mmap_set_pfn_dtor(unsigned long user_ptr, pid_t pid, uint64_t usr_size, mmap_pfn_dtor pfn_dtor)
{
    ...
    hash_for_each_possible(__mmap_entries, iter, h_entry, user_ptr) {
        if ((iter->pid == pid) && (iter->user_ptr == user_ptr)) {
            iter->pfn_dtor = pfn_dtor;
            iter->usr_size = usr_size;
            break;
        }
    }
