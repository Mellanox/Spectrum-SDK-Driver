...
static int sx_core_ioctl_synd_param_prepare(const enum ku_l2_type type, enum l2_type *listener_type_p)
{
    ...
    default:
        sxd_log_err("Err: invalid listener type : %d \n", type);
        err = -EINVAL;
        goto out;
    }
...
static void __sx_cq_monitor_sw_queue_handler(struct completion_info *comp_info, void *context)
{
    ...
    if (edata == NULL) {
        if (sx_dev && !priv->stats.eventlist_drops_counter) {
            sxd_log_warning("Memory allocation "
...
int check_valid_ku_synd(const struct ku_synd_ioctl *ku)
{
    ...
    if ((ku->is_default != false) && (ku->is_default != true)) {
        err = -EINVAL;
        sxd_log_debug("The given ku_synd_ioctl not valid: "
                      " ku->is_default=[%d]\n", ku->is_default);
    }
    ...
    if (file->evlist_size < SX_EVENT_LIST_SIZE) {
        ...
        if (file->queue_type == KU_QUEUE_TYPE_HEAD_DROP) {
            ...
            if ((sx_dev != NULL) && (!priv->stats.eventlist_drops_counter)) {
                sxd_log_rl_warning("Head drop channel: Event list is full, dropping oldest packet (trap_id=%u)\n",
                                   head_trap_id);
            }
            ...
            if ((sx_dev != NULL) && (!priv->stats.eventlist_drops_counter)) {
                sxd_log_rl_warning("Tail drop channel: Event list is full, dropping RX packet (trap_id=%u)\n",
                                   comp_info->hw_synd);
            }
            inc_eventlist_drops_counter(priv, comp_info->hw_synd);
            file->channel_experienced_drop = true;
...
static int sx_monitor_simulate_rx_skb(struct sx_dq          *bound_monitor_rdq,
                                      ...
                                      uint32_t              *buf_len_list,
                                      uint32_t              *total_packet_cnt_p,
                                      struct listener_entry *force_listener)
{
    ...
    for (i = 0; i < mon_rx_count && i < buffers_count; /* i incremented inside loop */) {
        ...
        if (cqe_params.is_send) {
            sxd_log_err("%s(): Error SDQ %d was provided when only RDQ is supported.\n",
        ...
        if (cqe_params.byte_count + sizeof(struct ku_read) > buf_len_list[i]) {
            ...
            sxd_log_err("%s(): i=%d , not enough space %d for packet size %d \n",
        ...
        if (!dq) {
            sxd_log_rl_warning("could not find dq context for %s dqn = %d\n",
        ...
        if (!skb) {
             ...
             */
            sxd_log_rl_notice("WJH: skipping NULL RDQ entry, maybe RDQ is also handled elsewhere "
        ...
        if (err) {
            sxd_log_warning("rx_skb error %d . Aborting\n", err);
            goto out;
        }
...
static int sx_trap_group_path_set(struct sx_dq *bound_monitor_rdq, uint8_t cpu_path_type)
{
    ...
    reg_data.dev_id = 255;
    sx_cmd_set_op_tlv(&reg_data.op_tlv, MLXSW_HTGT_ID, 1); /* 1 - query */
    reg_data.htgt_reg.swid = 0;
    reg_data.htgt_reg.trap_group = bound_monitor_rdq->dqn;
    err = sx_ACCESS_REG_HTGT(dev, &reg_data);
    if (err) {
        sxd_log_err("failed to disconnect trap group %d \n", reg_data.htgt_reg.trap_group);
        goto out;
    }

    /* Stop the RDQ */
    sx_cmd_set_op_tlv(&reg_data.op_tlv, MLXSW_HTGT_ID, 2); /* 2 - write */
    switch (cpu_path_type) {
    case SXD_HTGT_TYPE_NULL_E:
        reg_data.htgt_reg.type = SXD_HTGT_TYPE_NULL_E;
        reg_data.htgt_reg.trap_group = bound_monitor_rdq->dqn;
        break;

    case SXD_HTGT_TYPE_LOCAL_E:
        reg_data.htgt_reg.type = SXD_HTGT_TYPE_LOCAL_E;
        reg_data.htgt_reg.trap_group = bound_monitor_rdq->dqn; /* trap group num = dqn */
        reg_data.htgt_reg.path.rdq = bound_monitor_rdq->dqn;
        reg_data.htgt_reg.path.cpu_tclass = bound_monitor_rdq->cpu_tclass;
        break;

    default:
        sxd_log_err("invalid path type %d \n", cpu_path_type);
        err = -EINVAL;
        goto out;
    }
    err = sx_ACCESS_REG_HTGT(dev, &reg_data);
...
static int __monitor_buff_data_handle(struct list_head *list_p,
                                      ...
                                      uint32_t         *user_buffer_size_list_p,
                                      uint32_t          start_idx,
                                      uint32_t          end_idx,
                                      uint32_t          user_requested_num)
{
    int               err = 0;
    int               copied_size = 0;
    ...
    for (i = start_idx; i < end_idx && i < user_requested_num; i++) {
        ...
        if (err) {
            sxd_log_err("%s(): get_edata_from_elist buf_len:%d err: %d \n",
        ...
        if (copied_size < 0) {
            err = copied_size;
            sxd_log_err("%s(): copy_edata failed. err: %d \n",
                        __func__, err);
            goto out;
        }
...
static int __monitor_file_read(struct sx_dev   *dev,
                               struct file     *filp,
                               enum SX_FILE_OP  file_op,
                               uint32_t        *buffers_count_p,
                               ...
                               uint32_t        *buffer_size_list_p)
{
    ...
    int                   err = 0, rc = 0;
    uint32_t              total_packets_cnt = 0;
    uint32_t              sw_total_packets_cnt = 0;
    ...
    if (buffers_count_p == NULL) {
        sxd_log_err("%s(): buffers_count_p is  NULL \n", __func__);
        err = -EINVAL;
        goto out;
    }
    ...
    /* if PCI profile is not set, no point to read from the monitor-RDQ */
    ...
    err = sx_trap_group_path_set(file->bound_monitor_rdq, SXD_HTGT_TYPE_NULL_E);
    if (err) {
        sxd_log_err("%s(): disconnect rdq %d from TG failed. err: %d \n",
    ...
    if (err) {
        sxd_log_err("Monitor-File-Read: failed to pause CQ %d\n", cq->cqn);
        goto out;
    }
    resume_cq = true;
    ...
    if (err) {
        sxd_log_err("%s(): simulate_monitor_data_rx_skb for rdq %d failed. err: %d \n",
                    __func__, file->bound_monitor_rdq->dqn, err);
        goto out;
    }
    ...
    if (tmp_buff_pp == NULL) {
        sxd_log_err(" Failed to allocate memory for local buffer tmp_buff_pp. \n");
        err = -ENOMEM;
        goto out;
    }
    ...
    for (i = 0; i < buffers_count; i++) {
        ...
        if (tmp_buff_pp[i] == NULL) {
            sxd_log_err(" Failed to allocate memory for local buffer tmp_buff_pp, index=%d.\n", i);
    ...
    err = __monitor_buff_data_handle(&file->evlist.list,
                                     ...
                                     buffers_count);
    if (err) {
        sxd_log_err("%s(): __monitor_buff_data_handle for regular monitor RDQ failed. err: %d \n",
                    __func__, err);
        spin_unlock_irqrestore(&file->lock, flags);
        goto out;
    }
    ...
    err = __monitor_buff_data_handle(&file->bound_monitor_rdq->sw_dup_evlist_p->list,
                                     ...
                                     buffers_count);
    if (err) {
        sxd_log_err("%s(): __monitor_buff_data_handle for monitor SW queue failed. err: %d \n",
                    __func__, err);
        spin_unlock_irqrestore(&file->lock, flags);
        goto out;
    }
...
out:
    ...
    if (reconnect_trap_group) {
        ...
        rc = sx_trap_group_path_set(file->bound_monitor_rdq, SXD_HTGT_TYPE_LOCAL_E);
        if (rc) {
            if (!err) { /* if no errors till we got here, put 'rc' in 'err' to return */
                err = rc;
            }

            sxd_log_err("%s(): connect rdq %d to TG failed. err: %d \n",
                        __func__, file->bound_monitor_rdq->dqn, err);
        }
    }
                                     ...
                                     struct sx_dev                   *dev)
{
    ...
    sxd_log_err("PSAMPLE is not supported on this kernel (CONFIG_PSAMPLE)\n");
    return -ENOTSUPP;
...
bool __per_dev_synd_cfg(struct sx_dev *dev, void *context)
{
    ...
    case SX_KU_USER_CHANNEL_TYPE_PHY_NETDEV:
        if (priv->profile.pci_profile.swid_type[ku->swid] != SX_KU_L2_TYPE_ETH) {
            sxd_log_err("%s: Adding phy netdev on non ethernet device is not implemented\n", __func__);
            err = -EFAULT;
            goto out;
        }

        if (cmd == CTRL_CMD_ADD_SYND) {
            sxd_log_debug("%s: Adding SX_KU_USER_CHANNEL_TYPE_PHY_NETDEV\n", __func__);
            err = sx_core_add_synd_phy(ku->swid, ku->syndrome_num, dev, &(ku->port_vlan_params), ku->is_register);
        } else {
            sxd_log_debug("%s: Removing SX_KU_USER_CHANNEL_TYPE_PHY_NETDEV\n", __func__);
    ...
    case SX_KU_USER_CHANNEL_TYPE_L2_NETDEV:
        if (priv->profile.pci_profile.swid_type[ku->swid] != SX_KU_L2_TYPE_ETH) {
            sxd_log_debug("%s: Adding L2 netdev on non ethernet device is not implemented\n", __func__);
            err = -EFAULT;
            goto out;
        }

        if (cmd == CTRL_CMD_ADD_SYND) {
            sxd_log_debug("%s: Adding SX_KU_USER_CHANNEL_TYPE_L2_NETDEV\n", __func__);
            err = sx_core_add_synd_l2(ku->swid, ku->syndrome_num, dev, &(ku->port_vlan_params), ku->is_register);
        } else {
            sxd_log_debug("%s: Removing SX_KU_USER_CHANNEL_TYPE_L2_NETDEV\n", __func__);
    ...
    case SX_KU_USER_CHANNEL_TYPE_PSAMPLE:
        ...
        if (err) {
            sxd_log_err("failed to %s psample listener (err=%d)\n",
    ...
    case SX_KU_USER_CHANNEL_TYPE_DROP_MONITOR:
        ...
        if (err) {
            sxd_log_err("failed to %s drop monitor listener (err=%d)\n",
                        (cmd == CTRL_CMD_ADD_SYND) ? "add" : "remove",
                        err);
            goto out;
        }
    ...
    default:
        sxd_log_err("Err: invalid channel type : %d \n", ku->channel_type);
        err = -EINVAL;
        goto out;
    }
...
long ctrl_cmd_monitor_sw_queue_synd(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if (err) {
        sxd_log_err("Failed to copy data provided by user, err = %d.\n", err);
        goto out;
    }
    ...
    if (ku.monitor_hw_trap_group >= priv->dev_cap.max_num_rdqs) {
        sxd_log_err("CTRL_CMD_MONITOR_SYND: RDQ %d is out of range\n",
    ...
    if (!monitor_dq) {
        sxd_log_err("CTRL_CMD_MONITOR_SYND: RDQ %d is not valid\n",
                    ku.monitor_hw_trap_group);
        return -EINVAL;
    }
    ...
    if (monitor_dq->file_priv_p == NULL) {
        if (ku.cmd) {
            sxd_log_err("CTRL_CMD_MONITOR_SYND: monitor_dq->file_priv_p is NULL for hw trap group %d\n",
                        ku.monitor_hw_trap_group);
            return -EINVAL;
        } else {
            /*
             ...
             *
             */
            sxd_log_info("CTRL_CMD_MONITOR_SYND: monitor_dq->file_priv_p is NULL for hw trap group %d\n",
                         ku.monitor_hw_trap_group);
            return 0;
        }
    }
    ...
    err = sx_core_ioctl_synd_param_prepare(ku.type, &listener_type);
    if (err) {
        sxd_log_err("Failed to prepare listener type attribute for type %u, err = %u.\n", ku.type, err);
        goto out;
    }

    if (ku.cmd == true) { /* Add listener which will store matched packet to SW buffer of monitor RDQ */
        snprintf(synd_str, sizeof(synd_str) - 1, "WJH-queue (RDQ: %d)", monitor_dq->dqn);
        err = sx_core_add_synd(ku.swid,
                               ku.syndrome_num,
                               listener_type,
                               current->pid,
                               current->comm,
                               ...
                               &ku.critireas,
                               synd_str,
                               __sx_cq_monitor_sw_queue_handler,
                               monitor_dq->file_priv_p->owner,
                               /* use required filp */
                               CHECK_DUP_ENABLED_E,
                               dev,
                               &ku.port_vlan_params,
    ...
    if (err) {
        sxd_log_err("Failed to %s listener, err = %u.\n", ((ku.cmd == true) ? "add" : "remove"), err);
        goto out;
    }
...
long ctrl_cmd_multi_packet_enable(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if ((data != false) && (data != true)) {
        sxd_log_warning("ioctl MULTI_PACKET_ENABLE: " \
                        "error data = %lu > 1\n", data);
        return -EINVAL;
    }
...
long ctrl_cmd_set_owner(struct file *file, unsigned int cmd, unsigned long data)
{
    struct sx_rsc *rsc = file->private_data;

    sx_core_set_owner(rsc);
    return 0;
}
...
long ctrl_cmd_raise_event(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if ((event_data.buffer_size == 0) ||
        ...
        sxd_log_warning("sx_raise_event: Bad parameters\n");
        err = -EINVAL;
        goto out_err;
    }
...
long ctrl_cmd_set_rdq_timestamp_state(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if (!dev->pdev) {
        sxd_log_debug("will not set time stamp state since there's no PCI device\n");
        goto out;
    }
    ...
    if ((rdq_ts_state.rdq < 0) || (rdq_ts_state.rdq >= rdq_num)) {
        sxd_log_warning("Cannot set time stamp state, RDQ value (%d) is not valid\n",
                        rdq_ts_state.rdq);
        err = -EINVAL;
        goto out;
    }
...
long ctrl_cmd_set_rdq_cpu_priority(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if (!dev->pdev) {
        sxd_log_debug("will not set CPU priority since there's no PCI device\n");
        goto out;
    }
    ...
    if ((rdq_cpu_prio.rdq < 0) || (rdq_cpu_prio.rdq >= rdq_num)) {
        sxd_log_warning("Cannot set CPU priority, RDQ value (%d) is not valid\n",
                        rdq_cpu_prio.rdq);
        err = -EINVAL;
        goto out;
    }
...
long ctrl_cmd_set_truncate_params(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if (!dev->pdev) {
        sxd_log_debug("will not set truncate params since there's no "
                      "PCI device\n");
        goto out;
    }

    if (truncate_params.rdq >= priv->dev_cap.max_num_rdqs) {
        sxd_log_err("CTRL_CMD_SET_TRUNCATE_PARAMS: RDQ %d is not valid\n",
                    truncate_params.rdq);
        return -EINVAL;
    }
    ...
    if (truncate_params.truncate_enable) {
        if (truncate_params.truncate_size < SX_TRUNCATE_SIZE_MIN) {
            sxd_log_err("CTRL_CMD_SET_TRUNCATE_PARAMS: Truncate size %u is not valid\n",
                             ...
                             struct ku_set_monitor_rdq_params *monitor_rdq_params)
{
    ...
    if (!dev->pdev) {
        sxd_log_debug("will not set monitor rdq since there's no PCI device\n");
        err = -ENODEV;
        goto out;
    }

    if (monitor_rdq_params->rdq >= priv->dev_cap.max_num_rdqs) {
        sxd_log_err("Set-Monitor-RDQ: RDQ %d is out of range\n", monitor_rdq_params->rdq);
        err = -EINVAL;
        goto out;
    }

    if (monitor_rdq_params->is_monitor && monitor_rdq_params->del_all_fd_listeners) {
        sxd_log_err("Set-Monitor-RDQ: will delete listeners only if RDQ is set from monitoring to normal\n");
        err = -EINVAL;
        goto out;
    }
    ...
    if (!dq) {
        sxd_log_err("Set-Monitor-RDQ: RDQ %d is not valid\n", monitor_rdq_params->rdq);
    ...
    err = sx_cq_pause(cq, priv);
    if (err) {
        sxd_log_err("Set-Monitor-RDQ: failed to pause CQ %d (RDQ %d)\n", cq->cqn, dq->dqn);
        goto out;
    }
    ...
    if (monitor_rdq_params->del_all_fd_listeners) {
        sxd_log_notice("Set-Monitor-RDQ: Requested to remove all RDQ-owner-file listeners\n");
    ...
    if (monitor_rdq_params->is_monitor) {
        ...
        if (rsc->bound_monitor_rdq && (rsc->bound_monitor_rdq != dq)) {
            sxd_log_err("Set-Monitor-RDQ: Trying to bind file %p to RDQ %d but it is already bound to RDQ %d\n",
                        file,
                        monitor_rdq_params->rdq,
        ...
        if (dq->is_monitor) {
            if (dq->file_priv_p->owner == rsc->owner) { /* same owner, do nothing */
                ...
                sxd_log_notice("Set-Monitor-RDQ: Trying to do set over set on "
                               "RDQ %d with RDQ owner file=%p)\n", monitor_rdq_params->rdq, dq->file_priv_p->owner);
                goto out;
            }

            sxd_log_notice("Set-Monitor-RDQ: Replacing monitoring RDQ %d owner file from %p to %p\n",
                           monitor_rdq_params->rdq,
        ...
        if (dq->file_priv_p->bound_monitor_rdq->sw_dup_evlist_p == NULL) {
            sxd_log_err("Failed to allocate memory for SW duplication queue.\n");
            spin_unlock_irqrestore(&priv->rdq_table.lock, flags);
            err = -ENOMEM;
            goto out;
        }
        ...
        if (err) {
            sxd_log_err("Failed to add RDQ %d to monitoring RDQ list (err=%d)\n", dq->dqn, err);
        ...
        sxd_log_notice("Set-Monitor-RDQ: Setting RDQ %d from monitoring to normal (RDQ owner file=%p)\n",
                       monitor_rdq_params->rdq,
                       dq->file_priv_p->owner);
...
out:
    ...
    if (remove_from_ignore_list) {
        /* When setting a RDQ from monitoring to normal, we remove it from ignore list.
         *  The return value is not checked because it may fail due to the health check is not enabled yet. */
        sx_health_update_tg(dev->device_id, dq->dqn, false, true);
    }
...
long ctrl_cmd_set_monitor_rdq(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if (err) {
        sxd_log_err("Set-Monitor-RDQ: failed to get user configuration\n");
        goto out;
    }
    ...
    sxd_log_notice("Set-Monitor-RDQ: Agent=[%s/%d], file=%p, RDQ=%d, Monitoring=%s, CPU-TClass=%u\n",
                   agent,
                   agent_pid,
                   file,
                   monitor_rdq_params.rdq,
    ...
    if (err) {
        sxd_log_err("Set-Monitor-RDQ: operation failed (err=%d)\n", err);
    }
...
long ctrl_cmd_set_rdq_filter_ebpf_prog(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if (!dev->pdev) {
        sxd_log_debug("will not set rdq filter ebpf program since there's no PCI device\n");
        goto out;
    }

    if (rdq_filter_ebpf_prog_params.rdq >= priv->dev_cap.max_num_rdqs) {
        sxd_log_err("CTRL_CMD_SET_RDQ_FILTER_EBPF_PROG: RDQ %d is out of range\n",
    ...
    if (!dq) {
        sxd_log_err("CTRL_CMD_SET_RDQ_FILTER_EBPF_PROG: RDQ %d is not valid\n",
                    rdq_filter_ebpf_prog_params.rdq);
        spin_unlock_irqrestore(&priv->rdq_table.lock, flags);
        err = -EINVAL;
        goto out;
    ...
    if (rdq_filter_ebpf_prog_params.is_attach) {
        if (rdq_filter_ebpf_prog_params.ebpf_prog_fd < 0) {
            sxd_log_err("CTRL_CMD_SET_RDQ_FILTER_EBPF_PROG: eBPF program file descriptor %d is invalid\n",
        ...
        if (priv->filter_ebpf_progs[rdq_filter_ebpf_prog_params.rdq] != NULL) {
            sxd_log_info("CTRL_CMD_SET_RDQ_FILTER_EBPF_PROG: RDQ %d already has an eBPF program attached, "
        ...
        if (IS_ERR(bpf_prog_p)) {
            sxd_log_err("CTRL_CMD_SET_RDQ_FILTER_EBPF_PROG: failed to get ebpf program\n");
            spin_unlock_irqrestore(&priv->rdq_table.lock, flags);
        ...
        if (priv->filter_ebpf_progs[rdq_filter_ebpf_prog_params.rdq] == NULL) {
            sxd_log_info("CTRL_CMD_SET_RDQ_FILTER_EBPF_PROG: RDQ %d has no eBPF program attached to it.\n",
                         rdq_filter_ebpf_prog_params.rdq);
            spin_unlock_irqrestore(&priv->rdq_table.lock, flags);
            goto out;
        }
...
long ctrl_cmd_set_rdq_agg_ebpf_prog(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if (!dev->pdev) {
        sxd_log_debug("will not set rdq aggregation ebpf program since there's no PCI device\n");
        goto out;
    }

    if ((rdq_agg_ebpf_prog_params.rdq >= priv->dev_cap.max_num_rdqs) ||
        (rdq_agg_ebpf_prog_params.rdq < 0)) {
        sxd_log_err("CTRL_CMD_SET_RDQ_AGG_EBPF_PROG: RDQ %d is out of range\n",
    ...
    if ((rdq_agg_ebpf_prog_params.index >= SX_AGG_EBPF_PROG_NUM_PER_RDQ) ||
        ...
        sxd_log_err("CTRL_CMD_SET_RDQ_AGG_EBPF_PROG: index %d is out of range\n",
    ...
    if (!dq) {
        sxd_log_err("CTRL_CMD_SET_RDQ_AGG_EBPF_PROG: RDQ %d is not valid\n",
                    rdq);
        spin_unlock_irqrestore(&priv->rdq_table.lock, flags);
        err = -EINVAL;
        goto out;
    ...
    if (rdq_agg_ebpf_prog_params.is_attach) {
        if (rdq_agg_ebpf_prog_params.ebpf_prog_fd < 0) {
            sxd_log_err("CTRL_CMD_SET_RDQ_AGG_EBPF_PROG: eBPF program file descriptor %d is invalid\n",
        ...
        if (priv->agg_ebpf_progs[rdq][index] != NULL) {
            sxd_log_info("CTRL_CMD_SET_RDQ_AGG_EBPF_PROG: RDQ %d index %d already has an eBPF program attached, "
                         "detach it and attach the new one.\n",
                         rdq,
                         index);
        ...
        if (IS_ERR(bpf_prog_p)) {
            sxd_log_err("CTRL_CMD_SET_RDQ_AGG_EBPF_PROG: failed to get ebpf program\n");
            spin_unlock_irqrestore(&priv->rdq_table.lock, flags);
            err = PTR_ERR(bpf_prog_p);
            goto out;
        }
        ...
        if (priv->agg_ebpf_progs[rdq][index] == NULL) {
            sxd_log_info("CTRL_CMD_SET_RDQ_AGG_EBPF_PROG: RDQ %d index %d has no eBPF program attached to it.\n",
                         rdq,
                         index);
            spin_unlock_irqrestore(&priv->rdq_table.lock, flags);
            goto out;
...
long ctrl_cmd_read_multi(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if (!dev->pdev) {
        sxd_log_debug("will not execute multi packets read since there's no PCI device\n");
        goto out;
    }

    read_multi_params_p = vmalloc(sizeof(*read_multi_params_p));
    if (!read_multi_params_p) {
        sxd_log_debug("can't vmalloc read_multi_params_p\n");
        err = -ENOMEM;
        goto out;
    }
    ...
    if (rsc->bound_monitor_rdq) {
        ...
        if (err) {
            sxd_log_err("ioctl_read failed err:%d, buf_cnt:%d, buff_list:%p, buff_list_cnt:%p\n",
                        err,
        ...
        sxd_log_debug("multi_read from fd which is not monitor isn't supported .\n");
        err = -EINVAL;
    }
...
long ctrl_cmd_get_rdq_stat(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if (err) {
        sxd_log_err("Failed to copy provided by user data.\n");
        goto out;
    }
    ...
    if (ku.monitor_hw_trap_group >= priv->dev_cap.max_num_rdqs) {
        sxd_log_err("CTRL_CMD_GET_RDQ_STAT: RDQ %d is out of range\n",
                    ku.monitor_hw_trap_group);
        err = -EINVAL;
        goto out;
    }
    ...
    if (!monitor_dq) {
        sxd_log_err("CTRL_CMD_GET_RDQ_STAT: RDQ %d is not valid\n",
                    ku.monitor_hw_trap_group);
        err = -EINVAL;
        goto out;
    }
    ...
    if (err) {
        sxd_log_err("Failed to copy data to user, err = %d.\n", err);
        goto out;
    }
...
out:
    return err;
#else
    sxd_log_info("ioctl ignored because offload_fwd_mark_en module param not avail in this kernel version\n");
    return 0;
#endif
}
...
long ctrl_cmd_trap_filter_add(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if (filter_data.trap_id >= NUM_HW_SYNDROMES) {
        sxd_log_err("Received TRAP ID %d "
                    "is invalid\n", filter_data.trap_id);
        err = -EINVAL;
        goto out;
    }
    ...
    if (filter_data.is_lag) {
        if (filter_data.lag_id >= lag_max) {
            sxd_log_err("Received LAG ID 0x%x "
                        "is invalid\n",
                        filter_data.lag_id);
        ...
        if (idx == -1) {
            sxd_log_err("Cannot add LAG ID 0x%x to trap %d filter "
            ...
            sx_priv(dev)->lag_filter_db[filter_data.trap_id][idx] =
                filter_data.lag_id;
            sxd_log_info("LAG ID %u was added to filter list "
                         "of trap ID 0x%x\n", filter_data.lag_id,
                         filter_data.trap_id);
        }
    } else {
        for (i = 0; i < MAX_SYSTEM_PORTS_IN_FILTER; i++) {
            if (sx_priv(dev)->sysport_filter_db[filter_data.trap_id][i] ==
                filter_data.sysport) {
                sxd_log_err("Received system port 0x%x "
                            "is already in the filter list "
                            "of trap ID 0x%x\n",
                            filter_data.sysport,
                            filter_data.trap_id);
        ...
        if (idx == -1) {
            sxd_log_err("Cannot add system port 0x%x to trap %d filter "
                        "because DB filter DB for this trap is full\n",
            ...
            sx_priv(dev)->sysport_filter_db[filter_data.trap_id][idx] =
                filter_data.sysport;
            sxd_log_info("system port 0x%x was added to filter "
...
long ctrl_cmd_trap_filter_remove(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if (filter_data.is_lag) {
        ...
        for (i = 0; i < MAX_LAG_PORTS_IN_FILTER; i++) {
            if (sx_priv(dev)->lag_filter_db[filter_data.trap_id][i] ==
                ...
                sx_priv(dev)->lag_filter_db[filter_data.trap_id][i] =
                    LAG_ID_INVALID;
                sxd_log_info("LAG ID %u was removed from filter list "
                             "of trap ID 0x%x\n", filter_data.lag_id,
                             filter_data.trap_id);
                break;
            }
        ...
        if (i == MAX_LAG_PORTS_IN_FILTER) {
            sxd_log_err("Cannot find LAG ID 0x%x in trap ID %d filter DB\n",
                        filter_data.lag_id, filter_data.trap_id);
            err = -EINVAL;
        }
    } else {
        for (i = 0; i < MAX_SYSTEM_PORTS_IN_FILTER; i++) {
            if (sx_priv(dev)->sysport_filter_db[filter_data.trap_id][i] ==
                ...
                sxd_log_info("system port 0x%x was removed from filter "
                             "list of trap ID 0x%x\n", filter_data.sysport,
                             filter_data.trap_id);
                break;
            }
        ...
        if (i == MAX_SYSTEM_PORTS_IN_FILTER) {
            sxd_log_err("Cannot find system port 0x%x in trap ID %d "
                        "filter DB\n", filter_data.sysport, filter_data.trap_id);
            err = -EINVAL;
        }
    }
...
long ctrl_cmd_trap_filter_remove_all(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    spin_unlock_irqrestore(&sx_priv(dev)->db_lock, flags);
    sxd_log_info("Removed all ports and LAGs from the filter list "
                 "of trap ID %d\n", filter_data.trap_id);
...
long ctrl_cmd_set_fd_attributes(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if (fd_attributes_data.queue_type > KU_QUEUE_TYPE_MAX) {
        sxd_log_err("Received Queue Type %d "
