...
static int __create_caches(void)
{
    ...
    if (!__records_cache) {
        sxd_log_err("failed to allocate memory cache for PTP TX events\n");
        goto error;
    }
    ...
    if (!__rx_event_cache) {
        sxd_log_err("failed to allocate memory cache for PTP RX events\n");
        goto error;
    }
...
int sx_ptp_dev_init_spc1(struct sx_priv *priv, ptp_mode_t ptp_mode)
{
    ...
    if ((ptp_mode != KU_PTP_MODE_POLLING) &&
        ...
        sxd_log_err("invalid PTP mode for SPC: %u\n", ptp_mode);
        return -EINVAL;
    }
    ...
    if (err) {
        sxd_log_err("Failed to configure register MOGCR\n");
        goto out;
    }
    ...
    if (ptp_mode == KU_PTP_MODE_POLLING) {
        if (atomic_inc_return(&__ptp_rx_polling_thread_users) == 1) { /* first one here */
            ...
            if (IS_ERR(__ptp_rx_polling_thread)) {
                ...
                sxd_log_err("failed to create PTP RX polling thread (err=%d)\n", err);
