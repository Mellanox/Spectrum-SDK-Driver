...
int __module_log_dump(struct seq_file *m, void *v, void *context)
{
    struct sx_priv *priv = NULL;
    struct sx_dev  *dev = sx_dbg_dump_get_device(m);
    ...
    if (priv->module_log_init) {
        seq_printf(m, "Internal module log dump\n");
        seq_printf(m, "---------------------------\n");
        sx_internal_log_dump(&priv->module_log, m);
        seq_printf(m, "\n\n");
    }
...
bool __init sx_dbg_init(struct sx_dev *dev, void *context)
{
    ...
    sx_dbg_dump_read_handler_register("module_log_dump", __module_log_dump, NULL, dev, NULL);
