...
bad_emad_frame:
    sxd_log_err("EMAD bad RX frame [%s]!\n", reason);
...
static struct sx_emad_transaction * __tid_lookup(struct sx_priv *priv, sx_emad_tid_t tid)
{
    ...
    if (list_empty(&priv->emad.transactions_list)) {
        sxd_log_err("EMAD TID 0x%llx is unsolicited: there is no in-flight transaction (dev_id=%u)\n",
    ...
    if (!__emad_transaction_in_range(tid, head_tr->tid, tail_tr->tid)) {
        sxd_log_err("EMAD TID 0x%llx is unsolicited: transaction is out of in-flight range "
    ...
    list_for_each_entry_safe(emad_tr, tmp, &priv->emad.transactions_list, list) {
         ...
         * */
        sxd_log_err("EMAD transaction reply is out-of-order! "
...
static void __emad_rx(struct completion_info *ci, void *context)
{
    ...
    if (rx_dump) {
        print_hex_dump_debug("RX-EMAD    ", DUMP_PREFIX_OFFSET, 16, 1, skb->data, skb->len, 0);
    }
    ...
    if ((emad_tr->status.fw_status != SX_EMAD_STATUS_OK_E) && !emad_tr->in_params->silent) {
        sxd_log_err(
            "EMAD transaction FW error (tid=0x%llx, reg_id=0x%x, dev_id=%u, method=%u, priority=%u, fw_status=%u, fw_cause='%s')\n",
            emad_tr->tid,
            emad_tr->reg_id,
...
static int __send_emad(struct sx_dev *dev, struct sx_emad_transaction *emad_tr, struct sx_emad_operation_tlv *op_tlv)
{
    ...
    if (tx_dump) {
        print_hex_dump_debug("TX-EMAD    ", DUMP_PREFIX_OFFSET, 16, 1, skb->data, skb->len, 0);
    }
                       ...
                       u32                          timeout_usec)
{
    ...
    sxd_log_err("EMAD TIMEOUT [tid:%llx, method=%u, reg_id=%04x]\n",
                emad_tr->tid,
                emad_tr->method,
                emad_tr->reg_id);

    sx_health_report_error_emad_timeout(priv->dev.device_id, emad_tr->reg_id, timeout_usec, "SXD_driver");
...
int sx_emad_dev_init(struct sx_dev *dev)
{
    ...
    if (err) {
        sxd_log_err("failed to initialize EMAD on device %u\n", dev->device_id);
        goto out;
    }
    ...
    case SXD_MGIR_HW_DEV_ID_SWITCH_IB2:
        sxd_log_notice("device %d does not support EMAD\n", dev->device_id);
    ...
    sxd_log_info("EMAD is ready on device %u [string_tlv=%s, latency_tlv=%s]\n",
                 dev->device_id,
    ...
    err = sx_core_add_synd(0, /* swid */
                           ...
                           1 /* is register? */);
    if (err) {
        sxd_log_err("failed to register EMAD handler on device %u\n", dev->device_id);
        goto out;
    }
...
void sx_emad_dev_deinit(struct sx_dev *dev)
{
    ...
    do {
        ...
        if (!busy) {
            sxd_log_info("device %u does not have pending EMADs\n", dev->device_id);
            break;
        }

        sxd_log_warning("device %u has pending EMADs, waiting ...\n", dev->device_id);
        msleep(100);
    ...
    err = sx_core_remove_synd(0, /* swid */
                              ...
                              NULL);
    if (err) {
        sxd_log_err("failed to unregister EMAD handler on device %u\n", dev->device_id);
    }
}
...
static int __emad_from_driver_dump(struct seq_file* m, void *v, void* context)
{
    ...
    seq_printf(m, "EMAD from driver - Device %u\n", dev->device_id);
    sx_dbg_dump_print_empty_line(m);
    seq_printf(m, "Supports String TLV: %s\n", ((priv->emad.flags & SX_EMAD_F_STRING_TLV) ? "True" : "False"));
    seq_printf(m, "Supports Latency TLV: %s\n", ((priv->emad.flags & SX_EMAD_F_LATECNY_TLV) ? "True" : "False"));
    sx_dbg_dump_print_empty_line(m);

    seq_printf(m, "%-20s    %-18s    %-6s    %-6s    %-9s    %-8s    %-15s\n",
    ...
    list_for_each_entry(emad_tr, &priv->emad.transactions_list, list) {
        ...
        seq_printf(m, "%-20s    0x%016llx    0x%04x    %-6s    %-9s    %-8s    %-15lu\n",
                   agent,
                   emad_tr->tid,
                   emad_tr->reg_id,
...
int sx_emad_init(void)
{
    ...
    if (!__emad_transaction_cache) {
        sxd_log_err("failed to create EMAD transaction cache\n");
        err = -ENOMEM;
        goto out;
    }
...
{
    sx_dbg_dump_read_handler_register("emad_from_driver", __emad_from_driver_dump, NULL, dev, NULL);
    return true;
}

bool sx_emad_dump_hanlders_per_device_deinit(struct sx_dev *dev, void *context)
{
    sx_dbg_dump_read_handler_unregister("emad_from_driver", dev);
    return true;
}
