...
static int __sx_af_dbg_info_mafri_record_dump(struct seq_file *m, void *v, void *context, cbuf_record_t *record_p);
...
static inline int __sx_af_dbg_info_db_init(af_dbg_db_t *db_p, unsigned int size_in_bytes)
{
    ...
    if (db_p->cbuf.buf == NULL) {
        err = -ENOMEM;
        sxd_log_err("%s: failed to allocate memory for Accumulated counters debug buffer, err [%d]\n",
                    __func__, err);
        goto out;
    }
...
int sx_af_dbg_info_clear(void)
{
    ...
    sxd_log_notice("Clearing Accumulated counters' debug DB\n");
...
static void __sx_af_dbg_info_drop(void)
{
    ...
    if (count < sizeof(cbuf_record_t)) {
        sxd_log_rl_err(
            "Cannot drop a record, buffer is filled with something [%d] that is smaller than one record [%lu]\n",
            count,
...
int sx_af_dbg_info_push(cbuf_record_t *record_p)
{
    ...
    if (CIRC_SPACE(g_af_dbg_db.cbuf.head, g_af_dbg_db.cbuf.tail, g_af_dbg_db.size) >= sizeof(cbuf_record_t)) {
        ...
        sxd_log_rl_err("Cannot save a record, the buffer is full.\n");
        g_af_dbg_db.drops[record_p->type]++;
    }
...
static int __sx_af_dbg_info_db_mem_preallocate(af_dbg_db_t **db_p)
{
    ...
    *db_p = vmalloc(sizeof(af_dbg_db_t));
    if (*db_p == NULL) {
        err = -ENOMEM;
        sxd_log_err("%s: failed to allocate memory for a copy of Accumulated counters debug DB, err [%d]\n",
                    __func__, err);
        goto out;
    }
    ...
    if ((*db_p)->cbuf.buf == NULL) {
        err = -ENOMEM;
        sxd_log_err("failed to allocate memory for a copy of Accumulated counters debug buffer, err [%d]\n",
                    err);
        goto out;
    }
...
static int __sx_af_dbg_info_mafbi_record_dump(struct seq_file *m, void *v, void *context, cbuf_record_t *record_p)
{
    ...
    for (index = 0; index < num_rec; index++) {
        seq_printf(m, "\t\t\t %u \t\t %u \t\t %u \t %u \n", index,
                   mlxsw_reg_mafbi_counter_index_get(record_p->data.mafbi_reg.reg, 0, index),
                   mlxsw_reg_mafbi_bytes_inc_get(record_p->data.mafbi_reg.reg, 0, index),
                   mlxsw_reg_mafbi_packets_inc_get(record_p->data.mafbi_reg.reg, 0, index));
    }
...
static int __sx_af_dbg_info_mafti_record_dump(struct seq_file *m, void *v, void *context, cbuf_record_t *record_p)
{
    ...
    for (index = 0; index < num_rec; index++) {
        seq_printf(m, "\t\t\t %u \t\t %u \t\t %u \t %u \n", index,
                   mlxsw_reg_mafti_counter_index_get(record_p->data.mafti_reg.reg, 0, index),
                   mlxsw_reg_mafti_bytes_inc_get(record_p->data.mafti_reg.reg, 0, index),
                   mlxsw_reg_mafti_packets_inc_get(record_p->data.mafti_reg.reg, 0, index));
    }

    if (sx_af_counters_dbg_info_print_in_hex) {
        seq_printf(m, "\t\tHex: \n");
        dword_p = (u32 *)record_p->data.mafti_reg.reg;
        seq_printf(m, "\t\t\t offset \t dword 0 \t dword 1 \t dword 2 \t dword 3 \n");
...
static int __sx_af_dbg_info_mafri_record_dump(struct seq_file *m, void *v, void *context, cbuf_record_t *record_p)
{
    ...
    num_rec = mlxsw_reg_mafri_num_rec_get(record_p->data.mafri_reg.reg);
    seq_printf(m, "\t\tNumber of records: %u \n", num_rec);
    base = mlxsw_reg_mafri_counter_index_base_get(record_p->data.mafri_reg.reg),
    seq_printf(m, "\t\tIndex of the first counter: %u \n", base);
    seq_printf(m, "\t\t\t record_idx \t counter_idx \t bytes \t packets \n");
    for (index = 0; index < num_rec; index++) {
        seq_printf(m, "\t\t\t %u \t\t %u \t %u \n", index,
                   mlxsw_reg_mafri_bytes_inc_get(record_p->data.mafri_reg.reg, 0, index),
                   mlxsw_reg_mafri_packets_inc_get(record_p->data.mafri_reg.reg, 0, index));
    }

    if (sx_af_counters_dbg_info_print_in_hex) {
        seq_printf(m, "\t\tHex: \n");
        dword_p = (u32 *)record_p->data.mafri_reg.reg;
        seq_printf(m, "\t\t\t offset \t dword 0 \t dword 1 \t dword 2 \t dword 3 \n");
        for (index = 0; index < MLXSW_MAFRI_LEN / 16; index++) {
            seq_printf(m, "\t\t\t %04x: \t\t %08x \t %08x \t %08x \t %08x \n", index, *(dword_p + (4 * index)),
                       *(dword_p + (4 * index) + 1), *(dword_p + (4 * index) + 2), *(dword_p + (4 * index) + 3));
        }
    }
...
static int __sx_af_dbg_info_record_dump(struct seq_file *m, void *v, void *context, cbuf_record_t *record_p)
{
    ...
    case CBUF_RECORD_TYPE_MAFRI_E:
        __sx_af_dbg_info_mafri_record_dump(m, v, context, record_p);
        break;
...
static int __sx_af_dbg_info_records_dump(struct seq_file *m, void *v, void *context, af_dbg_db_t *db_p)
{
    ...
    do {
        ...
        if (count < sizeof(cbuf_record_t)) {
            sxd_log_rl_err(
                "Cannot dump a debug record[%d], the buffer is filled with something [%d] that is smaller than one record [%lu]\n",
                record_idx,
                count,
                sizeof(cbuf_record_t));
            break;
