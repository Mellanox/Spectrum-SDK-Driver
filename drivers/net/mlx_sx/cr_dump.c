...
static int __fw_crdump_precheck(struct sx_dev *dev, dma_addr_t dma_addr, u8 opcode, int dump_size)
{
    ...
    case SX_CR_DUMP_OP_START_FLAT:
        ...
        cap_dump_host_size = priv->cr_dump_info.cap_dump_host_size_flat;
        break;

    case SX_CR_DUMP_OP_START_REDUCED_FLAT:
        priv->cr_dump_info.dumped_bytes = 0;
        cap_dump_host_size = priv->cr_dump_info.cap_dump_host_size_reduced_flat;
        break;

    case SX_CR_DUMP_OP_START_GW:
        priv->cr_dump_info.dumped_bytes = 0;
        cap_dump_host_size = ROUNDUP(priv->cr_dump_info.cap_dump_host_size_gw, SX_CR_DUMP_MEMBLK_SIZE);
        break;

    case SX_CR_DUMP_OP_START_GDB:
        priv->cr_dump_info.dumped_bytes = 0;
        cap_dump_host_size = ROUNDUP(priv->cr_dump_info.cap_dump_host_size_gdb, SXD_CR_DUMP_HOST_MEM_ALIGN);
    ...
    if (priv->cr_dump_info.dumped_bytes + dump_size > cap_dump_host_size) {
        sxd_log_err(
