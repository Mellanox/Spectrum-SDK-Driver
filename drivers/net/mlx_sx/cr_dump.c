...
static int __fw_crdump_precheck(struct sx_dev *dev, dma_addr_t dma_addr, u8 opcode, int dump_size)
{
    ...
    case SX_CR_DUMP_OP_START_GW:
        __dumped_bytes = 0;
        cap_dump_host_size = ROUNDUP(__cap_dump_host_size_gw, SX_CR_DUMP_MEMBLK_SIZE);
        break;
...
void sx_core_cr_dump_notify_dump_completion(struct sx_dev *dev, bool query, fw_dump_completion_state_t* state)
{
    u32             complete_dword = 0;
    u8              fw_dump_completion_state = 0;
    struct sx_priv *priv = sx_priv(dev);
    u32             cr_dump_offset = priv->fw.cr_dump_offset;
    ...
    if (is_sgmii_supported()) {
        ...
        sx_dpt_cr_space_read(dev->device_id, BASE_INFO_PTR_MEM, (unsigned char*)&cr_dump_offset,
                             sizeof(cr_dump_offset));
        cr_dump_offset = be32_to_cpu(cr_dump_offset);
        sx_dpt_cr_space_read(dev->device_id,
                             cr_dump_offset + 4,
                             (unsigned char*)&cr_dump_offset,
                             sizeof(cr_dump_offset));
        cr_dump_offset = be32_to_cpu(cr_dump_offset);
    }

    if (!cr_dump_offset) {
        /* On some devices, older FW doesn't support cr_dump addresses. In this case, Query_FW returns 0
         * for cr_dump_offset, and we simply return FW_DUMP_COMPLETION_STATE_IDLE for backwards compatibility. */
        *state = FW_DUMP_COMPLETION_STATE_IDLE;
        return;
    }

    sx_dpt_cr_space_read(dev->device_id,
                         cr_dump_offset + SX_CR_DUMP_CMD_MEM_OPCODE_COMPLETION_OFFSET,
                         (unsigned char*)&complete_dword,
                         sizeof(complete_dword));
    complete_dword = be32_to_cpu(complete_dword);

    if (query) {
        /* Read the current completion state. */
        complete_dword = complete_dword & 0x0F000000;
        fw_dump_completion_state = (complete_dword >> 24) & 0x3;
        *state = fw_dump_completion_state;
        return;
    }

    /* Change only the completion state. */
    complete_dword &= 0xF0FFFFFF;
    complete_dword |= (*state & 0x3) << 24;
    complete_dword = cpu_to_be32(complete_dword);
    sx_dpt_cr_space_write(dev->device_id,
                          cr_dump_offset + SX_CR_DUMP_CMD_MEM_OPCODE_COMPLETION_OFFSET,
                          (unsigned char*)&complete_dword,
                          sizeof(complete_dword));
}
...
int sx_core_cr_dump_init(struct sx_priv *priv)
{
    ...
    if (!priv->cr_dump_base) {
        printk(KERN_ERR "Couldn't map cr dump command mem register, aborting.\n");
        return -ENOMEM;
    }
