...
static u32               __cap_dump_host_size_reduced_flat = SX_CR_DUMP_DEFAULT_HOST_SIZE;
...
static int __fw_crdump_precheck(struct sx_dev *dev, dma_addr_t dma_addr, u8 opcode, int dump_size)
{
    ...
    case SX_CR_DUMP_OP_START_REDUCED_FLAT:
        __dumped_bytes = 0;
        cap_dump_host_size = __cap_dump_host_size_reduced_flat;
        break;
...
int sx_core_cr_dump_init(struct sx_priv *priv)
{
    ...
    if (priv->fw.cap_dump_host_size_reduced_flat > 0) {
        __cap_dump_host_size_reduced_flat = priv->fw.cap_dump_host_size_reduced_flat;
    }
    ...
    sxd_log_notice(
        "cr_dump init: cap_dump_host_size_flat:%u, cap_dump_host_size_reduced_flat:%u, cap_dump_host_size_gw:%u, cap_dump_host_size_gdb:%u.\n",
        __cap_dump_host_size_flat,
        __cap_dump_host_size_reduced_flat,
