...
int bulk_cntr_db_add(struct ku_bulk_cntr_transaction_add *bulk_cntr_tr_add)
{
    ...
    /* we return BUSY when -
     * 1. When same bulk type is in progress or if HFT is in progress or if no more transactions are allowed,
     * 2. when another bulk type is in progress and a HFT bulk type is requested.
     */
    if ((__in_progress[type] != NULL) || (__in_progress[SXD_BULK_CNTR_KEY_TYPE_HFT_E] != NULL) ||
        (__in_progress_cnt == BULK_CNTR_MAX_IN_PROGRESS)) {
        err = -EBUSY;
        goto out;
    } else if ((__in_progress_cnt != 0) && (type == SXD_BULK_CNTR_KEY_TYPE_HFT_E)) {
        err = -EBUSY;
        goto out;
    }
...
static void __bulk_cntr_db_del(uint32_t                  type,
                               struct transaction_entry *entry,
                               pid_t                     client_pid,
                               unsigned long             buffer_id)
{
    /* Decrement reference to shared memory. */
...
int bulk_cntr_db_del(pid_t client_pid, unsigned long buffer_id)
{
    ...
    /* delete the transaction from DB and  decrement reference for shared memory so that memory is
     * deallocated if application has exited, after bulk transaction is triggered.
     */
    __bulk_cntr_db_del(type, entry, client_pid, buffer_id);
                          ...
                          uint32_t                         *cookie)
{
    ...
    /* delete the transaction from DB and decrement reference for shared memory so that memory is deallocated if application
     * has exited, after bulk transaction is triggered.
     */
    __bulk_cntr_db_del(event_id->event_id_fields.type,
                       entry,
                       entry->event_id.event_id_fields.client_id,
                       entry->buffer_id);
...
int bulk_cntr_db_buffer_stats_set(struct ku_bulk_cntr_buffer_stats *bulk_cntr_buffer_stats_data_p)
{
     ...
     * is set to FALSE when NOS unmaps the memory.
     */
    if (bulk_cntr_buffer_stats_data_p->is_allocated) {
        ...
        if (g_bulk_counter_buffer_hft_utilization - bulk_cntr_buffer_stats_data_p->size >= 0) {
            g_bulk_counter_buffer_hft_utilization -= bulk_cntr_buffer_stats_data_p->size;
        } else {
            err = -ENOMEM;
        }
