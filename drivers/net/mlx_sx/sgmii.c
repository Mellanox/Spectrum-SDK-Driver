...
int sgmii_get_send_attempts_by_transport(sxd_transport_type_t transport_type)
{
    ...
    case SXD_TRANSPORT_TYPE_CR_ACCESS:
...
static int __sgmii_wq_init(void)
{
    ...
    if (!__sgmii_worker_thread) {
        sxd_log_err("failed to create sgmii worker thread\n");
        return -ENOMEM;
    }
...
int sgmii_queue_task(sgmii_task_t task, const void *param_buff, int param_buff_size, unsigned long delay)
{
    ...
    if (__sgmii_wq_inc_ref()) {
        ...
        sxd_log_err("SGMII workqueue is not active!\n");
        kfree(sgmii_task);
    }
...
int sgmii_set_system_cfg(const struct ku_sgmii_system_cfg *sgmii_system_cfg)
{
    ...
    if (ret == 0) {
        ...
        if (ret) {
            sxd_log_err("failed to set MAD path to SGMII on default device (%d)\n", DEFAULT_DEVICE_ID);
        }

        ret = sx_dpt_set_cr_access_path(DEFAULT_DEVICE_ID, DPT_PATH_SGMII);
        if (ret) {
            sxd_log_err("failed to set CR-Access path to SGMII on default device (%d)\n", DEFAULT_DEVICE_ID);
        }
    }
