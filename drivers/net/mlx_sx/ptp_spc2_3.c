...
#define PTP_TX_ONE_STEP_MAGIC (0x1a2b3c4d5e6f7890)
    ...
    u8            msg_type;
    unsigned long sent_ts; /* timestamp of when the packet was sent to HW */
};
...
int sx_ptp_dev_init_spc2(struct sx_priv *priv, ptp_mode_t ptp_mode)
{
    ...
    if (ptp_mode != KU_PTP_MODE_EVENTS) {
        sxd_log_err("invalid PTP mode: %u\n", ptp_mode);
        return -EINVAL;
    }
    ...
    if (err) {
        sxd_log_err("PTP: failed to get max FID\n");
        goto out;
    }
    ...
    if (err) {
        sxd_log_err("failed to configure MTPCPC, err=%d\n", err);
        goto out;
    }
...
#endif
{
    ...
    struct skb_shared_hwtstamps hwts;
    unsigned long               diff_jiffies = jiffies - entry->sent_ts;
    ...
    if (diff_jiffies > priv->ptp.hwd.spc2.max_tx_ts_diff) {
        priv->ptp.hwd.spc2.max_tx_ts_diff = diff_jiffies;
        sx_int_log_info(&priv->ptp.log, "New max diff between TX-packet and timestamp: %u msecs",
...
int sx_ptp_dump_spc2(struct sx_priv *priv, struct seq_file *m, void *v, void *context)
{
    ...
    seq_printf(m, "%-40s   %-15s   %-15u\n",
               "Max diff (msecs) tx-packet to timestamp",
               "N/A",
               jiffies_to_msecs(priv->ptp.hwd.spc2.max_tx_ts_diff));
    seq_printf(m, "%-40s   %-15llu   %-15llu\n",
               "Empty timestamp",
               (u64)atomic64_read(&priv->ptp.hwd.spc2.counters[PTP_PACKET_INGRESS][
                                      PTP_COUNTER_SPC2_EMPTY_TIMESTAMP]),
...
static int __configure_MTPSPU(struct sx_dev *dev, u16 msg_type_bitmap)
{
    ...
    if (err) {
        sxd_log_err("Failed to configure register MTPSPU (msg_type_bitmap = 0x%x)\n", msg_type_bitmap);
    }
...
int sx_ptp_set_tx_type_spc2(struct sx_priv *priv, int tx_type)
{
    ...
    sxd_log_notice("PTP stepping mode is changing from %s to %s\n",
    ...
    if (err) {
        sxd_log_err("failed to set MTPSPU(0x%x)\n", tx_msg_type_one_step_bitmap);
