...
static int __qos_emad_queues_scheduling_thread(void *arg)
{
    ...
    sxd_log_debug("starting scheduling emads from queues thread\n");
            ...
            for (queue_number = EMAD_PRIO_HIGH; queue_number < EMAD_PRIO_MAX; queue_number++) {
                ...
                for (weight = 0; weight < queue_weight; weight++) {
                    ...
                    err = __sx_core_post_send(emad_to_send->dev,
                                              ...
                                              DONT_FORCE_SDQ_ID);
                    if (err) {
                        sxd_log_err("scheduler failed to send EMAD to queue (err=%d)\n", err);
...
int sx_qos_emad_init(void)
{
    ...
    sxd_log_info("QoS Emad initialization \n");
    ...
    if (!__emad_skb_cache) {
        sxd_log_err("failed to allocate memory cache for qos emad\n");
        err = -ENOMEM;
        goto out;
    }
    __num_of_emads = 0;
    ...
    if (IS_ERR(__scheduling_emads_from_queues_thread)) {
        ...
        sxd_log_err("failed to create scheduling emads from queues thread (err=%d)\n", err);
        goto out;
    }
...
int sx_qos_emad_push_to_queue(struct sx_dev *dev, struct sk_buff  *skb, struct isx_meta *meta, emad_prio_e priority)
{
    ...
    if ((int)priority >= EMAD_PRIO_MAX) {
        sxd_log_err("qos emad priority is out of range: [%d], \n", priority);
        err = -ERANGE;
        goto err_out;
    }
    ...
    if (is_send_without_thread) {
        err = __sx_core_post_send(dev,
                                  ...
                                  DONT_FORCE_SDQ_ID);
        if (err) {
            sxd_log_err("failed to push EMAD to queue (err=%d)\n", err);
        }
        goto out;
    }
    ...
    if (!skb_p) {
        err = -ENOMEM;
        sxd_log_err("allocation of spice skb emad failed \n");
        goto err_out;
    }
