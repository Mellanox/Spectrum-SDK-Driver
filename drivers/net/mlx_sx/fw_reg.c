                                               ...
                                               size_t                   typesize)
{
    ...
    if ((item->offset % typesize != 0) ||
        ...
        sxd_log_err("mlxsw: item bug (name=%s,offset=%x,step=%x,in_step_offset=%x,typesize=%zx)\n",
...
static inline u16 __mlxsw_item_bit_array_offset(const struct mlxsw_item *item, u16 outer_index, u16 index, u8 *shift)
{
    ...
    if ((item->offset % sizeof(u32) != 0) ||
        ...
        sxd_log_err("mlxsw: item bug (name=%s,offset=%x,element_size=%x)\n",
...
#define IN_MB_SIZE(reg_dword_size) (((reg_dword_size) * 4) + REG_TLV_SIZE + OPERATION_TLV_SIZE)
    ...
    if (!dev || !op_tlv || !ku_reg) {
        sxd_log_err("Access Reg invalid param dev:%p op_tlv:%p ku_reg:%p \n", dev, op_tlv, ku_reg);
        return -EINVAL;
    }

    if (sx_core_fw_is_faulty(dev)) {
        sxd_log_notice("Access Reg: Faulty FW - ignoring\n");
        return 0;
    }
    ...
    if (err) {
        if (flags & SX_ACCESS_REG_F_IGNORE_FW_RET_CODE) {
            ...
            sxd_log_err("Register access failed (reg_id=0x%04x, err=%d)\n",
                        op_tlv->register_id,
                        err);
    ...
    if (op_tlv->status) {
        if ((op_tlv->status == SX_EMAD_STATUS_BUSY_E) || (op_tlv->status == SX_EMAD_STATUS_ISSU_ONGOING_E)) {
            sxd_log_info("Register access notice (reg_id=0x%04x, status=0x%x)\n",
                         op_tlv->register_id,
                         op_tlv->status);
        } else {
            sxd_log_err("Register access failed (reg_id=0x%04x, status=0x%x)\n",
                        op_tlv->register_id,
                        op_tlv->status);
            print_hex_dump(KERN_NOTICE, "", DUMP_PREFIX_OFFSET, 16, 1, inbox, IN_MB_SIZE(reg_len_in_dwords), 0);
            dump_stack();
...
#define MLXSW_PMLP_ID  0x5002
#define MLXSW_PMLP_LEN 0x40
int sx_EMAD_ACCESS_REG_PMLP(struct sx_dev                  *dev,
                            emad_prio_e                     priority,
                            sx_emad_method_e                method,
                            const struct sx_emad_in_params *in_params,
                            struct sx_emad_out_params      *out_params,
                            struct ku_pmlp_reg             *reg)
{
    u16 reg_len_dword = (MLXSW_PMLP_LEN >> 2) + 1;

    if (MLXSW_PMLP_LEN % 4 > 0) {
        reg_len_dword++;
    }
...
EXPORT_SYMBOL(sx_EMAD_ACCESS_REG_PMLP);
...
int sx_ACCESS_REG_MCIA(struct sx_dev *dev, struct ku_access_mcia_reg *reg_data)
{
    ...
    if (!err && reg_data->op_tlv.status &&
        ...
        sxd_log_warning("MCIA failed (fw_err=%u, mcia_status=%u ['%s'])\n",
...
static int __MTBR_decode(u8 *outbox, void *ku_reg, void *context)
{
    ...
    if (mtbr_reg->num_rec > MTBR_MAX_TEMPERATURE_RECORDS) {
        sxd_log_err("response MTBR num_rec %d is greater than max records %d.\n",
...
int sx_ACCESS_REG_MTBR(struct sx_dev *dev, struct ku_access_mtbr_reg *reg_data)
{
    ...
    if (reg_data->mtbr_reg.num_rec > MTBR_MAX_TEMPERATURE_RECORDS) {
        sxd_log_err("request MTBR num_rec %d is greater than max records %d.\n",
...
#define REG_IPV4_RECORDS_PORT_2_OFFSET      0xbc
#define REG_IPV6_RECORDS_PORT_2_OFFSET      0xdc
...
static int __IBSNI_encode(u8 *inbox, void *ku_reg, void *context)
{
    ...
    for (i = 0; i < IBSNI_IPV4_RECORD_REC_MAX_COUNT; i++) {
        ...
        SX_PUT_REG_FIELD(inbox, ibsni_reg->ipv4_record_port2[i].ipv4,
                         (dword_inc++) * 4 + REG_IPV4_RECORDS_PORT_2_OFFSET);
        SX_PUT_REG_FIELD(inbox, ibsni_reg->ipv4_record[i].netmask, (dword_inc) * 4 + REG_IPV4_RECORDS_OFFSET);
        SX_PUT_REG_FIELD(inbox,
                         ibsni_reg->ipv4_record_port2[i].netmask,
                         (dword_inc++) * 4 + REG_IPV4_RECORDS_PORT_2_OFFSET);
    }
    dword_inc = 0;
    for (i = 0; i < IBSNI_IPV6_RECORD_REC_MAX_COUNT; i++) {
        for (j = 0; j < IBSNI_IPV6_RECORD_IPV6_MAX_COUNT; j++) {
            ...
            SX_PUT_REG_FIELD(inbox,
                             ibsni_reg->ipv6_record_port2[i].ipv6[j],
                             (dword_inc++) * 4 + REG_IPV6_RECORDS_PORT_2_OFFSET);
        }
        for (j = 0; j < IBSNI_IPV6_RECORD_NETMASK_MAX_COUNT; j++) {
            ...
            SX_PUT_REG_FIELD(inbox,
                             ibsni_reg->ipv6_record_port2[i].netmask[j],
                             (dword_inc++) * 4 + REG_IPV6_RECORDS_PORT_2_OFFSET);
        }
    }
...
static int __IBSNI_decode(u8 *outbox, void *ku_reg, void *context)
{
    ...
    for (i = 0; i < IBSNI_IPV4_RECORD_REC_MAX_COUNT; i++) {
        ...
        SX_GET_REG_FIELD(ibsni_reg->ipv4_record_port2[i].ipv4, outbox,
                         (dword_inc++) * 4 + REG_IPV4_RECORDS_PORT_2_OFFSET);
        SX_GET_REG_FIELD(ibsni_reg->ipv4_record[i].netmask, outbox, (dword_inc) * 4 + REG_IPV4_RECORDS_OFFSET);
        SX_GET_REG_FIELD(ibsni_reg->ipv4_record_port2[i].netmask,
                         outbox,
                         (dword_inc++) * 4 + REG_IPV4_RECORDS_PORT_2_OFFSET);
    }
    dword_inc = 0;
    for (i = 0; i < IBSNI_IPV6_RECORD_REC_MAX_COUNT; i++) {
        for (j = 0; j < IBSNI_IPV6_RECORD_IPV6_MAX_COUNT; j++) {
            ...
            SX_GET_REG_FIELD(ibsni_reg->ipv6_record_port2[i].ipv6[j],
                             outbox,
                             (dword_inc++) * 4 + REG_IPV6_RECORDS_PORT_2_OFFSET);
        }
        for (j = 0; j < IBSNI_IPV6_RECORD_NETMASK_MAX_COUNT; j++) {
            ...
            SX_GET_REG_FIELD(ibsni_reg->ipv6_record_port2[i].netmask[j], outbox,
                             (dword_inc++) * 4 + REG_IPV6_RECORDS_PORT_2_OFFSET);
        }
    }
...
static int __SFN_decode(u8 *outbox, void *ku_reg, void *context)
{
    ...
    for (i = 0; i < sfn_reg->num_records; i++) {
        ...
        default:
            sxd_log_err("SFN type (decode) is unsupported (rec_index=%d, type=0x%x)\n", i, curr_rec_usr->sfn_type);
            break;
        }
