...
static int sx_drop_monitor_init(void)
{
    ...
    if (!new_listener) {
        sxd_log_err("%s: Failed allocating memory for new listener\n", __func__);
        return -ENOMEM;
    }
...
static bool sx_drop_monitor_listener_active(void)
{
    ...
    if (!g_drop_monitor_listener) {
        ...
        if (err) {
            sxd_log_err("%s: sx_drop_monitor_init failed with err %d\n", __func__, err);
            return false;
        }
    }
                                        ...
                                        struct sk_buff         *skb)
{
    ...
    if (enable_drop_monitor_report && sx_drop_monitor_listener_active()) {
        ...
        *rx_ts_set = true;
        ...
        if (err) {
            sxd_log_err("%s: rx_skb failed with err %d\n", __func__, err);
        }
    }
}
...
int sx_drop_monitor_set_buffer_drop_params(unsigned long data)
{
    ...
    if (err) {
        sxd_log_err("%s: copy_from_user failed with err %d\n", __func__, err);
        goto out;
    }

    if (buffer_drop_params.is_set) {
        if (buffer_drop_params.trap_id > NUM_HW_SYNDROMES) {
            sxd_log_err("%s: CTRL_CMD_BUFFER_DROP_PARAMS: trap ID %u is not valid\n",
                             ...
                             struct sx_dev                   *dev)
{
    ...
    if (cmd == CTRL_CMD_ADD_SYND) {
        err = sx_core_add_synd(ku->swid, ku->syndrome_num,
                               ...
                               critireas, "Drop-Monitor", sx_drop_monitor_handler, dev,
