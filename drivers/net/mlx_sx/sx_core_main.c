...
int sx_core_get_port_module_temperature(struct sx_dev *dev, uint16_t local_port, struct sx_temperature_params *params)
{
    ...
    if (pmaos_reg_data.pmaos_reg.oper_status != 1) {
        err = -ENODEV;
        printk(KERN_INFO PFX "Module %d is not enabled\n", module_id);
        goto out;
    }
    ...
    if ((temperature == 0) || (temperature == SXD_MTBR_NO_CABLE) || (temperature == SXD_MTBR_NO_READ) ||
        ...
        err = -ENODEV;
        printk(KERN_ERR PFX "Temperature sensing is not supported on module %d, status=0x%x\n", module_id,
               temperature);
        goto out;
    }
...
static void __exit sx_core_cleanup(void)
{
    ...
    if (sx_glb.fake_dev) { /* we're running on a VM or OOB system */
        sx_core_remove_one(sx_priv(sx_glb.fake_dev), false);
