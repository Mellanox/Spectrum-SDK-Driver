...
int rx_debug_metadata;
module_param_named(rx_debug_metadata, rx_debug_metadata, int, 0644);
MODULE_PARM_DESC(rx_debug_metadata, "en/dis print of rx packets metadata");
...
int tx_debug_metadata;
module_param_named(tx_debug_metadata, tx_debug_metadata, int, 0644);
MODULE_PARM_DESC(tx_debug_metadata, "en/dis print of tx packets metadata");
...
int rx_debug_hft_event = 0;
module_param_named(rx_debug_hft_event, rx_debug_hft_event, int, 0644);
MODULE_PARM_DESC(rx_debug_hft_event, "en/dis print of hft events.");

static DEFINE_SPINLOCK(sx_files_db_lock);
static LIST_HEAD(sx_files_db);
...
int sx_core_skb_add_vlan(struct sk_buff **untagged_skb, uint16_t vid, uint16_t pcp)
{
    ...
    if (skb_headroom(skb) < ISX_HDR_SIZE + VLAN_HLEN) {
        ...
        if (!new_skb) {
            sxd_log_err("adding vlan to skb failed because of allocation\n");
            return -ENOMEM;
        }
...
int config_l2_force_snoop(void)
{
    ...
    if (0 == dcrbase_l2c) {
        ...
        if (prop == NULL) {
            sxd_log_err("%s: Can't get cache-size!\n", np->full_name);
            of_node_put(np);
            return -ENODEV;
        }
        l2_size = prop[0];
        ...
        if (!dcrreg || (len != 4 * sizeof(u32))) {
            sxd_log_err("%s: Can't get DCR register base !",
                        np->full_name);
            of_node_put(np);
            return -ENODEV;
        }
...
int sx_core_get_local(struct sx_dev *dev, uint16_t lag_id, uint16_t lag_subport, uint16_t *local_p)
{
    ...
    if (sx_core_get_lag_max(dev, &lag_max, &lag_member_max)) {
        sxd_log_err("Failed to get max number of LAGs.\n");
        return -EINVAL;
    }

    if (lag_id > lag_max) {
        sxd_log_rl_err("lag_id %d is invalid. (MAX %d).\n",
                       lag_id, lag_max);
        return -EINVAL;
    }

    if (lag_subport > lag_member_max) {
        sxd_log_rl_err("lag_subport %d is invalid. (MAX %d).\n",
    ...
    if (sx_core_get_phy_port_max(dev, &phy_port_max)) {
        sxd_log_err("Failed to get max number of phy ports.\n");
        return -EINVAL;
    }

    if (local > phy_port_max) {
        sxd_log_rl_err("Local %d is invalid. (MAX %d).\n",
                       local, phy_port_max);
        return -EINVAL;
    }
...
int sx_core_get_prio2tc(struct sx_dev *dev, uint16_t port_lag_id, uint8_t is_lag, uint8_t pcp, uint8_t *tc)
{
    ...
    if (pcp > MAX_PRIO_NUM) {
        sxd_log_err("PCP %d is invalid. (MAX %d).\n",
                    pcp, MAX_PRIO_NUM);
        return -EINVAL;
    }
    ...
    if (is_lag) {
        *tc = dev_priv->lag_prio2tc[port_lag_id][pcp];
        ...
        if (local > phy_port_max) {
            sxd_log_err("Local %d is invalid. (MAX %d).\n",
                        local, phy_port_max);
...
int sx_core_get_prio_tagging(struct sx_dev *dev, uint16_t port_lag_id, uint8_t is_lag, uint8_t *is_port_prio_tagged)
{
    ...
    if (sx_core_get_phy_port_max(dev, &phy_port_max)) {
        sxd_log_err("Failed to get max number of phy port.\n");
        return -EINVAL;
    }
...
int sx_core_get_swid(struct sx_dev *dev, struct completion_info *comp_info, uint8_t *swid)
{
    ...
    *swid = 0;
    ...
    if (!dev) {
        sxd_log_err("get_swid: dev is NULL\n");
        return -EINVAL;
    }

    rc = __sx_core_dev_specific_cb_get_reference(dev);
    if (rc) {
        sxd_log_err("__sx_core_dev_specific_cb_get_reference failed.\n");
        return rc;
    }
    ...
    if (sx_priv(dev)->dev_specific_cb.get_swid_cb != NULL) {
        ...
        sxd_log_err("Error retrieving get_swid_cb callback\n");
...
int sx_core_get_phy_port_max(struct sx_dev *dev, uint16_t *port)
{
    ...
    if (dev == NULL) {
        sxd_log_err("get phy port max: dev is NULL.\n");
        return -EINVAL;
    }

    if (port == NULL) {
        sxd_log_err("port is NULL.\n");
        return -EINVAL;
    }
    ...
    *port = 0;
    ...
    if (sx_priv(dev)->dev_specific_cb.sx_get_phy_port_max_cb != NULL) {
        if (sx_priv(dev)->dev_specific_cb.sx_get_phy_port_max_cb(port)) {
            sxd_log_err("Error retrieving max phy port number from sx_get_phy_port_max_cb callback\n");
            rc = -EINVAL;
        }
    } else {
        sxd_log_err("Error retrieving sx_get_phy_port_max_cb callback\n");
        rc = -EINVAL;
    }
...
int sx_core_get_lag_max(struct sx_dev *dev, uint16_t *lags, uint16_t *pors_per_lag)
{
    ...
    if (dev == NULL) {
        sxd_log_err("lag_max: dev is NULL.\n");
        return -EINVAL;
    }

    if (lags == NULL) {
        sxd_log_err("lags is NULL.\n");
        return -EINVAL;
    }

    if (pors_per_lag == NULL) {
        sxd_log_err("lags_per_port is NULL.\n");
        return -EINVAL;
    }
    ...
    if (sx_priv(dev)->dev_specific_cb.sx_get_lag_max_cb != NULL) {
        if (sx_priv(dev)->dev_specific_cb.sx_get_lag_max_cb(lags, pors_per_lag)) {
            sxd_log_err("Error retrieving max values for LAG from sx_get_lag_max_cb callback\n");
            rc = -EINVAL;
        }
    } else {
        sxd_log_err("Error retrieving sx_get_lag_max_cb callback\n");
        rc = -EINVAL;
    }
...
int sx_core_get_lag_mid(struct sx_dev *dev, u16 lag_id, u16 *mid)
{
    ...
    if (!dev) {
        sxd_log_err("get_lad_mid: dev is NULL\n");
        return -EINVAL;
    }
    ...
    if (dev && (sx_priv(dev)->dev_specific_cb.get_lag_mid_cb != NULL)) {
        ...
        sxd_log_err("Error retrieving get_lag_mid_cb callback\n");
        __sx_core_dev_specific_cb_release_reference(dev);
        return -EINVAL;
    }
...
int sx_core_is_eqn_cmd_ifc_only(struct sx_dev *dev, int eqn, u8 *is_cmd_ifc_only)
{
    ...
    if (!dev) {
        sxd_log_err("Error in sx_core_is_eqn_cmd_ifc_only: *dev is NULL.\n");
        return -EINVAL;
    }
...
int sx_core_get_rp_rfid(struct sx_dev *dev, uint16_t port_lag_id, uint8_t is_lag, uint16_t vlan_id, uint16_t *rfid)
{
    ...
    if (vlan_id >= SXD_MAX_VLAN_NUM) {
        sxd_log_err("vlan_id %d is invalid. (MAX %d).\n",
    ...
    *rfid = 0;
    ...
    if (is_lag) {
        if (port_lag_id > lag_max) {
            sxd_log_rl_err("port_lag_id %d is invalid. (MAX %d).\n",
                           port_lag_id, lag_max);
            spin_unlock_irqrestore(&sx_priv(dev)->db_lock, flags);
            return -EINVAL;
        }
        if (!dev_priv->lag_rp_rif_valid[port_lag_id][vlan_id]) {
            sxd_log_err("No RP on LAG ID %d and vlan %d.\n",
                        port_lag_id, vlan_id);
            spin_unlock_irqrestore(&sx_priv(dev)->db_lock, flags);
            return -EINVAL;
        }
        ...
        *rfid = dev_priv->rif_id_to_hwfid[rif_id];
        ...
        if (!dev_priv->port_rp_rif_valid[local][vlan_id]) {
            sxd_log_err("No RP on port %d and vlan %d.\n",
                        local, vlan_id);
            spin_unlock_irqrestore(&sx_priv(dev)->db_lock, flags);
            return -EINVAL;
        }
...
int sx_core_get_rp_vlan(struct sx_dev *dev, struct completion_info *comp_info, uint16_t *vlan_id)
{
    ...
    if (!dev) {
        sxd_log_err("get_rp_vlan: dev is NULL\n");
        return -EINVAL;
    }
    ...
    *vlan_id = 0;
    ...
    if (sx_priv(dev)->dev_specific_cb.get_rp_vid_cb != NULL) {
        ...
        sxd_log_err("Error retrieving get_rp_vid_cb callback\n");
        __sx_core_dev_specific_cb_release_reference(dev);
        return -EINVAL;
    }
...
int sx_core_get_rp_mode(struct sx_dev *dev, u8 is_lag, u16 sysport_lag_id, u16 vlan_id, u8 *is_rp)
{
    ...
    if (is_lag) {
        if (sysport_lag_id > lag_max) {
            sxd_log_err("LAG ID %d is invalid. (MAX %d).\n",
                        lag_id, lag_max);
            spin_unlock_irqrestore(&sx_priv(dev)->db_lock, flags);
            return -EINVAL;
        }
...
int sx_core_get_vlan2ip(struct sx_dev *dev, uint16_t vid, uint32_t *ip_addr)
{
    ...
    if (vid >= SXD_MAX_VLAN_NUM) {
        sxd_log_err("vid %d is invalid. (MAX %d).\n",
...
int sx_core_get_ib_system_port_mid(struct sx_dev *dev, u16 ib_port, u16* sys_port_mid)
{
    ...
    if (sx_priv(dev)->dev_specific_cb.get_ib_system_port_mid != NULL) {
        ...
        sxd_log_err("Error retrieving get_ib_system_port_mid callback structure!\n");
        sxd_log_err("Hint: Check call to IB code from ETH flow!\n");
        __sx_core_dev_specific_cb_release_reference(dev);
        return -EINVAL;
    }
...
int sx_core_get_module_by_port(struct sx_dev *dev, uint16_t local_port, uint8_t *slot, uint8_t *module)
{
    ...
    if (local_port > MAX_PHYPORT_NUM) {
        sxd_log_rl_err("Local port %d is invalid. (MAX %d).\n",
...
static int __sx_core_get_low_power_mode(struct sx_dev *dev, uint8_t slot_id, uint8_t module_id, bool *low_power_p)
{
    struct sx_priv           *priv = sx_priv(dev);
    int                       err = 0;
    ...
    sx_cmd_set_op_tlv(&reg_data.op_tlv, MLXSW_PMMP_ID, EMAD_METHOD_QUERY);
    ...
    if (err) {
        sxd_log_err("Failed to query PMMP, err=%d\n", err);
        goto out;
    }
    if (reg_data.op_tlv.status) {
        err = -EINVAL;
        sxd_log_err("Failed to query PMMP, status=%d\n", reg_data.op_tlv.status);
        goto out;
    }

    *low_power_p = !!(reg_data.pmmp_reg.eeprom_override & MLXSW_REG_PMMP_EEPROM_OVERRIDE_LOW_POWER_MASK);
...
static int __sx_core_set_low_power_mode(struct sx_dev *dev, uint8_t slot_id, uint8_t module_id, bool low_power)
{
    ...
    if (priv->apply_im_supported == false) {
    ...
    reg_data.pmmp_reg.slot_index = slot_id;
    reg_data.pmmp_reg.module = module_id;
    reg_data.pmmp_reg.apply_im = priv->apply_im_supported;
    ...
    err = sx_ACCESS_REG_PMMP(dev, &reg_data);
    if (err) {
        sxd_log_err("Failed to access PMMP, err=%d\n", err);
        goto bring_module_up;
    }
    if (reg_data.op_tlv.status) {
        err = -EINVAL;
        sxd_log_err("Failed to access PMMP, status=%d\n", reg_data.op_tlv.status);
        goto bring_module_up;
    }
...
int sx_core_update_module_power_mode(struct sx_dev *dev, uint8_t slot_id, uint8_t module_id, bool port_up)
{
    ...
    if (err) {
        sxd_log_err("Failed to set power mode (%d), slot id (%d), module id (%d).\n",
                    !port_up, slot_id, module_id);
        goto out;
    }
...
int sx_core_set_module(struct sx_dev *dev, uint8_t slot_id, uint8_t module_id, uint8_t enable, uint8_t reset)
{
    ...
    spin_lock_irqsave(&sx_priv(dev)->db_lock, flags);
    is_system_ind =
        (sx_priv(dev)->independent_module_params.module_support_type == SXD_MODULE_MASTER_MODE_SW_CONTROL_E);
    spin_unlock_irqrestore(&sx_priv(dev)->db_lock, flags);
                                         ...
                                         enum sx_module_power_mode_policy power_mode_policy)
{
    ...
    if (is_independent) {
        sxd_log_err("%s: Failed: module is independent (SW control).\n", __func__);
        err = -EFAULT;
        goto out;
    }
    ...
    if ((slot_id >= MAX_SLOT_NUM) || (module_id >= MAX_MODULE_NUM)) {
        err = -EINVAL;
        sxd_log_rl_err("slot id %d or module id %d is out of range.\n", slot_id, module_id);
        spin_unlock_irqrestore(&priv->db_lock, flags);
        goto out;
    }

    if ((priv->apply_im_supported == false) && (power_mode_policy == SX_MODULE_POWER_MODE_POLICY_LOW)) {
        err = -EINVAL;
        sxd_log_err("Apply immediate is not supported: can't change to power mode policy to low.\n");
        spin_unlock_irqrestore(&priv->db_lock, flags);
        goto out;
    }
    ...
    if ((any_port_up == true) && (power_mode_policy == SX_MODULE_POWER_MODE_POLICY_AUTO)) {
        goto out_set_policy;
    }

    low_power = (power_mode_policy != SX_MODULE_POWER_MODE_POLICY_HIGH);

    err = __sx_core_get_low_power_mode(dev, slot_id, module_id, &is_low_power);
    if (err) {
        goto out;
    }
    if (is_low_power == low_power) {
        sxd_log_debug("Current power mode and desired power mode are same.\n");
        goto out_set_policy;
    }
                                  ...
                                  struct sx_module_power_mode_params *params)
{
    ...
    if (!(reg_data.mcion_reg.module_status_bits & MCION_MODULE_STATUS_BITS_PRESENT_MASK)) {
        err = -ENODEV;
        sxd_log_info("Module %d is not presented\n", module_id);
        goto out;
    }
...
int sx_core_reset_module(struct sx_dev *dev, uint8_t slot_id, uint8_t module_id)
{
    ...
    if (is_independent) {
        sxd_log_err("%s: Failed reset: module is independent (SW control).\n", __func__);
        err = -EFAULT;
        goto out;
    }
                                   ...
                                   struct sx_temperature_params *params)
{
    ...
    if (pmaos_reg_data.pmaos_reg.oper_status != SXD_PMAOS_OPER_STATUS_PLUGGED_ENABLED_E) {
        err = -ENODEV;
        sxd_log_debug("Module %d oper status (%d) is not plugged enabled.\n", module_id,
    ...
    if ((temperature == 0) || (temperature == SXD_MTBR_NO_CABLE) || (temperature == SXD_MTBR_NO_READ) ||
        ...
        err = -ENODEV;
        sxd_log_debug("Temperature sensing is not supported on module %d, status=0x%x\n", module_id,
                      temperature);
        goto out;
    }
    ...
    params->threshold_hi = mtmp_reg_data.mtmp_reg.temperature_threshold_hi;
    params->threshold_lo = mtmp_reg_data.mtmp_reg.temperature_threshold_lo;
...
static int check_valid_meta(struct sx_dev *dev, struct isx_meta *meta)
{
    ...
    if (meta->etclass >= NUMBER_OF_ETCLASSES) {
        sxd_log_rl_warning("Error: etclass (%u) "
                           "is invalid\n",
                           meta->etclass);
    ...
    if (rc) {
        sxd_log_err("Error: failed to get max RDQ num\n");
        return rc;
    }

    if ((meta->rdq >= rdq_num) &&
        (meta->rdq != 0x1F)) {
        sxd_log_rl_warning("Error: rdq (%u) is invalid\n",
                           meta->rdq);
    ...
    if ((meta->swid >= NUMBER_OF_SWIDS) &&
        ...
        sxd_log_rl_warning("Error: swid (%u) is invalid\n",
                           meta->swid);
    ...
    if (dev != NULL) {
        if ((meta->swid != SWID_NUM_DONT_CARE) &&
            ...
            sxd_log_warning("Error: swid (%u) does not exists\n", meta->swid);
            return -EINVAL;
        }
    }

    if ((meta->to_cpu != false) && (meta->to_cpu != true)) {
        sxd_log_rl_warning("Error: to_cpu (%u) "
                           "is invalid\n",
                           meta->to_cpu);
    ...
    if ((meta->type < SX_PKT_TYPE_MIN) ||
        ...
        sxd_log_rl_warning("Error: type (%d) is invalid\n",
                           meta->type);
        return -ERANGE;
    }
                            ...
                            gfp_t            gfp_mask)
{
    ...
    if (is_from_user) {
        ...
        if (err) {
            sxd_log_warning("failed to copy buffer from user\n");
            goto out_free;
        }
    } else {
        iov = write_data->iov;
    ...
    *skb = alloc_skb(packet_size + max_headers_size, gfp_mask);
    ...
    for (index = 0; index < write_data->vec_entries; index++) {
        ...
        if (p_skb_data == NULL) {
            sxd_log_warning("copy_ku_to_skb: "
                            "skb_put failed\n");
            err = -EFAULT;
            goto out_free_skb;
        }
        ...
        if (is_from_user) {
            err = copy_from_user(p_skb_data, iov[index].iov_base,
                                 iov[index].iov_len);
            if (err) {
                sxd_log_warning("failed to copy buffer from user index:[%d]\n", index);
                goto out_free_skb;
            }
        } else {
                            ...
                            struct listener_entry *listener)
{
    ...
    if (err) {
        sxd_log_warning("sx_send_loopback: failed copying buffer to SKB err:[%d]\n", err);
        goto out;
    }
    ...
    SX_RX_TIMESTAMP_INIT(&ci.rx_timestamp, timestamp.tv_sec, timestamp.tv_nsec, SXD_TS_TYPE_LINUX);
                        ...
                        struct listener_entry *listener)
{
    ...
    if (is_sgmii_device(device_id)) {
        ...
        if (!dev) {
            sxd_log_warning("device is NULL\n");
            err = 1;
            goto out;
        }
    }
                         ...
                         struct listener_entry   *listener)
{
    ...
    sxd_log_notice("Sending SDK health event [dev=%u, cause=%d, severity=%d, irisc=%u, sync_new_listener=%s]\n",
...
void sx_copy_pkt_metadata_prepare(struct ku_read *metadata_p, struct event_data *edata_p)
{
    ...
    if (!sx_core_has_predefined_devices()) {
        ...
        if (edata_p->trap_id == SXD_TRAP_ID_PMPE) {
            /* For module related PMPE event, it is trapped with (dev_id+local_module_id) in multi-asic systems.
             * Then dev_id should remain unchanged for later use, e.g, to send rearm event to kernel to select
             * the corresponding device to access PMAOS. */
            metadata_p->dev_id = edata_p->dev_id;
        } else {
                     ...
                     u8                                is_register)
{
    ...
    if (sx_dev == NULL) {
        ...
        if (sx_dev == NULL) {
            sxd_log_err("sx_dev_db_get_default_device return sx_dev == NULL\n");
            return -EINVAL;
        }
    }
    ...
    if (!handler) {
        sxd_log_warning("sx_core_add_synd: Cannot add listener, handler is NULL\n");
        return -EINVAL;
    }

    new_listener = kzalloc(sizeof(*new_listener), GFP_ATOMIC);
    if (!new_listener) {
        sxd_log_warning("sx_core_add_synd: Failed allocating memory for the new listener\n");
        return -ENOMEM;
    }
    ...
    if (handler_desc) {
        strncpy(new_listener->handler_str, handler_desc, sizeof(new_listener->handler_str) - 1);
    }
    ...
    if (!info) {
        sxd_log_err("listeners database does not exist!\n");
        kfree(new_listener);
        goto out;
    }
    ...
    list_for_each_entry(listener, &info->per_synd_list[hw_synd], list) {
        if (is_same_listener(swid, type, is_default, crit, context, listener, handler)) {
            if (port_vlan == NULL) {
                found_same_port_vlan_listener = 1;
            } else {
...
static int check_valid_profile(struct sx_dev *dev, struct sx_pci_profile *profile)
{
    ...
    for (i = 0; i < NUMBER_OF_SWIDS; i++) {
        for (j = 0; j < NUMBER_OF_ETCLASSES; j++) {
            if (profile->tx_prof[i][j].sdq >=
                ...
                sxd_log_warning("sdq num is > max\n");
                return -EINVAL;
            }
            if (profile->tx_prof[i][j].stclass >= 8) {
                sxd_log_warning("stclass num is > "
                                "max\n");
                return -EINVAL;
            }
        }
    ...
    if (profile->emad_tx_prof.sdq >= priv->dev_cap.max_num_sdqs) {
        sxd_log_warning("emad sdq num is > max\n");
        return -EINVAL;
    }
    if (profile->emad_tx_prof.stclass >= 8) {
        sxd_log_warning("emad stclass num is > max\n");
        return -EINVAL;
    }
    ...
    for (i = 0; i < NUMBER_OF_SWIDS; i++) {
        ...
        if (profile->rdq_count[i] > (priv->dev_cap.max_num_rdqs - 1)) {
            sxd_log_warning("Normalize rdq num %d to dev_cap_max_num_rdq %d\n",
    ...
    if ((profile->pci_profile >= PCI_PROFILE_EN_SINGLE_SWID) &&
        ...
        sxd_log_warning("emad_rdq = %d > max\n",
                        profile->emad_rdq);
        return -EINVAL;
    }
    ...
    for (i = 0; i < NUMBER_OF_SWIDS; i++) {
        if ((profile->swid_type[i] != SX_KU_L2_TYPE_IB) &&
            ...
            sxd_log_warning("for swid %d type = "
                            "%d is wrong!\n",
    ...
    if (rc) {
        sxd_log_err("Error: failed to get max RDQ params\n");
        return rc;
    }
    ...
    for (i = 0; i < rdq_num; i++) {
        if ((profile->rdq_properties[i].number_of_entries >
            ...
            (profile->rdq_properties[i].entry_size >
             entry_size)) {
            sxd_log_warning("%s:%d\n", __FILE__, __LINE__);
            sxd_log_warning("%d: n_enties %d , max_rdq_sz:"
    ...
    for (i = 0; i < NUMBER_OF_SDQS; i++) {
        if (profile->cpu_egress_tclass[i] >= 64) {
            sxd_log_warning("cpu_egress_tclass[%d] = "
                            "%d > max\n",
...
int sx_send_enable_ib_swid_events(struct sx_dev *dev, u8 swid)
{
    ...
    if (priv->profile.pci_profile.swid_type[swid] == SX_KU_L2_TYPE_IB) {
        ...
        if (!event_data) {
            sxd_log_err("%s: failed to allocate event data\n", __FUNCTION__);
            err = -ENOMEM;
            goto out;
        }
        ...
        if (first_ib_swid) {
            ...
            if (!rpa_init_event_data) {
                sxd_log_err("%s: failed to allocate rpa_event_data\n", __FUNCTION__);
                err = -ENOMEM;
                goto out;
            }
        ...
        sxd_log_err("Error: try to send IB_SWID_UP event on swid %d from non-IB type %d, ",
...
int sx_enable_swid(struct sx_dev *dev, int sx_dev_id, u8 swid, int synd, u64 mac)
{
    ...
    if (!sx_dpt_is_path_valid(sx_dev_id, DPT_PATH_PCI_E)) {
        sxd_log_info("PCIe path is not valid for device %u, "
                     "will not open DQs\n",
                     sx_dev_id);
        goto send_events;
    }
    ...
    for (i = 0; i < NUMBER_OF_ETCLASSES; i++) {
        ...
        if (err) {
            sxd_log_err("Error retrieving sx_get_sdq_cb callback structure! err=[%d]\n", err);
            goto out;
        }
    ...
    for (i = 0; i < priv->profile.pci_profile.rdq_count[swid]; i++) {
        ...
        if (!dq) {
            err = sx_core_create_rdq(dev, priv->profile.pci_profile.rdq_properties[dqn].number_of_entries,
                                     dqn, &dq);
            if (err) {
                sxd_log_err("Failed to create RDQ %d \n", dqn);
                goto out;
            }
...
send_events:
    ...
    if (is_sgmii_supported()) {
        ...
        priv->dev_info.dev_info_ro.flags |= SX_DEV_INFO_F_DEV_PROFILE_SET;
    }
...
void sx_disable_swid(struct sx_dev *dev, u8 swid)
{
    ...
    event_data = kzalloc(sizeof(union sx_event_data), GFP_KERNEL);
    if (!event_data) {
        sxd_log_warning("Failed to allocate memory for event data\n");
        goto out;
    }

    if (!sx_bitmap_test(&priv->swid_bitmap, swid)) {
        sxd_log_warning("SWID %u is not set\n", swid);
        goto out;
    }
...
int sx_handle_set_profile(struct sx_dev *dev)
{
    ...
    if (err) {
        sxd_log_err("input profile is not valid\n");
        goto out;
    }
    ...
    if ((priv->profile.pci_profile.pci_profile >= PCI_PROFILE_EN_SINGLE_SWID) &&
        ...
        if (err) {
            sxd_log_err("create EMAD sdq %d failed. err: %d\n",
                        dqn, err);
            goto out;
        }
        ...
        err = sx_core_create_rdq(dev, priv->profile.pci_profile.rdq_properties[dqn].number_of_entries,
                                 dqn, &rdq);
        if (err) {
            sxd_log_err("create EMAD rdq %d failed. err: %d\n",
                        dqn, err);
            goto out;
        }
    }
...
int sx_core_get_rdq_param_max(struct sx_dev *dev, uint8_t *rdq_num_max_p, uint16_t *rdq_max_entry_size_p)
{
    ...
    if (dev && (sx_priv(dev)->dev_specific_cb.sx_get_rdq_max_cb != NULL)) {
        ...
        sxd_log_err("Error retrieving sx_get_rdq_max_cb callback\n");
        rc = -EINVAL;
    }
...
int __sx_core_dev_specific_cb_get_reference(struct sx_dev *dev)
{
    ...
    if (!priv->dev_specific_cb_init_done) {
        err = -EFAULT;
        spin_unlock_irqrestore(&priv->db_lock, flags);
        sxd_log_info("dev_specific_cb is not init\n");
        goto out;
    }
...
int sx_change_configuration(struct sx_dev *dev)
{
    ...
    if (priv->profile.pci_profile_set) {
        ...
        priv->dev_info.dev_info_ro.flags &= ~SX_DEV_INFO_F_PCI_PROFILE_SET;
        spin_unlock(&priv->profile.profile_lock);
...
static int sx_core_open(struct inode *inode, struct file *filp)
{
    ...
    atomic_set(&file->mmap_refcnt, 0);

    file->pid = current->pid;
    strncpy(file->name, current->comm, sizeof(file->name) - 1);
    file->since = jiffies;

    spin_lock_bh(&sx_files_db_lock);
    list_add_tail(&file->list, &sx_files_db);
    spin_unlock_bh(&sx_files_db_lock);
    return 0;
}
...
void sx_core_set_owner(struct sx_rsc *file)
{
    spin_lock_bh(&sx_files_db_lock);
    file->pid = current->pid;
    strncpy(file->name, current->comm, sizeof(file->name) - 1);
...
static ssize_t sx_core_write(struct file *filp, const char __user *buf, size_t count, loff_t *pos)
{
    ...
    while (user_buffer_copied_size + sizeof(write_data) <= count) {
        ...
        if (is_sgmii_device(write_data.meta.dev_id)) {
            ...
            if (err) {
                sxd_log_warning("Cannot execute because meta "
                                "is invalid\n");
                goto out;
            }
        }
        ...
        if ((write_data.meta.type == SX_PKT_TYPE_DROUTE_EMAD_CTL) ||
            ...
            if (!dev || !priv->profile.pci_profile_set || is_sgmii_device(write_data.meta.dev_id)) {
                ...
                if (i == NUMBER_OF_SWIDS) { /* no ETH or IB swids found */
                    sxd_log_warning("sx_core_write: Err: "
        ...
        if (down_timeout(&rsc->write_sem, msecs_to_jiffies(10000)) == -ETIME) {
            sxd_log_err("packet is waiting too long on TX queue, discarding.\n");
            err = -ETIMEDOUT;
            kfree_skb(skb);
            goto out;
        }
        ...
        if (err) {
            sxd_log_rl_warning("FD write: could not push packet towards ASIC (ret=%d)\n", err);
...
static int sx_core_close(struct inode *inode, struct file *filp)
{
    ...
    if (sx_glb.pci_drivers_in_use & PCI_DRIVER_F_SX_DRIVER) {
        ...
        if (dqn >= 0) {
            ...
            if (ret) {
                sxd_log_err("failed to disable monitoring RDQ on file close (err=%d)\n", ret);
            }
        }
    }
...
static int __fd_dump_handler(struct seq_file* m, void *v, void* context)
{
    char              agent[64] = { 0 };
    char              mon_rdq[5] = "N/A";
    char              time_str[32] = { 0 };
    ...
    struct sx_rsc    *iter;
    struct file      *owner_p = NULL;
    struct dentry    *dentry_p = NULL;

    seq_printf(m, "%-20s    %-8s    %-8s    %-23s    %-6s    %-7s    %-7s\n",
               "Agent", "File", "D-Entry", "Created", "RefCnt", "SHM-Cnt", "Mon RDQ");
    ...
    list_for_each_entry(iter, &sx_files_db, list) {
        snprintf(agent, sizeof(agent) - 1, "%s/%u", iter->name, iter->pid);
        sx_dbg_jiffies_to_time_str(&now, iter->since, time_str, sizeof(time_str));
        if (iter->bound_monitor_rdq) {
            snprintf(mon_rdq, sizeof(mon_rdq) - 1, "%d", iter->bound_monitor_rdq->dqn);
        }
        owner_p = iter->owner;
        if (owner_p != NULL) {
            dentry_p = file_dentry(owner_p);
        } else {
            dentry_p = NULL;
        }
        seq_printf(m, "%-20s    %-8p    %-8p    %-23s    %-6d    %-7d    %-7s\n",
                   agent,
                   owner_p,
                   dentry_p,
                   time_str,
                   (int)atomic_read(&iter->refcnt),
                   (int)atomic_read(&iter->mmap_refcnt),
                   mon_rdq);
    }
    spin_unlock_bh(&sx_files_db_lock);
    return 0;
...
int sx_add_char_dev(struct cdev *cdev_p, int dev_id)
{
    ...
    sxd_log_notice("Create char dev with major:%d minor:%d \n", major, minor);
    if (ret) {
        sxd_log_err("Couldn't add char device for device %d. Aborting... err: %d\n",
                    dev_id, ret);
        goto out;
    }
...
{
    sxd_log_notice("Deinit char dev: %p , usage:%d\n",
                   cdev_p, cdev_p->count);
    cdev_del(cdev_p);
}
...
static int __init sx_core_init(void)
{
    ...
    ret = sx_emad_init();
    if (ret) {
        sxd_log_err("failed to initialize EMAD (err=%d)\n", ret);
        goto emad_init_failed;
    }

    ret = sx_qos_emad_init();
    if (ret) {
        sxd_log_err("failed to initialize EMAD QOS (err=%d)\n", ret);
        goto emad_qos_init_failed;
    }

    ret = sx_health_check_init();
    if (ret) {
        sxd_log_err("failed to initialize health-check (err=%d)\n", ret);
    ...
    if (sx_cr_mode()) {
        ...
        if (ret) {
            sxd_log_err("PCI probe failed, aborting (err=%d)\n", ret);
            goto out_cdev;
        }
        ...
        if (probes_total == 0) { /* no PCI device found, create a temporary device */
            ...
            if (ret) {
                sxd_log_err("sx_init_char_dev() failed (err=%d)\n", ret);
                goto out_unreg_dev;
            }
        } else if ((probes_total > 1) && !sx_core_has_predefined_devices()) {
            sxd_log_err(
                "Detected more than one PCI device, driver must have predefined mapping of PCI bus to device ID\n");
            ret = -EINVAL;
            goto out_unreg_dev;
        }
    }
    ...
    sx_dbg_dump_read_handler_register("fd_dump", __fd_dump_handler, NULL, NULL, NULL);

    ret = sx_core_synce_init();
    if (ret < 0) {
        sxd_log_err("sx_core_synce_init() failed (err=%d)\n", ret);
...
out_close_proc:
    if (sx_core_has_predefined_devices()) {
        ...
        sx_dbg_dump_read_handler_unregister("fd_dump", NULL);
...
static bool __dev_iterate_listeners_cleanup(struct sx_dev *dev, void *context)
{
    ...
    sxd_log_info("sx_core_listeners_cleanup (device %d)\n", dev->device_id);
...
static void __exit sx_core_cleanup(void)
{
    ...
    sxd_log_info("sx_core_cleanup_module \n");
    ...
    if (sx_core_has_predefined_devices()) {
        /* Since removing the pci device is tearing down the dev_db,
         *  on multi asic devices we need to deinit some modules before
         *  calling sx_dev_deinit_core_pci */
        sx_dev_db_iterate(sx_health_check_dev_deinit, NULL);
        sx_health_check_deinit();
        sx_dev_db_iterate(sx_dbg_deinit, NULL);
        sx_dev_db_iterate(sx_dpt_dump_hanlders_per_device_deinit, NULL);
        sx_dev_db_iterate(sx_emad_dump_hanlders_per_device_deinit, NULL);
