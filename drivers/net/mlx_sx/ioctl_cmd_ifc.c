...
long ctrl_cmd_memory_to_cpu_map(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if (sx_cr_mode()) {
        printk(KERN_NOTICE "Memory-to-CPU map is not supported in CR mode, ignoring\n");
        return 0;
    }
...
long ctrl_cmd_set_device_profile(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if (profile.module_master_fw_default) {
    ...
    if (err) {
        printk(KERN_ERR PFX "Post set profile event failed, err:%d\n", err);
        goto out;
    }
...
long ctrl_cmd_send_fatal_failure_detect_info_set(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    if (sx_cr_mode()) {
        printk(KERN_NOTICE "Health-Check feature is not supported in CR mode, ignoring\n");
        return 0;
    }
