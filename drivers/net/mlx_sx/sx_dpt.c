...
int sx_dpt_init_dev_pci(struct sx_dev *sx_dev)
{
    ...
    sxd_log_notice("Called PCI DPT initialize with device_id: %d \n", dev_id);
...
{
    sxd_log_err("MST interface is not attached [WRITE: offset=0x%x, len=%d]\n", offset, len);
    return 0;
}

static int sx_dpt_stub_mst_read(int offset, int len, u8 *buf)
{
    sxd_log_err("MST interface is not attached [READ: offset=0x%x, len=%d]\n", offset, len);
    return 0;
}
...
int sx_dpt_add_dev_path(int sx_dev_id, enum ku_dpt_path_type path, union ku_dpt_path_info path_data, uint8_t is_local)
{
    ...
    if (sx_i2c_mode() && (path != DPT_PATH_I2C)) {
        sxd_log_notice("*** In I2C configuration, device set to use %s but will use I2C! ***\n",
                       __dpt_type2str(path));
        path = DPT_PATH_I2C;
        memset(&path_data, 0, sizeof(path_data));
        path_data.sx_i2c_info.sx_i2c_dev = DEFAULT_I2C_DEV_ID;
    ...
    if (sx_mst_mode() && (path != DPT_PATH_MST)) {
        sxd_log_notice("*** In MST configuration, device set to use %s but will use MST! ***\n",
                       __dpt_type2str(path));
        path = DPT_PATH_MST;
        memset(&path_data, 0, sizeof(path_data));
    }
    ...
    case DPT_PATH_I2C:
        ...
        if (sx_glb.sx_i2c.get_local_mbox) {
            ...
            sxd_log_notice("I2C (dev %d): In-Mailbox [offset=0x%x, size=0x%x], Out-Mailbox [offset=0x%x, size=0x%x]\n",
                           sx_dev_id,
                           sx_glb.sx_dpt.dpt_info[sx_dev_id].in_mb_offset[HCR2],
                           sx_glb.sx_dpt.dpt_info[sx_dev_id].in_mb_size[HCR2],
                           sx_glb.sx_dpt.dpt_info[sx_dev_id].out_mb_offset[HCR2],
        ...
        if (!is_local && sx_glb.sx_i2c.get_fw_rev) {
            ...
            if (err) {
                sxd_log_err("I2C bus %d of device %d is not ready. "
        ...
        if (sx_cr_mode() && is_local) {
            ...
            if (err) {
                sxd_log_err("Failed to reassign default device with dev_id %u\n", sx_dev_id);
            }
        }
    ...
    case DPT_PATH_SGMII:
        if (!is_sgmii_supported()) {
            sxd_log_err("SGMII is disabled\n");
            err = -EPERM;
            break;
        }

        if (sx_glb.sx_dpt.dpt_info[sx_dev_id].is_ifc_valid[DPT_PATH_SGMII]) {
            sxd_log_notice("SGMII path for dev %d is already set\n", sx_dev_id);
            err = -EEXIST;
            break;
        }
        ...
        if (err) {
            sxd_log_err("Failed to add SGMII information to 'sgmii_info_to_dev_id' database (err=%d)\n", err);
            break;
        }
    ...
    default:
        sxd_log_err("Try to set invalid path to sx_dev_id: %d, dpt_path: %s\n",
    ...
    if (!err) {
        sxd_log_info("Successfully added path %s to device %d\n",
...
int sx_dpt_remove_dev_path(int sx_dev_id, enum ku_dpt_path_type path)
{
    ...
    if (sx_glb.sx_dpt.dpt_info[sx_dev_id].cmd_path == path) {
        sxd_log_err("Can't remove path because %d "
    ...
    case DPT_PATH_SGMII:
        ...
        if (err) {
            sxd_log_err("Failed to del SGMII information from 'sgmii_info_to_dev_id' database (err=%d)\n", err);
            break;
        }
    ...
    default:
        sxd_log_err("Invalid path.\n");
        err = -EINVAL;
    }

    if (!err) {
        sxd_log_info("Successfully removed path %s from device %d\n",
                     __dpt_type2str(path), sx_dev_id);
    }
...
void __sx_dpt_remove_dev(int sx_dev_id)
{
    ...
    sxd_log_info("Successfully removed device %d from the DPT\n", sx_dev_id);
}
...
int sx_dpt_set_cmd_path(int sx_dev_id, enum  ku_dpt_path_type cmd_path)
{
    if (sx_i2c_mode() && (cmd_path != DPT_PATH_I2C)) {
        sxd_log_notice("*** In I2C configuration, device set to use %s for command-interface but will use I2C! ***\n",
                       __dpt_type2str(cmd_path));
        cmd_path = DPT_PATH_I2C;
    }

    if (sx_mst_mode() && (cmd_path != DPT_PATH_MST)) {
        sxd_log_notice("*** In MST configuration, device set to use %s for command-interface but will use MST! ***\n",
                       __dpt_type2str(cmd_path));
        cmd_path = DPT_PATH_MST;
    }

    if (sx_glb.sx_dpt.dpt_info[sx_dev_id].is_ifc_valid[cmd_path] == false) {
        sxd_log_err("sx_dev_id:%d , path %d is not valid !\n", sx_dev_id, cmd_path);
        return -EINVAL;
    }
...
int sx_dpt_set_emad_path(int sx_dev_id, enum  ku_dpt_path_type emad_path)
{
    if (sx_i2c_mode() && (emad_path != DPT_PATH_I2C)) {
        sxd_log_notice("*** In I2C configuration, device set to use %s for EMAD but will use I2C! ***\n",
                       __dpt_type2str(emad_path));
        emad_path = DPT_PATH_I2C;
    }

    if (sx_mst_mode() && (emad_path != DPT_PATH_MST)) {
        sxd_log_notice("*** In MST configuration, device set to use %s for EMAD but will use MST! ***\n",
                       __dpt_type2str(emad_path));
        emad_path = DPT_PATH_MST;
    }

    if (!sx_glb.sx_dpt.dpt_info[sx_dev_id].is_ifc_valid[emad_path]) {
        sxd_log_err("path %d is not valid !\n", emad_path);
        return -EINVAL;
    }
...
int sx_dpt_set_mad_path(int sx_dev_id, enum  ku_dpt_path_type mad_path)
{
    if ((sx_dev_id < 0) ||
        ...
        sxd_log_err("Device id is not valid\n");
        return -EINVAL;
    }

    if ((mad_path < DPT_PATH_MIN) ||
        (mad_path > DPT_PATH_MAX)) {
        sxd_log_err("MAD path is not valid\n");
        return -EINVAL;
    }

    if (sx_i2c_mode() && (mad_path != DPT_PATH_I2C)) {
        sxd_log_notice("*** In I2C configuration, device set to use %s for MAD but will use I2C! ***\n",
                       __dpt_type2str(mad_path));
        mad_path = DPT_PATH_I2C;
    }

    if (sx_mst_mode() && (mad_path != DPT_PATH_MST)) {
        sxd_log_notice("*** In MST configuration, device set to use %s for MAD but will use MST! ***\n",
                       __dpt_type2str(mad_path));
...
int sx_dpt_set_cr_access_path(int sx_dev_id, enum ku_dpt_path_type cr_access_path)
{
    ...
    if ((cr_access_path < DPT_PATH_MIN) ||
        ...
        sxd_log_err("CR_ACCESS path is not valid.\n");
        return -EINVAL;
    }

    if (sx_i2c_mode() && (cr_access_path != DPT_PATH_I2C)) {
        sxd_log_notice("*** In I2C configuration, device set to use %s for CR-Space but will use I2C! ***\n",
                       __dpt_type2str(cr_access_path));
        cr_access_path = DPT_PATH_I2C;
    }

    if (sx_mst_mode() && (cr_access_path != DPT_PATH_MST)) {
        sxd_log_notice("*** In MST configuration, device set to use %s for CR-Space but will use MST! ***\n",
                       __dpt_type2str(cr_access_path));
    ...
    sxd_log_info("Successfully set CR_ACCESS path for device %d to %s\n",
...
enum ku_dpt_path_type sx_dpt_get_cr_access_path(int sx_dev_id)
{
    ...
    if (sx_glb.sx_dpt.dpt_info[sx_dev_id].cr_access_path != DPT_PATH_INVALID) {
        return sx_glb.sx_dpt.dpt_info[sx_dev_id].cr_access_path;
    }

    dev = sx_dev_db_get_dev_by_id(sx_dev_id);
    if (dev && (sx_priv(dev)->dev_info.dev_info_ro.flags & SX_DEV_INFO_F_PCI_PROBE_FAILURE)) {
        union ku_dpt_path_info path_data; /* dummy data */

        sx_dpt_add_dev_path(sx_dev_id, DPT_PATH_PCI_E, path_data, 1);
        sx_dpt_set_cmd_path(sx_dev_id, DPT_PATH_PCI_E);
        sx_dpt_set_cr_access_path(sx_dev_id, DPT_PATH_PCI_E);
        return DPT_PATH_PCI_E;
    }

    return DPT_PATH_INVALID;
}
...
int sx_get_hw_etclass(struct isx_meta *meta, u8* hw_etclass)
{
    ...
    if (!dev) {
        sxd_log_err("Invalid device id %d\n", meta->dev_id);
        return -EINVAL;
    }

    err = __sx_core_dev_specific_cb_get_reference(dev);
    if (err) {
        sxd_log_err("__sx_core_dev_specific_cb_get_reference failed for device id %d\n", meta->dev_id);
        return err;
    }
...
int sx_get_sqd_num(struct sx_dev *dev, u8 swid, u8 etclass, u8 *sdq)
{
    ...
    if (dev && (sx_priv(dev)->dev_specific_cb.sx_get_sdq_num_cb != NULL)) {
        ...
        err = -EINVAL;
        sxd_log_err("Error retrieving sx_get_sdq_num_to_create_cb callback structure! err=[%d]\n", err);
        goto out;
    }
               ...
               u16             *cap_max_mtu_p)
{
    ...
    if (!dev) {
        sxd_log_err("get sdq: dev is NULL\n");
        return -EINVAL;
    }

    /* this func also take priv->db_lock */
    if (sx_get_hw_etclass(meta, &hw_etclass) != 0) {
        sxd_log_err("Error retrieving HW Etclass!\n");
        return -EINVAL;
    }
    ...
    if (sx_priv(dev)->dev_specific_cb.cap_max_mtu_get_cb != NULL) {
        *cap_max_mtu_p = sx_priv(dev)->dev_specific_cb.cap_max_mtu_get_cb();
        ...
        sxd_log_err("Error retrieving cap_max_mtu_get_cb callback structure!\n");
        ret = -EINVAL;
        goto out;
    }
    ...
    if (sx_priv(dev)->dev_specific_cb.sx_get_sdq_cb != NULL) {
        ...
        sxd_log_err("Error retrieving sx_get_sdq_cb callback structure!\n");
        ret = -EINVAL;
        goto out;
    }
...
int sx_build_isx_header(struct isx_meta *meta, struct sk_buff *skb, u8 stclass)
{
    ...
    if (!dev) {
        sxd_log_err("build isx header: dev is NULL\n");
        return -EINVAL;
    }
    ...
    err = __sx_core_dev_specific_cb_get_reference(dev);
    if (err) {
        sxd_log_err("__sx_core_dev_specific_cb_get_reference failed for dev id %d \n", meta->dev_id);
        return err;
    }
    ...
    if (sx_priv(dev)->dev_specific_cb.sx_build_isx_header_cb != NULL) {
        ...
        sxd_log_err("Error retrieving sx_build_isx_header callback structure!\n");
        __sx_core_dev_specific_cb_release_reference(dev);
        return -EINVAL;
    }
...
int sx_dpt_send_emad(int sx_dev_id, struct sk_buff *skb, struct isx_meta *meta)
{
    ...
    case DPT_PATH_I2C:
        sxd_log_err("Send emad over I2C isn't supported.\n");
        err = -EINVAL;
    ...
    default:
        sxd_log_err("dpt_info[%d].emad_path: %d is invalid\n",
                    sx_dev_id,
...
int sx_dpt_send_mad(int sx_dev_id, struct sk_buff *skb, struct isx_meta *meta)
{
    ...
    case DPT_PATH_I2C:
        sxd_log_err("Send mad over I2C isn't supported.\n");
        err = -EINVAL;
        sx_skb_free(skb);   /* drop packet flow, use kfree_skb */
        break;
    ...
    default:
        sxd_log_err("dpt_info[%d].mad_path: %d is INVALID !\n",
                    sx_dev_id,
    ...
    *dev = NULL;
    ...
    if (sx_glb.sx_dpt.dpt_info[sx_dev_id].cmd_path == DPT_PATH_PCI_E) {
        if (sx_glb.sx_dpt.dpt_info[sx_dev_id].is_ifc_valid[DPT_PATH_PCI_E] == true) {
            *dev = sx_dev_db_get_dev_by_id(sx_dev_id);
            if (!*dev) {
                sxd_log_err("PCIE path is configured but there is no PCI device\n");
                ret = -ENODEV;
            }
        } else {
            sxd_log_err("no PCIE path configured for dev_id %d\n", sx_dev_id);
            ret = -ENXIO;
            goto out;
        }
    } else {
        *dev = sx_dev_db_get_default_device();
        if (!*dev) {
            sxd_log_err("There is no device attached\n");
            ret = -ENODEV;
        }
    }
...
int sx_dpt_get_i2c_dev_by_id(int sx_dev_id, int *i2c_dev)
{
    if (sx_glb.sx_dpt.dpt_info[sx_dev_id].is_ifc_valid[DPT_PATH_I2C] == false) {
        sxd_log_err("Path DPT_PATH_I2C is not valid.\n");
        *i2c_dev = 0;
        return -EINVAL;
    }
...
int sx_dpt_i2c_writel(int dev_id, u32 reg, u32 value)
{
    ...
    if (err) {
        sxd_log_err("sx_dpt_get_i2c_dev_by_id for dev_id: %d "
                    "failed !\n", dev_id);
        return -EINVAL;
    }
    ...
    if (err) {
        sxd_log_warning("sx_dpt_i2c_writel for dev_id: %d failed "
                        "after %d tries!\n", dev_id, i);
        return -EINVAL;
    }
...
int sx_dpt_i2c_write_buf(int dev_id, unsigned int i2c_offset, unsigned char *buf, int size)
{
    ...
    err = sx_dpt_get_i2c_dev_by_id(dev_id, &i2c_dev_id);
    if (err) {
        sxd_log_err("sx_dpt_get_i2c_dev_by_id for dev_id: "
                    "%d failed !\n", dev_id);
        return -EINVAL;
    }
    ...
    if (err) {
        sxd_log_err("sx_dpt_i2c_write_buf for dev_id: %d offset %u "
...
int sx_dpt_i2c_read_buf(int dev_id, unsigned int i2c_offset, unsigned char *buf, int size)
{
    ...
    if (err) {
        sxd_log_err("sx_dpt_i2c_read_buf for dev_id: %d failed "
                    "after %d tries! err: %d \n", dev_id, i, err);
        return -EINVAL;
    }
...
u32 sx_dpt_i2c_readl(int dev_id, u32 address, int *err)
{
    ...
    *err = sx_dpt_get_i2c_dev_by_id(dev_id, &i2c_dev_id);
    ...
    if (*err) {
        sxd_log_debug("sx_dpt_i2c_readl for dev_id: %d failed "
                      "after %d tries!\n", dev_id, i);
        return 0;
    }

    if (i > 0) {
        sxd_log_info("sx_dpt_i2c_read_buf for dev_id: %d "
...
void sx_dump_dpt_entry(int i)
{
    ...
    sxd_log_info("%d:   c:%s e:%s m:%s cr:%s ; ",
                 i,
    ...
    if (dpt_info->is_ifc_valid[DPT_PATH_I2C]) {
        sxd_log_info("i2c (HCR2):0x%x mbox in 0x%x out 0x%x",
    ...
    if (dpt_info->is_ifc_valid[DPT_PATH_MST]) {
        sxd_log_info("MST (HCR2): mbox in 0x%x out 0x%x",
                     dpt_info->in_mb_offset[HCR2],
                     dpt_info->out_mb_offset[HCR2]);
    }

    if (dpt_info->is_ifc_valid[DPT_PATH_SGMII]) {
        sxd_log_info("sgmii: DMAC %02x:%02x:%02x:%02x:%02x:%02x, VID %u",
    ...
    if (dpt_info->is_ifc_valid[DPT_PATH_PCI_E]) {
        sxd_log_info("pci (HCR1): mbox in 0x%x out 0x%x",
...
void sx_dpt_dump(void)
{
    ...
    sxd_log_info("Active DPT entries : %d \n", cnt);
}
...
{
    sxd_log_err("i2c module non registered !!!\n");
    return 0;
}
...
{
    sxd_log_err("i2c module not registered !\n");
    return 0;
}
...
int sx_dpt_cr_space_read(int dev_id, unsigned int address, unsigned char *buf, int size)
{
    ...
    case (DPT_PATH_I2C):
        ...
        if (err) {
            sxd_log_warning("Can't get I2C info of device %d. "
        ...
        if (err) {
            sxd_log_warning("I2C bus of device %d is not ready. "
                            "cr space read will not be performed. err = %d\n",
                            dev_id, err);
            goto out;
        }
    ...
    case (DPT_PATH_PCI_E):
        ...
        if (err) {
            sxd_log_err("Device ID [%u] failed on pci read access\n", dev_id);
            goto out;
        }
    ...
    case DPT_PATH_SGMII:
        ...
        if (err) {
            sxd_log_notice("failed to send CR-Space read-request device:[%d], address:[0x%x] over SGMII (err=%d)\n",
                           dev_id,
                           address,
                           err);
            goto out;
    ...
    case DPT_PATH_MST:
        ...
        if (err) {
            sxd_log_warning("failed to read CR-Space with MST (err=%d)\n", err);
            goto out;
        }
    ...
    default:
        sxd_log_err("sx_dpt_cr_space_read: Can't read from CR space "
...
int sx_dpt_cr_space_write(int dev_id, unsigned int address, unsigned char *buf, int size)
{
    ...
    case (DPT_PATH_PCI_E):
        ...
        if (err) {
            sxd_log_err("Device ID [%u] failed on pcie write access\n", dev_id);
            goto out;
        }
    ...
    case DPT_PATH_SGMII:
        ...
        if (err) {
            sxd_log_err("failed to send CR-Space write-request device:[%d], address:[0x%x] over SGMII (err=%d)\n",
                        dev_id, address, err);
            goto out;
        }
    ...
    case DPT_PATH_MST:
        ...
        if (err) {
            sxd_log_warning("failed to write CR-Space with MST (err=%d)\n", err);
            goto out;
        }
    ...
    default:
        sxd_log_err("sx_dpt_cr_space_write: Can't write to CR space "
                    "of device %u because the DPT path isn't valid\n", dev_id);
        err = -EINVAL;
        break;
    }
                            ...
                            cr_access_method access_method)
{
    ...
    if (buff == NULL) {
        sxd_log_err("__cr_pcie_access: Invalid buffer address for"
                    "device %u\n", dev_id);
        err = -EINVAL;
        goto out;
    }
    ...
    if ((access_method < CR_ACCESS_METHOD_MIN) || (access_method > CR_ACCESS_METHOD_MAX)) {
        sxd_log_err("__cr_pcie_access: Can't access CR space "
    ...
    if ((size % 4) != 0) {
        sxd_log_err("__cr_pcie_access: size %u is invalid, must be a multiply of 4\n", size);
        err = -EINVAL;
        goto out;
    }

    if (!sx_dpt_is_dev_pci_attached(dev_id)) {
        sxd_log_err("__cr_pcie_access: Device ID [%u] is not PCI device\n", dev_id);
        err = -ENODEV;
        goto out;
    }
