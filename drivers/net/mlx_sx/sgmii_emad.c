                             ...
                             struct sgmii_sync_transaction_context *sync_context)
{
    ...
    tr_meta.tr_id = tr_id;
    tr_meta.transport_type = SXD_TRANSPORT_TYPE_EMAD;
...
int sgmii_emad_init(void)
{
    ...
    if (ret) {
        sxd_log_err("failed to create emad transaction db (ret=%d)\n", ret);
        return ret;
    }

    dummy.dont_care.sysport = SYSPORT_DONT_CARE_VALUE;
    ret = sx_core_add_synd(0, SXD_TRAP_ID_GENERAL_ETH_EMAD, L2_TYPE_DONT_CARE,
                           0, "sgmii_driver", 0, &dummy, "SGMII EMAD handler", __sgmii_rx_emad, NULL,
                           CHECK_DUP_DISABLED_E, NULL, NULL, 1);
    if (ret) {
        sxd_log_err("failed to register EMAD handler (ret=%d)\n", ret);
    }
