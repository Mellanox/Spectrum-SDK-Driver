...
static bool __is_long_command_detected(struct sx_health_dev_info *info)
{
    ...
    sx_core_cr_dump_notify_dump_completion(info->dev, true /* query */, &state);
    if (FW_DUMP_COMPLETION_STATE_REQUEST_SENT == state) {
        printk(KERN_NOTICE "Health check - detected long command\n");
                        ...
                        sxd_event_health_data_t     *event_data)
{
    ...
    if (__is_qtm_device(info->dev) && (cause != SXD_HEALTH_CAUSE_FW_LONG_COMMAND) &&
        ...
        printk(
            KERN_NOTICE "Health check issue found: device=%d, cause=%s cause id=%d, severity=%s irisc_id=0x%x - Event is not sent due to long command that was detected!\n",
            info->dev->device_id,
...
static void __sx_health_check_cycle(struct work_struct *work)
{
    ...
    if (__is_qtm_device(info->dev) && __is_long_command_detected(info)) {
        ...
        if (info->long_command_event_counter >= LONG_COMMAND_MAX_COUNTER) {
            ...
            printk(
                KERN_ERR "Health check: long command was not released after %d seconds, Stopping further device monitoring!\n",
...
static void __mfde_work(struct work_struct *work)
{
    ...
    error_msg = ecw->cause_data.external_data.err_msg;
    ...
    case SXD_MFDE_EVENT_ID_CRSPACE_TIMEOUT_E:
        ...
        snprintf(error_msg,
                 ...
                 "FW CR Space timeout: irisc_id = [%d], log_address = [0x%x], old_event = [%d], is_yu = [%d], is_iron = [%d], is_main_farm = [%d], log_id = [%d], log_ip = [0x%llx] [%s], reg_attr_id = [0x%x], mgmt_class = [%d], method = [%d], event_severity=[%d], packet_state = [%d]",
                 mfde_reg->irisc_id,
    ...
    case SXD_MFDE_EVENT_ID_KVD_IM_STOP_E:
        snprintf(error_msg,
                 HEALTH_CHECK_EVENT_MSG_MAX,
                 "FW KVM Stopped: irisc_id = [%d],  old_event = [%d], pipe mask = [%d], reg_attr_id = [0x%x], mgmt_class = [%d], method = [%d], event_severity=[%d], packet_state = [%d]",
                 mfde_reg->irisc_id,
    ...
    case SXD_MFDE_EVENT_ID_FW_ASSERT_E:
        ...
        snprintf(error_msg,
                 HEALTH_CHECK_EVENT_MSG_MAX,
                 "FW Assert: assert_string = [%s], irisc_id = [%d], assert id = [0x%x], assert_var0 = [0x%x], assert_var1 = [0x%x], assert_var2 = [0x%x],"
                 " assert_var3 = [0x%x], assert_var4 = [0x%x], assert_existptr = [0x%x], assert_callra = [0x%x],"
                 " ext_synd = [0x%x], old_event = [0x%x], tile_v = [0x%x], tile_index = [0x%x], reg_attr_id = [0x%x], mgmt_class = [%d], method = [%d], event_severity=[%d], packet_state = [%d]",
                 assert_string,
                 mfde_reg->irisc_id,
    ...
    case SXD_MFDE_EVENT_ID_FATAL_CAUSE_E:
        ...
        if (true == mfde_reg->event_params.fatal_cause.tile_v) {
            ...
            snprintf(error_msg,
                     HEALTH_CHECK_EVENT_MSG_MAX,
                     "FW Fatal: fw_cause = [0x%x], cause_id = [0x%x], irisc_id = [%d], tile_index = [0x%x], reg_attr_id = [0x%x], mgmt_class = [%d], method = [%d], event_severity=[%d], packet_state = [%d]",
            ...
            snprintf(error_msg,
                     HEALTH_CHECK_EVENT_MSG_MAX,
                     "FW Fatal:fw_cause = [0x%x], cause_id = [0x%x], irisc_id = [%d], reg_attr_id = [0x%x], mgmt_class = [%d], method = [%d], event_severity=[%d], packet_state = [%d]",
                     mfde_reg->event_params.fatal_cause.fw_cause,
                     mfde_reg->event_params.fatal_cause.cause_id,
                     mfde_reg->irisc_id,
                     mfde_reg->reg_attr_id,
    ...
    case SXD_MFDE_EVENT_ID_LONG_CMD_TIMEOUT_E:
        ...
        if (__is_qtm_device(dev)) {
            snprintf(error_msg,
                     HEALTH_CHECK_EVENT_MSG_MAX,
                     "FW Long Command Timeout: irisc_id = [%d],  reg_attr_id = [0x%x], mgmt_class = [%d], method = [%d], event_severity=[%d], packet_state = [%d]",
                     mfde_reg->irisc_id,
                     mfde_reg->reg_attr_id,
                     mfde_reg->mgmt_class,
                     mfde_reg->method,
            ...
            printk(KERN_ERR "Long command event received from FW and is being ignored \n");
            goto out;
        }
        break;
    ...
    if ((mfde_reg->severity == SXD_MFDE_SEVERITY_INTR_E) && !is_test) {
        if (printk_ratelimit()) {
            printk(KERN_NOTICE "Health check: got an internal event - ignored [%s]\n", error_msg);
        }
    ...
    case SXD_MFDE_LOG_ERROR:
        printk(KERN_ERR "SXD_HEALTH_FW_FATAL: %s\n", error_msg);
        break;

    case SXD_MFDE_LOG_NOTICE:
        printk(KERN_NOTICE "Health check: %s\n", error_msg);
        break;
    ...
    ecw->cause_data.external_data.err_msg[HEALTH_CHECK_EVENT_MSG_MAX - 1] = 0;
