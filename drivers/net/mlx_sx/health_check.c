...
#define NUM_OF_HC_CYCLE_ITER_TD_WORKER_NOT_RESPONSE 200
                                                ...
                                                ku_thread_status_changed_t *threads_info)
{
    ...
    unsigned long now = (jiffies_to_msecs(jiffies) / 1000);

    /* bitmap and bitmap_subset share the same size */
    max = sx_bitmap_num_bits(running_bitmap);
    for (bit_id = 0; bit_id < max; bit_id++) {
        if (sx_bitmap_test(running_bitmap, bit_id) && sx_bitmap_test(last_bitmap, bit_id)) {
            threads_info[bit_id].time_passed_from_last_update = now - threads_info[bit_id].last_update_time;
...
static void __check_and_report_sdq_failure(struct sx_health_dev_info *info, u32 sub_bitmap_res)
{
    ...
    if (info->config.sdq.num_of_check_iter >= info->config.sdq.max_iter_allowed) {
        sxd_log_notice("Health-Check:Did not receive completion for SDQ dqn (%u) \n",
                       sub_bitmap_res);
        snprintf(err_msg, sizeof(err_msg) - 1, "SDQ #%u", sub_bitmap_res);
    ...
    for (sdq_id = 0; sdq_id < max_sdq; sdq_id++) {
        ...
        if (sdq) {
            ...
            if (skb_headroom(skb) < ISX_HDR_SIZE) {
                ...
                if (ret != 0) {
                    sxd_log_err("Health-Check:__test_sdq failed to post and send"
                                " via SDQ_Id =%d ,(ret=%d)\n", sdq_id, ret);
                    goto out;
                }
                ...
                if (ret != 0) {
                    sxd_log_err("Health-Check:_test_sdq failed to post and send"
                                " via SDQ_Id =%d ,(ret=%d)\n", sdq_id, ret);
                    goto out;
                }
            }
...
static void __check_sdq_from_last_cycle(struct sx_health_dev_info *info)
{
    ...
    sub_bitmap_res = __bitmaps_covered(&priv->health_check.operational_state.sdq_bitmap, &info->config.sdq.sdq_bitmap);
...
static void __check_rdq_from_last_cycle(struct sx_health_dev_info *info)
{
    ...
    sub_bitmap_res =
        __bitmaps_covered(&priv->health_check.operational_state.rdq_bitmap, &info->config.last_rdq_bitmap);
    if (sub_bitmap_res != ((u32) - 1)) {
        snprintf(err_msg, sizeof(err_msg) - 1, "RDQ #%u", sub_bitmap_res);
...
static void __check_sdk_hc_monitor_init_thread(struct sx_health_dev_info *info)
{
    ...
    if ((info->config.thread_monitoring_status == THREAD_MONITOR_ACTIVE_E) &&
        ...
        if (info->config.sdk_thread_info.num_of_iter_without_cnt_increase ==
            NUM_OF_HC_CYCLE_ITER_TD_WORKER_NOT_RESPONSE) {
...
static void __set_threads_last_time_update(ku_thread_status_changed_t *thread_status)
{
    ...
    for (thread_index = 0; thread_index < NUM_OF_SDK_THREADS_MAP_BY_BITS; thread_index++) {
        ...
        if (thread_status[thread_index].thread_id) {
            thread_status[thread_index].last_update_time = now;
        }
    }
}
...
static int __sx_health_update_sdk_thread_monitor(u8 dev_id, const threads_monitor_info_t* threads_info)
{
    ...
    sub_bitmap_res = __bitmaps_covered(&info->config.sdk_thread_info.running_bitmap,
                                       &info->config.sdk_thread_info.last_sent_bitmap);
    if ((sub_bitmap_res != ((u32) - 1)) && (info->config.thread_monitoring_status == THREAD_MONITOR_ACTIVE_E)) {
        if (info->config.sdk_thread_info.bit_to_thread_status_arr[sub_bitmap_res].thread_id == 0) {
            sxd_log_err("Health check:sdk monitor -Internal error the missing bit = [%u] contain thread id 0 \n",
                        info->config.sdk_thread_info.bit_to_thread_status_arr[sub_bitmap_res].bit_index);
            goto out;
        }
        snprintf(error_msg,
                 sizeof(error_msg),
                 "SDK thread [%s] TID [0x%llx] bit [%d] does not respond [%lu] seconds",
                 info->config.sdk_thread_info.bit_to_thread_status_arr[sub_bitmap_res].name,
                 info->config.sdk_thread_info.bit_to_thread_status_arr[sub_bitmap_res].thread_id,
                 info->config.sdk_thread_info.bit_to_thread_status_arr[sub_bitmap_res].bit_index,
                 info->config.sdk_thread_info.bit_to_thread_status_arr[sub_bitmap_res].time_passed_from_last_update);
        sxd_log_err("%s\n", error_msg);
...
static int __sx_health_update_sdk_thread_status_changed(u8 dev_id, const ku_thread_status_changed_t* threads_info)
{
    ...
    if (SX_HEALTH_CHECK_DELETE_THREAD == threads_info->cmd) {
        ...
        info->config.sdk_thread_info.bit_to_thread_status_arr[threads_info->bit_index].time_passed_from_last_update =
            0;
        info->config.sdk_thread_info.bit_to_thread_status_arr[threads_info->bit_index].last_update_time = now;
    }
                        ...
                        bool                        fatal_error_mode_active)
{
    ...
    for (thread_index = 0; thread_index < NUM_OF_SDK_THREADS_MAP_BY_BITS; thread_index++) {
        ...
        if (priv->prev_sdk_thread_status_arr[thread_index].thread_id) {
            ...
            info->config.sdk_thread_info.sdk_thread_monitor_cnt++;
            info->config.sdk_thread_info.bit_to_thread_status_arr[thread_index].last_update_time = now;
        }
    }
    info->config.thread_monitoring_status = THREAD_MONITOR_ACTIVE_E;
...
static int __health_check_dbg_cmd_handler(int argc, const char *argv[], void *context)
{
    struct sx_health_dev_info *info = NULL;
    int                        dev_id;
    int                        err = 0;
    ...
    if (err) {
        sxd_log_notice("Health-Check: debug command - device ID is invalid\n");
        return -EINVAL;
    }
    ...
    info = __info_find(dev_id);
    if (!info) {
        sxd_log_notice("Health-Check: debug command - device not found\n");
        err = -ENODEV;
        goto out;
    }

    if (strcmp(argv[2], "stop") == 0) {
        sxd_log_notice("Health-Check: debug command - stopping health-check for device %d\n", dev_id);
        __info_dealloc(info);
        goto out;
    }
    ...
    if (strcmp(argv[2], "mute") == 0) {
        sxd_log_notice("Health-Check: debug command - muting health events for device %d\n", dev_id);
        info->config.debug_state = HEALTH_DEBUG_MUTE;
    } else if (strcmp(argv[2], "normal") == 0) {
        sxd_log_notice("Health-Check: debug command - normal health-check functionality for device %d\n", dev_id);
        info->config.debug_state = HEALTH_DEBUG_NORMAL;
    } else if (strcmp(argv[2], "dont_stop_on_fatal") == 0) {
        sxd_log_notice("Health-Check: debug command - don't stop health cycles even if device %d is fatal\n", dev_id);
        info->config.debug_state = HEALTH_DEBUG_DONT_STOP_ON_FATAL;
    } else {
        sxd_log_notice("Health-Check: debug command - invalid argument [%s]\n", argv[2]);
        err = -EINVAL;
        goto out_info_unlock;
    }
...
int sx_health_check_init(void)
{
    ...
    /* release this lock during sx_dbg_dump_write_handler_register to avoid circular lock of __proc_list_lock while holding __health_check_lock. */
    mutex_unlock(&__health_check_lock);
    err = sx_dbg_dump_write_handler_register("health_check",
                                             "<dev_id> <mute | dont_stop_on_fatal | stop | normal>",
                                             "health check debug commands",
                                             __health_check_dbg_cmd_handler,
                                             NULL);
    if (err) {
        sxd_log_warning("Health-Check: failed to register health-check debug handler (not mandatory) (err=%d)\n",
                        err);
        err = 0;
    }
    mutex_lock(&__health_check_lock);
...
int sx_health_check_deinit(void)
{
    ...
    /* release this lock during sx_dbg_dump_write_handler_unregister to avoid circular lock of __proc_list_lock while holding __health_check_lock. */
    mutex_unlock(&__health_check_lock);
...
int sx_health_check_configure(ku_dbg_health_check_params_t *params)
{
    ...
    case SXD_HEALTH_FATAL_FAILURE_ENABLE_E:
        sxd_log_debug("Health-Check: Enable device %u, issu_on %d\n",
                      params->dev_id,
    ...
    case SXD_HEALTH_FATAL_FAILURE_DISABLE_E:
        sxd_log_debug("Health-Check: Disable device %u\n", params->dev_id);
    ...
    case SXD_HEALTH_FATAL_FAILURE_UPDATE_SDK_THREAD_STATUS_CHANGED_E:
        sxd_log_debug("Health-Check: Update SDK thread ('%s') status changed to '%s' on bit index %d for device %u \n",
    ...
    case SXD_HEALTH_FATAL_FAILURE_ADD_EMAD_TIMEOUT_FAILURE_E:
        sxd_log_debug("Health-Check: Add timeout error for device %u \n", params->dev_id);
    ...
    case SXD_HEALTH_FATAL_FAILURE_ISSU_ON_E:
        sxd_log_debug("Health-Check: Set ISSU on mode for device %u\n",  params->dev_id);
        err = __sx_health_set_issu_on(params->dev_id, &cyc_finish_wq_completion);
        break;

    case SXD_HEALTH_FATAL_FAILURE_ISSU_OFF_E:
        sxd_log_debug("Health-Check: Set ISSU off mode for device %u\n",  params->dev_id);
...
static void __dump_info(struct seq_file *m, void *v, void *context, struct sx_health_dev_info *info)
{
    ...
    seq_printf(m, "    Last time that running counter increased [%d] msec",
    ...
    if (info->catas_iomap) {
        seq_printf(m, "        CATAS value (@offset 0x%x) ............... 0x%08x\n",
    ...
    seq_printf(m, "    Interrupt Handler (tasklet)\n");
    ...
    for (thread_index = 0; thread_index < NUM_OF_SDK_THREADS_MAP_BY_BITS; thread_index++) {
        if (info->config.sdk_thread_info.bit_to_thread_status_arr[thread_index].thread_id) {
            seq_printf(m,
                       "            %15s ,bit_id [%2d] last response time [%2lu] sec, MUTED [%3s]\n",
...
void sx_health_check_set_debug_trigger(struct sx_health_check_trigger_params *params)
{
    ...
    case SX_HEALTH_CHECK_TRIGGER_OP_EXTEND_THRESHOLD_THREAD_MONITOR:
        sxd_log_notice("Health-Check: dev id = [%d] multiplied SDK threads threshold time by %u\n",
                       params->dev_id,
                       params->params.sdk_threshold_mult_params.mult_value);
        info->config.sdk_thread_info.mult_threshold_time = params->params.sdk_threshold_mult_params.mult_value;
        break;
