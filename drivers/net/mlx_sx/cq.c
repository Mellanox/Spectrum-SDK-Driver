...
void sx_printk_cqe_v0(union sx_cqe *u_cqe)
{
    ...
    sxd_log_rl_debug("CQEv0 %p contents:\n%08x\n%08x\n%08x\n%08x\n",
                     u_cqe->v0,
...
void sx_printk_cqe_v1(union sx_cqe *u_cqe)
{
    ...
    sxd_log_rl_debug("CQEv1 %p contents:\n%08x\n%08x\n%08x\n%08x\n",
                     u_cqe->v1,
                     be32_to_cpu(cqebuf[0]),
                     be32_to_cpu(cqebuf[1]),
                     be32_to_cpu(cqebuf[2]),
...
static void sx_cpu_port_loopback(struct completion_info *ci)
{
    ...
    if (!tmp_skb) {
        sxd_log_info("%s: fail to alloc_skb\n", __func__);
        err = -ENOMEM;
        goto out;
    }
    ...
    if (err) {
        sxd_log_info("%s: sx_core_post_send FAILED\n", __func__);
        goto out;
    }
...
static inline void sx_reg_sbctr_tclass_set(void *reg_data, u64 tc_vec)
{
    struct sxd_emad_sbctr_reg* sbctr = (struct sxd_emad_sbctr_reg *)reg_data;

    sbctr->tc_vec = cpu_to_be64(tc_vec);
}

static void sx_handle_sbctr_event(struct completion_info *ci)
{
    struct ku_sbctr_reg        sbctr_reg;
    struct sxd_emad_sbctr_reg* sbctr = (struct sxd_emad_sbctr_reg *)ci->skb->data;
    u8                        *sbctr_outbox = (u8*)ci->skb->data + sizeof(struct sx_emad) +
    ...
    __SBCTR_decode(sbctr_outbox, &sbctr_reg, NULL);

    SX_PORT_BUILD_PHY_ID_FROM_LSB_MSB(local_port,
                                      sbctr_reg.local_port,
                                      sbctr_reg.lp_msb);

    if (local_port > MAX_PHYPORT_NUM) {
        sxd_log_err("Received local_port %d is invalid (max. %d) \n",
                    sbctr_reg.local_port, MAX_PHYPORT_NUM);
        return;
    }

    dir_ing = sbctr_reg.dir_ing;
    entity = sbctr_reg.entity;
    fp = sbctr_reg.fp;
    tc_vec = (((u64)sbctr_reg.tclass_vector_high << 32) | sbctr_reg.tclass_vector_low);
    ...
    if (entity == 0) { /* entity is port tc */
        ...
        if (tc_vec == old_tc_vec) {
            ...
            /* re-write the event */
            sx_reg_sbctr_tclass_set(ci->skb->data, new_tc_vec);
        }
...
static void sx_handle_fshe_event(struct completion_info *ci)
{
    ...
    if (err) {
        sxd_log_err("__FSHE_decode returned with error %d\n", err);
        return;
    }
...
static void sx_handle_meccc_event(struct completion_info *ci)
{
    ...
    if (err) {
        sxd_log_err("__MECCC_decode returned with error %d\n", err);
        return;
    }
...
static void sx_handle_mfde_event(struct completion_info *ci)
{
    ...
    if (err) {
        sxd_log_err("__MFDE_decode returned with error %d\n", err);
        return;
    }
...
static void sx_parse_event(struct completion_info *ci)
{
    ...
    case SXD_TRAP_ID_MOFTD:
        sx_bulk_cntr_handle_moftd(ci);
        break;
...
int dispatch_pkt(struct sx_dev *dev, struct completion_info *ci, u16 entry, int dispatch_default)
{
    ...
    if (entry > NUM_HW_SYNDROMES) {
        sxd_log_err("Error: arrived synd %d is out of range (1..%d) \n",
...
static int __verify_hw_synd(struct sx_priv *priv, struct completion_info *ci)
{
    ...
    if (ci->hw_synd == SXD_TRAP_ID_PUDE) {
        ...
        if ((pude->oper_status == PORT_OPER_STATUS_UP) &&
            ...
            sxd_log_debug("Skipping PUDE [%d] event for port [%d] - admin status: %d \n",
                          pude->oper_status,
                          pude->local_port,
...
static void __print_rx_packet(struct completion_info *ci, union sx_cqe *u_cqe)
{
    ...
    if (rx_debug_metadata) {
        sxd_log_debug("RX_META: swid=%d, sysport=%d, hw_synd=%d%s, skb_len=%d, pkt_type=%d, byte_count=%d, "
           ...
           u8                            dev_id)
{
    ...
    if ((ci->sysport != 0) || (ci->is_lag != 0)) {
        ...
        if (swid == 255) { /* port is in migration between 2 swids */
            if (rx_debug) {
                sxd_log_err("port (sysport=%u, is_lag=%u) is temporarily disabled! dropping packet!\n",
    ...
    case PKT_TYPE_FCoETH:
        ...
        if (ci->info.eth.ethtype == ETHTYPE_EMAD) {
            ...
            if (ci->info.eth.emad_tid == 0xffffffff) {
                sxd_log_debug("emad_tid =  0x%x HEALTH CHECK packet via "
    ...
    case PKT_TYPE_ETHoIB:
        ...
        if (ci->info.ib.qpn == 0xffffffff) {
            sxd_log_rl_warning("Received IB packet "
...
static int post_skb(struct sx_dq *dq)
{
    ...
    if (!new_skb) {
        sxd_log_err("failed to allocate a new buffer with size of %u for RDQ %d\n", size, dq->dqn);
    ...
    if (skb_put(new_skb, size) == NULL) {
        sxd_log_err("failed to set buffer size %u to a new buffer for RDQ %d\n", size, dq->dqn);
...
#endif
{
    ...
    if (cqe_params.is_send) {
        ...
        if (err) {
            sxd_log_err("Error: failed to get max RDQ params (err=%d)\n", err);
            return err;
        }
    ...
    if (!dq) {
        sxd_log_rl_warning("could not find dq context for %s "
                           "dqn = %d\n",
    ...
    if (cqe_params.is_send) {
        ...
        do {
            ...
            /* update occupancy when completion is received for SDQ.*/
            sx_dq_update_occupancy(dq);
            ...
            if (!found) {
                sxd_log_err("SDQ [%u]: completion for an unknown index (idx=%u)\n",
                            cqe_params.dqn, idx);
            } else {
                list_del(pos);
                ...
                if (time_after(jiffies, curr_pkt->since + SX_LATE_COMPLETION_THRESHOLD)) {
                    ...
                    sxd_log_rl_warning("Receive late completion for dqn (%d) idx (%d)\n",
                                       cqe_params.dqn, idx);
                } else {
        ...
        idx = dq->tail & (dq->wqe_cnt - 1);
        while (idx != wqe_ctr) {
            sxd_log_rl_debug("sx_poll_one: Err wqe_ctr [%u] != idx [%u] (dq->tail [%u])\n",
                             wqe_ctr, idx, dq->tail);
                     ...
                     struct sx_bitmap      *prio_bitmap)
{
    ...
    if (!cq) {
        if (!priv->global_flushing) {
            sxd_log_rl_warning("Completion event for bogus CQ %08x\n", cqn);
        }
...
void sx_core_destroy_cq(struct sx_dev *dev, struct sx_cq *cq, struct sx_dq *dq)
{
    ...
    if (err) {
        sx_warn(dev, "HW2SW_CQ failed (%d) for CQN %06x\n", err, cq->cqn);
    }

    synchronize_irq(priv->eq_table.eq[SX_EQ_COMP].irq);

    __sx_cq_dec_refcnt(cq);
    if (!err && (wait_for_completion_timeout(&cq->free, msecs_to_jiffies(10000)) == 0)) {
        sxd_log_err("sx_core: Error - trying to delete CQ but it is still in use! "
                    "[cqn=%d, is_monitor=%s, DQ_head=%u, DQ_tail=%u, cq_active_bitmap=0x%x]\n",
                    cq->cqn,
                    (dq->is_monitor ? "true" : "false"),
                    dq->head,
                    dq->tail,
                    sx_cq_active_bitmap_get(dev, cq->cqn));
    }
...
int iterate_active_cqs(struct sx_dev *dev, struct sx_bitmap *active_cq_bitmap)
{
    ...
    for (cqn = 0; cqn < priv->dev_cap.max_num_cqs; cqn++) {
        ...
        if (num_of_cqe_handled < 0) {
            if (!priv->global_flushing) {
                sxd_log_warning("CQ completion failed (err=%d)\n", num_of_cqe_handled);
            }
...
static int __monitor_cq_handler_thread(void *arg)
{
    ...
    sxd_log_debug("starting new device's monitor CQ handler thread\n");
    ...
    sxd_log_debug("terminating new device's monitor CQ handler thread\n");
...
static int __low_priority_cq_handler_thread(void *arg)
{
    ...
    sxd_log_debug("starting new device's low-priority CQ handler thread\n");
    ...
    sxd_log_debug("terminating new device's low-priority CQ handler thread\n");
...
int __cpu_traffic_priority_init(struct sx_dev *dev, struct cpu_traffic_priority *cpu_traffic_prio)
{
    ...
    if (IS_ERR(cpu_traffic_prio->low_prio_cq_thread)) {
        ...
        sxd_log_err("failed to create low-priority thread (err=%d)\n", err);
        goto del_timer;
    }
    ...
    if (err) {
        sxd_log_err("Monitor RDQ bitmap init failed. Aborting...\n");
        goto stop_low_prio_cq_thread;
    }

    err = sx_bitmap_init(&sx_priv(dev)->active_monitor_cq_bitmap, priv->dev_cap.max_num_cqs);
    if (err) {
        sxd_log_err("Active monitor RDQ bitmap init failed. Aborting...\n");
        goto stop_low_prio_cq_thread;
    }
    ...
    if (IS_ERR(cpu_traffic_prio->monitor_cq_thread)) {
        ...
        sxd_log_err("failed to create monitor cq thread (err=%d)\n", err);
        goto stop_low_prio_cq_thread;
    }
...
void __cpu_traffic_priority_deinit(struct sx_dev *dev, struct cpu_traffic_priority *cpu_traffic_prio)
{
    ...
    if (err) {
        sxd_log_err("Monitor RDQ bitmap init failed.\n");
    }
    ...
    for (entry = 0; entry < NUM_HW_SYNDROMES + 1; entry++) {
        ...
        if (!list_empty(&info->per_synd_list[entry])) {
            list_for_each(pos, &info->per_synd_list[entry]) {
                ...
                case L2_TYPE_ETH:
                    sxd_log_info("list_type: ETH, crit "
                ...
                case L2_TYPE_FC:
                    sxd_log_info("list_type: FC \n");
                    break;

                default:
                    sxd_log_info("list_type: UNKNOWN \n");
                    break;
                }
            }
        }
...
void sx_cq_show_cq(struct sx_dev *dev, int cqn)
{
    ...
    if (!cq) {
        sxd_log_err("cq %d doesn't exist \n", cqn);
        return;
    }

    sxd_log_info("[cq %d]: cqn:%d, cons_index:%d, nent:%d cons_index&(nent-1):%d"
...
void sx_cq_flush_rdq(struct sx_dev *dev, int dqn)
{
    ...
    if (err) {
        sxd_log_err("failed to pause RDQ %d\n", dqn);
        return;
    }
    ...
    sxd_log_notice("RDQ flushed %d packets\n", num_of_packets);
...
/* use this function to make sure that a CQ and its associated DQ are idle,
 * which means that no tasklet, no low-priority or monitoring thread runs
 * the CQ/DQ. Currently we use this function is two cases:
 ...
 */
u32 sx_cq_active_bitmap_get(struct sx_dev *dev, int cqn)
{
    struct sx_priv              *priv = sx_priv(dev);
...
int sx_cq_pause(struct sx_cq *cq, struct sx_priv* priv)
{
    ...
    if (wait_for_completion_timeout(&priv->pause_cqn_completion, msecs_to_jiffies(5000)) == 0) {
        sxd_log_err("CQ is still active (CQ=%d, cq_active_bitmap=0x%x)\n",
                    priv->pause_cqn,
                    sx_cq_active_bitmap_get(&priv->dev, cq->cqn));
        priv->pause_cqn = -1;
        err = -ETIMEDOUT;
        goto out;
    }
