...
static const char * const __sxd_trap_id_str_s[] = {
    ...
    [SXD_TRAP_ID_FSED] = "SXD_TRAP_ID_FSED",   /**< FSED (Bulk counters) */
    [SXD_TRAP_ID_IPAC_DONE] = "SXD_TRAP_ID_IPAC_DONE", /**< IPAC DONE */
    ...
    [SXD_TRAP_ID_MOFRB] = "SXD_TRAP_ID_MOFRB",    /**< MOFRB */
    ...
    [SXD_TRAP_ID_MAFRI] = "SXD_TRAP_ID_MAFRI", /**< MAFRI (Bulk counters) */
    ...
    [SXD_TRAP_ID_MCION] = "SXD_MCION", /**< Management IO cable notification. */
    [SXD_TRAP_ID_PTSE] = "SXD_PTSE", /**< TX ready event. */
    [SXD_TRAP_ID_MFCDR] = "SXD_MFCDR", /**< MFCDR (Fake cable) . */
    [SXD_TRAP_ID_UPCNT] = "SXD_TRAP_ID_UPCNT", /**< UPCNT (Bulk counters) */
    [SXD_TRAP_ID_UTCC] = "SXD_TRAP_ID_UTCC", /**< UTCC (Bulk counters) */
    [SXD_TRAP_ID_USACN] = "SXD_TRAP_ID_USACN", /**< USACN (Bulk counters) */
    [SXD_TRAP_ID_MFRI] = "SXD_TRAP_ID_MFRI", /**< MFRI (Fast Recovery) */
    [SXD_TRAP_ID_PPIR] = "SXD_TRAP_ID_PPIR", /**< PPIR */
    [SXD_TRAP_ID_MOFTD] = "SXD_TRAP_ID_MOFTD", /**< MOFTD (Bulk counters) */
    [SXD_TRAP_ID_SPZR] = "SXD_TRAP_ID_SPZR", /**< SPZR */
    [SXD_TRAP_ID_IBISSU] = "SXD_TRAP_ID_IBISSU", /**< IBISSU */
    ...
    [SXD_TRAP_ID_ETH_L3_URPF_PROTECTION] = "SXD_L3_URPF_PROTECTION",
    ...
    [SXD_TRAP_ID_ARN_RECEIVE_OK] = "SXD_ARN_RECEIVE_OK",
    ...
    [SXD_TRAP_ID_DISCARD_ING_PACKET_SMAC0] = "SXD_DISCARD_ING_PACKET_SMAC0",
    ...
    [SXD_TRAP_ID_DISCARD_DEC_NVE_OPTIONS] = "SXD_DISCARD_DEC_NVE_OPTIONS",
    ...
    [SXD_TRAP_ID_DISCARD_OVERLAY_SWITCH_SMAC0] = "SXD_DISCARD_OVERLAY_SWITCH_SMAC0",
...
static void __print_listener(struct seq_file *m, u16 synd, struct listener_entry           *listener)
{
    ...
    seq_printf(m, "%-40s | %-8d | %-4u | %-21s | %-50s | %-25s | %-15llu | %u  \n",
               __trap_id_str(synd),
               synd,
               listener->swid,
               source_name,
               listener->handler_str,
               type_name,
               listener->rx_pkts,
               listener->is_default);
}
...
static int __trap_reg_dump(struct seq_file *m, void *v, void *context)
{
    ...
    priv = sx_priv(dev);
    sx_dbg_dump_print_header(m, "Device Trap Registration");

    sx_dbg_dump_print_table_header(m, "%-40s | %-8s | %-4s | %-21s | %-50s | %-25s | %-15s | %s",
    ...
    for (synd = 0; synd < NUM_HW_SYNDROMES + 1; synd++) {
        if (!list_empty(&info->per_synd_list[synd])) {
            list_for_each(pos, &info->per_synd_list[synd]) {
                ...
                __print_listener(m, synd, listener);
            }
        }
    }
...
static int __trap_filter_register_dump(struct seq_file *m, void *v, void *context)
{
    ...
    priv = sx_priv(dev);
    sx_dbg_dump_print_header(m, "Device Trap Registrations and Filters");
    ...
    for (synd = 0; synd < NUM_HW_SYNDROMES + 1; synd++) {
        if (!list_empty(&info->per_synd_list[synd])) {
            list_for_each(pos, &info->per_synd_list[synd]) {
                ...
                sx_dbg_dump_print_table_header(m, "%s", "listener Reg/Filter info:");

                seq_printf(m, "%s", "Global Filter: ");
                listener->listener_register_filter.is_global_filter ? seq_printf(m, "%s", "True") : seq_printf(
                    m,
                    "%s",
                    "False");
                ...
                seq_printf(m, "%s", "\nGlobal Register: ");
                listener->listener_register_filter.is_global_register ? seq_printf(m, "%s", "True") : seq_printf(
                    m,
                    "%s",
                    "False");
...
static int __host_ifc_stats_dump(struct seq_file *m, void *v, void *context)
{
    ...
    sx_dbg_dump_print_table_header(m, "%-30s   %-15s",
                                   "RX-type", "RX-packets");
    ...
    sx_dbg_dump_print_table_header(m, "%-30s   %-15s   %-15s",
    ...
    sx_dbg_dump_print_table_header(m, "%-4s   %-15s   %-15s",
    ...
    sx_dbg_dump_print_table_header(m, "%-3s   %-15s   %-15s",
...
static int __unconsumed_pkt_dump(struct seq_file *m, void *v, void *context)
{
    ...
    sx_dbg_dump_print_table_header(m, "%-40s|%-8s| %s",
...
static int __trap_filter_dump(struct seq_file *m, void *v, void *context)
{
    ...
    sx_dbg_dump_print_table_header(m, "%-40s   %-7s   %-7s",
    ...
    sx_dbg_dump_print_table_header(m, "%-40s   %-7s   %-6s",
...
{
    sx_dbg_dump_read_handler_register("trap_reg_dump", __trap_reg_dump, NULL, dev, NULL);
    sx_dbg_dump_read_handler_register("trap_filter_register_dump", __trap_filter_register_dump, NULL, dev, NULL);
    sx_dbg_dump_read_handler_register("host_ifc_stats_dump", __host_ifc_stats_dump, NULL, dev, NULL);
    sx_dbg_dump_read_handler_register("unconsumed_pkt_dump", __unconsumed_pkt_dump, NULL, dev, NULL);
    sx_dbg_dump_read_handler_register("trap_filter_dump", __trap_filter_dump, NULL, dev, NULL);
    return 0;
}

void sx_dbg_host_ifc_deinit(struct sx_dev *dev)
{
    sx_dbg_dump_read_handler_unregister("trap_reg_dump", dev);
    sx_dbg_dump_read_handler_unregister("trap_filter_register_dump", dev);
    sx_dbg_dump_read_handler_unregister("host_ifc_stats_dump", dev);
    sx_dbg_dump_read_handler_unregister("unconsumed_pkt_dump", dev);
    sx_dbg_dump_read_handler_unregister("trap_filter_dump", dev);
}
