                         ...
                         ...)
{
    ...
    err = sx_dbg_dump_read_handler_register(dump_name, __internal_log_dump, NULL, NULL, (void*)ilog);
    if (err) {
        sxd_log_err("Internal log: failed to create proc entry '%s'\n", dump_name);
        goto dump_handler_failed;
    }

    if (__num_of_logs == 0) { /* this is the first log */
        err = sx_dbg_dump_write_handler_register("log-cmd",
                                                 "log-cmd clear <log_name>\n"
                                                 "log-cmd set-min-severity <log_name> <critical|error|warning|notice|info|debug>",
                                                 "Clean log",
                                                 __internal_log_cmd,
                                                 NULL);
        if (err) {
            sxd_log_err("Internal log: failed to add 'log-cmd' command\n");
...
cmd_handler_failed:
    sx_dbg_dump_read_handler_unregister(dump_name, NULL);
