...
static void __sgmii_send_mad_deferred(void *task_param)
{
    ...
    tr_meta.transport_type = SXD_TRANSPORT_TYPE_MAD;
...
int sgmii_mad_init(void)
{
    ...
    if (ret) {
        sxd_log_err("failed to create mad transaction db (ret=%d)\n", ret);
        return ret;
    }

    crit.ib.qpn = 0;
    ret = sx_core_add_synd(0,
                           SXD_TRAP_ID_INFINIBAND_QP0,
                           L2_TYPE_IB,
                           0,
                           "sgmii_driver",
                           0,
                           &crit,
                           "SGMII MAD (QP0) handler",
                           __sgmii_rx_mad,
                           NULL,
                           ...
                           1);
    if (ret) {
        sxd_log_err("failed to register MAD QP0 handler (ret=%d)\n", ret);
        goto mad_qp0_failed;
    }

    crit.ib.qpn = 1;
    ret = sx_core_add_synd(0,
                           SXD_TRAP_ID_INFINIBAND_QP1,
                           L2_TYPE_IB,
                           0,
                           "sgmii_driver",
                           0,
                           &crit,
                           "SGMII MAD (QP1) handler",
                           __sgmii_rx_mad,
                           NULL,
                           CHECK_DUP_DISABLED_E,
                           NULL,
                           ...
                           1);
    if (ret) {
        sxd_log_err("failed to register MAD QP1 handler (ret=%d)\n", ret);
        goto mad_qp1_failed;
    }
    ...
    ret = sx_core_add_synd(0,
                           ...
                           1);
    if (ret) {
        sxd_log_err("failed to register MAD other-QPs handler (ret=%d)\n", ret);
