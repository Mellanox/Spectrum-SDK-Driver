...
static long __ctrl_cmd_not_implemented(struct file *file, unsigned int cmd, unsigned long data)
{
    ...
    sxd_log_err("ioctl() command %u is not implemented\n", cmd);
    return -ENOTTY;
}
...
static const ioctl_handler_cb_t
    ...
    [IOCTL_CMD_INDEX(CTRL_CMD_BULK_CNTR_BUFFER_STATS_SET)] = ctrl_cmd_bulk_cntr_buffer_stats_set,
    [IOCTL_CMD_INDEX(CTRL_CMD_BULK_CNTR_BUFFER_INFO_GET)] = ctrl_cmd_bulk_cntr_buffer_info_get,
    [IOCTL_CMD_INDEX(CTRL_CMD_BULK_CNTR_BUFFER_INFO_SET)] = ctrl_cmd_bulk_cntr_buffer_info_set,
    ...
    [IOCTL_CMD_INDEX(CTRL_CMD_SET_MMAP_MEM_ALLOC_SCHEME)] = ctrl_cmd_set_mmap_alloc_scheme,
    [IOCTL_CMD_INDEX(CTRL_CMD_SET_SYSFS_SNIFFER)] = ctrl_cmd_set_sysfs_sniffer_mode,
    [IOCTL_CMD_INDEX(CTRL_CMD_SET_OWNER)] = ctrl_cmd_set_owner,
...
long sx_core_ioctl(struct file *filp, unsigned int cmd, unsigned long data)
{
    ...
    if (handler_uses_pci_restart_lock) {
        ...
        err = down_read_interruptible(&sx_glb.pci_restart_lock);
        if (err) { /* -EINTR in case of interrupt */
            goto out;
        }
#else
