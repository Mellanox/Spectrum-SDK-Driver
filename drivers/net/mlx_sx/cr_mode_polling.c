...
static bool __cr_check_for_rx_packet(struct sx_dev *dev, struct ku_access_mopr_reg *mopr_reg_data)
{
    ...
    if (err) {
        sxd_log_err("CR check for RX packet: failed to access MOPR (err=%d)\n", err);
        return false;
    }
...
static int __cr_receive_packet(struct sx_dev *dev, struct ku_access_mopr_reg *mopr_reg_data)
{
    ...
    if (!skb) {
        sxd_log_err("CR receive packet: No memory to allocate buffer\n");
        return -ENOMEM;
    }
    ...
    err = sx_ACCESS_REG_MOPR(dev, mopr_reg_data);
    if (err) {
        sxd_log_err("CR receive packet: failed to access MOPR (err=%d)\n", err);
        goto out_free_skb;
    }
    ...
    if (cr_rx_dump) {
        sxd_log_notice("CR RX buffer (len=%d)\n", skb->len);
    ...
    if (err) {
        sxd_log_err("CR receive packet: failed to handle incoming packet (err=%d)\n", err);
        goto out_free_skb;
    }
...
static void __cr_send_packet(struct sx_dev *dev)
{
    ...
    if (cr_tx_dump) {
        sxd_log_notice("CR TX buffer (dst_port=%u, len=%d)\n", entry->meta.system_port_mid, entry->skb->len);
...
static int __cr_polling_thread(void *arg)
{
    ...
    sxd_log_info("CR polling thread started\n");
    ...
    while (!kthread_should_stop()) {
        ...
        for (local_port = 1; local_port < MAX_PHYPORT_NUM + 1; local_port++) {
            ...
            /* Check if port was up and now it is down */
                ...
                sxd_log_notice("Local port %u is down\n", local_port);
                __generate_pude(dev,
                                swid,
                                local_port,
                                paos_reg_data.paos_reg.admin_status,
...
int sx_cr_polling_thread_init(struct sx_dev *dev)
{
    ...
    if (IS_ERR(priv->cr_mode.polling_thread)) {
        ...
        sxd_log_err("Failed to start CR polling thread (err=%d)\n", err);
