...
#define PROC_DUMP(fmt, args ...)     sxd_log_err(fmt, ## args)
#define PROC_DUMP_DBG(fmt, args ...) sxd_log_debug(fmt, ## args)
...
static void print_sx_proc_help(void);
extern int                rx_debug;
...
static void sx_proc_handle_access_reg(struct sx_dev *dev, char *p, u32 dev_id, u32 reg_id, char *running)
{
    ...
    case MLXSW_HTGT_ID:
    {
        ...
        reg_data.htgt_reg.path.rdq = 21;
        reg_data.htgt_reg.path.cpu_tclass = 1;
        ...
        PROC_DUMP("rdq = %u\n",
                  reg_data2.htgt_reg.path.rdq);
        PROC_DUMP("cpu_tclass = %u\n",
                  reg_data2.htgt_reg.path.cpu_tclass);
        break;
    }

    case MCIA_REG_ID:
        sxd_log_warning("%s() reg_id %u not implemented yet\n",
                        __func__, reg_id);
        break;
    ...
    default:
        ...
        if (err == EINVAL) {
            sxd_log_warning("%s() reg_id %u doesn't exist\n",
                            __func__, reg_id);
        } else if (err) {
            goto print_err;
        }
...
print_err:
    sxd_log_warning("%s() execution of ACCESS_REG on reg_id %x Failed."
                    "error code = %d \n", __func__, reg_id, err);
}
...
void __query_fw(struct sx_dev *my_dev, char *running)
{
    ...
    sx_QUERY_FW_2(my_dev, dev_id, HCR1);
...
static ssize_t sx_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
    ...
    char          *cmd_str;
    size_t         buf_len = 0;
    struct sx_dev *my_dev = sx_core_has_predefined_devices() ? NULL : sx_dev_db_get_default_device();
    int            dev_id;


    if (!my_dev && !sx_core_has_predefined_devices()) {
        err = -ENODEV;
    ...
    if (sx_core_has_predefined_devices() && !strstr(cmd_str, "health_check_trigger")) {
        running = sx_proc_str_get_u32(running, &dev_id);
        my_dev = sx_dev_db_get_dev_by_id(dev_id);
        if (!my_dev) {
            PROC_DUMP("sx_proc_write: Invalid device received Null\n");
            print_sx_proc_help();
            return -ENODEV;
    ...
    if (p != NULL) {
        ...
        if (profile == NULL) {
            sxd_log_err("Error allocate mem for profile\n");
            goto out;
        }
        params = vmalloc(sizeof(*params));
        if (params == NULL) {
            sxd_log_err("Error allocate mem for params\n");
            vfree(profile);
            goto out;
        }
    ...
    if (p != NULL) {
        ...
        sxd_log_info("Write value %lx to addr: 0x%lx  \n", val, addr);
    ...
    if (p != NULL) {
        ...
        sxd_log_info("Read value addr: 0x%lx  val: 0x%lx \n",
                     addr, val);
        cmd++;
    }
    ...
    if (p != NULL) {
        ...
        running = sx_proc_str_get_str(running, &arg_reset);

        if (arg_reset) {
            if (strcmp(arg_reset, "no_reset") == 0) {
                do_reset = false;
            } else {
                PROC_DUMP("wrong argument to pcidrv_restart command\n");
                goto out;
            }
        }

        sxd_log_notice("Executing restart pci driver. \n");
        err = sx_restart_one_pci(my_dev, do_reset);
        if (err) {
    ...
    if (p) {
        ...
        } else if (strcmp(op, "cancel_all") == 0) {
            params.op = SX_HEALTH_CHECK_TRIGGER_OP_CANCEL_ALL;
        } else if (strcmp(op, "kernel_threads") == 0) {
            params.op = SX_HEALTH_CHECK_TRIGGER_OP_KERNEL_THREADS;
        } else if (strcmp(op, "sx_dbg_test_fw_fatal_event") == 0) {
            params.op = SX_HEALTH_CHECK_TRIGGER_OP_FW_FATAL_EVENT_TEST;
        } else if (strcmp(op, "sx_dbg_test_fw_fatal_cause") == 0) {
            params.op = SX_HEALTH_CHECK_TRIGGER_OP_FW_CAUSE_TEST;
        } else if (strcmp(op, "sx_dbg_test_fw_assert") == 0) {
            params.op = SX_HEALTH_CHECK_TRIGGER_OP_FW_ASSERT_TEST;
        } else if (strcmp(op, "sx_dbg_test_fw_tile_assert") == 0) {
            params.op = SX_HEALTH_CHECK_TRIGGER_OP_FW_TILE_ASSERT_TEST;
        } else if (strcmp(op, "sx_dbg_test_fw_tile_fatal_cause") == 0) {
            params.op = SX_HEALTH_CHECK_TRIGGER_OP_FW_TILE_FATAL_CAUSE_TEST;
        } else if (strcmp(op, "sx_dbg_test_pll_lock_cause") == 0) {
            params.op = SX_HEALTH_CHECK_TRIGGER_OP_FW_PLL_FATAL_CAUSE_TEST;
        } else if (strcmp(op, "sx_dbg_extend_thread_monitor_threshold") == 0) {
            params.op = SX_HEALTH_CHECK_TRIGGER_OP_EXTEND_THRESHOLD_THREAD_MONITOR;
        } else if (strcmp(op, "sx_dbg_set_new_thread_monitor_threshold") == 0) {
            params.op = SX_HEALTH_CHECK_TRIGGER_OP_SET_NEW_THRESHOLD_THREAD_MONITOR;
        } else {
        ...
        if (params.op == SX_HEALTH_CHECK_TRIGGER_OP_EXTEND_THRESHOLD_THREAD_MONITOR) {
            running = sx_proc_str_get_u32(running, &params.params.sdk_threshold_debug_params.mult_value);
            /* This is the last argument, Running will be NULL At this point - Don't validate it, only the mult_value */
            if (params.params.sdk_threshold_debug_params.mult_value == 0) {
                PROC_DUMP("health-check-trigger: sdk_threshold_debug_params.mult_value is 0\n");
                goto out_health_check_trigger;
            }
        }
        if (params.op == SX_HEALTH_CHECK_TRIGGER_OP_SET_NEW_THRESHOLD_THREAD_MONITOR) {
            running = sx_proc_str_get_u32(running, &params.params.sdk_threshold_debug_params.new_debug_threshold_time);
            /* This is the last argument, Running will be NULL At this point - Don't validate it, only the new_debug_threshold_time
             * we choose 200 as a max value because the longest time for thread is 200  */
            if ((params.params.sdk_threshold_debug_params.new_debug_threshold_time > 200) ||
                (params.params.sdk_threshold_debug_params.new_debug_threshold_time == 0)) {
                PROC_DUMP(
                    "health-check-trigger: sdk_threshold_debug_params.new_debug_threshold_time [%d] is out of range 1-200 0\n",
...
static void print_sx_proc_help(void)
{
    ...
    if (sx_core_has_predefined_devices()) {
        PROC_DUMP("This is a multi asic system - every command should have [dev_id] as the first parameter!\n");
    }
    ...
    PROC_DUMP("  health_check_trigger <add_dev|del_dev|sysfs|catas|cmd_ifc|sdq|rdq> [dev-id] [dqn]\n");
...
int sx_core_init_proc_fs(void)
{
    ...
    if (dir_proc_entry == NULL) {
        sxd_log_warning("create proc dir %s failed\n",
                        proc_dir_name);
        return -EINVAL;
    }
    ...
    if (proc_entry == NULL) {
        sxd_log_warning("create proc %s failed\n",
                        proc_file_name);
        return -EINVAL;
    }
    ...
    sxd_log_info("create proc %s succeeded\n", proc_file_name);
...
{
    if (addr == 0) {
        sxd_log_warning("%s() unsupported addr NULL \n", __func__);
    }
    ...
    default:
        sxd_log_warning("%s() unsupported width %d \n",
                        __func__, width);
    }
}
...
void __sx_proc_dump_swids(struct sx_dev *dev)
{
    ...
    for (i = 0; i < NUMBER_OF_SWIDS; i++) {
        if (sx_bitmap_test(&sx_priv(dev)->swid_bitmap, i)) {
            sxd_log_info("Swid %i is ACTIVE.\n", i);
        } else {
            sxd_log_info("Swid %i is DISABLED.\n", i);
        }
    }
}
...
void __sx_proc_dump_fw_icm(struct sx_dev *dev)
{
    ...
    if ((icm = priv->fw.fw_icm) == NULL) {
        sxd_log_info("dev_priv->fw->fw_icm is null \n");
        return;
    }
    ...
    for (sx_icm_first(icm, &iter);
         !sx_icm_last(&iter);
         sx_icm_next(&iter)) {
        sxd_log_info("[%d] icm_addr:%#llxp icm_size:%lu",
                     iii,
        ...
        for (jjj = 0; jjj < (sx_icm_size(&iter) >> lg); ++jjj) {
            ...
            ts += 1 << (lg - 10);
            ++tc;
            sxd_log_info("[%d/%d] nent:%d page:%#llx", iii, jjj, nent, pages);
...
void __sx_proc_dump_sdq(struct sx_dev *dev)
{
    ...
    if (sdq_table == NULL) {
        sxd_log_info("sdq table is empty \n");
        return;
    }

    for (i = 0; i < priv->dev_cap.max_num_sdqs; i++) {
        if (sdq_table->dq[i]) {
            sxd_log_info("[sdq %d]: dqn:%d, is_send:%d, "
                         "head:%d, tail:%d,"
...
void __sx_proc_dump_rdq(struct sx_dev *dev)
{
    ...
    if (rdq_table == NULL) {
        sxd_log_info("rdq table is empty \n");
        return;
    }

    if (cq_table == NULL) {
        sxd_log_info("cq table is empty \n");
        return;
    }
    ...
    for (i = 0; i < priv->dev_cap.max_num_rdqs; i++) {
        if (rdq_table->dq[i]) {
            ...
            sxd_log_info("[rdq %d]: dqn:%d, is_send:%d, "
...
void __sx_proc_dump_eq(struct sx_dev *dev)
{
    ...
    if (eq_table == NULL) {
        sxd_log_info("eq_table is empty \n");
        return;
    }

    for (i = 0; i < SX_NUM_EQ; i++) {
        sxd_log_info("[eq %d]: eqn:%d, nent:%d, cons_index:%d \n",
...
void __sx_proc_dump_rdq_single(struct sx_dev *dev, int rdq)
{
    ...
    if (rdq_table->dq[i]) {
        ...
        sxd_log_info("rdq %d doesn't exist \n", i);
    }
...
#define DUMP_BYTES_NUM 16
    ...
    for (j = 0; j < rdq_table->dq[i]->wqe_cnt; j++) {
        ...
        sxd_log_info("==================\n");
        sxd_log_info("rdq %d , wqe %d , buf: %p, first %d bytes :\n",
                     i, j, buf, DUMP_BYTES_NUM);
        for (k = 0; k < DUMP_BYTES_NUM; k++) {
            if ((k == 0) || (k % 4 == 0)) {
                sxd_log_info("\n 0x%04x : ", k);
            }
            sxd_log_info(" 0x%02x", buf[k]);
        }
        sxd_log_info("\n");
    }
}
...
void __sx_proc_rdq_start(struct sx_dev *dev, uint8_t rdq)
{
    ...
    sxd_log_info("Enabling handling events for RDQ [%d] \n", rdq);

    if (rdq_table == NULL) {
        sxd_log_err("RDQ table is empty \n");
        return;
    }

    if (cq_table == NULL) {
        sxd_log_err("CQ table is empty \n");
        return;
    }

    spin_lock_irqsave(&rdq_table->lock, rdq_table_flags);
    if (!rdq_table->dq[rdq]) {
        sxd_log_err("RDQ %d doesn't exist \n", rdq);
...
void __sx_proc_rdq_stop(struct sx_dev *dev, uint8_t rdq)
{
    ...
    sxd_log_info("Disabling handling events for RDQ [%d] \n", rdq);
...
void __dump_stats(struct sx_dev* sx_dev)
{
    ...
    for (swid = 0; swid < NUMBER_OF_SWIDS + 1; swid++) {
        ...
        if (NUMBER_OF_SWIDS == swid) {
            sxd_log_info("Packets stats for swid 254 \n");
        } else {
            sxd_log_info("Packets stats for swid %d \n", swid);
        }

        for (pkt_type = 0; pkt_type < PKT_TYPE_NUM; pkt_type++) {
            if (0 != priv->stats.rx_by_pkt_type[swid][pkt_type]) {
                sxd_log_info("rx pkt of type [%s (%d)]: %llu \n",
            ...
            if (0 != priv->stats.tx_by_pkt_type[swid][pkt_type]) {
                sxd_log_info("tx pkt of type [%s (%d)]: %llu (%llu bytes)\n",
        ...
        for (synd = 0; synd < NUM_HW_SYNDROMES; synd++) {
            if (0 != priv->stats.rx_by_synd[swid][synd]) {
                sxd_log_info("rx pkt on synd [%d]: %llu (%llu bytes)\n", synd,
        ...
        for (rdq = 0; rdq < NUMBER_OF_RDQS; rdq++) {
            if (0 != priv->stats.rx_by_rdq[swid][rdq]) {
                sxd_log_info("rx pkt on rdq [%d]: %llu (%llu bytes)\n", rdq,
    ...
    for (synd = 0; synd < NUM_HW_SYNDROMES; synd++) {
        for (pkt_type = 0; pkt_type < PKT_TYPE_NUM; pkt_type++) {
            if (priv->stats.rx_unconsumed_by_synd[synd][pkt_type] != 0) {
                sxd_log_info("rx unconsumed on synd [%d] type [%s]: %llu \n",
    ...
    for (synd = 0; synd < NUM_HW_SYNDROMES; synd++) {
        if (priv->stats.rx_eventlist_drops_by_synd[synd] != 0) {
            sxd_log_info("rx event list drops on synd [%d]: %llu \n",
                         synd, priv->stats.rx_eventlist_drops_by_synd[synd]);
        }
    }

    sxd_log_info("=========================\n");
    sxd_log_info("event list_drops_counter: %llu \n"
...
void __sx_proc_dump_emad(struct sx_dev *my_dev, u32 emad_type, u32 max_cnt, u32 direction)
{
    if ((SX_PROC_DUMP_PKT_DISABLE == direction) ||
        ...
        rx_debug_metadata = SX_DBG_DISABLE;
        ...
        tx_debug_metadata = SX_DBG_DISABLE;
    ...
    if (direction & SX_PROC_DUMP_PKT_RX) {
        ...
        rx_debug_metadata = SX_DBG_ENABLE;
    ...
    if (direction & SX_PROC_DUMP_PKT_TX) {
        ...
        tx_debug_metadata = SX_DBG_ENABLE;
...
void __debug_dump_netdev(struct sx_dev *dev)
{
    sxd_log_err("'debug_dump_netdev' is obsolete. use debug-dump proc entry 'netdev_dump' instead.\n");
}
...
void __debug_netdev_traps(struct sx_dev *dev, u32 trap_id, u32 is_add)
{
    ...
    if (!event_data) {
        sxd_log_err("Failed to allocate memory for event data.\n");
        return;
    }
...
void __set_net_dev_oper_state(char *dev_name, char *oper_state)
{
    ...
    while (dev) {
        if ((dev_name != NULL) &&
            ...
            sxd_log_rl_info("Device [%s] set oper state to %s \n",
                            dev->name, oper_state);
            ...
            if (strstr(oper_state, "up")) {
                ...
                sxd_log_rl_err("Unsupported oper_state: %s \n", oper_state);
            }
            break;
        }
...
void __dump_net_dev_oper_state(void)
{
    ...
    dev = first_net_device(&init_net);
    while (dev) {
        sxd_log_debug("Found dev [%s]: dormant:%d, carrier:%d  \n",
...
void __dump_kdbs(void)
{
    ...
    if (sx_core_get_phy_port_max(my_dev, &phy_port_max)) {
        sxd_log_info("Failed to get max number of phy ports.\n");
    }

    /* IB only */
    sxd_log_info("============================\n");
    sxd_log_info("IB to LOCAL :\n");
    for (i = 0; i < MAX_IBPORT_NUM + 1; i++) {
        if (priv->ib_to_local_db[i] != 0) {
            sxd_log_info("[ib%d : local %d] \n",
    ...
    /* ETH only */
    sxd_log_info("============================\n");
    sxd_log_info("ETH SYS to LOCAL :\n");
    for (i = 0; i < MAX_SYSPORT_NUM; i++) {
        if (priv->system_to_local_db[i] != 0) {
            sxd_log_info("[system 0x%x : local %d : pvid %d] \n",
    ...
    sxd_log_info("============================\n");
    sxd_log_info("FID to HW_FID mapping dump :\n");
    sxd_log_info("%-7s| %-7s| %-7s\n", "#", "FID", "HW_FID");
    sxd_log_info("------------------------------------------\n");
    i = 0;
    for (fid = 0; fid < MAX_FIDS_NUM; fid++) {
        if (priv->fid_to_hwfid[fid] != INVALID_HW_FID_ID) {
            i++;
            sxd_log_info("%-7u| %-7u| %-7u\n",
    ...
    sxd_log_info("============================\n");
    sxd_log_info("RIF to HW_FID mapping dump :\n");
    sxd_log_info("%-7s| %-7s| %-7s\n", "#", "RIF", "HW_FID");
    sxd_log_info("------------------------------------------\n");
    i = 0;
    ...
    sxd_log_info("============================\n");
    sxd_log_info("RIF data dump :\n");
    sxd_log_info("%-7s| %-7s| %-15s| %-7s|\n", "RIF", "IS_LAG", "LAG/PORT", "VLAN");
    sxd_log_info("------------------------------------------\n");
    i = 0;
    for (rif_id = 0; rif_id < MAX_RIFS_NUM; rif_id++) {
        if (priv->rif_data[rif_id].is_valid) {
            sxd_log_info("%-7u| %-7u| %-15u| %-7u|\n",
    ...
    /* common */
    sxd_log_info("============================\n");
    sxd_log_info("COMMON LOCAL TO SWID :\n");
    sxd_log_info("LOCAL\t| SWID\t|\n");
    sxd_log_info("------------------------------------------\n");
    ...
    sxd_log_info("============================\n");
    sxd_log_info("COMMON TRUNCATE SIZE:\n");
    sxd_log_info("RDQ\t| tr_sz\t|\n");
    ...
    for (i = 0; i < (phy_port_max + 1); i++) { /* system port */
        ...
        if (is_valid) {
            sxd_log_info("sysport 0x%x : ", i);
            for (j = 0; j < (MAX_PRIO_NUM + 1); j++) { /* prio */
                sxd_log_info("p%d:tc%d, ",
    ...
    for (i = 0; i < (phy_port_max + 1); i++) { /* system port */
        ...
        if (is_valid) {
            sxd_log_info("sys_port 0x%x tagged on vlans: ", i);
            for (j = 0; j < (SXD_MAX_VLAN_NUM); j++) { /* vlan */
    ...
    for (i = 0; i < (lag_max + 1); i++) { /* lid */
        ...
        if (is_valid) {
            sxd_log_info("lag 0x%x tagged on vlans: ", i);
            for (j = 0; j < (SXD_MAX_VLAN_NUM); j++) { /* vlan */
    ...
    sxd_log_info("============================\n");
    sxd_log_info("prio_tag_sysports:\n");
    for (i = 0; i < (phy_port_max + 1); i++) { /* system port */
    ...
    sxd_log_info("============================\n");
    sxd_log_info("prio_tag_lags:\n");
    for (i = 0; i < (lag_max + 1); i++) { /* lid */
        if (priv->lag_prio_tagging_mode[i]) {
            sxd_log_info("lag%d, ", i);
        }
    }
    sxd_log_info("\n");
    ...
    for (i = 0; i < (SXD_MAX_VLAN_NUM - 1); i++) { /* vlan */
        if (priv->icmp_vlan2ip_db[i]) {
            sxd_log_info("vlan%d: 0x%x, ", i, priv->icmp_vlan2ip_db[i]);
        }
    }
    sxd_log_info("\n");

    sxd_log_info("============================\n");
    sxd_log_info("port_rp_rif:\n");
    sxd_log_info("LOCAL\t| VLAN\t| RIF\t|\n");
    sxd_log_info("-------------------------\n");
    ...
    sxd_log_info("============================\n");
    sxd_log_info("lag_rp_rif:\n");
    sxd_log_info("LID\t| VLAN\t| RIF\t|\n");
    sxd_log_info("-------------------------\n");
    for (i = 0; i < lag_max; i++) {
        for (j = 0; j < SXD_MAX_VLAN_NUM; j++) {
    ...
    sxd_log_info("============================\n");
    sxd_log_info("port_vid_to_fid:\n");
    sxd_log_info("PORT\t| VLAN\t| FID\t|\n");
    sxd_log_info("-------------------------\n");
    ...
    sxd_log_info("============================\n");
    sxd_log_info("lag_oper_state:\n");
    sxd_log_info("LID\t| oper_s\n");
    sxd_log_info("------------------\n");
    for (i = 0; i < lag_max; i++) {
