...
#define SX_CORE_IOCTL_ACCESS_REG_HANDLER(reg_name, ku_reg_struct_name)                             \
    ...
    {                                                                                              \
        struct ku_reg_struct_name *reg_data = NULL;                                                \
        struct sx_dev             *dev;                                                            \
        int                        err;                                                            \
                                                                                                   \
        /* TODO: dynamic registers to be supported */                                              \
        reg_data = kmalloc(sizeof(*reg_data), GFP_KERNEL);                                         \
        if (!reg_data) {                                                                           \
            sxd_log_err("Failed to allocate register %s\n", #reg_name);                            \
            err = -ENOMEM;                                                                         \
            goto out;                                                                              \
        }                                                                                          \
                                                                                                   \
        ...
        if (sx_core_has_predefined_devices()) {                                                    \
            reg_data->dev_id = get_device_id_from_fd(file);                                        \
        }                                                                                          \
                                                                                                   \
        err = sx_dpt_get_cmd_sx_dev_by_id(reg_data->dev_id, &dev);                                 \
        if (err) {                                                                                 \
            sxd_log_warning("sx_core_access_reg " #reg_name ": Device doesn't exist. Aborting\n"); \
            goto out;                                                                              \
        }                                                                                          \
                                                                                                   \
