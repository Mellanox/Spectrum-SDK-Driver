                                     ...
                                     struct sgmii_cr_space_sync_context *context)
{
    ...
    tr_meta.transport_type = SXD_TRANSPORT_TYPE_CR_ACCESS;
                                 ...
                                 enum sgmii_cr_space_direction dir)
{
    ...
    while (tmp_size > 0) {
        ...
        if (ret) {
            ...
            sxd_log_err("failed SKB creation to read CR address:[0x%x] error:[%d]\n",
                        tmp_address, ret);
            goto out;
        }
        ...
        if (ret) {
            sxd_log_notice(
                "failed sending CR address:[0x%x] bytes:[%d] out of remaining request size:[%d] error:[%d]\n",
                tmp_address,
                bytes_to_send,
                tmp_size,
                ret);
...
int sgmii_cr_space_init(void)
{
    ...
    if (ret) {
        sxd_log_err("failed to create cr-space transaction db (ret=%d)\n", ret);
        return ret;
    }
