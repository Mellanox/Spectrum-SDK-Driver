...
static int __sgmii_dev_init_hopf(struct sgmii_dev *sgmii_dev, bool hopf_on_remote_mgmt)
{
    ...
    for (i = 0; i < NUMBER_OF_ETCLASSES; i++) {
        ...
        if (err) {
            sxd_log_err("failed to init HOPF (SDQ %d) for dev id %d [err=%d]\n", i, sgmii_dev->dev_id, err);
            goto out;
        }
    }
    ...
    if (err) {
        sxd_log_err("failed to get SGMII netdev MAC address\n");
        goto out;
    }
    ...
    for (i = 0; i < MAX_OOB_CPU_INGRESS_TCLASS; i++) {
        ...
        err = sgmii_emad_access_hopf(sgmii_dev->dev_id, &reg_hopf);
        if (err) {
            sxd_log_err("failed to init HOPF (RDQ %d) for dev id %d [err=%d]\n", i, sgmii_dev->dev_id, err);
            goto out;
        }
    }
...
int sgmii_dev_init(int dev_id, uint8_t init_hopf)
{
    ...
    if (err) {
        sxd_log_err("Dev %d is not SGMII device\n", dev_id);
        return err;
    }
    ...
    if (err) {
        sxd_log_err("failed to init PPAD for dev_id %d [err=%d]\n", dev_id, err);
        goto out;
    }
...
int sgmii_default_dev_set(int dev_id, bool hopf_on_remote_mgmt)
{
    ...
    if (err) {
        sxd_log_err("cannot set default device to %d - not a SGMII device\n", dev_id);
        goto out;
    }
...
static int __sgmii_dev_proc_show(struct seq_file *m, void *v)
{
    ...
    case SXD_MGIR_HW_DEV_ID_SPECTRUM5:
        dev_type_name = "Switch-Spectrum-5";
        break;
...
static int __sgmii_dev_proc_open(struct inode *inode, struct file *file)
{
    ...
    dev_id = (int)(long)PDE_DATA_DEF(inode);
...
static void __sgmii_dev_create_proc_file(struct sgmii_dev *sgmii_dev)
{
    ...
    if (!sgmii_dev->proc_file) {
        sxd_log_err("failed to create proc file for dev_id %d. ignoring\n", sgmii_dev->dev_id);
    }
}
...
static void __sgmii_dev_delete_proc_file(struct sgmii_dev *sgmii_dev)
{
    ...
    sxd_log_info("delete proc file %s\n", proc_file_name);
...
int sgmii_dev_add(int dev_id, const struct ku_dpt_sgmii_info *dpt_sgmii_info)
{
    ...
    if (!DEV_ID_IS_VALID(dev_id) ||
        ...
        sxd_log_err("sgmii information has invalid data\n");
        return -EINVAL;
    }

    sgmii_dev = kzalloc(sizeof(struct sgmii_dev), GFP_KERNEL);
    if (!sgmii_dev) {
        sxd_log_err("failed to allocate sgmii device\n");
        return -ENOMEM;
    }
    ...
    if (__is_sgmii_device_LOCKED(dev_id)) {
        sxd_log_err("failed to insert a new sgmii dev - already exists\n");
        ret = -EEXIST;
        goto out;
    }
    ...
    if (ret) {
        sxd_log_err("failed to insert a new sgmii dev. ret=%d\n", ret);
        goto out;
    }
...
int sgmii_dev_del(int dev_id)
{
    ...
    if (!__is_sgmii_device_LOCKED(dev_id)) {
        sxd_log_err("failed to delete sgmii dev %d - not SGMII device\n", dev_id);
        ret = -ENODEV;
        goto out;
    }
    ...
    if (__sgmii_default_dev_id == dev_id) {
        sxd_log_warning("delete sgmii dev %d - this is currently the default device!\n", dev_id);
...
int sgmii_dev_set_mft_info(const struct ku_sgmii_mft_info *mft_info)
{
    ...
    if (!mft_info || !DEV_ID_IS_VALID(mft_info->dev_id)) {
        sxd_log_err("set_mft_info() invalid parameters\n");
        return -EINVAL;
    }
    ...
    if (!__is_sgmii_device_LOCKED(mft_info->dev_id)) {
        sxd_log_err("failed to set MFT info for sgmii dev %d - not SGMII device\n", mft_info->dev_id);
        ret = -ENODEV;
        goto out;
    }
...
void sgmii_dev_dec_ref(struct sgmii_dev *sgmii_dev)
{
    ...
    if (refs < 0) {
        sxd_log_err("SGMII device %d reference count is negative!\n", sgmii_dev->dev_id);
        dump_stack();
    }
...
int sgmii_dev_db_init(void)
{
    ...
    if (!__fw_tool_proc_dir) {
        sxd_log_err("failed to create fw_tool proc dir\n");
        return -ENOMEM;
    }

    ret = sx_core_map_init(&__sgmii_dev_map, __sgmii_info_compare, 6);
    if (ret) {
        sxd_log_err("failed to create sgmii device map (err=%d)\n", ret);
        return ret;
    }

    return sx_dbg_dump_read_handler_register("sgmii_dev_db", __sgmii_dev_db_handler, NULL, NULL, NULL);
}


void sgmii_dev_db_deinit(void)
{
    sx_dbg_dump_read_handler_unregister("sgmii_dev_db", NULL);
