...
find_good_timing:
    ...
    if (err) {
        sxd_log_err("sx_ACCESS_REG_MTPPS (set_pps) returned with error %d\n", err);
    }
    ...
    if (freq_adj != 0) {
        ...
        if (err) {
            sxd_log_err("sx_ACCESS_REG_MTUTC (adj_freq) returned with error %d\n", err);
        }
    }
    ...
    err = sx_ACCESS_REG_MTUTC(&priv->dev, &reg_mtutc);
    if (err) {
        sxd_log_err("sx_ACCESS_REG_MTUTC (set_time) returned with error %d\n", err);
    }
...
static int __adjfreq_spc1(struct ptp_clock_info *ptp, s32 delta)
{
    ...
    if (delta > ptp->max_adj) {
        sxd_log_rl_warning("sx_ptp_adjfreq: "
...
int sx_clock_dev_init_spc1(struct sx_priv *priv)
{
    ...
    if (!priv->hw_clock.hwd.spc1.frc_base) {
        sxd_log_err("could not remap FRC register\n");
        err = -ENOMEM;
        goto out_err;
    }
    ...
    sxd_log_info("HW clock frequency: %u KHz\n", SPC1_CLOCK_FREQ_KHZ);
