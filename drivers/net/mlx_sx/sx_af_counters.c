...
static inline int __sx_af_counters_can_init(void)
{
    ...
    if (g_af_counters_initialized != false) {
        err = -EINVAL;
        sxd_log_err("%s: Accumulated counters have already been initialized, err [%d]\n",
                    __func__, err);
        goto out;
    }
    ...
    if (g_af_deinit_in_progress != false) {
        err = -EINVAL;
        sxd_log_err("%s: The deinitialization of Accumulated counters is in progress, err [%d]\n",
                    __func__, err);
        goto out;
    }
...
static inline int __sx_af_counters_ctx_init(struct ku_accuflow_counters_params *af_params_p)
{
    ...
    if (err) {
        sxd_log_err("%s: Failed to initialize kernel buffer for Accumulated counters, err [%d]\n",
                    __func__, err);
        goto out;
    }
    ...
    if (IS_ERR(g_af_counters_ctx.af_counters_wq_p)) {
        err = -ENOMEM;
        sxd_log_err("%s: Failed to create a work queue for Accumulated counters, err [%d]\n",
                    __func__, err);
        goto out;
    }
    ...
    if (err) {
        sxd_log_err("%s: Failed to initialize debug infrastructure for Accumulated counters, err [%d]\n",
                    __func__, err);
        goto out;
    }
...
static inline int __sx_af_counters_deinit_state_in_progress_set(void)
{
    ...
    if (__sx_af_counters_context_acquire()) {
        sxd_log_notice("%s: Trying to de-initialize Accumulated counters module "
...
static int __sx_af_counters_buffer_init(u32 counters_num)
{
    ...
    if (g_af_counters_ctx.af_counters_buffer_p == NULL) {
        err = -ENOMEM;
        sxd_log_err("%s: failed to allocate memory for Accumulated counters buffer, err [%d]\n",
                    __func__, err);
        goto out;
    }
...
static inline void __sx_af_counters_buffer_deinit(void)
{
    if (g_af_counters_ctx.af_counters_buffer_p) {
        ...
        sxd_log_err("%s: g_af_counters_ctx.af_counters_buffer_p is NULL \n",
                    __func__);
    }
}
...
static int __sx_af_counter_refresh_done_event_send(void)
{
    ...
    err = send_trap(&event,
                    ...
                    GFP_ATOMIC);
    if (err) {
        sxd_log_err("failed to send notification for bulk-counter refresh completion.\n");
    }

    return err;
}

int sx_af_counters_tr_reservation_set(struct sx_dev                                     *dev,
                                      ku_accuflow_counters_params_data_tr_reservation_t *tr_reservation_p)
{
    int err = 0;

    switch (tr_reservation_p->action) {
    case KU_ACCUFLOW_COUNTERS_PARAMS_DATA_TR_RESERVATION_ACTION_ACQUIRE_E:
        err = bulk_cntr_db_mocs_session_acquire(dev, SXD_BULK_CNTR_KEY_TYPE_ACCUFLOW_E);
        if (err) {
            goto out;
        }
    ...
    case KU_ACCUFLOW_COUNTERS_PARAMS_DATA_TR_RESERVATION_ACTION_RELEASE_E:
        err = bulk_cntr_db_mocs_session_release(dev, SXD_BULK_CNTR_KEY_TYPE_ACCUFLOW_E);
        if (err) {
            goto out;
        }
    ...
    default:
        sxd_log_err("Invalid action for the refresh reservation: %d.\n",
...
void sx_af_counters_mocs_done(struct sx_dev *dev)
{
    __sx_af_counter_refresh_done_event_send();
    mb();
    bulk_cntr_db_mocs_session_release(dev, SXD_BULK_CNTR_KEY_TYPE_ACCUFLOW_E);
    mb();
    __sx_af_counters_stats_refresh_end();
}

int sx_af_counters_user_job_schedule(struct sx_dev *dev, struct ku_accuflow_counters_params *af_params_p)
{
    int                                        err = 0;
    ...
    if (af_counter_work_p == NULL) {
        err = -ENOMEM;
        sxd_log_err("Memory allocation for Accumulated counter data failed, "
    ...
    case KU_ACCUFLOW_COUNTERS_PARAMS_OP_TRASACTION_START_E:
        ...
        if (af_params_p->op_data.transaction_data.cancel == 0) {
            ...
            if (!layout_common) {
                err = -EINVAL;
                sxd_log_err("Failed to start an Accumulated transaction - event id %llu, buffer_id %lu \n",
    ...
    default:
        sxd_log_err("Unknown Accumulated operation: %d.\n", af_params_p->op);
        err = -EINVAL;
        goto out;
    }
    ...
    if (wq_err == false) {
        err = -EAGAIN;
        sxd_log_warning("Failed to schedule the Accumulated counter ws, dropping RX packet\n");
        goto out;
    }
    ...
    if (af_params_p->op == KU_ACCUFLOW_COUNTERS_PARAMS_OP_READ_E) {
        if (!err) {
            ...
            if (wait_for_completion_timeout(&read_completion, msecs_to_jiffies(2000)) == 0) {
                err = -ETIMEDOUT;
                sxd_log_err("After the 2 seconds wait, the Accumulated thread still didn't return a counter \n");
...
static void __sx_af_counters_alloc_handler_cb(struct work_struct *work_p)
{
    ...
    if (index >= __sx_af_counters_prm_indices_num_get()) {
        sxd_log_err("Failed to allocate a counter because of invalid index %u [%u, %u]\n",
...
static void __sx_af_counters_move_handler_cb(struct work_struct *work_p)
{
    ...
    if (old_index >= __sx_af_counters_prm_indices_num_get()) {
        sxd_log_err("Failed to move counters data because of invalid old index %u [%u, %u]\n",
    ...
    if (new_index >= __sx_af_counters_prm_indices_num_get()) {
        sxd_log_err("Failed to move counters data because of invalid new index %u [%u, %u]\n",
...
static void __sx_af_counters_clear_handler_cb(struct work_struct *work_p)
{
    ...
    if (zero_based_counter_index >= __sx_af_counters_prm_indices_num_get()) {
        sxd_log_err("Failed to clear a counter because of invalid index %u [%u, %u]\n",
...
static void __sx_af_counters_read_handler_cb(struct work_struct *work_p)
{
    ...
    if (zero_based_counter_index >= __sx_af_counters_prm_indices_num_get()) {
        sxd_log_err("Failed to read a counter because of invalid index %u [%u, %u]\n",
...
static void __sx_af_counters_tr_handler_cb(struct work_struct *work_p)
{
    ...
    if (index >= __sx_af_counters_prm_indices_num_get()) {
        sxd_log_err("Failed to start a transaction for counters because of invalid index %u [%u, %u]\n",
    ...
    err = bulk_cntr_db_in_progress_get(af_work_p->dev_p, SXD_BULK_CNTR_KEY_TYPE_ACCUFLOW_E, &in_progress);
    if (err) {
        goto out;
    }
    ...
    err = sx_bulk_cntr_handle_ack(af_work_p->dev_p,
                                  &af_params_p->op_data.transaction_data.ev_id,
                                  af_params_p->op_data.transaction_data.buffer_id);
    if (err) {
        goto out;
    }
...
void sx_af_counters_event_job_schedule(struct completion_info *comp_info_p)
{
    ...
    if (af_counter_work_p == NULL) {
        ...
        err = -ENOMEM;
        sxd_log_warning("Memory allocation for Accumulated counter data failed, "
    ...
    if (comp_info_p->hw_synd == SXD_TRAP_ID_ACCU_FLOW_INC) {
        ...
        __sx_af_counters_stats_event_jobs_drop_set(comp_info_p);
        err = -EINVAL;
        sxd_log_warning("Unexpected event in the Accumulated handler");
        goto out;
    }
    ...
    if (wq_err == false) {
        __sx_af_counters_stats_event_jobs_drop_set(comp_info_p);
        err = -EAGAIN;
        sxd_log_warning("Failed to schedule an Accumulated event job\n");
        goto out;
    }
                           ...
                           void                                      *context)
{
    ...
    if (reg_len != MLXSW_MAFTI_LEN) {
        sxd_log_err("MAFTI parse: reg_len is less than expected len (reg_len=%u, expected=%u)\n",
    ...
    for (index = 0; index < num_rec; index++) {
        ...
        if (counter_index >= __sx_af_counters_prm_indices_num_get()) {
            sxd_log_err("counter_index %u exceeds number of counters %u \n",
                           ...
                           void                                      *context)
{
    ...
    if (reg_len != MLXSW_MAFRI_LEN) {
        sxd_log_err("MAFRI parse: reg_len is less than expected len (reg_len=%u, expected=%u)\n",
                           ...
                           void                                      *context)
{
    ...
    if (reg_len != MLXSW_MAFBI_LEN) {
        sxd_log_err("MAFBI parse: reg_len is less than expected len (reg_len=%u, expected=%u)\n",
    ...
    if (mlxsw_reg_mafbi_type_get(mafbi->reg) != SXD_MAFBI_TYPE_ENTRY_TYPE0_E) {
        sxd_log_err("MAFBI parse: Unexpected type - %u (expected=%u)\n",
...
int sx_af_counters_proc_fs_handler(sx_af_counters_proc_fs_handler_op_e op)
{
    ...
    default:
        sxd_log_notice("Accumulated proc fs: received unknown operation - %d\n", op);
        break;
    }
