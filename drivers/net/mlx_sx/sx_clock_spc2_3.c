...
#define ADJTIME_RANGE_EXT (46)
#define ADJPHASE_OFFSET   (54)
#define IS_ADJTIME_RANGE_EXTENDED(feature_cap_mask1)   ((feature_cap_mask1) & (1 << (ADJTIME_RANGE_EXT - 32)))
...
#define DEFAULT_FREQ_ADJUSTMENT_PPB_RANGE (50000000)

#define FREQ_ADJ_UNIT_SCALED_PPM_OFFSET 47
#define IS_SCALED_PPM_FREQ_ADJ_SUPPORTED_BY_HW(feature_cap_mask1) \
    ((feature_cap_mask1) &                                        \
     (1 << (FREQ_ADJ_UNIT_SCALED_PPM_OFFSET - 32)))

#define IS_VALID_ADJFREQ_PPB_VALUE(p_sx_priv, ppb)                    \
    (((ppb) >= -((p_sx_priv)->hw_clock.hwd.spc2.max_freq_adj_ppb)) && \
     ((ppb) <= (p_sx_priv)->hw_clock.hwd.spc2.max_freq_adj_ppb))

#define IS_VALID_ADJFINE_PPB_VALUE(p_sx_priv, ppb)                               \
    (((ppb) >= -((p_sx_priv)->hw_clock.hwd.spc2.max_scaled_ppm_freq_adj_ppb)) && \
     ((ppb) <= (p_sx_priv)->hw_clock.hwd.spc2.max_scaled_ppm_freq_adj_ppb))

#define IS_VALID_ADJFINE_SCALED_PPM_VALUE(scaled_ppm) \
    (((scaled_ppm) >= S32_MIN) && ((scaled_ppm) <= S32_MAX))

#define IS_CRSDUMP_ON_GOING(p_sx_priv) ((p_sx_priv)->crsdump_in_process)
...
static int __write_mtutc(struct sx_priv *priv, struct ku_access_mtutc_reg *mtutc, const char *op)
{
    ...
    /* Crspace dump will race bandwidth with command interface and we choose emad instead (for bug#3779478).
     * We use command interface for other cases (e.g, only cmd is OK during init flow, for bug#3959130) */
    if (IS_CRSDUMP_ON_GOING(priv)) {
        err = sx_EMAD_ACCESS_REG_MTUTC(dev, EMAD_PRIO_LOW, EMAD_METHOD_WRITE, &in_params, NULL, &(mtutc->mtutc_reg));
    } else {
    ...
    if (err) {
        sxd_log_err("MTUTC register write returned with error %d\n", err);
        ...
        memset(&mtutc_reg, 0, sizeof(mtutc_reg));
        err2 = sx_EMAD_ACCESS_REG_MTUTC(dev, EMAD_PRIO_LOW, EMAD_METHOD_QUERY, &in_params, NULL, &mtutc_reg);
        if (err2) {
            __print_mtutc("HW", &mtutc_reg);
        }

        __print_mtutc("Erroneous", &mtutc_reg);
    }
...
#endif /* #if (LINUX_VERSION_CODE < KERNEL_VERSION(6, 2, 0)) */

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 10, 0))
static int __init_adjfine_params(struct sx_priv *priv)
{
    struct ku_access_mtutc_reg reg_mtutc;
    struct ku_access_mcam_reg  reg_mcam;
    int                        err = 0;
    u8                         log_max_freq_adjustment = 0;
    bool                       adjfine_ppm_supported = false;
    ...
    if (err) {
        sxd_log_err("Cannot query whether phase adjustment is supported, error: %d\n", err);
        err = -EFAULT;
        goto out;
    }

    if (IS_SCALED_PPM_FREQ_ADJ_SUPPORTED_BY_HW(reg_mcam.mcam_reg.mng_feature_cap_mask[1])) {
        adjfine_ppm_supported = true;
    }
    if (adjfine_ppm_supported) {
        /* query range for scaled_ppb unit */
        memset(&reg_mtutc, 0, sizeof(reg_mtutc));
        reg_mtutc.dev_id = priv->dev.device_id;
        reg_mtutc.mtutc_reg.freq_adj_units = SXD_MTUTC_FREQ_ADJ_UNITS_UNITS_ARE_PARTS_PER_MILLION_E;
        sx_cmd_set_op_tlv(&reg_mtutc.op_tlv, MTUTC_REG_ID, EMAD_METHOD_QUERY);
        err = sx_ACCESS_REG_MTUTC(&priv->dev, &reg_mtutc);
        if (err) {
            sxd_log_err("sx_ACCESS_REG_MTUTC query returned with error %d\n", err);
            err = -EFAULT;
            goto out;
        }

        log_max_freq_adjustment = reg_mtutc.mtutc_reg.log_max_freq_adjustment;
        if (log_max_freq_adjustment == 0) {
            priv->hw_clock.hwd.spc2.max_scaled_ppm_freq_adj_ppb = DEFAULT_FREQ_ADJUSTMENT_PPB_RANGE;
        } else {
            priv->hw_clock.hwd.spc2.max_scaled_ppm_freq_adj_ppb = 1 << log_max_freq_adjustment;
        }
        sxd_log_notice("Frequency adjustment (scaled PPM unit) is supported, with log_max_freq_adjustment being %u.\n",
                       log_max_freq_adjustment);
    }

    /* query range for ppb unit */
    memset(&reg_mtutc, 0, sizeof(reg_mtutc));
    reg_mtutc.dev_id = priv->dev.device_id;
    reg_mtutc.mtutc_reg.freq_adj_units = SXD_MTUTC_FREQ_ADJ_UNITS_UNITS_ARE_PPB_E;
    sx_cmd_set_op_tlv(&reg_mtutc.op_tlv, MTUTC_REG_ID, EMAD_METHOD_QUERY);
    err = sx_ACCESS_REG_MTUTC(&priv->dev, &reg_mtutc);
    if (err) {
        sxd_log_err("sx_ACCESS_REG_MTUTC query returned with error %d\n", err);
    ...
    log_max_freq_adjustment = reg_mtutc.mtutc_reg.log_max_freq_adjustment;
    if (log_max_freq_adjustment == 0) {
        priv->hw_clock.hwd.spc2.max_freq_adj_ppb = DEFAULT_FREQ_ADJUSTMENT_PPB_RANGE;
    } else {
        priv->hw_clock.hwd.spc2.max_freq_adj_ppb = 1 << log_max_freq_adjustment;
    }
    sxd_log_notice("Frequency adjustment PPB log_max_freq_adjustment is %u.\n",
                   log_max_freq_adjustment);

out:
    priv->hw_clock.hwd.spc2.is_scaled_ppm_freq_adj_supported = adjfine_ppm_supported;
    return err;
}

#if (LINUX_VERSION_CODE < KERNEL_VERSION(6, 2, 0))
/* Try to fall back to adjfreq() handle flow + static check */
static int __fallback_to_adjfreq(struct ptp_clock_info *ptp, long ppb)
{
    struct sx_priv *priv = container_of(ptp, struct sx_priv, hw_clock.clock_info);

    sxd_log_debug("__fallback_to_adjfreq: ppb %ld.\n", ppb);

    if (!IS_VALID_ADJFREQ_PPB_VALUE(priv, ppb)) {
        sxd_log_notice(
            "scaled_ppm adjustment to ppb: input freq adjustment value %ld ppb is out of supported ppb range (%ld).\n",
            ppb,
            priv->hw_clock.hwd.spc2.max_scaled_ppm_freq_adj_ppb);
        return -ERANGE;
    }

    return __adjfreq_spc2(ptp, ppb);
}
#endif /* #if (LINUX_VERSION_CODE < KERNEL_VERSION(6, 2, 0)) */

#if (LINUX_VERSION_CODE <= KERNEL_VERSION(4, 19, 195) && !defined(CONFIG_SX_RHEL_8_6))
/* Copied from ptp_clock.c because it is static function in linux versions 4.19.195 or earlier versions.
 * Red-hat 8.6 is ignored because it has back-ported the function. */
...
static int __adjfine_spc2(struct ptp_clock_info *ptp, long scaled_ppm)
{
    struct sx_priv            *priv = container_of(ptp, struct sx_priv, hw_clock.clock_info);
    struct ku_access_mtutc_reg reg_mtutc;
    int                        err;
    long                       ppb = scaled_ppm_to_ppb(scaled_ppm);
    bool                       try_fallback_to_ppb_unit = false;
    long                       origin_scaled_ppm = scaled_ppm;

    sxd_log_debug("%s: scaled_ppm %ld (= ppb %ld).\n", __func__, scaled_ppm, ppb);

    if (!priv->hw_clock.hwd.spc2.is_scaled_ppm_freq_adj_supported) {
        sxd_log_notice(
            "ASIC does not support frequency adjustment with scaled_ppm unit, will try to fallback to use ppb unit.\n");
        try_fallback_to_ppb_unit = true;
    } else if (!IS_VALID_ADJFINE_PPB_VALUE(priv, ppb)) {
        sxd_log_notice(
            "Input %ld ppb is out of supported range (%ld) supported by scaled_ppm unit, will try to fallback to use ppb unit.\n",
            ppb,
            priv->hw_clock.hwd.spc2.max_scaled_ppm_freq_adj_ppb);
        try_fallback_to_ppb_unit = true;
    } else if (!IS_VALID_ADJFINE_SCALED_PPM_VALUE(scaled_ppm)) {
        sxd_log_notice(
            "The %ld scaled_ppm (calculated from input %ld ppb) is out of supported range (S32_MAX), will try to fallback to use ppb unit.\n",
            scaled_ppm,
            ppb);
        try_fallback_to_ppb_unit = true;
    }

    if (try_fallback_to_ppb_unit) {
#if (LINUX_VERSION_CODE < KERNEL_VERSION(6, 2, 0))
        return __fallback_to_adjfreq(ptp, ppb);
#else
        return -EOPNOTSUPP;
#endif /* #if (LINUX_VERSION_CODE < KERNEL_VERSION(6, 2, 0)) */
    }
    ...
    memset(&reg_mtutc, 0, sizeof(reg_mtutc));
    reg_mtutc.mtutc_reg.freq_adj_units = SXD_MTUTC_FREQ_ADJ_UNITS_UNITS_ARE_PARTS_PER_MILLION_E;
    reg_mtutc.mtutc_reg.operation = SXD_MTUTC_OPERATION_ADJUST_FREQ_UTC_E;
    reg_mtutc.mtutc_reg.freq_adjustment = scaled_ppm;
    err = __write_mtutc(priv, &reg_mtutc, "adj-fine");

    if (!err) {
        sx_clock_dbg_add_setter(priv);
        sx_clock_log_add_adjfine(priv, origin_scaled_ppm);
    } else {
        err = -EFAULT;
    }
...
static int __adjtime_spc2(struct ptp_clock_info *ptp, s64 delta)
{
    ...
    if ((delta < -priv->hw_clock.hwd.spc2.adjtime_range) ||
        (delta > priv->hw_clock.hwd.spc2.adjtime_range)) { /* if it is out of range, convert it to 'set_time' */
...
static int __init_adjphase_params(struct sx_priv *priv, struct ku_mcam_reg *reg_mcam)
{
    struct ku_access_mtutc_reg reg_mtutc;
    int                        err = 0;
    ...
    if (IS_ADJPHASE_SUPPORTED_BY_HW(reg_mcam->mng_feature_cap_mask[1])) {
        adjphase_supported = true;
    }
    if (!adjphase_supported) {
        sxd_log_debug("ADJ_PHASE is not supported by the asic.\n");
        goto out;
    }
    ...
    sxd_log_notice("phase adjustment is supported, with log_max_phase_adjustment being %u.\n",
...
static int __adjphase_spc2(struct ptp_clock_info *ptp, s32 delta)
{
    ...
    if (!err) {
        sx_clock_dbg_add_setter(priv);
        sx_clock_log_add_adjphase(priv, delta);
    } else {
        err = -EFAULT;
    }
...
int sx_pps_system_type_get(struct sx_priv *priv, int *dev_id)
{
    ...
    if (err) {
        sxd_log_err("PPS: Failed to get system info from MGIR register, err=%d\n", err);
        return err;
    }
...
/* For kernels that doesn't support duty cycle flag, leave the out pulse duration as 0, then device default will be used */
    ...
    if (err) {
        sxd_log_err("PPS perout configure: failed to set MTPPS, err=%d\n", err);
        return err;
    }
    ...
    if (err) {
        sxd_log_err("PPS perout configure: failed to set MTPPSE, err=%d\n", err);
        return err;
    }
...
static int __ptp_pps_extts_configure(struct sx_priv *priv, struct ptp_clock_request *rq, int on)
{
    ...
    err = sx_ACCESS_REG_MTPPS(&priv->dev, &mtpps);
    if (err) {
        sxd_log_err("PPS extts configure: failed to set MTPPS, err=%d\n", err);
        return err;
    }
    ...
    err = sx_ACCESS_REG_MTPPSE(&priv->dev, &mtppse);
    if (err) {
        sxd_log_err("PPS extts configure: failed to set MTPPSE, err=%d\n", err);
        return err;
    }
...
static int __ptp_pps_capabilities_get(struct sx_priv *priv)
{
    ...
    err = sx_ACCESS_REG_MTPPS(&priv->dev, &mtpps);
    if (err) {
        sxd_log_err("failed to query MTPPS, err=%d\n", err);
    }
...
static int __ptp_pps_pin_config_init(struct sx_priv *priv)
{
    ...
    if (!priv->hw_clock.clock_info.pin_config) {
        sxd_log_err("failed to allocate PPS pin config\n");
        return -1;
    }
...
int sx_clock_ptp_pps_init_spc2(struct sx_priv *priv)
{
    ...
    if (err) {
        sxd_log_err("failed to get PPS capabilities, err=%d\n", err);
        return err;
    }

    err = __ptp_pps_pin_config_init(priv);
    if (err) {
        sxd_log_err("failed to init PPS pin config, err=%d\n", err);
    }
...
int sx_clock_dev_init_spc2(struct sx_priv *priv)
{
    ...
    if (!priv->hw_clock.hwd.spc2.utc_sec) {
        sxd_log_err("could not remap UTC-sec register\n");
        err = -ENOMEM;
        goto failed_utc;
    }
    ...
    if (!priv->hw_clock.hwd.spc2.utc_nsec) {
        sxd_log_err("could not remap UTC-nsec register\n");
        err = -ENOMEM;
        goto failed_nsec;
    }
    ...
    if (IS_ADJTIME_RANGE_EXTENDED(reg_mcam.mcam_reg.mng_feature_cap_mask[1])) {
        priv->hw_clock.hwd.spc2.adjtime_range = 125000; /* -125000 to 125000 */
    } else {
        priv->hw_clock.hwd.spc2.adjtime_range = 32768; /* -32768 to 32768 */
    }

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 8, 0))
    err = __init_adjphase_params(priv, &reg_mcam.mcam_reg);
    if (err) {
        sxd_log_err("failed to query phase adjustment related params clock (err=%d)\n", err);
        goto failed_reg;
    }
#endif

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 10, 0))
    err = __init_adjfine_params(priv);
    if (err) {
        sxd_log_err("failed to query fine adjustment related params (err=%d)\n", err);
        goto failed_reg;
    }
#endif
    ...
    if (err) {
        sxd_log_err("failed to register clock (err=%d)\n", err);
        goto failed_reg;
    }
...
int sx_clock_dump_spc2(struct sx_priv *priv, struct seq_file *m, void *v, void *context)
{
    ...
    seq_printf(m, "\n");
    seq_printf(m, "adj_time range: -/+ %d\n", priv->hw_clock.hwd.spc2.adjtime_range);
    seq_printf(m, "\n\n");
...
void sx_core_ptp_handle_mtppst_event(struct completion_info *ci)
{
    ...
    if (err) {
        sxd_log_err("__MTPPST_decode returned with error %d\n", err);
        return;
    }

    if (mtppst_reg.pin > clock_info->n_pins) {
        sxd_log_err("PPS clock event unsupported pin=%d\n", mtppst_reg.pin);
        return;
    }
    ...
    case PTP_PF_PEROUT:
        sxd_log_err("Unsupported PPS clock OUT event\n");
        break;

    default:
        sxd_log_err("PPS clock event unsupported pin function=0x%x, pin=%d\n",
