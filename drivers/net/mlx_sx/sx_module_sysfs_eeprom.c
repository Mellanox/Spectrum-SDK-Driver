...
static inline int __reg_mcia_pack(struct ku_mcia_reg *mcia_reg,
                                  uint8_t             module,
                                  uint8_t             lock,
                                  uint8_t             page,
                                  uint8_t             pnv,
                                  ...
                                  uint8_t             slot)
{
    ...
    err = sx_core_get_possible_local_module(module, &local_module);
    if (err) {
        goto out;
    }
                                 ...
                                 struct ku_access_mcia_reg *reg_data)
{
    ...
    err = __reg_mcia_pack(&(reg_data->mcia_reg), module, lock, page, pnv, device_addr, size, i2c_addr, slot);
    if (err) {
        goto out;
    }
...
int sx_core_get_module_page_from_eeprom(struct sx_dev *dev, uint8_t module, uint8_t slot, uint8_t *page_p)
{
    ...
    err = __reg_mcia_pack(&reg_data.mcia_reg, module, 0, 0, 1, 127, 1, 0x50, slot);
    if (err) {
        goto out;
    }
    ...
    *page_p = MCIA_DWOARD_BYTE1(reg_data.mcia_reg.dword_0);
...
/* This function is valid for independent module only.
 * It updates the last access page in DB, and sets pnv=0 if last access page changed. */
                                           ...
                                           uint8_t       *pnv_p)
{
    ...
    /* If last page accessed is not cached in independent module DB:
     * Read it from module's EEPROM. */
    if (sx_priv(dev)->module_data[slot][module].independent_params.last_page_valid == false) {
        err = sx_core_get_module_page_from_eeprom(dev,
                                                  module,
                                                  slot,
                                                  &last_page);
        if (err) {
            sxd_log_err("Failed to get module page, err: %d.\n", err);
            goto out;
        }
        /* Cache the read value in independent module DB */
        sx_priv(dev)->module_data[slot][module].independent_params.last_page = last_page;
        sx_priv(dev)->module_data[slot][module].independent_params.last_page_valid = true;
    }

    if (sx_priv(dev)->module_data[slot][module].independent_params.last_page == page) {
        ...
        *pnv_p = 1;
         ...
         * Use legacy MCIA.PNV=0, and cache page value in independent module DB. */
        *pnv_p = 0;
        sx_priv(dev)->module_data[slot][module].independent_params.last_page = page;
    }
...
static int __sx_core_get_module_type(struct sx_dev *dev, uint8_t module, uint8_t slot, uint8_t *type)
{
    ...
    err = __reg_mcia_pack(&reg_data.mcia_reg, module, 0, 0, 0, 0, 1, 0x50, slot);
    if (err) {
        goto out;
    }
    ...
    *device_addr = pos;
    ...
    if (is_independent) {
        *limit_arr_len = 0;
         ...
         * page offset write.
         * Driver is caching the last access page to reduce number of EEPROM accesses.
         * The caching logic is done for every module type (flat / non-flat).
         * */
...
err_out:
    if (is_independent) {
        /* If error occur - reset last page stored in DB back to unknown */
        sx_priv(dev)->module_data[slot][module].independent_params.last_page = 0;
        sx_priv(dev)->module_data[slot][module].independent_params.last_page_valid = false;
    }
    if (release_lock) {
        mutex_unlock(&priv->module_access_mutex);
    }
