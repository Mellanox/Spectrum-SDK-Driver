...
static void rx_timeout_handler(uint32_t session_id)
{
    ...
    if (!session) {
        sxd_log_err("RX timeout error: session id %d isn't found.\n",
                    session_id);
...
static int rx_sess_add(struct bfd_offload_info *request_hdr, char *data, int update, void *stats)
{
    ...
    if (!session) {
        sxd_log_err("Failed to allocate new RX session.\n");
        err = -ENOMEM;
        goto bail;
    }
    ...
    if (session->packet == NULL) {
        sxd_log_err("Failed to allocate packet of size %zu in RX session.\n",
                    request_hdr->size);
        err = -ENOMEM;
        goto bail;
    }
    ...
    if (err < 0) {
        sxd_log_err("Failed to copy RX packet from user.\n");
        goto bail;
    }
    ...
    if (err < 0) {
        sxd_log_err("Failed to create RX delayed work.\n");
        goto bail;
    }
    ...
    if (!entry_session) {
        sxd_log_err("RX entry session allocation failed.\n");
        err = -ENOMEM;
        goto bail;
    }
    ...
    if (sx_bfd_rx_sess_lkp_entry_by_ip_addr_vrf(&session->ip_addr,
        ...
        spin_unlock_bh(&rx_sess_lock);
        if (session->ip_addr.family == AF_INET) {
            sxd_log_warning("RX session [ip:%pI4, vrf_id:%d] already exists.\n",
                            &(session->ip_addr.ipv4), session->vrf_id);
        } else {
            sxd_log_warning("RX session [ip:%pI6, vrf_id:%d] already exists.\n",
    ...
    if (entry_vrf) {
        ...
        entry_vrf = sx_bfd_rx_vrf_init(session->vrf_id,
                                       ...
                                       request_hdr->bfd_pid);
        if (!entry_vrf) {
            sxd_log_err("Initialization of VRF %d (%s) socket FAILED.\n",
...
int sx_bfd_rx_sess_add(char * data)
{
    ...
    if (err < 0) {
        sxd_log_err("Failed to copy RX offload request from user.\n");
        goto bail;
    }

    err = rx_sess_add(&request_hdr, data, 0, NULL);
    if (err < 0) {
        sxd_log_err("rx_sess_add for session %d VRF %d FAILED.\n",
                    request_hdr.session_id, request_hdr.vrf_id);
        goto bail;
    }

    sxd_log_debug("Session added session_id %d VRF %d.\n",
                  request_hdr.session_id, request_hdr.vrf_id);
...
static int rx_sess_del(uint32_t session_id, int update, void *stats)
{
    ...
    if (!entry) {
        spin_unlock_bh(&rx_sess_lock);
        sxd_log_err("RX session %d doesn't exist.\n",
                    session_id);
        return -ENOENT;
    }
...
int sx_bfd_rx_sess_del(char * data)
{
    ...
    if (err) {
        sxd_log_err("Failed to delete rx_session %d.\n", request_hdr.session_id);
        goto bail;
    }
    sxd_log_debug("rx session %u was deleted\n", request_hdr.session_id);
...
int sx_bfd_get_rx_sess_stats(char * data, uint8_t clear_stats)
{
    ...
    if (err) {
        sxd_log_err("Failed to copy bfd_offload_get_stats from user.\n");
        goto bail;
    }
    ...
    if (!session) {
        sxd_log_debug("RX session %d doesn't exist.\n",
                      request_hdr.session_id);
        goto bail;
    }
...
int sx_bfd_rx_sess_update(char *data)
{
    ...
    if (err != 0) {
        sxd_log_err("Failed to add rx_session %d.\n.\n", request_hdr.session_id);
        goto bail;
    }

    sxd_log_debug("Session session_id %d VRF %d updated.\n",
                  request_hdr.session_id, request_hdr.vrf_id);
