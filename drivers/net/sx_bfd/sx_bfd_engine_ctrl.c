...
static int sx_bfd_parse_cmd(char* data, int cmd)
{
    ...
    default:
        sxd_log_debug("Unsupported sx bfd command %d", cmd);
    }
...
int sx_bfd_engine_ctrl_init(void)
{
    ...
    if (err < 0) {
        sxd_log_err("Kernel BFD chardevice failed to initialize.\n");
        goto bail;
    }

    err = sx_bfd_workqueue_init();
    if (err < 0) {
        sxd_log_err("Kernel BFD workqueue initialization failed.\n");
        goto bail;
    }

    err = sx_bfd_rx_session_init();
    if (err < 0) {
        sxd_log_err("Kernel BFD RX session initialization failed.\n");
        goto bail;
    }

    err = sx_bfd_tx_session_init();
    if (err < 0) {
        sxd_log_err("Kernel BFD TX session initialization failed.\n");
        goto bail;
    }
