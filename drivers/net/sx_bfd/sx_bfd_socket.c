...
int sx_bfd_socket_recv(struct socket *sock, char* buf, size_t buf_len, struct metadata* metadata)
{
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 19, 0)) && !defined(CONFIG_SX_RHEL_8_6) && !defined(CONFIG_SX_CENTOS_8_4)
    int peer_addr_size;
#endif
    ...
    /* Peer address */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)) || defined(CONFIG_SX_RHEL_8_6) || defined(CONFIG_SX_CENTOS_8_4)
    sock->ops->getname(sock, (struct sockaddr *)&metadata->peer_addr, 1);
#else
    sock->ops->getname(sock, (struct sockaddr *)&metadata->peer_addr,
                       &peer_addr_size, 1);
#endif

    /* if peer address is IPv4 mapped IPv6 address, then fill in IPv4 socket information
     * by manually retrieving socket parameters from IPv6 address.
     */
    if (ipv6_addr_type(&(metadata->peer_addr.peer_in6.sin6_addr)) == IPV6_ADDR_MAPPED) {
        ...
        memcpy(&peer_in.sin_addr,
               ...
               4);
        peer_in.sin_port = metadata->peer_addr.peer_in6.sin6_port;
    ...
    while (scheduler.scheduler_thread_active) {
        ...
        if (user_info) {
            ...
            if (down_interruptible(&scheduler.scheduler_sem)) {
                sxd_log_err("unexpected return value of down_timeout \n");
            }
...
static int sk_bfd_socket_bindtodevice(struct socket *sock, char *linux_vrf_name)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
    sockptr_t optval = KERNEL_SOCKPTR(linux_vrf_name);

    err = sock_setsockopt(sock,
                          SOL_SOCKET,
                          SO_BINDTODEVICE,
                          optval, BFD_LINUX_VRF_NAME_LENGTH);
#else
    mm_segment_t oldfs;
    oldfs = get_fs();
    set_fs(KERNEL_DS);
    ...
    if (err < 0) {
        sxd_log_warning("Failed to set BFD socket SO_BINDTODEVICE option (vrf:%s). Err = %d\n",
                        linux_vrf_name, err);
    }
                          ...
                          char            *linux_vrf_name)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
    struct __kernel_sock_timeval time;
    sockptr_t                    optval;
#else
    mm_segment_t   oldfs;
    struct timeval time;
    ...
    if (linux_vrf_name == NULL) {
        sxd_log_err("Unexpected NULL <linux_vrf_name>.\n");
        return -EIO;
    }

    /* Create Socket */
    err = sock_create(AF_INET6, SOCK_DGRAM, IPPROTO_UDP, &t_sock);
    if (err < 0) {
        sxd_log_err("Failed to create BFD socket (err:%d) .\n", err);
        return -EIO;
    }

    if (use_vrf_device) {
        if (sk_bfd_socket_bindtodevice(t_sock, linux_vrf_name) < 0) {
            sxd_log_err("Failed to bind rx BFD socket to vrf device: %s.\n",
                        linux_vrf_name);
            return -EIO;
        }
    }
    ...
    if (sk_user_data == NULL) {
        sxd_log_err("Failed to allocate sk_user_data.\n");
        err = -ENOMEM;
        return err;
    }
    ...
    err = t_sock->ops->bind(t_sock,
                            (struct sockaddr *)&s6addr,
                            sizeof(struct sockaddr_in6));
    if (err < 0) {
        sxd_log_err("Failed to bind BFD socket to port %d (err:%d).\n",
                    port, err);
        return -EIO;
    }

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
    optval = KERNEL_SOCKPTR(&value);
#else
    oldfs = get_fs();
    set_fs(KERNEL_DS);
#endif
    ...
    err = t_sock->ops->setsockopt(t_sock,
                                  IPPROTO_IP,
                                  IP_RECVTTL,
    #if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
                                  optval,
                                  ...
                                  sizeof(value));
    if (err < 0) {
        sxd_log_warning("Failed to set BFD socket IP_RECVTTL option (err:%d).\n",
                        err);
    }
    ...
    err = t_sock->ops->setsockopt(t_sock,
                                  ...
                                  sizeof(value));
    if (err < 0) {
        sxd_log_warning("Failed to set BFD socket IPV6_RECVHOPLIMIT option (err:%d).\n",
                        err);
    }
    ...
    err = t_sock->ops->setsockopt(t_sock,
                                  ...
                                  sizeof(value));
    if (err < 0) {
        sxd_log_warning("Failed to set BFD socket IP_PKTINFO option (err:%d).\n",
                        err);
    }
    ...
    err = t_sock->ops->setsockopt(t_sock,
                                  ...
                                  sizeof(value));
    if (err < 0) {
        sxd_log_warning("Failed to set BFD socket IPV6_RECVPKTINFO option (err:%d).\n",
                        err);
    }
    ...
    time.tv_usec = 20000;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
    optval = KERNEL_SOCKPTR(&time);
    err = sock_setsockopt(t_sock,
                          SOL_SOCKET,
                          SO_RCVTIMEO_NEW,
                          optval,
                          sizeof(time));
    ...
    if (err < 0) {
#endif
        sxd_log_warning("Failed to set BFD socket SO_RCVTIMEO option (err:%d).\n",
                        err);
    }

    rcvbuf = 4096 * 4096;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
    optval = KERNEL_SOCKPTR(&rcvbuf);
    err = sock_setsockopt(t_sock, SOL_SOCKET, SO_RCVBUFFORCE,
                          optval, sizeof(rcvbuf));
    if (err) {
        sxd_log_warning("setting %d-byte socket receive buffer failed (err:%d)",
                        rcvbuf, err);
    }
#else
    err = sock_setsockopt(t_sock, SOL_SOCKET, SO_RCVBUFFORCE,
                            ...
                            char            * linux_vrf_name)
{
    ...
    if (local_addr == NULL) {
        sxd_log_err("Unexpected NULL <local_addr>.\n");
        err = -EIO;
        goto bail;
    }

    if (sock == NULL) {
        sxd_log_err("Unexpected NULL <sock>.\n");
        err = -EIO;
        goto bail;
    }
    ...
    /* Create Socket */
    err = sock_create(sa_family, SOCK_DGRAM, IPPROTO_UDP, &t_sock);
    if (err < 0) {
        sxd_log_err("Failed to create BFD socket (err:%d) .\n", err);
        err = -EIO;
        goto bail;
    ...
    if (use_vrf_device) {
        err = sk_bfd_socket_bindtodevice(t_sock, linux_vrf_name);
        if (err < 0) {
            sxd_log_err("Failed to bind tx BFD socket to vrf device: %s (err:%d).\n",
                        linux_vrf_name, err);
            err = -EIO;
            goto bail;
        }
    ...
    if (err < 0) {
        if (sa_family == AF_INET) {
            sxd_log_err("Failed to bind BFD socket to local_addr (ip:%pI4 ,port:%d) (err:%d).\n",
                        &(((struct sockaddr_in*)local_addr)->sin_addr),
                        ntohs(((struct sockaddr_in*)local_addr)->sin_port),
                        err);
        } else {
            sxd_log_err("Failed to bind BFD socket to local_addr (ip:%pI6 ,port:%d) (err:%d).\n",
...
void * sx_bfd_rx_vrf_init(uint32_t vrf_id, uint8_t use_vrf_device, char *linux_vrf_name, unsigned long bfd_pid)
{
    ...
    if (!vrf) {
        sxd_log_err("Failed to allocate VRF DS.\n");
        err = -ENOMEM;
        goto bail;
    }
    ...
    if (!entry_vrf) {
        sxd_log_err("RX entry session VRF allocation failed.\n");
        err = -ENOMEM;
        goto bail;
    }
    ...
    err = sx_bfd_rx_socket_init(SX_BFD_SINGLEHOP_PORT,
                                ...
                                linux_vrf_name);
    if (err) {
        sxd_log_err("Create SX_BFD_SINGLEHOP_PORT socket (vrf_d:%d,bfd_pid:%lu,vrf_name:%s) failed (err:%d).\n",
                    vrf->vrf_id, bfd_pid, linux_vrf_name, err);
        err = -EIO;
        goto bail;
    }
    ...
    err = sx_bfd_rx_socket_init(SX_BFD_MULTIHOP_PORT,
                                ...
                                linux_vrf_name);
    if (err) {
        sxd_log_err("Create SX_BFD_MULTIHOP_PORT socket (vrf_d:%d,bfd_pid:%lu,vrf_name:%s) failed (err:%d).\n",
                    vrf->vrf_id, bfd_pid, linux_vrf_name, err);
        err = -EIO;
        goto bail;
    }
...
int sx_bfd_socket_init(void)
{
    ...
    if (!g_initialized) {
        ...
        if (bfd_thread == NULL) {
            ...
            if (IS_ERR(bfd_thread)) {
                err = PTR_ERR(bfd_thread);
                sxd_log_err("Kernel BFD single-hop thread failed to start (err=%d).\n", err);
                goto bail;
            }
        }
        hash_init(rx_vrfs);
