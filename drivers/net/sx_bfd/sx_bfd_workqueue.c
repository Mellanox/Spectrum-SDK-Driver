...
static void handler(struct work_struct * work)
{
    ...
    /* sxd_log_err("sx_bfd_work %p.\n", sx_bfd_work); */
    /* sxd_log_err("sx_bfd_work->data %d.\n", sx_bfd_work->data); */
...
int sx_bfd_create_delayed_work(sx_bfd_delayed_work_t ** dwork, handler_func func, uint32_t data)
{
    ...
    if (t_dwork == NULL) {
        sxd_log_err("Memory allocation for delayed work failed.\n");
        err = -ENOMEM;
        goto bail;
    }
...
int sx_bfd_workqueue_init(void)
{
    ...
    if (!g_initialized) {
        ...
        if (workqueue == NULL) {
            sxd_log_err("Kernel BFD work queue initialization failed.\n");
            err = -EIO;
            goto bail;
        }
