...
static void tx_timeout_handler(uint32_t session_id)
{
    ...
    if (session == NULL) {
        sxd_log_debug("TX timeout, but session id %d doesn't exists.\n",
                      session_id);
...
struct sx_bfd_tx_session * sx_bfd_tx_sess_get(uint32_t session_id)
{
    ...
    if (!entry) {
        spin_unlock_bh(&tx_sess_lock);
        sxd_log_warning("TX Session %u doesn't exist.\n", session_id);
        return NULL;
    }
...
int sx_bfd_tx_sess_add(char *data, void* stats)
{
    ...
    if (err) {
        sxd_log_err("Failed to copy TX offload request from user.\n");
        goto bail;
    }
    ...
    if (!session) {
        sxd_log_err("Failed to allocate new TX session.\n");
        err = -ENOMEM;
        goto bail;
    }
    ...
    if (session->packet == NULL) {
        sxd_log_err("Failed to allocate packet in TX session.\n");
        err = -ENOMEM;
        goto bail;
    }
    ...
    if (err < 0) {
        sxd_log_err("Failed to copy TX packet from user.\n");
        goto bail;
    }
    ...
    if (err < 0) {
        sxd_log_err("Failed to create TX delayed work for session:%d (err:%d).\n",
    ...
    if (err < 0) {
        sxd_log_err("Failed to create TX socket for session %d (err:%d).\n",
                    request_hdr.session_id, err);
        goto bail;
    }
    ...
    if (!entry) {
        sxd_log_err("TX entry allocation failed for session %d.\n",
    ...
    if (sx_bfd_tx_sess_lkp_entry(request_hdr.session_id)) {
        spin_unlock_bh(&tx_sess_lock);
        sxd_log_warning("TX Session %d already exists.\n",
                        request_hdr.session_id);
        err = -EINVAL;
        goto bail;
    }
    ...
    sxd_log_debug("TX Session %u was added successfully.\n", request_hdr.session_id);
...
static int tx_sess_del(uint32_t session_id, void *stats)
{
    ...
    if (!entry) {
        spin_unlock_bh(&tx_sess_lock);
        sxd_log_err("TX session %d doesn't exist.\n",
                    session_id);
        return -ENOENT;
    }
...
int sx_bfd_tx_sess_del(char *data, void* stats)
{
    ...
    if (err) {
        sxd_log_err("Failed to delete tx_session. (session id  %u)\n", request_hdr.session_id);
        goto bail;
    }

    sxd_log_debug("tx session %u was deleted\n", request_hdr.session_id);
...
int sx_bfd_get_tx_sess_stats(char * data, uint8_t clear_stats)
{
    ...
    if (session == NULL) {
        sxd_log_err("TX timeout, but session id %d wasn't found.\n",
                    request_hdr.session_id);
        /* Noting to do, session is probably deleted */
        return -ENOENT;
    }
    ...
    if (err) {
        sxd_log_err("Failed to copy bfd_offload_get_tx_stats from user.\n");
        goto bail;
    }
