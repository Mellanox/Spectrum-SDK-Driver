                              ...
                              unsigned long             bfd_user_space_pid)
{
    ...
    if (!event_msg) {
        sxd_log_err("event_msg allocation failed (event_msg_size:%d, pkt_size:%d).\n",
                    event_msg_size, size);
        goto out;
    }
    ...
    if (send_trap(event_msg,
        ...
        sxd_log_err("Failed to send data trap 0x%x for session (%d).\n",
...
void sx_bfd_event_send_timeout(struct sx_bfd_rx_session *session)
{
    ...
    if (send_trap(&event,
        ...
        sxd_log_err("Failed to send timeout trap 0x%x for session_id %d and VRF %d.\n",
