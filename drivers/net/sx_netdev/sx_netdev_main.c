...
int send_netdev_sniffer_event(struct kobject          *kobj,
                              const char              *cb_name,
                              ...
                              enum sx_net_kobject_type kobj_type)
{
    ...
    CALL_SX_CORE_FUNC_WITH_RET(sx_netdev_get_net_device_by_kobj, ret, kobj, &netdev, kobj_type);
    if (ret) {
        sxd_log_err("Failed to get the net device of the kobj\n");
        return ret;
    }
...
static ssize_t _get_pddr(struct kobject *kobj, struct kobj_attribute *attr, char *buf, sxd_pddr_page_select_t page)
{
    int                       ret = 0;
    struct net_device        *netdev = NULL;
    struct sx_net_priv       *net_priv = NULL;
    ...
    return _access_reg_pddr_data_to_buffer_print(buf, PAGE_SIZE, &reg_data.pddr_reg);
}
...
static ssize_t show_netdev_pipeline_info(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    if (net_priv->pipeline == SX_NETDEV_PIPELINE_ADAPTIVE) {
        return snprintf(buf, PAGE_SIZE, "adaptive\n");
    } else if (net_priv->pipeline == SX_NETDEV_PIPELINE_BYPASS) {
        return snprintf(buf, PAGE_SIZE, "bypass\n");
    } else {
...
static ssize_t show_pddr_operation_info(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    ret = send_netdev_sniffer_event(kobj, __FUNCTION__, attr->attr.name, NULL, 0, SX_NET_KOBJECT_PORT);
    if (ret) {
    ...
    return _get_pddr(kobj, attr, buf, SXD_PDDR_PAGE_SELECT_OPERATIONAL_INFO_PAGE_E);
}
...
static ssize_t show_pddr_troubleshooting_info(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    return _get_pddr(kobj, attr, buf, SXD_PDDR_PAGE_SELECT_TROUBLESHOOTING_INFO_PAGE_E);
}
static ssize_t show_pddr_link_down_info(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    int ret = 0;
    ...
    return _get_pddr(kobj, attr, buf, SXD_PDDR_PAGE_SELECT_LINK_DOWN_INFO_E);
}
...
static ssize_t show_pddr_module_info(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    return _get_pddr(kobj, attr, buf, SXD_PDDR_PAGE_SELECT_MODULE_INFO_PAGE_E);
}
static ssize_t show_pddr_latched_flag_info(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    int ret = 0;
    ...
    return _get_pddr(kobj, attr, buf, SXD_PDDR_PAGE_SELECT_MODULE_LATCHED_FLAG_INFO_PAGE_E);
}
...
static int sx_netdev_register_global_event_handler(void)
{
    ...
    CALL_SX_CORE_FUNC_WITH_RET(sx_core_add_synd, err, 0, SXD_TRAP_ID_PMPE, L2_TYPE_DONT_CARE, 0, "sx_netdev", 0,
                               &crit, sx_netdev_handle_pmpe_event, NULL,
...
static void * sx_netdev_init_one(struct sx_dev *dev)
{
    ...
    if (g_sx_dev) {
        sxd_log_warning("netdev does not support more than one PCI device!\n");
        return NULL;
    }
...
static ssize_t reset_module(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t len)
{
    ...
    ret = send_netdev_sniffer_event(kobj, __FUNCTION__, attr->attr.name, buf, len, SX_NET_KOBJECT_MODULE);
    if (ret) {
        sxd_log_err("send_netdev_sniffer_event from sysfs netdev entry %s failed [%d]\n", __FUNCTION__, ret);
        ret = 0;
        /*continue */
...
static ssize_t show_module_power_mode_policy(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    ret = send_netdev_sniffer_event(kobj, __FUNCTION__, attr->attr.name, NULL, 0, SX_NET_KOBJECT_MODULE);
    if (ret) {
        sxd_log_err("send_netdev_sniffer_event from sysfs netdev entry %s failed [%d]\n", __FUNCTION__, ret);
        ret = 0;
        /*continue */
...
static ssize_t show_module_temp_input(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    ret = send_netdev_sniffer_event(kobj, __FUNCTION__, attr->attr.name, NULL, 0, SX_NET_KOBJECT_MODULE_TEMP);
    if (ret) {
        sxd_log_err("send_netdev_sniffer_event from sysfs netdev entry %s failed [%d]\n", __FUNCTION__, ret);
        ret = 0;
        /*continue */
