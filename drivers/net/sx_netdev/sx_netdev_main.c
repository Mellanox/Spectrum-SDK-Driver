...
static ssize_t _get_pddr(struct kobject *kobj, struct kobj_attribute *attr, char *buf, sxd_pddr_page_select_t page)
{
    ...
    if (is_independent) {
        printk(KERN_ERR PFX "%s: Failed to read PDDR: port module is independent (SW control).\n", __func__);
        return -EFAULT;
    }
...
static ssize_t show_netdev_pipeline_info(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    if (net_priv->pipeline == SX_NETDEV_PIPELINE_ADAPTIVE) {
        ...
        printk(KERN_ERR PFX "net_priv->pipeline is invalid %d\n", net_priv->pipeline);
...
static void sx_netdev_handle_pmpe_event(struct completion_info *comp_info, void *context)
{
    ...
    CALL_SX_CORE_FUNC_WITH_RET(sx_core_get_ports_by_module, ret, g_sx_dev, slot_id, module_id,
    ...
    for (i = 0; i < ports_cnt; i++) {
        for (j = 0; j < MAX_PORT_NETDEV_NUM; j++) {
            ...
            if (netdev) {
                net_priv = netdev_priv(netdev);
                present =
                    PMPE_MODULE_PLUGGED(pmpe->module_status) ? SX_MODULE_PRESENT_PLUGGED : SX_MODULE_PRESENT_UNPLUGGED;
...
static void sx_netdev_handle_pude_event(struct completion_info *comp_info, void *context)
{
    ...
    if ((ethertype != ETHTYPE_EMAD) || (reg_id != PUDE_REG_ID) ||
        ((type_len >> EMAD_TLV_TYPE_SHIFT) != TLV_TYPE_REG_E) ||
        ((type_len & 0x7FF) != 4)) {
        printk(KERN_ERR "Called wrongly with ethtype = %04X and reg-id = %04X, type = %d, len = %d\n",
               comp_info->info.eth.ethtype, reg_id,
               type_len >> EMAD_TLV_TYPE_SHIFT, type_len & 0x7FF);
        return;
    }
    ...
    if (!carrier_set_on_pude_disable) {
        printk(KERN_NOTICE "Called for logical port - %05X status %s\n", logical_port, is_up ? "UP" : "DOWN");
    }
                                        ...
                                        cq_handler                      *netdev_callback_p)
{
    ...
    default:
        printk(KERN_ERR "unsupported user channel type (%d).\n", uc_type);
        err = -ERANGE;
        goto out;
        break;
    }
    ...
    *tc = 0;
    ...
    if (ntohs(veth->h_vlan_proto) != ETH_P_8021Q) {
        ...
        if (sx_netdev_tx_debug) {
            pr_debug(PFX "VETH proto (0x%x) is not ETH_P_8021Q !\n",
                     veth->h_vlan_proto);
        }
...
static int sx_netdev_set_vlan_according_hw(struct net_device *netdev, struct sk_buff *skb)
{
    ...
    if (!vlan_child_exist) {
        if (sx_netdev_tx_debug) {
            pr_debug(PFX "%s: netdev (%s) No vlan child.\n", __func__, netdev->name);
        }
        return 0;
    }
    ...
    if (sx_netdev_tx_debug) {
        pr_debug(PFX "is_lag:%d, port:%d, vlan (from header):%d port is_tagged:%d, port is_prio_tag:%d.\n",
    ...
    if (!is_tagged && is_prio_tagged) {
        ...
        if (sx_netdev_tx_debug) {
            pr_debug(PFX "Port is not vlan tag but prio tag, clear the vlan: vlan_tci:0x%x.\n",
                     veth->h_vlan_TCI);
        }
    } else if (!is_tagged && !is_prio_tagged) {
        if (sx_netdev_tx_debug) {
            pr_debug(PFX "Port is not vtag and not prio tag, remove vlan header.\n");
        }
        ...
        if (sx_netdev_tx_debug) {
            pr_debug(PFX "Port is vlan tagged. do nothing.\n");
        }
    }
...
static int __get_link_ext_state(struct net_device *dev, struct sx_link_ext_state_info *link_ext_state_info)
{
    ...
    if (is_independent) {
        printk(KERN_WARNING PFX "%s: Failed to read link ext state: port module is independent (SW control).\n",
               __func__);
        return -ENODATA;
    }
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 18, 0))
    return port_devlink_port_split(devlink, port->port_index, count, extack);
#else/* LINUX_VERSION_CODE >= KERNEL_VERSION(5, 18, 0) */
...
#if defined(CONFIG_NET_PORT) && (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 18, 0))
    return port_devlink_port_unsplit(devlink, port->port_index, extack);
#else /* LINUX_VERSION_CODE >= KERNEL_VERSION(5, 18, 0) */
...
static int sx_netdev_init_one_netdev(struct sx_dev *dev, struct sx_netdev_rsc *resources, int swid, int synd, u64 mac)
{
    ...
    if (resources->allocated[swid]) {
        mutex_unlock(&resources->rsc_lock);
        pr_debug(PFX  "Net Device for swid %d was already"
                 " allocated\n", swid);
...
static void * sx_netdev_init_one(struct sx_dev *dev)
{
    ...
    pr_debug(PFX "sx_netdev_init_one\n");
...
#ifdef SX_DEBUG
    pr_debug(PFX "sx_netdev_remove_one\n");
#endif
...
static ssize_t show_link_ext_state(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    if (ret) {
        pr_debug(PFX "Cannot get state info in __get_link_ext_state\n");
        return 0;
    }
