...
static int __psample_listener_get(u32 group_id, struct psample_listener_context **listener_ctx_pp)
{
    ...
    if (listener_ctx == NULL) {
        if (__psample_listener_db_size == MAX_PSAMPLE_LISTENERS) {
            sxd_log_err("PSAMPLE [add group_id=%u]: database is full, cannot add another entry\n", group_id);
            err = -EUSERS;
            goto out;
        }
        ...
        if (!listener_ctx) {
            sxd_log_err("PSAMPLE [add group_id=%u]: could not allocate context\n", group_id);
            err = -ENOMEM;
            goto out;
        }
        ...
        if (!listener_ctx->group) {
            sxd_log_err("PSAMPLE [add group_id=%u]: could not get group\n", group_id);
            err = -ENOMEM;
            goto cleanup;
        }
...
static int __psample_listener_put(u32 group_id)
{
    ...
    if (!listener_ctx) { /* did not find it in DB */
        sxd_log_err("PSAMPLE [del group_id=%u]: could not find listener in database\n", group_id);
        err = -ENOENT;
        goto out;
    }

    if (--listener_ctx->sx_psample_context.refcnt > 0) {
        sxd_log_info("PSAMPLE [del group_id=%u]: listener still has references\n", group_id);
        goto out;
    }
...
static int __psample_synd_add(struct sx_dev *dev, u16 synd, union sx_event_data *event_data)
{
    ...
    if (err) {
        sxd_log_err("__psample_listener_get failed for group_id=%u (err=%d)\n", group_id, err);
        goto out;
    }

    snprintf(synd_str, sizeof(synd_str) - 1, "PSAMPLE [g=%u,rc=%u]",
             listener_ctx->sx_psample_context.group_num, listener_ctx->sx_psample_context.refcnt);
    ...
    if (err) {
        sxd_log_err("PSAMPLE [add group_id=%u]: could not add synd listener (err=%d)\n", group_id, err);
        goto out;
    }
...
static int __psample_synd_remove(struct sx_dev *dev, u16 synd, union sx_event_data *event_data)
{
    ...
    if (err) {
        ...
        sxd_log_err("PSAMPLE [del group_id=%u]: could not remove synd listener (err=%d)\n", group_id, err);
        goto out;
    }
...
int sx_psample_set_synd(struct sx_dev *dev, u16 synd, enum sx_dev_event event, union sx_event_data *event_data)
{
    int err = 0;

    switch (event) {
    case SX_DEV_EVENT_ADD_SYND_PSAMPLE:
        err = __psample_synd_add(dev, synd, event_data);
        break;

    case SX_DEV_EVENT_REMOVE_SYND_PSAMPLE:
        err = __psample_synd_remove(dev, synd, event_data);
        break;

    default:
        err = -EINVAL;
        sxd_log_err("invalid PSAMPLE sx-dev-event (event=%d)\n", event);
        break;
    }
...
int sx_psample_set_port_sample_ret(u16 local_port, u32 rate)
{
    if (local_port > MAX_PHYPORT_NUM) {
        sxd_log_err("PSAMPLE sample rate update got invalid local port (%u)\n", local_port);
        return -EINVAL;
    } else {
        sxd_log_info("PSAMPLE - update port %u sampling rate to %u\n", local_port, rate);
...
static void __psample_wq_add_work(struct psample_work_data * external_data)
{
    ...
    if (data == NULL) {
        sxd_log_err("PSAMPLE : failed to alloc work_data \n");
        return;
    }
...
static int sx_psample_add_cls_matchall(struct sx_net_priv *sx_port, struct tc_cls_matchall_offload *f, bool ingress)
{
    ...
    if (!tcf_exts_has_one_action(f->exts)) {
        sxd_log_err("only singular actions are supported\n");
        return -EOPNOTSUPP;
    }
