...
#endif
{
    ...
    if (!data[IFLA_SX_NETDEV_SWID]) {
        sxd_log_info("%s: SWID doesn't exists\n", __func__);
        return -EINVAL;
    }

    id = nla_get_u32(data[IFLA_SX_NETDEV_SWID]);
    if (id >= NUMBER_OF_SWIDS) {
        sxd_log_info("%s: SWID is out of range - %d\n", __func__, id);
        return -ERANGE;
    }

    if (data[IFLA_SX_NETDEV_PIPELINE]) {
        id = nla_get_u32(data[IFLA_SX_NETDEV_PIPELINE]);
        if (id >= SX_NETDEV_PIPELINE_MAX) {
            sxd_log_err("pipeline is out of range - %d\n", id);
            return -ERANGE;
        }
    }
    ...
    if (data[IFLA_SX_NETDEV_PORT]) {
        if (sx_netdev_sx_core_if_get_reference()) {
            if (g_sx_dev) {
                ...
                sxd_log_info("%s: g_sx_dev is NULL\n", __func__);
        ...
        if (id > MAX_SYSPORT_NUM) {
            sxd_log_info("%s: PORT is out of range - %d\n", __func__, id);
            return -ERANGE;
        }
        if (is_lag) {
            CALL_SX_CORE_FUNC_WITH_RET(sx_core_get_lag_max, err, g_sx_dev, &lag_max, &lag_member_max);
            if (err) {
                sxd_log_err("Failed to get max number of LAGs. err = %d.\n", err);
                return err;
            }

            if (id >= lag_max) {
                sxd_log_info("%s: LAG is out of range - %d\n", __func__, id);
                return -ERANGE;
            }
        }
    }
...
static size_t sx_netdev_get_size(const struct net_device *dev)
{
    return nla_total_size(sizeof(__u32)) +  /* IFLA_SX_NETDEV_SWID */
           ...
           nla_total_size(sizeof(__u32)) + /* IFLA_SX_NETDEV_PIPELINE */
           0;
}
...
static int sx_netdev_fill_info(struct sk_buff *skb, const struct net_device *dev)
{
    ...
    if (nla_put_u32(skb, IFLA_SX_NETDEV_PIPELINE, net_priv->pipeline)) {
        goto nla_put_failure;
    }
    return 0;
...
static int __sx_netdev_oper_state_get(struct sx_dev *dev, u32 log_port, u8 *is_oper_state_up)
{
    ...
    if (SX_PORT_TYPE_ID_GET(log_port) == SX_PORT_TYPE_LAG) {
        ...
        if (err) {
            sxd_log_info("%s: Unable to get lag [0x%x] oper state."
                         " err = %d\n", __func__, log_port, err);
            *is_oper_state_up = 0;
        }
        sxd_log_info("%s: LAG port state for 0x%x = %d\n", __func__,
        ...
        if (err) {
            sxd_log_info("%s: Unable to get port state, port = 0x%x err = %d\n", __func__, log_port, err);
            paos_reg_data.paos_reg.oper_status = 0;
        }
        sxd_log_info("%s: Port state for 0x%x = %d\n", __func__, log_port, paos_reg_data.paos_reg.oper_status);
        sxd_log_debug("%s: Port state for 0x%x = %d\n", __func__, log_port, paos_reg_data.paos_reg.oper_status);
...
static int sx_netdev_port_mac_get(struct sx_dev *dev, u32 log_port, u64 *mac)
{
    ...
    if (SX_PORT_TYPE_ID_GET(log_port) == SX_PORT_TYPE_LAG) {
        *mac = 0;
        ...
        if (err) {
            sxd_log_info("%s: Unable to get port MAC address, err = %d\n", __func__, err);
...
static void __sx_netdev_oper_state_get_work_handler(struct work_struct *work_p)
{
    ...
    if (err) {
        sxd_log_info("%s: Unable to get port state, port = %d,"
...
static int sx_netdev_port_init(struct net_device *dev, struct nlattr *tb[], struct nlattr *data[])
{
    ...
    u32                               pipeline = SX_NETDEV_PIPELINE_ADAPTIVE;
    ...
    if (data[IFLA_SX_NETDEV_PIPELINE]) {
        pipeline = nla_get_u32(data[IFLA_SX_NETDEV_PIPELINE]);
        if (pipeline >= SX_NETDEV_PIPELINE_MAX) {
            sxd_log_err("pipeline is out of range - %d\n", pipeline);
            return -EINVAL;
        }
        sxd_log_debug("%s: pipeline is %d\n", __func__, pipeline);
    }
    ...
    CALL_SX_CORE_FUNC_WITH_RET(sx_core_get_send_to_port_as_data,
                               ...
                               1);
    if (err) {
        sxd_log_err("sx_core_get_send_to_port_as_data failed, err: %d \n", err);
        return err;
    }
    ...
    if (g_netdev_resources->port_allocated[port_type][net_priv->port] == MAX_PORT_NETDEV_NUM) {
        ...
        sxd_log_err("No more resources - Max Net Device for port %u was already"
                    " allocated\n", sys_port);
        return -EEXIST;
    }
    ...
    if (err) {
        sxd_log_debug("%s: Unable to get mac, port = %d,"
                      " is_lag = %d, err = %d\n", __func__, net_priv->port, net_priv->is_lag, err);
        mac = 0;
    }
    net_priv->mac = mac;
    ...
    sxd_log_debug("%s: Newly device %s log port 0x%x MAC address = %llx\n",
                  __func__,
                  dev->name,
                  logical_port,
                  net_priv->mac);
    ...
    if (net_priv->is_lag) {
        ...
        if (err) {
            sxd_log_info("%s: sx_core_get_lag_mid() failed error - %d\n", __func__, err);
    ...
    CALL_SX_CORE_FUNC_WITH_RET(sx_core_get_port_module_present, err, net_priv->dev, net_priv->port,
                               &(present));
    if (err) {
        sxd_log_info("%s: sx_core_get_port_module_present() failed error - %d\n", __func__, err);
        return err;
    }
    ...
    if (net_priv->kobj[SX_NET_KOBJECT_PORT] == NULL) {
        ...
        if (err) {
            sxd_log_err("%s: sx_netdev_create_port_sysfs_nodes() failed error - %d\n", __func__, err);
            mutex_unlock(&g_netdev_resources->rsc_lock);
            return err;
        }
    }
...
#endif
{
    ...
    if (!sx_core_if.init_done) {
        ...
        sxd_log_err("%s: can't add new port netdevice during sx_core detach/reset. \n", __func__);
        return -ENXIO;
    }
    read_unlock_irqrestore(&sx_core_if.access_lock, flags);
...
#endif
{
    ...
    if (!sx_core_if.init_done) {
        read_unlock_irqrestore(&sx_core_if.access_lock, flags);
        sxd_log_err("%s: can't change port netdevice during sx_core detach/reset. \n", __func__);
        return -ENXIO;
    }
    read_unlock_irqrestore(&sx_core_if.access_lock, flags);
...
static void sx_netdev_dellink(struct net_device *dev, struct list_head *head)
{
    ...
    if (!sx_core_if.init_done) {
        read_unlock_irqrestore(&sx_core_if.access_lock, flags);
        sxd_log_err("%s: can't delete port netdevice during sx_core detach/reset. \n", __func__);
        return;
    }
    read_unlock_irqrestore(&sx_core_if.access_lock, flags);
    ...
    /* rsc_lock is put after sysfs node operations to avoid circular lock from parallel sysfs access */
    mutex_lock(&g_netdev_resources->rsc_lock);
    if (single_netdev_per_port_enable) {
        for (i = 0; i < MAX_PORT_NETDEV_NUM; i++) {
            rcu_assign_pointer(g_netdev_resources->port_netdev[port_type][net_priv->port][i], NULL);
...
static const struct nla_policy                 sx_netdev_policy[IFLA_SX_NETDEV_MAX + 1] = {
    ...
    [IFLA_SX_NETDEV_PIPELINE] = { .type = NLA_U32 },
};
