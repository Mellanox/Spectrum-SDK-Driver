...
static index_type_data_t pddr_page_select_data_arr_g[13];
static index_type_data_t pemi_page_select_data_arr_g[1];
...
static void pddr_page_select_arr_init(void);
static void pemi_page_select_arr_init(void);
...
static void pddr_page_select_arr_init(void)
{
    memset(&pddr_page_select_data_arr_g, 0, sizeof(index_type_data_t) * 13);

    pddr_page_select_data_arr_g[0].field_value = 0;
    sprintf(pddr_page_select_data_arr_g[0].field_name, "Operational_info_page");
    pddr_page_select_data_arr_g[1].field_value = 1;
    sprintf(pddr_page_select_data_arr_g[1].field_name, "Troubleshooting_info_page");
    pddr_page_select_data_arr_g[2].field_value = 2;
    sprintf(pddr_page_select_data_arr_g[2].field_name, "Phy_info_page");
    pddr_page_select_data_arr_g[3].field_value = 3;
    sprintf(pddr_page_select_data_arr_g[3].field_name, "Module_info_page");
    pddr_page_select_data_arr_g[4].field_value = 4;
    sprintf(pddr_page_select_data_arr_g[4].field_name, "Port_events_page");
    pddr_page_select_data_arr_g[5].field_value = 5;
    sprintf(pddr_page_select_data_arr_g[5].field_name, "Device_events_page");
    pddr_page_select_data_arr_g[6].field_value = 6;
    sprintf(pddr_page_select_data_arr_g[6].field_name, "Link_down_info");
    pddr_page_select_data_arr_g[7].field_value = 7;
    sprintf(pddr_page_select_data_arr_g[7].field_name, "Link_down_events");
    pddr_page_select_data_arr_g[8].field_value = 8;
    sprintf(pddr_page_select_data_arr_g[8].field_name, "Link_up_info");
    pddr_page_select_data_arr_g[9].field_value = 9;
    sprintf(pddr_page_select_data_arr_g[9].field_name, "Module_latched_flag_info_page");
    pddr_page_select_data_arr_g[10].field_value = 10;
    sprintf(pddr_page_select_data_arr_g[10].field_name, "link_down_info_ext");
    pddr_page_select_data_arr_g[11].field_value = 0xFE;
    sprintf(pddr_page_select_data_arr_g[11].field_name, "mng_debug_page");
    pddr_page_select_data_arr_g[12].field_value = 0XFF;
    sprintf(pddr_page_select_data_arr_g[12].field_name, "phy_debug_page");

}

static void pemi_page_select_arr_init(void)
{
    memset(&pemi_page_select_data_arr_g, 0, sizeof(index_type_data_t) * 1);

    pemi_page_select_data_arr_g[0].field_value = 1;
    sprintf(pemi_page_select_data_arr_g[0].field_name, "Snr_samples");
...
static void enum_index_arr_init(void)
{
    port_type_arr_init();
    grp_arr_init();
    pddr_page_select_arr_init();
    pemi_page_select_arr_init();
    slot_index_arr_init();
    proto_mask_arr_init();
    ib_sel_arr_init();
    unit_arr_init();
...
{
    sxd_log_info("sx_spice_tree_init\n");
...
{
    sxd_log_info("In sx_spice_tree_deinit function \n");
    debugfs_remove_recursive(root_p);
}
...
int sx_spice_tree_asic_counters_eth_create(dev_private_data_t *dev_data)
{
    ...
    sxd_log_info("sx_spice_tree_asic_counters_eth_create\n");
    ...
    err = sx_spice_access_reg_asic_ports_file_create(asic_p, dev_data);
    if (err) {
        goto allocation_failed;
    }
...
int sx_spice_tree_ber_eth_create(dev_private_data_t *dev_data)
{
    ...
    struct dentry * asic_p;
    struct dentry * pemi_page_select_p;
    struct dentry * ib_sel_p;
    struct dentry * pddr_page_select_p;
    struct dentry * port_type_p;
    struct dentry * sensor_index_p;
    struct dentry * module_index_p;
    ...
    int pemi_page_select_it = 0;
    int ib_sel_it = 0;
    int pddr_page_select_it = 0;
    int port_type_it = 0;
    int sensor_index_it = 0;
    int module_index_it = 0;
    ...
    sxd_log_info("sx_spice_tree_ber_eth_create\n");
    ...
    for (local_port_it = 1; local_port_it <= dev_data->max_local_port; local_port_it++) {
        ...
        err = sx_spice_access_reg_pmlp_file_create(local_port_p, dev_data);
        if (err) {
            goto allocation_failed;
        }
        for (pemi_page_select_it = 0; pemi_page_select_it < 1; pemi_page_select_it++) {
            snprintf(dname, sizeof(dname), "page_select.%s.0x%X",
                pemi_page_select_data_arr_g[pemi_page_select_it].field_name,
                pemi_page_select_data_arr_g[pemi_page_select_it].field_value);
            pemi_page_select_p = debugfs_create_dir(dname, local_port_p);

            if (!pemi_page_select_p) {
                goto allocation_failed;
            }
            indexes_arr_g[PAGE_SELECT] = pemi_page_select_data_arr_g[pemi_page_select_it].field_value;

            err = sx_spice_access_reg_pemi_file_create(pemi_page_select_p, dev_data);
            if (err) {
                goto allocation_failed;
            }
        }
        for (port_type_it = 0; port_type_it < 6; port_type_it++) {
            ...
            for (pddr_page_select_it = 0; pddr_page_select_it < 13; pddr_page_select_it++) {
                snprintf(dname, sizeof(dname), "page_select.%s.0x%X",
                    pddr_page_select_data_arr_g[pddr_page_select_it].field_name,
                    pddr_page_select_data_arr_g[pddr_page_select_it].field_value);
                pddr_page_select_p = debugfs_create_dir(dname, port_type_p);

                if (!pddr_page_select_p) {
                    goto allocation_failed;
                }
                indexes_arr_g[PAGE_SELECT] = pddr_page_select_data_arr_g[pddr_page_select_it].field_value;

                err = sx_spice_access_reg_pddr_file_create(pddr_page_select_p, dev_data);
                if (err) {
                    goto allocation_failed;
                }
            }
    ...
    for (slot_index_it = 0; slot_index_it < 1; slot_index_it++) {
        ...
        for (module_index_it = 0; module_index_it <= dev_data->max_module_index; module_index_it++) {
            snprintf(dname, sizeof(dname), "module_index.%d", module_index_it );
            module_index_p = debugfs_create_dir(dname, slot_index_p);
            if (!module_index_p) {
                goto allocation_failed;
            }
            indexes_arr_g[MODULE_INDEX] = module_index_it;

            err = sx_spice_access_reg_pmaos_file_create(module_index_p, dev_data);
            if (err) {
                goto allocation_failed;
            }
        }
        ...
        for (i_it = 1; i_it <= 1; i_it++) {
            ...
            for (sensor_index_it = 0; sensor_index_it <= 0; sensor_index_it++) {
                snprintf(dname, sizeof(dname), "sensor_index.%d", sensor_index_it );
                sensor_index_p = debugfs_create_dir(dname, i_p);
                if (!sensor_index_p) {
                    goto allocation_failed;
                ...
                err = sx_spice_access_reg_mvcr_file_create(sensor_index_p, dev_data);
                if (err) {
                    goto allocation_failed;
                }
            }
...
int sx_spice_tree_ber_ib_create(dev_private_data_t *dev_data)
{
    ...
    sxd_log_info("sx_spice_tree_ber_ib_create\n");
