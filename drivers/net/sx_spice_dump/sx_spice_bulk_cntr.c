...
static inline void __sx_spice_bulk_cntr_ctx_finalize(struct sx_dev                *dev,
                                                     enum sxd_bulk_cntr_key_type_e key_type,
...
{
#ifdef SPICE_BULK_CNTR_LOG_EN
    sxd_log_info("%s: current state: %s new state: %s", __func__,
                                                     ...
                                                     spice_bulk_cntr_file_db_t    *file_db)
{
    ...
    bulk_cntr_db_mocs_session_release(dev, key_type);
...
static inline int __sx_spice_bulk_cntr_get_trap_id(enum sxd_bulk_cntr_key_type_e key_type, sxd_trap_id_t *trap_id_p)
{
    ...
    default:
        err = -EINVAL;
        sxd_log_err("%s: invalid argument for key_type_e %d err: %d", __func__,
                    key_type, err);
        break;
    }
    return err;
...
static inline int __sx_spice_bulk_cntr_verify_lbuf(enum sxd_bulk_cntr_key_type_e key_type, size_t lbuf)
{
    ...
    if (lbuf < min_size) {
        err = -EPERM;
        sxd_log_info("%s: Read operation not permitted insufficient buffer size lbuf: %zu"
...
static void __sx_spice_bulk_cntr_handle_completion_event(struct completion_info *comp_info, void *context)
{
    ...
    if (emad_tid != file_db->tr_ctx.ev_id.event_id_value) {
        ...
        sxd_log_err("%s: syndrom: 0x%x unable to match event_id from db emad_tid=%llu, event_tid=%llu "
...
#ifdef SPICE_BULK_CNTR_LOG_EN
    sxd_log_info("%s: syndrom: 0x%x trapped successfully: emad_tid=%llu, event_tid=%llu\n", __func__,
                            ...
                            spice_bulk_cntr_ku_reg_u     *reg_data)
{
    ...
    case SXD_BULK_CNTR_KEY_TYPE_PERFORMANCE_E:
        ...
        if (err) {
            err = -EFAULT;
            sxd_log_err("%s: failed to decode MOPCE reg_tlv", __func__);
            goto out;
        }
        break;
                              ...
                              void                                      *context)
{
    ...
    if (reg_tlv_node_p == NULL) {
        sxd_log_err("%s: Memory allocation for spice bulk cntr"
        ...
        *kbuf_alloc_size = ksize;
        ...
        if (ksize > file_db->tr_ctx.kbuf_alloc_size) {
            *kbuf = (char *)kzalloc(sizeof(char) * ksize, GFP_KERNEL);
            if (*kbuf == NULL) {
                err = -ENOMEM;
                sxd_log_err("%s: Memory reallocation for Spice bulk cntr"
                            " kbuf_p failed err : %d \n", __func__, err);
                goto out;
            }
            kfree(file_db->tr_ctx.kbuf);
...
static int __sx_spice_bulk_cntr_tr_set(spice_bulk_cntr_event_params_t *params)
{
    ...
    if (params->op == SPICE_BULK_COUNTER_CANCEL_OP_E) {
        ...
        spin_lock_bh(&file_db->tr_ctx_lock);
        err = bulk_cntr_db_mocs_session_acquire(dev, key_type);
        if (err) {
            sxd_log_err("%s: ,transaction for counter type: %d denied  - "
    ...
    if (err) {
        sxd_log_err("SPICE: sx_ACCESS_REG_MOCS failed with err: %d\n", err);
...
/*roll backs*/
mocs_set_rollback:
    bulk_cntr_db_mocs_session_release(params->dev, key_type);
    file_db->tr_ctx.tr_progress_state = SPICE_BULK_COUNTER_TR_IDLE_E;
...
static ssize_t __sx_spice_bulk_cntr_tr_ongoing(spice_bulk_cntr_event_params_t *params_p)
{
    ...
    size_t                        total_perf_cntr_groups =
        file_db->tr_ctx.perf_cntr_groups_max > 0 ? file_db->tr_ctx.perf_cntr_groups_max : 1;
    ...
    * process all reg_tlv's that are queued (copy data to kernel buffer)*/
    while (true) {
        if (!file_db->tr_ctx.is_skb_inproc) {
             ...
             * For performance counters we send one MOCS start to FW. FW starts reading the counters.
             * The FW reads each counter group the interval time that configured using MOPGCR.
             * FW reads one counter group and send MOPCE event to SDK, then reads the next counter group and so on.
             * The timeout for the session should reflect the number of counter groups.
             */
            if (wait_for_completion_interruptible_timeout(skb_queue_not_empty_p,
                                                          msecs_to_jiffies(WAIT_TIME_OUT * total_perf_cntr_groups)) ==
                0) {
                ret = -ETIMEDOUT;
                sxd_log_err("Failure - spice bulk counters timed out after %d [msec] with no completion skb_queue\n",
                            WAIT_TIME_OUT);
                goto tr_finalize;
            }
        }
        ...
        if (!skb_node) {
            ret = -EFAULT;
            sxd_log_err(" %s: invalid state - skb_queue is empty ", __func__);
            goto tr_finalize;
        }
        ...
        if (reg_tlv_node) {
            ...
            if (!reg_tlv_node->is_decoded) {
                ...
                if (ret) {
                    sxd_log_err("%s: failed to decode reg_tlv", __func__);
                    goto tr_finalize;
                }
...
#ifdef SPICE_BULK_CNTR_LOG_EN
                sxd_log_info(" %s: COPY DONE reg_tlv: [%p] of skb: [%p] \n",
...
#ifdef SPICE_BULK_CNTR_LOG_EN
                sxd_log_info(
                    "insufficient buffer space - reg_tlv: [%p] of skb: [%p] wasn't copied - waiting for next read cb\n",
    ...
    if (kbuf_size_cnt) {
        ...
        if (ret < 0) {
            sxd_log_err("%s: simple_read_from_buffer failure err: %zu\n", __func__, ret);
            goto tr_finalize;
        }
...
#ifdef SPICE_BULK_CNTR_LOG_EN
    sxd_log_info("%s: user buf: [%p] lbuf: %zu user_ppos: %llu kbuf [%p]\n",
                 __func__,
                 buf,
                 lbuf,
                 *user_ppos,
                 file_db->tr_ctx.kbuf);
    sxd_log_info("%s: kbuf_size_cnt: %zu returned size for the user is : %zu\n", __func__, kbuf_size_cnt, ret);
#endif
...
tr_finalize:
    spin_lock_bh(&file_db->tr_ctx_lock);
    __sx_spice_bulk_cntr_ctx_finalize(params_p->dev, key_type, file_db);
    spin_unlock_bh(&file_db->tr_ctx_lock);
    return ret;
}
...
static int __sx_spice_bulk_cntr_file_init(spice_bulk_cntr_event_params_t *params)
{
    ...
    if (err) {
        sxd_log_err("%s: failed to set trap_id : 0x%x", __func__, trap_id);
        goto out;
    }
...
static int __sx_spice_bulk_cntr_file_tr_start_on_idle(spice_bulk_cntr_event_params_t *params_p)
{
    ...
    sxd_log_err("%s: spice_bulk_cntr module is not initialized"
...
static int __sx_spice_bulk_cntr_file_deinit(spice_bulk_cntr_event_params_t *params)
{
    ...
    if (err) {
        sxd_log_err("%s: %s op: %d failed with err %d", __func__,
    ...
    case SPICE_BULK_COUNTER_TR_PENDING_MOCS_DONE:
        ...
        if (err) {
            sxd_log_warning("%s: Mocs transaction failure counter type: %d op: %d err : %d",
                            __func__,
                            key_type,
                            params->op,
                            err);
        ...
        /*wait for mocs done only if we haven't received it yet - ano not necessarily from the cancel op*/
        if (SPICE_BULK_COUNTER_TR_PENDING_MOCS_DONE == file_db->tr_ctx.tr_progress_state) {
            /*wait for completion*/
            if (wait_for_completion_interruptible_timeout(&cancel_completion, msecs_to_jiffies(WAIT_TIME_OUT)) == 0) {
                err = -ETIMEDOUT;
                sxd_log_err("%s: Failure - spice bulk counters timed out after %d [msec] with no completion \n",
                            __func__,
                            WAIT_TIME_OUT);
            }
        }
    ...
    case SPICE_BULK_COUNTER_TR_MOCS_DONE_E:
        ...
        __sx_spice_bulk_cntr_ctx_finalize(params->dev, params->key_type, file_db);
...
static int __sx_spice_bulk_cntr_file_tr_start(spice_bulk_cntr_event_params_t *params)
{
    ...
    if (err) {
        sxd_log_err("%s: Read operation not permitted for counter type : %d insufficient buffer size.\n"
                    , __func__, key_type);
        goto out;
    }
...
#ifdef SPICE_BULK_CNTR_LOG_EN
    sxd_log_info("%s: transaction status: %s", __func__, __sx_spice_bulk_cntr_transaction_e_str(req_status));
#endif
    ...
    case SPICE_BULK_COUNTER_TR_NEW_E:
        ...
        if (err) {
            sxd_log_err("%s: Mocs transaction failure counter type: %d  err : %d", __func__, key_type, err);
            goto out;
        }
    ...
    case SPICE_BULK_COUNTER_TR_ONGOING_E:
        ...
        if (size < 0) {
            err = size;
            sxd_log_err("%s: 1. Mocs transaction unable to execute for counter type: %d  err : %d",
    ...
    case SPICE_BULK_COUNTER_TR_BUSY_E:
        err = -EBUSY;
        sxd_log_err("%s: 2. Mocs transaction unable to execute for counter type: %d  err : %d",
                    __func__,
                    key_type,
                    err);
        break;
    ...
    case SPICE_BULK_COUNTER_TR_ERROR_E:
        err = -EINVAL;
        sxd_log_err("%s: 3. Mocs transaction unable to execute for counter type: %d  err : %d",
                    __func__,
                    key_type,
                    err);
        break;
...
#ifdef SPICE_BULK_CNTR_LOG_EN
    sxd_log_info(" %s: tr_progress_state: %d \n",
...
static int __sx_spice_bulk_cntr_file_trap_done(spice_bulk_cntr_event_params_t *params)
{
    ...
    if (hw_synd == SXD_TRAP_ID_MOCS_DONE) {
        ...
        sx_bulk_cntr_multi_emad_parse(node_p->skb->data, node_p->skb->len,
                                      __reg_tlv_enqueue, params->dev, (void *)node_p);
    }
    complete(file_db->tr_ctx.skb_queue_not_empty_p);

#ifdef SPICE_BULK_CNTR_LOG_EN
    sxd_log_info("%s: Trap completed synd: 0x%x skb : [%p] is queued \n", __func__,
                 hw_synd, node_p->skb);
#endif
                                    ...
                                    void            *context)
{
    ...
    if (cmd == CTRL_CMD_ADD_SYND) {
        err = sx_core_add_synd(ku.swid,
                               ...
                               ku.is_default,
                               &critireas,
                               "SPICE (bulk-completion)",
                               __sx_spice_bulk_cntr_handle_completion_event,
                               context,
                               CHECK_DUP_ENABLED_E,
                               dev,
                               &(ku.port_vlan_params),
    ...
    if (err) {
        sxd_log_err("%s: %s failed with err %d", __func__,
                    (cmd == CTRL_CMD_ADD_SYND) ? "sx_core_add_synd" : "sx_core_remove_synd", err);
        goto out;
    }

#ifdef  SPICE_BULK_CNTR_LOG_EN
    sxd_log_info("%s: successfully executed %s num: 0x%x file db ptr: [%p]", __func__,
...
int sx_spice_access_reg_bulk_counters_file_create(struct dentry *parent, dev_private_data_t *dev_data)
{
    ...
    for (; typeidx < SXD_BULK_CNTR_KEY_TYPE_LAST_E; typeidx++) {
        if (SPICE_SUPPORTED_BULK_CNTR_TRAP_MASK & (0x1 << typeidx)) {
            ...
            if (!file_params) {
                sxd_log_err(" %s: Failed to allocate private data for counter type: %d\n", __func__, typeidx);
                goto out;
            }
...
ssize_t sx_spice_bulk_counters_read_cb(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    if (NULL == buf) {
        err = -EINVAL;
        sxd_log_err("%s: invalid NULL argument", __func__);
        return err;
    }
    ...
    if (!sx_core_is_profile_set(file_params->dev) || spice_bulk_cntr_is_issu) {
        ...
        sxd_log_notice("sx_spice: bulk counter read ignore, pci profile is not yet set or issu in progress,"
    ...
    if (*ppos == 0) {
        ...
        if (err) {
            sxd_log_err("SPICE: sx_sysfs_asic_perf_cntr_counters_db_get failed with err: %d\n", err);
            goto out_unlock;
        }
        ...
        if (err) {
            sxd_log_err("SPICE: __sx_spice_bulk_cntr_perf_cntr_config_table failed with err: %d\n", err);
            goto out_unlock;
        }
                                                                 ...
                                                                 const struct group_id_counters_list* group_id_to_add)
{
    ...
    list_for_each_entry(iter, &group_id_to_add->mopct_list, group_id_list) {
        ...
        if (err) {
            sxd_log_err("SPICE: sx_ACCESS_REG_MOPCT failed with err: %ld\n", err);
            goto out;
        }
    }
                                                           ...
                                                           ssize_t       *group_id_num_left)
{
    ...
    if ((group_id_to_set_map == NULL) || (group_id_num_left == NULL)) {
        err = -EINVAL;
        sxd_log_err("SPICE: invalid parameters, err: %ld\n", err);
        goto out;
    }
    ...
    if (err) {
        sxd_log_err("SPICE: sx_sysfs_asic_perf_cntr_counters_db_get failed with err: %ld\n", err);
        goto out;
    }
    ...
    for (i = 0; i < max_groups_id_num; i++) {
        ...
        if (group_id_to_set_map[i]) {
            ...
            if ((alocated_entries + perf_cntr_groups_arr[i].mopct_in_group) < PERF_CNTR_MAX_TABLE_ENTRIES) {
                ...
                if (err) {
                    sxd_log_err(
                        "SPICE: __sx_spice_bulk_cntr_perf_cntr_add_group_to_table for group id [%d] failed with err: %ld\n",
                        i,
                        err);
                    goto out;
                }
    ...
    if (err) {
        sxd_log_err("SPICE: __sx_spice_bulk_cntr_perf_cntr_set_table_last_entry failed with err: %ld\n", err);
        goto out;
    }
...
ssize_t sx_spice_bulk_cntr_event(spice_bulk_cntr_event_params_t *params_p)
{
    ...
    if (!file_db) {
        sxd_log_err("%s: db is not initialized invalid device_id : %d", __func__, dev->device_id);
        goto out;
    }
...
#ifdef SPICE_BULK_CNTR_LOG_EN
    sxd_log_info("%s: Invoke event: [%s]  on [%s] state on device_id : %d counter type: %d",
                 __func__,
