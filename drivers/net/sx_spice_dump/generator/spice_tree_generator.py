...
def spice_tree_index_enum_gen():
    ...
    # enum index should be unique in index_type_e
    enum_index_set = set()
    for idx in INDEXES_DB['enum_idxs']:
        idx_name = idx[0]
        enum_index_set.add(idx_name.upper())
    for enum_idx in enum_index_set:
        code += pattern % (INDENT, enum_idx)
...
"""
    ...
    user_view_index_name = index_name
    index_type_name = index_name.upper()
    if reg != "":
        index_name = reg + '_' + index_name
    ...
    for idx1, idx2 in INDEX_SET_DB[protocol]:
        if index_name == idx2:
            ...
            code = index_set_pattern % (nest0, index_name, index_name, dir_num, index_name,
                                        ...
                                        nest2, user_view_index_name, mod_sign, mod_sign,
                                        nest3, prefix, index_name,
                                        nest3, prefix, index_name,
                                        nest2, index_name, parent_str,
                                        nest2, index_name,
                                        nest3, nest2,
                                        nest2, index_type_name, prefix, index_name)

            return code, True

    code = pattern % (nest0, index_name, index_name, dir_num, index_name,
                      nest1, user_view_index_name, mod_sign, mod_sign,
                      nest2, prefix, index_name,
                      nest2, prefix, index_name,
                      nest1, index_name, parent_str,
                      nest1, index_name,
                      nest2, nest1,
                      nest1, index_type_name, prefix, index_name)
...
%s
%ssxd_log_info(\"sx_spice_tree_%s_%s_create\\n\");
...
{
%ssxd_log_info(\"sx_spice_tree_init\\n\");
...
{
%ssxd_log_info(\"In sx_spice_tree_deinit function \\n\");
%sdebugfs_remove_recursive(root_p);
}\n
"""
...
def spice_tree_local_idxs_list_populate(tree, local_idxs_list):
    ...
    for idx in tree.iter('enum_index'):
        reg_field = idx.attrib.get('reg')
        if reg_field is not None and reg_field != '':
            local_idxs_list.add(idx.attrib['reg'] + '_' + idx.attrib['name'])
        else:
            local_idxs_list.add(idx.attrib['name'])
