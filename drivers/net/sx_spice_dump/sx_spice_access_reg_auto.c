...
int sx_spice_access_reg_mgir_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_mgir_reg * reg_data)
{
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "cfiv: 0x%x\n", reg_data->hw_info.cfiv);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "chassis_module_id: 0x%x\n", reg_data->hw_info.chassis_module_id);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "chassis_slot_index: 0x%x\n", reg_data->hw_info.chassis_slot_index);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
