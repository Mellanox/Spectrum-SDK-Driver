...
int sx_spice_access_reg_ptys_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_ptys_reg * reg_data)
{
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "ext_ib_proto_cap: 0x%x\n", reg_data->ext_ib_proto_cap);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "ext_ib_proto_admin: 0x%x\n", reg_data->ext_ib_proto_admin);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "ext_ib_proto_oper: 0x%x\n", reg_data->ext_ib_proto_oper);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
...
int sx_spice_access_reg_pemi_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_pemi_reg * reg_data)
{
    ...
    case SXD_PEMI_PAGE_SELECT_TEMPERATURE_SAMPLES_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "lasers_temp: 0x%x\n", reg_data->page_data.pemi_temperature_samples.lasers_temp);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
