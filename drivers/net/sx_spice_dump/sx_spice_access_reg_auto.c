...
static LIST_HEAD(__mvcr_list_head);
...
int sx_spice_prepare_access_reg(dev_private_data_t *priv_data)
{
    ...
    if (down_timeout(&priv_data->reg_access.reg_sem, msecs_to_jiffies(0)) == -ETIME) {
        sxd_log_debug("SPICE: FW is busy, can't send SPICE query");
        err = -EBUSY;
    }
...
int release_mtcap(struct inode *inodep, struct file *filp)
{
    const mtcap_data_t *data = (mtcap_data_t*)filp->f_inode->i_private;

    if (filp->private_data == (void *)0xdeadbeaf) { /* The application acquired the semaphore following read operation */
        sx_spice_kbuffer_free(data->dev_data);
    }

    return 0;
}

int release_mvcap(struct inode *inodep, struct file *filp)
{
...
int release_mtmp(struct inode *inodep, struct file *filp)
{
...
int release_mvcr(struct inode *inodep, struct file *filp)
{
    const mvcr_data_t *data = (mvcr_data_t*)filp->f_inode->i_private;
...
int release_mgir(struct inode *inodep, struct file *filp)
{
...
int release_msgi(struct inode *inodep, struct file *filp)
{
...
int release_mdir(struct inode *inodep, struct file *filp)
{
...
int release_spad(struct inode *inodep, struct file *filp)
{
...
int release_spzr(struct inode *inodep, struct file *filp)
{
...
int release_ptys(struct inode *inodep, struct file *filp)
{
...
int release_ppcnt(struct inode *inodep, struct file *filp)
{
...
int release_pmaos(struct inode *inodep, struct file *filp)
{
    const pmaos_data_t *data = (pmaos_data_t*)filp->f_inode->i_private;
...
int release_sltp(struct inode *inodep, struct file *filp)
{
...
int release_pddr(struct inode *inodep, struct file *filp)
{
...
int release_pnlpnr(struct inode *inodep, struct file *filp)
{
    const pnlpnr_data_t *data = (pnlpnr_data_t*)filp->f_inode->i_private;
...
int release_pmdr(struct inode *inodep, struct file *filp)
{
...
int release_pphcr(struct inode *inodep, struct file *filp)
{
...
int release_sllm(struct inode *inodep, struct file *filp)
{
...
int release_pemi(struct inode *inodep, struct file *filp)
{
    const pemi_data_t *data = (pemi_data_t*)filp->f_inode->i_private;
...
int sx_spice_access_reg_mtmp_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_mtmp_reg * reg_data)
{
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "temperature_critical_threshold_hi: 0x%x\n", reg_data->temperature_critical_threshold_hi);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "temperature_critical_threshold_low: 0x%x\n", reg_data->temperature_critical_threshold_low);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
...
int sx_spice_access_reg_mvcr_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_mvcr_reg * reg_data)
{
    int buffer_size = 0, snp_res = 0;
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "voltage_sensor_value: 0x%x\n", reg_data->voltage_sensor_value);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "current_sensor_value_msb: 0x%x\n", reg_data->current_sensor_value_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "current_sensor_value: 0x%x\n", reg_data->current_sensor_value);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "sensor_name: 0x%llx\n", reg_data->sensor_name);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
...
int sx_spice_access_reg_mgir_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_mgir_reg * reg_data)
{
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "ib_mad_gen: 0x%x\n", reg_data->hw_info.ib_mad_gen);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "development: 0x%x\n", reg_data->hw_info.development);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "chip_type: 0x%x\n", reg_data->hw_info.chip_type);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "ga: 0x%x\n", reg_data->hw_info.ga);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "device_ticks_per_msec: 0x%x\n", reg_data->hw_info.device_ticks_per_msec);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "pds: 0x%x\n", reg_data->fw_info.pds);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "issu_able: 0x%x\n", reg_data->fw_info.issu_able);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "life_cycle_msb: 0x%x\n", reg_data->fw_info.life_cycle_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
...
int sx_spice_access_reg_spzr_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_spzr_reg * reg_data)
{
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "v_key_h: 0x%x\n", reg_data->v_key_h);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "v_key_l: 0x%x\n", reg_data->v_key_l);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "infrastructure_ip_type: 0x%x\n", reg_data->infrastructure_ip_type);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "infrastructure_ip: 0x%x\n", reg_data->infrastructure_ip);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
...
int sx_spice_access_reg_ptys_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_ptys_reg * reg_data)
{
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "dis_optic_neg_admin: 0x%x\n", reg_data->dis_optic_neg_admin);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    ...
    switch (reg_data->proto_mask) {
    case SXD_PTYS_PROTO_MASK_INFINIBAND_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ext_proto_ib: 0x%x\n", reg_data->ext_ib_proto_cap.ptys_ext_proto_ib.ext_proto_ib);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PTYS_PROTO_MASK_NVLINK_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ext_proto_nvlink: 0x%x\n", reg_data->ext_ib_proto_cap.ptys_ext_proto_nvlink.ext_proto_nvlink);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    switch (reg_data->proto_mask) {
    case SXD_PTYS_PROTO_MASK_INFINIBAND_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ext_proto_ib: 0x%x\n", reg_data->ext_ib_proto_admin.ptys_ext_proto_ib.ext_proto_ib);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PTYS_PROTO_MASK_NVLINK_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ext_proto_nvlink: 0x%x\n", reg_data->ext_ib_proto_admin.ptys_ext_proto_nvlink.ext_proto_nvlink);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    switch (reg_data->proto_mask) {
    case SXD_PTYS_PROTO_MASK_INFINIBAND_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ext_proto_ib: 0x%x\n", reg_data->ext_ib_proto_oper.ptys_ext_proto_ib.ext_proto_ib);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PTYS_PROTO_MASK_NVLINK_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ext_proto_nvlink: 0x%x\n", reg_data->ext_ib_proto_oper.ptys_ext_proto_nvlink.ext_proto_nvlink);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
...
int sx_spice_access_reg_ppcnt_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_ppcnt_reg * reg_data)
{
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "counters_cap: 0x%x\n", reg_data->counters_cap);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "plane_ind: 0x%x\n", reg_data->plane_ind);
    ...
    case SXD_PPCNT_GRP_ETHERNET_DISCARD_COUNTERS_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "egress_link_down_high: 0x%x\n", reg_data->counter_set.eth_discard_cntrs_grp.egress_link_down_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "egress_link_down_low: 0x%x\n", reg_data->counter_set.eth_discard_cntrs_grp.egress_link_down_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    ...
    case SXD_PPCNT_GRP_PHYSICAL_LAYER_COUNTERS_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rs_fec_total_blocks_high: 0x%x\n", reg_data->counter_set.phys_layer_cntrs.rs_fec_total_blocks_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rs_fec_total_blocks_low: 0x%x\n", reg_data->counter_set.phys_layer_cntrs.rs_fec_total_blocks_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PPCNT_GRP_PHYSICAL_LAYER_STATISTICAL_COUNTERS_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "fc_zero_hist: 0x%x\n", reg_data->counter_set.phys_layer_stat_cntrs.fc_zero_hist);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "estimated_effective_ber_magnitude: 0x%x\n", reg_data->counter_set.phys_layer_stat_cntrs.estimated_effective_ber_magnitude);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "estimated_effective_ber_coef_float: 0x%x\n", reg_data->counter_set.phys_layer_stat_cntrs.estimated_effective_ber_coef_float);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "estimated_effective_ber_coef: 0x%x\n", reg_data->counter_set.phys_layer_stat_cntrs.estimated_effective_ber_coef);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    ...
    case SXD_PPCNT_GRP_INFINIBAND_PKTS_COUNTERS_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_llu_rx_icrc_error_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packets_counter.port_llu_rx_icrc_error_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_llu_rx_icrc_error_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packets_counter.port_llu_rx_icrc_error_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_tx_parity_error_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packets_counter.port_tx_parity_error_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_tx_parity_error_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packets_counter.port_tx_parity_error_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PPCNT_GRP_INFINIBAND_GENERAL_COUNTERS_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "general_receive_discard_external_contain_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_general_counter.general_receive_discard_external_contain_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "general_receive_discard_external_contain_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_general_counter.general_receive_discard_external_contain_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_receive_poison_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_general_counter.port_receive_poison_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_receive_poison_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_general_counter.port_receive_poison_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PPCNT_GRP_L1_GENERAL_COUNTERS_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "l1_total_entries_high: 0x%x\n", reg_data->counter_set.ppcnt_l1_general_counters.l1_total_entries_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "l1_total_entries_low: 0x%x\n", reg_data->counter_set.ppcnt_l1_general_counters.l1_total_entries_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "l1_total_local_exits_high: 0x%x\n", reg_data->counter_set.ppcnt_l1_general_counters.l1_total_local_exits_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "l1_total_local_exits_low: 0x%x\n", reg_data->counter_set.ppcnt_l1_general_counters.l1_total_local_exits_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "l1_total_exits_high: 0x%x\n", reg_data->counter_set.ppcnt_l1_general_counters.l1_total_exits_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "l1_total_exits_low: 0x%x\n", reg_data->counter_set.ppcnt_l1_general_counters.l1_total_exits_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "relative_high_speed_steady_state_time: 0x%x\n", reg_data->counter_set.ppcnt_l1_general_counters.relative_high_speed_steady_state_time);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "relative_l1_steady_state_time: 0x%x\n", reg_data->counter_set.ppcnt_l1_general_counters.relative_l1_steady_state_time);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "relative_other_state_time: 0x%x\n", reg_data->counter_set.ppcnt_l1_general_counters.relative_other_state_time);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PPCNT_GRP_INFINIBAND_PACKET_SIZE_HISTOGRAM_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_9_16_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_9_16_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_9_16_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_9_16_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_17_32_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_17_32_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_17_32_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_17_32_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_65_127_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_65_127_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_65_127_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_65_127_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_128_255_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_128_255_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_128_255_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_128_255_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_256_511_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_256_511_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_256_511_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_256_511_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_512_1023_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_512_1023_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_512_1023_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_512_1023_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_1024_1518_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_1024_1518_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_1024_1518_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_1024_1518_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_1519_2047_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_1519_2047_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_1519_2047_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_1519_2047_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_2048_4095_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_2048_4095_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_2048_4095_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_2048_4095_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_4096_8191_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_4096_8191_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_4096_8191_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_4096_8191_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_mtu_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_mtu_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_r_mtu_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_r_mtu_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_9_16_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_9_16_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_9_16_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_9_16_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_17_32_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_17_32_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_17_32_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_17_32_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_65_127_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_65_127_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_65_127_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_65_127_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_128_255_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_128_255_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_128_255_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_128_255_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_256_511_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_256_511_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_256_511_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_256_511_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_512_1023_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_512_1023_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_512_1023_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_512_1023_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_1024_1518_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_1024_1518_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_1024_1518_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_1024_1518_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_1519_2047_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_1519_2047_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_1519_2047_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_1519_2047_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_2048_4095_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_2048_4095_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_2048_4095_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_2048_4095_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_4096_8191_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_4096_8191_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_4096_8191_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_4096_8191_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_mtu_high: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_mtu_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cnt_t_mtu_low: 0x%x\n", reg_data->counter_set.ppcnt_infiniband_packet_histograms.cnt_t_mtu_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
...
int sx_spice_access_reg_pmaos_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_pmaos_reg * reg_data)
{
    ...
    snp_res = snprintf(buffer, buffer_length - buffer_size, "rst: 0x%x\n", reg_data->rst);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "slot_index: 0x%x\n", reg_data->slot_index);
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "admin_status: 0x%x\n", reg_data->admin_status);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "oper_status: 0x%x\n", reg_data->oper_status);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "secondary: 0x%x\n", reg_data->secondary);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "rev_incompatible: 0x%x\n", reg_data->rev_incompatible);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "operational_notification: 0x%x\n", reg_data->operational_notification);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "error_type: 0x%x\n", reg_data->error_type);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "e: 0x%x\n", reg_data->e);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
...
int sx_spice_access_reg_sltp_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_sltp_reg * reg_data)
{
    ...
    if ((reg_data->port_type == SXD_SLTP_PORT_TYPE_NETWORK_PORT_E || reg_data->port_type == SXD_SLTP_PORT_TYPE_INTERNAL_IC_LR_PORT_E) && (reg_data->version == SXD_SLTP_VERSION_PROD_7NM_E) && (reg_data->conf_mod == SXD_SLTP_CONF_MOD_TX_LINEAR_E)) {
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ph_pol_admin: 0x%x\n", reg_data->page_data.sltp_7nm.ph_pol_admin);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    }

    if ((reg_data->port_type == SXD_SLTP_PORT_TYPE_NETWORK_PORT_E || reg_data->port_type == SXD_SLTP_PORT_TYPE_INTERNAL_IC_LR_PORT_E) && (reg_data->version == SXD_SLTP_VERSION_PROD_7NM_E) && (reg_data->conf_mod == SXD_SLTP_CONF_MOD_TX_LUT_E)) {
    ...
    if ((reg_data->port_type == SXD_SLTP_PORT_TYPE_NETWORK_PORT_E || reg_data->port_type == SXD_SLTP_PORT_TYPE_INTERNAL_IC_LR_PORT_E) && (reg_data->version == SXD_SLTP_VERSION_PROD_5NM_E)) {
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ph_pol_admin: 0x%x\n", reg_data->page_data.sltp_5nm.ph_pol_admin);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "drv_amp: 0x%x\n", reg_data->page_data.sltp_5nm.drv_amp);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ob_bad_stat: 0x%x\n", reg_data->page_data.sltp_5nm.ob_bad_stat);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tap3: 0x%x\n", reg_data->page_data.sltp_5nm.tap3);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tap2: 0x%x\n", reg_data->page_data.sltp_5nm.tap2);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tap1: 0x%x\n", reg_data->page_data.sltp_5nm.tap1);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tap0: 0x%x\n", reg_data->page_data.sltp_5nm.tap0);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tap7: 0x%x\n", reg_data->page_data.sltp_5nm.tap7);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tap6: 0x%x\n", reg_data->page_data.sltp_5nm.tap6);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tap5: 0x%x\n", reg_data->page_data.sltp_5nm.tap5);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tap4: 0x%x\n", reg_data->page_data.sltp_5nm.tap4);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tap11: 0x%x\n", reg_data->page_data.sltp_5nm.tap11);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tap10: 0x%x\n", reg_data->page_data.sltp_5nm.tap10);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tap9: 0x%x\n", reg_data->page_data.sltp_5nm.tap9);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tap8: 0x%x\n", reg_data->page_data.sltp_5nm.tap8);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    }
...
int sx_spice_access_reg_pddr_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_pddr_reg * reg_data)
{
    ...
    case SXD_PDDR_PAGE_SELECT_PHY_INFO_PAGE_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "phy_manager_disable_mask: 0x%x\n", reg_data->page_data.pddr_phy_info_page.phy_manager_disable_mask);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    ...
    case SXD_PDDR_PAGE_SELECT_MODULE_INFO_PAGE_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cable_attenuation_53g: 0x%x\n", reg_data->page_data.pddr_module_info.cable_attenuation_53g);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cdr_vendor: 0x%x\n", reg_data->page_data.pddr_module_info.cdr_vendor);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    ...
    case SXD_PDDR_PAGE_SELECT_PORT_EVENTS_PAGE_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "merger_tx_eb_underrun: 0x%x\n", reg_data->page_data.pddr_port_events_page_layout.merger_tx_eb_underrun);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "bubble_in_pkt: 0x%x\n", reg_data->page_data.pddr_port_events_page_layout.bubble_in_pkt);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    ...
    case SXD_PDDR_PAGE_SELECT_LINK_DOWN_INFO_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "mean_est_eff_ber_coef: 0x%x\n", reg_data->page_data.pddr_link_down_info_page.mean_est_eff_ber_coef);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "mean_est_eff_ber_magnitude: 0x%x\n", reg_data->page_data.pddr_link_down_info_page.mean_est_eff_ber_magnitude);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "min_est_eff_ber_coef: 0x%x\n", reg_data->page_data.pddr_link_down_info_page.min_est_eff_ber_coef);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "min_est_eff_ber_magnitude: 0x%x\n", reg_data->page_data.pddr_link_down_info_page.min_est_eff_ber_magnitude);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "max_est_eff_ber_coef: 0x%x\n", reg_data->page_data.pddr_link_down_info_page.max_est_eff_ber_coef);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "max_est_eff_ber_magnitude: 0x%x\n", reg_data->page_data.pddr_link_down_info_page.max_est_eff_ber_magnitude);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "num_of_est_eff_ber_warnings: 0x%x\n", reg_data->page_data.pddr_link_down_info_page.num_of_est_eff_ber_warnings);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "num_of_est_eff_ber_alarms: 0x%x\n", reg_data->page_data.pddr_link_down_info_page.num_of_est_eff_ber_alarms);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PDDR_PAGE_SELECT_LINK_PARTNER_INFO_PAGE_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "bubble_agreement: 0x%x\n", reg_data->page_data.pddr_link_partner_info.bubble_agreement);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    ...
    case SXD_PDDR_PAGE_SELECT_PHY_DEBUG_PAGE_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pport_second: 0x%x\n", reg_data->page_data.pddr_phy_debug_page.pport_second);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "second_port_valid: 0x%x\n", reg_data->page_data.pddr_phy_debug_page.second_port_valid);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "trigger_cond_port: 0x%x\n", reg_data->page_data.pddr_phy_debug_page.trigger_cond_port);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "revision: 0x%x\n", reg_data->page_data.pddr_phy_debug_page.revision);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "page_size: 0x%x\n", reg_data->page_data.pddr_phy_debug_page.page_size);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "page_index: 0x%x\n", reg_data->page_data.pddr_phy_debug_page.page_index);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        for (i = 0; i < SXD_PDDR_PDDR_PHY_DEBUG_PAGE_TRACER_DATA_ENTRY_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "tracer_data_entry[%d]: 0x%x\n", i, reg_data->page_data.pddr_phy_debug_page.tracer_data_entry[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
...
int sx_spice_access_reg_pnlpnr_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_pnlpnr_reg * reg_data)
{
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "nlpn_v: 0x%x\n", reg_data->nlpn_v);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "link_up: 0x%x\n", reg_data->link_up);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "nlpn_rev: 0x%x\n", reg_data->nlpn_rev);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "page_req: 0x%llx\n", reg_data->page_req);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "page_valid: 0x%llx\n", reg_data->page_valid);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    for (i = 0; i < SXD_PNLPNR_PAGES_NUM; i++) {
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pages[%d]: 0x%x\n", i, reg_data->pages[i]);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    }
...
int sx_spice_access_reg_pmdr_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_pmdr_reg * reg_data)
{
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "tile_pport_msb: 0x%x\n", reg_data->tile_pport_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lbp_query_msb: 0x%x\n", reg_data->lbp_query_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "pll_cnt_rx: 0x%x\n", reg_data->pll_cnt_rx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "pll_cnt_tx: 0x%x\n", reg_data->pll_cnt_tx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane7_ph_tx_msb: 0x%x\n", reg_data->lane7_ph_tx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane6_ph_tx_msb: 0x%x\n", reg_data->lane6_ph_tx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane5_ph_tx_msb: 0x%x\n", reg_data->lane5_ph_tx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane4_ph_tx_msb: 0x%x\n", reg_data->lane4_ph_tx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane3_ph_tx_msb: 0x%x\n", reg_data->lane3_ph_tx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane2_ph_tx_msb: 0x%x\n", reg_data->lane2_ph_tx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane1_ph_tx_msb: 0x%x\n", reg_data->lane1_ph_tx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane0_ph_tx_msb: 0x%x\n", reg_data->lane0_ph_tx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane7_ph_rx_msb: 0x%x\n", reg_data->lane7_ph_rx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane6_ph_rx_msb: 0x%x\n", reg_data->lane6_ph_rx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane5_ph_rx_msb: 0x%x\n", reg_data->lane5_ph_rx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane4_ph_rx_msb: 0x%x\n", reg_data->lane4_ph_rx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane3_ph_rx_msb: 0x%x\n", reg_data->lane3_ph_rx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane2_ph_rx_msb: 0x%x\n", reg_data->lane2_ph_rx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane1_ph_rx_msb: 0x%x\n", reg_data->lane1_ph_rx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane0_ph_rx_msb: 0x%x\n", reg_data->lane0_ph_rx_msb);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "ph_cluster_tx_cnt: 0x%x\n", reg_data->ph_cluster_tx_cnt);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "ph_cluster_rx_cnt: 0x%x\n", reg_data->ph_cluster_rx_cnt);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane7_cluster_tx: 0x%x\n", reg_data->lane7_cluster_tx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane6_cluster_tx: 0x%x\n", reg_data->lane6_cluster_tx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane5_cluster_tx: 0x%x\n", reg_data->lane5_cluster_tx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane4_cluster_tx: 0x%x\n", reg_data->lane4_cluster_tx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane3_cluster_tx: 0x%x\n", reg_data->lane3_cluster_tx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane2_cluster_tx: 0x%x\n", reg_data->lane2_cluster_tx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane1_cluster_tx: 0x%x\n", reg_data->lane1_cluster_tx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane0_cluster_tx: 0x%x\n", reg_data->lane0_cluster_tx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "pll_index_rx: 0x%x\n", reg_data->pll_index_rx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane7_cluster_rx: 0x%x\n", reg_data->lane7_cluster_rx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane6_cluster_rx: 0x%x\n", reg_data->lane6_cluster_rx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane5_cluster_rx: 0x%x\n", reg_data->lane5_cluster_rx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane4_cluster_rx: 0x%x\n", reg_data->lane4_cluster_rx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane3_cluster_rx: 0x%x\n", reg_data->lane3_cluster_rx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane2_cluster_rx: 0x%x\n", reg_data->lane2_cluster_rx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane1_cluster_rx: 0x%x\n", reg_data->lane1_cluster_rx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane0_cluster_rx: 0x%x\n", reg_data->lane0_cluster_rx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
...
int sx_spice_access_reg_pemi_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_pemi_reg * reg_data)
{
    ...
    snp_res = snprintf(buffer, buffer_length - buffer_size, "clr_statistic: 0x%x\n", reg_data->clr_statistic);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    SX_PORT_BUILD_PHY_ID_FROM_LSB_MSB(local_port, reg_data->local_port, reg_data->lp_msb);
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "group_cap_mask: 0x%x\n", reg_data->group_cap_mask);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "page_select: 0x%x\n", reg_data->page_select);
    ...
    case SXD_PEMI_PAGE_SELECT_MODULE_STATUS_SAMPLES_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "voltage: 0x%x\n", reg_data->page_data.pemi_module_status_samples.voltage);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rx_power_lane0: 0x%x\n", reg_data->page_data.pemi_module_status_samples.rx_power_lane0);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rx_power_lane1: 0x%x\n", reg_data->page_data.pemi_module_status_samples.rx_power_lane1);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rx_power_lane2: 0x%x\n", reg_data->page_data.pemi_module_status_samples.rx_power_lane2);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rx_power_lane3: 0x%x\n", reg_data->page_data.pemi_module_status_samples.rx_power_lane3);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rx_power_lane4: 0x%x\n", reg_data->page_data.pemi_module_status_samples.rx_power_lane4);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rx_power_lane5: 0x%x\n", reg_data->page_data.pemi_module_status_samples.rx_power_lane5);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rx_power_lane6: 0x%x\n", reg_data->page_data.pemi_module_status_samples.rx_power_lane6);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rx_power_lane7: 0x%x\n", reg_data->page_data.pemi_module_status_samples.rx_power_lane7);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_power_lane0: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_power_lane0);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_power_lane1: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_power_lane1);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_power_lane2: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_power_lane2);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_power_lane3: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_power_lane3);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_power_lane4: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_power_lane4);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_power_lane5: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_power_lane5);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_power_lane6: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_power_lane6);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_power_lane7: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_power_lane7);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_bias_lane0: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_bias_lane0);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_bias_lane1: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_bias_lane1);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_bias_lane2: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_bias_lane2);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_bias_lane3: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_bias_lane3);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_bias_lane4: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_bias_lane4);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_bias_lane5: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_bias_lane5);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_bias_lane6: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_bias_lane6);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_bias_lane7: 0x%x\n", reg_data->page_data.pemi_module_status_samples.tx_bias_lane7);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "module_st: 0x%x\n", reg_data->page_data.pemi_module_status_samples.module_st);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "dp_st_lane0: 0x%x\n", reg_data->page_data.pemi_module_status_samples.dp_st_lane0);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "dp_st_lane1: 0x%x\n", reg_data->page_data.pemi_module_status_samples.dp_st_lane1);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "dp_st_lane2: 0x%x\n", reg_data->page_data.pemi_module_status_samples.dp_st_lane2);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "dp_st_lane3: 0x%x\n", reg_data->page_data.pemi_module_status_samples.dp_st_lane3);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "dp_st_lane4: 0x%x\n", reg_data->page_data.pemi_module_status_samples.dp_st_lane4);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "dp_st_lane5: 0x%x\n", reg_data->page_data.pemi_module_status_samples.dp_st_lane5);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "dp_st_lane6: 0x%x\n", reg_data->page_data.pemi_module_status_samples.dp_st_lane6);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "dp_st_lane7: 0x%x\n", reg_data->page_data.pemi_module_status_samples.dp_st_lane7);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PEMI_PAGE_SELECT_SNR_SAMPLES_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_media_lane0: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_media_lane0);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_media_lane1: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_media_lane1);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_media_lane2: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_media_lane2);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_media_lane3: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_media_lane3);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_media_lane4: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_media_lane4);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_media_lane5: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_media_lane5);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_media_lane6: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_media_lane6);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_media_lane7: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_media_lane7);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_host_lane0: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_host_lane0);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_host_lane1: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_host_lane1);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_host_lane2: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_host_lane2);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_host_lane3: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_host_lane3);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_host_lane4: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_host_lane4);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_host_lane5: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_host_lane5);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_host_lane6: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_host_lane6);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_host_lane7: 0x%x\n", reg_data->page_data.pemi_snr_samples.snr_host_lane7);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PEMI_PAGE_SELECT_LASER_MONITORS_SAMPLES_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "laser_age: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.laser_age);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tec_current: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.tec_current);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "laser_frequency_error_lane0: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.laser_frequency_error_lane0);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "laser_frequency_error_lane1: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.laser_frequency_error_lane1);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "laser_frequency_error_lane2: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.laser_frequency_error_lane2);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "laser_frequency_error_lane3: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.laser_frequency_error_lane3);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "laser_frequency_error_lane4: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.laser_frequency_error_lane4);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "laser_frequency_error_lane5: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.laser_frequency_error_lane5);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "laser_frequency_error_lane6: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.laser_frequency_error_lane6);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "laser_frequency_error_lane7: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.laser_frequency_error_lane7);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cooled_laser_temperature_lane0: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.cooled_laser_temperature_lane0);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cooled_laser_temperature_lane1: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.cooled_laser_temperature_lane1);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cooled_laser_temperature_lane2: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.cooled_laser_temperature_lane2);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cooled_laser_temperature_lane3: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.cooled_laser_temperature_lane3);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cooled_laser_temperature_lane4: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.cooled_laser_temperature_lane4);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cooled_laser_temperature_lane5: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.cooled_laser_temperature_lane5);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cooled_laser_temperature_lane6: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.cooled_laser_temperature_lane6);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cooled_laser_temperature_lane7: 0x%x\n", reg_data->page_data.pemi_laser_monitors_samples.cooled_laser_temperature_lane7);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PEMI_PAGE_SELECT_PAM4_LEVEL_TRANSITION_SAMPLES_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_media_lane0: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_media_lane0);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_media_lane1: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_media_lane1);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_media_lane2: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_media_lane2);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_media_lane3: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_media_lane3);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_media_lane4: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_media_lane4);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_media_lane5: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_media_lane5);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_media_lane6: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_media_lane6);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_media_lane7: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_media_lane7);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_host_lane0: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_host_lane0);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_host_lane1: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_host_lane1);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_host_lane2: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_host_lane2);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_host_lane3: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_host_lane3);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_host_lane4: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_host_lane4);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_host_lane5: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_host_lane5);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_host_lane6: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_host_lane6);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_host_lane7: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_samples.pam4_level_transition_host_lane7);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PEMI_PAGE_SELECT_PRE_FEC_BER_SAMPLES_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_min_media: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_samples.pre_fec_ber_min_media);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_min_host: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_samples.pre_fec_ber_min_host);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_max_media: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_samples.pre_fec_ber_max_media);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_max_host: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_samples.pre_fec_ber_max_host);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_avg_media: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_samples.pre_fec_ber_avg_media);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_avg_host: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_samples.pre_fec_ber_avg_host);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_val_media: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_samples.pre_fec_ber_val_media);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_val_host: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_samples.pre_fec_ber_val_host);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PEMI_PAGE_SELECT_FERC_SAMPLES_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ferc_min_media: 0x%x\n", reg_data->page_data.pemi_ferc_samples.ferc_min_media);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ferc_min_host: 0x%x\n", reg_data->page_data.pemi_ferc_samples.ferc_min_host);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ferc_max_media: 0x%x\n", reg_data->page_data.pemi_ferc_samples.ferc_max_media);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ferc_max_host: 0x%x\n", reg_data->page_data.pemi_ferc_samples.ferc_max_host);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ferc_avg_media: 0x%x\n", reg_data->page_data.pemi_ferc_samples.ferc_avg_media);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ferc_avg_host: 0x%x\n", reg_data->page_data.pemi_ferc_samples.ferc_avg_host);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ferc_val_media: 0x%x\n", reg_data->page_data.pemi_ferc_samples.ferc_val_media);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ferc_val_host: 0x%x\n", reg_data->page_data.pemi_ferc_samples.ferc_val_host);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PEMI_PAGE_SELECT_TEMPERATURE_SAMPLES_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "module_temp: 0x%x\n", reg_data->page_data.pemi_temperature_samples.module_temp);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "uc_temp: 0x%x\n", reg_data->page_data.pemi_temperature_samples.uc_temp);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "lasers0_temp: 0x%x\n", reg_data->page_data.pemi_temperature_samples.lasers0_temp);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "lasers1_temp: 0x%x\n", reg_data->page_data.pemi_temperature_samples.lasers1_temp);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cdr0_temp: 0x%x\n", reg_data->page_data.pemi_temperature_samples.cdr0_temp);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cdr1_temp: 0x%x\n", reg_data->page_data.pemi_temperature_samples.cdr1_temp);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cdr2_temp: 0x%x\n", reg_data->page_data.pemi_temperature_samples.cdr2_temp);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cdr3_temp: 0x%x\n", reg_data->page_data.pemi_temperature_samples.cdr3_temp);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PEMI_PAGE_SELECT_MODULE_STATUS_PROPERTIES_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "module_status_cap: 0x%x\n", reg_data->page_data.pemi_module_status_properties.module_status_cap);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "voltage_high_alarm: 0x%x\n", reg_data->page_data.pemi_module_status_properties.voltage_high_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "voltage_low_alarm: 0x%x\n", reg_data->page_data.pemi_module_status_properties.voltage_low_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "voltage_high_warning: 0x%x\n", reg_data->page_data.pemi_module_status_properties.voltage_high_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "voltage_low_warning: 0x%x\n", reg_data->page_data.pemi_module_status_properties.voltage_low_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rx_power_high_alarm: 0x%x\n", reg_data->page_data.pemi_module_status_properties.rx_power_high_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rx_power_low_alarm: 0x%x\n", reg_data->page_data.pemi_module_status_properties.rx_power_low_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rx_power_high_warning: 0x%x\n", reg_data->page_data.pemi_module_status_properties.rx_power_high_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rx_power_low_warning: 0x%x\n", reg_data->page_data.pemi_module_status_properties.rx_power_low_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_power_high_alarm: 0x%x\n", reg_data->page_data.pemi_module_status_properties.tx_power_high_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_power_low_alarm: 0x%x\n", reg_data->page_data.pemi_module_status_properties.tx_power_low_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_power_high_warning: 0x%x\n", reg_data->page_data.pemi_module_status_properties.tx_power_high_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_power_low_warning: 0x%x\n", reg_data->page_data.pemi_module_status_properties.tx_power_low_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_bias_high_alarm: 0x%x\n", reg_data->page_data.pemi_module_status_properties.tx_bias_high_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_bias_low_alarm: 0x%x\n", reg_data->page_data.pemi_module_status_properties.tx_bias_low_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_bias_high_warning: 0x%x\n", reg_data->page_data.pemi_module_status_properties.tx_bias_high_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_bias_low_warning: 0x%x\n", reg_data->page_data.pemi_module_status_properties.tx_bias_low_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PEMI_PAGE_SELECT_SNR_PROPERTIES_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_cap: 0x%x\n", reg_data->page_data.pemi_snr_properties.snr_cap);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_media_low_alarm: 0x%x\n", reg_data->page_data.pemi_snr_properties.snr_media_low_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_media_low_warning: 0x%x\n", reg_data->page_data.pemi_snr_properties.snr_media_low_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_host_low_alarm: 0x%x\n", reg_data->page_data.pemi_snr_properties.snr_host_low_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "snr_host_low_warning: 0x%x\n", reg_data->page_data.pemi_snr_properties.snr_host_low_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PEMI_PAGE_SELECT_LASER_MONITORS_PROPERTIES_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "laser_monitor_cap: 0x%x\n", reg_data->page_data.pemi_laser_monitors_properties.laser_monitor_cap);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "laser_age_high_alarm: 0x%x\n", reg_data->page_data.pemi_laser_monitors_properties.laser_age_high_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "laser_age_high_warning: 0x%x\n", reg_data->page_data.pemi_laser_monitors_properties.laser_age_high_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tec_current_high_alarm: 0x%x\n", reg_data->page_data.pemi_laser_monitors_properties.tec_current_high_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tec_current_low_alarm: 0x%x\n", reg_data->page_data.pemi_laser_monitors_properties.tec_current_low_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tec_current_high_warning: 0x%x\n", reg_data->page_data.pemi_laser_monitors_properties.tec_current_high_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tec_current_low_warning: 0x%x\n", reg_data->page_data.pemi_laser_monitors_properties.tec_current_low_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "laser_frequency_error_high_alarm: 0x%x\n", reg_data->page_data.pemi_laser_monitors_properties.laser_frequency_error_high_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "laser_frequency_error_high_warning: 0x%x\n", reg_data->page_data.pemi_laser_monitors_properties.laser_frequency_error_high_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cooled_laser_temp_high_alarm: 0x%x\n", reg_data->page_data.pemi_laser_monitors_properties.cooled_laser_temp_high_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cooled_laser_temp_low_alarm: 0x%x\n", reg_data->page_data.pemi_laser_monitors_properties.cooled_laser_temp_low_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cooled_laser_temp_high_warning: 0x%x\n", reg_data->page_data.pemi_laser_monitors_properties.cooled_laser_temp_high_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "cooled_laser_temp_low_warning: 0x%x\n", reg_data->page_data.pemi_laser_monitors_properties.cooled_laser_temp_low_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PEMI_PAGE_SELECT_PAM4_LEVEL_TRANSITION_PROPERTIES_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_cap: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_properties.pam4_level_transition_cap);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_media_low_alarm: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_properties.pam4_level_transition_media_low_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_media_low_warning: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_properties.pam4_level_transition_media_low_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_host_low_alarm: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_properties.pam4_level_transition_host_low_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pam4_level_transition_host_low_warning: 0x%x\n", reg_data->page_data.pemi_pam4_level_transition_properties.pam4_level_transition_host_low_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PEMI_PAGE_SELECT_PRE_FEC_BER_PROPERTIES_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_cap: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_properties.pre_fec_ber_cap);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_min_high_media_alarm: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_properties.pre_fec_ber_min_high_media_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_min_high_media_warning: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_properties.pre_fec_ber_min_high_media_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_min_high_host_alarm: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_properties.pre_fec_ber_min_high_host_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_min_high_host_warning: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_properties.pre_fec_ber_min_high_host_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_max_high_media_alarm: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_properties.pre_fec_ber_max_high_media_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_max_high_media_warning: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_properties.pre_fec_ber_max_high_media_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_max_high_host_alarm: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_properties.pre_fec_ber_max_high_host_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_max_high_host_warning: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_properties.pre_fec_ber_max_high_host_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_avg_high_media_alarm: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_properties.pre_fec_ber_avg_high_media_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_avg_high_media_warning: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_properties.pre_fec_ber_avg_high_media_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_avg_high_host_alarm: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_properties.pre_fec_ber_avg_high_host_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pre_fec_ber_avg_high_host_warning: 0x%x\n", reg_data->page_data.pemi_pre_fec_ber_properties.pre_fec_ber_avg_high_host_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PEMI_PAGE_SELECT_FERC_PROPERTIES_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ferc_cap: 0x%x\n", reg_data->page_data.pemi_ferc_properties.ferc_cap);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PEMI_PAGE_SELECT_TEMPERATURE_PROPERTIES_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "temp_cap: 0x%x\n", reg_data->page_data.pemi_temperature_properties.temp_cap);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "temp_high_alarm: 0x%x\n", reg_data->page_data.pemi_temperature_properties.temp_high_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "temp_low_alarm: 0x%x\n", reg_data->page_data.pemi_temperature_properties.temp_low_alarm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "temp_high_warning: 0x%x\n", reg_data->page_data.pemi_temperature_properties.temp_high_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "temp_low_warning: 0x%x\n", reg_data->page_data.pemi_temperature_properties.temp_low_warning);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
...
ssize_t sx_spice_access_reg_mtcap_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    mtcap_data_t     *data = (mtcap_data_t*)filp->f_inode->i_private;

    if (*ppos == 0) {
        if (down_interruptible(&data->dev_data->kbuffer_sem)) {
            sxd_log_info("received signal while trying to read from a file and waiting for semaphore \n");
            return -ERESTARTSYS;
        }
        filp->private_data = (void*) 0xdeadbeaf; /* indication that the semahpore was acquired by the file owner in the first read operation */
        data->dev_data->print_data_size = 0;
        data->dev_data->kbuffer = sx_spice_buffer_alloc(GFP_KERNEL);
        if (!data->dev_data->kbuffer) {
            printk(KERN_ERR "Failed to allocate kernel buffer for MTCAP SPICE file generation\n");
            return 0;
        }

        memset(data->dev_data->kbuffer, 0, sizeof(char) * KERNEL_BUFFER_SIZE);
        ...
        if (err) {
            data->dev_data->print_data_size = snprintf(data->dev_data->kbuffer, sizeof(char) * 5, "N/A\n");
            size = simple_read_from_buffer(buf, lbuf, ppos, data->dev_data->kbuffer, data->dev_data->print_data_size + 1);
            goto out;
        }

        data->dev_data->print_data_size = sx_spice_access_reg_mtcap_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }

    if (*ppos > data->dev_data->print_data_size) {
        sxd_log_debug("no more data to read \n");
        return 0;
    }

    data_left_to_read = (size_t)(data->dev_data->print_data_size - *ppos);

    /*This line should always run*/
    if (data_left_to_read < lbuf) {
        size = simple_read_from_buffer(buf,
                                       data_left_to_read,
                                       ppos,
                                       data->dev_data->kbuffer,
                                       data->dev_data->print_data_size + 1);
    } else {
        size = simple_read_from_buffer(buf,
                                       lbuf,
                                       ppos,
...
ssize_t sx_spice_access_reg_mvcap_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                  data_left_to_read;
    mvcap_data_t     *data = (mvcap_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_mvcap_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_mtmp_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                 data_left_to_read;
    mtmp_data_t     *data = (mtmp_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_mtmp_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_mvcr_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    int                    err = 0;
    ssize_t                size = 0;
    struct ku_mvcr_reg     reg_data;
    size_t                 data_left_to_read;
    mvcr_data_t     *data = (mvcr_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->kbuffer = sx_spice_buffer_alloc(GFP_KERNEL);
        if (!data->dev_data->kbuffer) {
            printk(KERN_ERR "Failed to allocate kernel buffer for MVCR SPICE file generation\n");
            return 0;
        }
        ...
        err = SX_SPICE_EMAD_ACCESS_REG(MVCR, data->dev_data, &reg_data);
        if (err) {
            data->dev_data->print_data_size = snprintf(data->dev_data->kbuffer, sizeof(char) * 5, "N/A\n");
            size = simple_read_from_buffer(buf, lbuf, ppos, data->dev_data->kbuffer, data->dev_data->print_data_size + 1);
            goto out;
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_mvcr_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_mgir_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                 data_left_to_read;
    mgir_data_t     *data = (mgir_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_mgir_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_msgi_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                 data_left_to_read;
    msgi_data_t     *data = (msgi_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_msgi_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_mdir_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                 data_left_to_read;
    mdir_data_t     *data = (mdir_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_mdir_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_spad_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                 data_left_to_read;
    spad_data_t     *data = (spad_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_spad_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_spzr_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                 data_left_to_read;
    spzr_data_t     *data = (spzr_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_spzr_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_ptys_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                 data_left_to_read;
    ptys_data_t     *data = (ptys_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_ptys_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_ppcnt_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                  data_left_to_read;
    ppcnt_data_t     *data = (ppcnt_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_ppcnt_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_pmaos_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                  data_left_to_read;
    pmaos_data_t     *data = (pmaos_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->kbuffer = sx_spice_buffer_alloc(GFP_KERNEL);
        if (!data->dev_data->kbuffer) {
            printk(KERN_ERR "Failed to allocate kernel buffer for PMAOS SPICE file generation\n");
            return 0;
        }
        ...
        memset(&reg_data, 0, sizeof(reg_data));
        reg_data.slot_index = data->slot_index;
        reg_data.module = data->module;


        err = SX_SPICE_EMAD_ACCESS_REG(PMAOS, data->dev_data, &reg_data);
        if (err) {
            data->dev_data->print_data_size = snprintf(data->dev_data->kbuffer, sizeof(char) * 5, "N/A\n");
            size = simple_read_from_buffer(buf, lbuf, ppos, data->dev_data->kbuffer, data->dev_data->print_data_size + 1);
            goto out;
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_pmaos_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_sltp_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                 data_left_to_read;
    sltp_data_t     *data = (sltp_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_sltp_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_pddr_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                 data_left_to_read;
    pddr_data_t     *data = (pddr_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_pddr_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_pnlpnr_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                   data_left_to_read;
    pnlpnr_data_t     *data = (pnlpnr_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->kbuffer = sx_spice_buffer_alloc(GFP_KERNEL);
        if (!data->dev_data->kbuffer) {
            printk(KERN_ERR "Failed to allocate kernel buffer for PNLPNR SPICE file generation\n");
            return 0;
        }
        ...
        err = SX_SPICE_EMAD_ACCESS_REG(PNLPNR, data->dev_data, &reg_data);
        if (err) {
            data->dev_data->print_data_size = snprintf(data->dev_data->kbuffer, sizeof(char) * 5, "N/A\n");
            size = simple_read_from_buffer(buf, lbuf, ppos, data->dev_data->kbuffer, data->dev_data->print_data_size + 1);
            goto out;
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_pnlpnr_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_pmdr_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                 data_left_to_read;
    pmdr_data_t     *data = (pmdr_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_pmdr_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_pphcr_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                  data_left_to_read;
    pphcr_data_t     *data = (pphcr_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_pphcr_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_sllm_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t                 data_left_to_read;
    sllm_data_t     *data = (sllm_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_sllm_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
ssize_t sx_spice_access_reg_pemi_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    int                    err = 0;
    ssize_t                size = 0;
    struct ku_pemi_reg     reg_data;
    size_t                 data_left_to_read;
    pemi_data_t     *data = (pemi_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->kbuffer = sx_spice_buffer_alloc(GFP_KERNEL);
        if (!data->dev_data->kbuffer) {
            printk(KERN_ERR "Failed to allocate kernel buffer for PEMI SPICE file generation\n");
            return 0;
        }
        ...
        err = SX_SPICE_EMAD_ACCESS_REG(PEMI, data->dev_data, &reg_data);
        if (err) {
            data->dev_data->print_data_size = snprintf(data->dev_data->kbuffer, sizeof(char) * 5, "N/A\n");
            size = simple_read_from_buffer(buf, lbuf, ppos, data->dev_data->kbuffer, data->dev_data->print_data_size + 1);
            goto out;
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_pemi_data_to_buffer_print(data->dev_data->kbuffer, sizeof(char) * KERNEL_BUFFER_SIZE, &reg_data);
    }
...
int sx_spice_access_reg_mvcr_file_create(struct dentry *parent, dev_private_data_t *dev_data)
{
    mvcr_data_t *mvcr = NULL;

    mvcr = kzalloc(sizeof(mvcr_data_t), GFP_KERNEL);
    if (!mvcr) {
        printk(KERN_ERR "Failed to allocate mvcr for SPICE\n");
        goto out;
    }

    list_add_tail(&mvcr->mvcr_list, &__mvcr_list_head);

    mvcr->dev_data = dev_data;
    mvcr->i = sx_spice_tree_index_value_get(I);
    mvcr->slot_index = sx_spice_tree_index_value_get(SLOT_INDEX);
    mvcr->sensor_index = sx_spice_tree_index_value_get(SENSOR_INDEX);


    debugfs_create_file("MVCR", 0644, parent, (void *)mvcr, &mvcr_fops);
...
int sx_spice_access_reg_ppcnt_file_create(struct dentry *parent, dev_private_data_t *dev_data)
{
    ...
    ppcnt->lp_gl = sx_spice_tree_index_value_get(LP_GL);
    ppcnt->plane_ind = sx_spice_tree_index_value_get(PLANE_IND);
...
{
    pmaos_data_t *pmaos = NULL;

    pmaos = kzalloc(sizeof(pmaos_data_t), GFP_KERNEL);
    if (!pmaos) {
        printk(KERN_ERR "Failed to allocate pmaos for SPICE\n");
        goto out;
    }

    list_add_tail(&pmaos->pmaos_list, &__pmaos_list_head);

    pmaos->dev_data = dev_data;
    pmaos->slot_index = sx_spice_tree_index_value_get(SLOT_INDEX);
    pmaos->module = sx_spice_tree_index_value_get(MODULE_INDEX);


    debugfs_create_file("PMAOS", 0644, parent, (void *)pmaos, &pmaos_fops);
...
{
    pnlpnr_data_t *pnlpnr = NULL;

    pnlpnr = kzalloc(sizeof(pnlpnr_data_t), GFP_KERNEL);
    if (!pnlpnr) {
        printk(KERN_ERR "Failed to allocate pnlpnr for SPICE\n");
        goto out;
    }

    list_add_tail(&pnlpnr->pnlpnr_list, &__pnlpnr_list_head);

    pnlpnr->dev_data = dev_data;
    pnlpnr->local_port = sx_spice_tree_index_value_get(LOCAL_PORT);


    debugfs_create_file("PNLPNR", 0644, parent, (void *)pnlpnr, &pnlpnr_fops);
...
int sx_spice_access_reg_pmdr_file_create(struct dentry *parent, dev_private_data_t *dev_data)
{
    ...
    pmdr->plane_ind = sx_spice_tree_index_value_get(PLANE_IND);
...
int sx_spice_access_reg_pphcr_file_create(struct dentry *parent, dev_private_data_t *dev_data)
{
    ...
    pphcr->port_type = sx_spice_tree_index_value_get(PORT_TYPE);
    pphcr->plane_ind = sx_spice_tree_index_value_get(PLANE_IND);
...
{
    pemi_data_t *pemi = NULL;

    pemi = kzalloc(sizeof(pemi_data_t), GFP_KERNEL);
    if (!pemi) {
        printk(KERN_ERR "Failed to allocate pemi for SPICE\n");
        goto out;
    }

    list_add_tail(&pemi->pemi_list, &__pemi_list_head);

    pemi->dev_data = dev_data;
    pemi->local_port = sx_spice_tree_index_value_get(LOCAL_PORT);
    pemi->pnat = sx_spice_tree_index_value_get(PNAT);
    pemi->page_select = sx_spice_tree_index_value_get(PAGE_SELECT);


    debugfs_create_file("PEMI", 0644, parent, (void *)pemi, &pemi_fops);
...
void sx_spice_access_reg_mvcr_list_free(void)
{
    mvcr_data_t *iter_mvcr, *tmp_mvcr;

    list_for_each_entry_safe(iter_mvcr, tmp_mvcr, &__mvcr_list_head, mvcr_list) {
        list_del(&iter_mvcr->mvcr_list);
        kfree(iter_mvcr);
    }
}
...
{
    pmaos_data_t *iter_pmaos, *tmp_pmaos;

    list_for_each_entry_safe(iter_pmaos, tmp_pmaos, &__pmaos_list_head, pmaos_list) {
        list_del(&iter_pmaos->pmaos_list);
        kfree(iter_pmaos);
    }
}
...
{
    pnlpnr_data_t *iter_pnlpnr, *tmp_pnlpnr;

    list_for_each_entry_safe(iter_pnlpnr, tmp_pnlpnr, &__pnlpnr_list_head, pnlpnr_list) {
        list_del(&iter_pnlpnr->pnlpnr_list);
        kfree(iter_pnlpnr);
    }
}
...
{
    pemi_data_t *iter_pemi, *tmp_pemi;

    list_for_each_entry_safe(iter_pemi, tmp_pemi, &__pemi_list_head, pemi_list) {
        list_del(&iter_pemi->pemi_list);
        kfree(iter_pemi);
    }
}
