...
static LIST_HEAD(__pmlp_list_head);
...
ssize_t sx_spice_access_reg_slrip_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    if (!kbuffer) {
        sxd_log_err("Failed to allocate kernel buffer for SLRIP SPICE file generation\n");
        return 0;
    }
...
int sx_spice_access_reg_slrip_file_create(struct dentry *parent, dev_private_data_t *dev_data)
{
    ...
    if (!slrip) {
        sxd_log_err("Failed to allocate slrip for SPICE\n");
        goto out;
    }
...
ssize_t sx_spice_access_reg_slrg_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    kbuffer = sx_spice_buffer_alloc(GFP_KERNEL);
    if (!kbuffer) {
        sxd_log_err("Failed to allocate kernel buffer for SLRG SPICE file generation\n");
        return 0;
    }
...
int sx_spice_access_reg_slrg_file_create(struct dentry *parent, dev_private_data_t *dev_data)
{
    ...
    if (!slrg) {
        sxd_log_err("Failed to allocate slrg for SPICE\n");
        goto out;
    }
...
ssize_t sx_spice_access_reg_slrp_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    kbuffer = sx_spice_buffer_alloc(GFP_KERNEL);
    if (!kbuffer) {
        sxd_log_err("Failed to allocate kernel buffer for SLRP SPICE file generation\n");
        return 0;
    }
...
int sx_spice_access_reg_slrp_file_create(struct dentry *parent, dev_private_data_t *dev_data)
{
    ...
    if (!slrp) {
        sxd_log_err("Failed to allocate slrp for SPICE\n");
        goto out;
    }
...
int sx_spice_access_reg_peucg_data_to_buffer_print(char *buffer, size_t buffer_length, struct ku_peucg_reg *reg_data)
{
    ...
    for (index = 0; index < PEUCG_PAGE_DATA_REC_MAX_COUNT; index++) {
        ...
        if (reg_info == NULL) {
            sxd_log_err("REG 0x%x: Register is not initialized!\n", reg_data->page_data[index].address);
        } else {
            snp_res = snprintf(buffer,
                               buffer_length - buffer_size,
                               "rxtx: 0x%x\n",
...
ssize_t sx_spice_access_reg_peucg_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    if (*ppos == 0) {
        ...
        if (!data->dev_data->kbuffer) {
            sxd_log_err("Failed to allocate kernel buffer for PEUCG SPICE file generation\n");
            return 0;
        }
        ...
        if (data->unit == 0) { /* DLN */
            for (i = 0; i < dln_tx_arr_size; i++, pd_it++) {
                if (pd_it == (dln_tx_arr_size - 1)) {
                    ...
                    data->dev_data->print_data_size += sx_spice_access_reg_peucg_data_to_buffer_print(
                        data->dev_data->kbuffer + data->dev_data->print_data_size,
                        kernel_buff_size - data->dev_data->print_data_size,
                        &reg_data);
            ...
            data->dev_data->print_data_size = sx_spice_access_reg_peucg_data_to_buffer_print(data->dev_data->kbuffer,
                                                                                             sizeof(char) * KERNEL_BUFFER_SIZE,
                                                                                             &reg_data);
        }
    }
    ...
    if (data_left_to_read < lbuf) {
        size = simple_read_from_buffer(buf,
                                       ...
                                       ppos,
                                       data->dev_data->kbuffer,
                                       data->dev_data->print_data_size);
    } else {
        size = simple_read_from_buffer(buf, lbuf, ppos, data->dev_data->kbuffer, data->dev_data->print_data_size);
    }
...
int sx_spice_access_reg_peucg_file_create(struct dentry *parent, dev_private_data_t *dev_data)
{
    ...
    if (!peucg) {
        sxd_log_err("Failed to allocate peucg for SPICE\n");
        goto out;
    }
...
ssize_t sx_spice_access_reg_ppll_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    kbuffer = sx_spice_buffer_alloc(GFP_KERNEL);
    if (!kbuffer) {
        sxd_log_err("Failed to allocate kernel buffer for PPLL SPICE file generation\n");
        return 0;
    }
...
int sx_spice_access_reg_ppll_file_create(struct dentry *parent, dev_private_data_t *dev_data)
{
    ...
    if (!ppll) {
        sxd_log_err("Failed to allocate ppll for SPICE\n");
        goto out;
    }
...
int sx_spice_access_reg_pmlp_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_pmlp_reg * reg_data)
{
    ...
    snp_res = snprintf(buffer,
                       buffer_length - buffer_size,
                       "use_different_rx_tx: 0x%x\n",
                       reg_data->use_different_rx_tx);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "autosplit: 0x%x\n", reg_data->autosplit);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    SX_PORT_BUILD_PHY_ID_FROM_LSB_MSB(local_port, reg_data->local_port, reg_data->lp_msb);
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "width: 0x%x\n", reg_data->width);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    for (i = 0; i < NUMBER_OF_SERDESES; i++) {
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rx_lane: 0x%x\n", reg_data->rx_lane[i]);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "lane: 0x%x\n", reg_data->lane[i]);
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "slot: 0x%x\n", reg_data->slot[i]);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
...
ssize_t sx_spice_access_reg_pmlp_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    size_t             data_left_to_read;
    const pmlp_data_t *data = (pmlp_data_t*)filp->f_inode->i_private;
    ...
    if (*ppos == 0) {
        ...
        data->dev_data->kbuffer = sx_spice_buffer_alloc(GFP_KERNEL);
        if (!data->dev_data->kbuffer) {
            printk(KERN_ERR "Failed to allocate kernel buffer for PMLP SPICE file generation\n");
            return 0;
        }
        ...
        err = SX_SPICE_EMAD_ACCESS_REG(PMLP, data->dev_data, &reg_data);
        if (err) {
            data->dev_data->print_data_size = snprintf(data->dev_data->kbuffer, sizeof(char) * 5, "N/A\n");
            size = simple_read_from_buffer(buf,
                                           lbuf,
        ...
        data->dev_data->print_data_size = sx_spice_access_reg_pmlp_data_to_buffer_print(data->dev_data->kbuffer,
                                                                                        sizeof(char) * KERNEL_BUFFER_SIZE,
                                                                                        &reg_data);
    }
...
static const struct file_operations pmlp_fops = {
    .read = sx_spice_access_reg_pmlp_read
};

int sx_spice_access_reg_pmlp_file_create(struct dentry *parent, dev_private_data_t *dev_data)
{
    pmlp_data_t *pmlp = NULL;

    pmlp = kzalloc(sizeof(pmlp_data_t), GFP_KERNEL);
    if (!pmlp) {
        printk(KERN_ERR "Failed to allocate pmlp for SPICE\n");
        goto out;
    }

    list_add_tail(&pmlp->pmlp_list, &__pmlp_list_head);

    pmlp->dev_data = dev_data;
    pmlp->local_port = sx_spice_tree_index_value_get(LOCAL_PORT);
    /*pmlp->plane_ind = sx_spice_tree_index_value_get(PLANE_IND); */


    debugfs_create_file("PMLP", 0644, parent, (void *)pmlp, &pmlp_fops);
...
{
    pmlp_data_t *iter_pmlp, *tmp_pmlp;

    list_for_each_entry_safe(iter_pmlp, tmp_pmlp, &__pmlp_list_head, pmlp_list) {
        list_del(&iter_pmlp->pmlp_list);
        kfree(iter_pmlp);
    }
}
