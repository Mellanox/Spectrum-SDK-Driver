...
static int __sx_spice_module_count_get(dev_private_data_t *priv_data);
...
void sx_spice_kbuffer_free(dev_private_data_t *dev_data)
{
    if (dev_data->kbuffer) {
        sx_spice_buffer_free(dev_data->kbuffer);
        dev_data->kbuffer = NULL;
        up(&dev_data->kbuffer_sem);
    }
}
...
static void * sx_spice_register(struct sx_dev *dev)
{
    ...
    sema_init(&priv_data->kbuffer_sem, 1);
    ...
    if (err) {
        sxd_log_err("SPICE: Failed to get system type\n");
        vfree(priv_data);
        return NULL;
    }
    ...
    if (err) {
        if (err == -EFAULT) {
            sxd_log_err("SPICE: SPICE isn't supported with dev_id: %d \n", priv_data->hw_device_id);
            vfree(priv_data);
            return NULL;
        }
        sxd_log_err("SPICE: Failed to create tree\n");
    }

    if (priv_data->eth_mode) {
        err = __sx_spice_bulk_cntr_init(priv_data);
        if (err) {
            sxd_log_err("SPICE: Failed to init spice_bulk_cntr\n");
        }
    }

    if ((priv_data->hw_device_id == SXD_MGIR_HW_DEV_ID_QUANTUM2) ||
        (priv_data->hw_device_id == SXD_MGIR_HW_DEV_ID_SPECTRUM4) ||
        (priv_data->hw_device_id == SXD_MGIR_HW_DEV_ID_SPECTRUM5)) {
        err = sx_spice_uphy_reg_init();
        if (err) {
            sxd_log_err("SPICE: Failed to initialize uPHY reg module\n");
        }
    }
...
static void sx_spice_unregister(struct sx_dev *dev, void *private_data)
{
    ...
    if (priv_data->is_available) {
        if (down_timeout(&priv_data->reg_access.reg_sem, msecs_to_jiffies(5000)) == -ETIME) {
            sxd_log_err("SPICE: FW is busy, cannot proceed with spice deinit process");
        }
    }
    ...
    if (priv_data->eth_mode) {
        ...
        sx_spice_asic_ports_data_list_free();
                          ...
                          union sx_event_data *event_data)
{
    ...
    case SX_DEV_EVENT_PRE_RESET:
        sxd_log_notice("sx_spice_event: Got ISSU/PRE_RESET start event. "
                       "dev = %p, dev_id = %u",
                       dev, dev->device_id);
        ...
        if (dev_data->eth_mode) {
            ...
            if (err) {
                sxd_log_err("SPICE: Failed to unregister spice_bulk_cntr "
    ...
    case SX_DEV_EVENT_POST_SET_PROFILE:
        sxd_log_notice("sx_spice_event: Got POST RESET event. "
                       "dev = %p, dev_id = %u",
                       dev, dev->device_id);
        ...
        if (dev_data->eth_mode) {
            ...
            if (err) {
                sxd_log_err("SPICE: Failed to register spice_bulk_cntr"
...
static int cache_create(void)
{
    __kbuffer_cache = kmem_cache_create_usercopy("spice_kbuffer",
                                                 ...
                                                 NULL);
    if (!__kbuffer_cache) {
        sxd_log_err("failed to allocate memory cache for spice kernel buffer\n");
        return -ENOMEM;
    }
...
static int __sx_spice_tree_app_directories_create(void)
{
    ...
    if (err) {
        sxd_log_err("SPICE: Failed to create root directory for BER application\n");
        goto out;
    }

    err = sx_spice_tree_asic_counters_app_dir_create();
    if (err) {
        sxd_log_err("SPICE: Failed to create root directory for asic_counters application\n");
        goto out;
    }
out:
    return err;
...
int __init sx_spice_dump_init(void)
{
    ...
    if (err) {
        sxd_log_err("SPICE: Failed to initialize spice tree module\n");
        goto out;
    }
    ...
    err = cache_create();
    if (err) {
        sxd_log_err("Failed to create cache for SPICE\n");
    ...
    if (err) {
        sxd_log_err("Failed to register SPICE kernel module\n");
...
static int __sx_spice_bulk_cntr_init(dev_private_data_t *dev_data)
{
    ...
    list_for_each_entry(iter, &dev_data->spice_bulk_cntr_dev_files_list, list_file_params) {
        ...
        if (err) {
            sxd_log_err("%s: Failed to initialize spice bulk counters type: %d on device: %d\n",
        ...
        sxd_log_info("%s: successfully initialized spice bulk counter type:%d of device_id: %d\n",
                     __func__, iter->key_type, iter->dev->device_id);
#endif
    }
    ...
    if (err) {
        sxd_log_err("%s: failed to set trap_id SXD_TRAP_ID_MOCS_DONE", __func__);
        goto out;
    }
...
static int __sx_spice_bulk_cntr_deinit(dev_private_data_t *dev_data)
{
    ...
    list_for_each_entry(iter, &dev_data->spice_bulk_cntr_dev_files_list, list_file_params) {
        ...
        err = sx_spice_bulk_cntr_event(&bulk_cntr_params);
        if (err) {
            sxd_log_err("%s: Failed to deinit spice bulk counters type: %d on device: %d\n",
                        __func__, iter->key_type, iter->dev->device_id);
            mutex_unlock(&iter->file_db.tr_file_op_mutex);
            goto out;
        }
...
#ifdef SPICE_BULK_CNTR_LOG_EN
        sxd_log_info("%s: successfully deinit spice bulk counter type:%d of device_id: %d\n",
                     __func__, iter->key_type, iter->dev->device_id);
#endif
    }
    ...
    err = sx_spice_bulk_cntr_listener_set(CTRL_CMD_REMOVE_SYND,
                                          ...
                                          (void *)&dev_data->spice_bulk_cntr_dev_files_list);
    if (err) {
        sxd_log_err("%s: failed to remove trap_id SXD_TRAP_ID_MOCS_DONE", __func__);
        goto out;
    }
...
static void __sx_spice_access_reg_lists_free(void)
{
    ...
    sx_spice_access_reg_pmaos_list_free();

    sx_spice_access_reg_pmlp_list_free();

    sx_spice_access_reg_pemi_list_free();
    ...
    sx_spice_access_reg_mvcr_list_free();
...
{
    sxd_log_info("finish spice-dump\n");
...
int sx_spice_system_type_get(dev_private_data_t *priv_data)
{
    ...
    if (err) {
        sxd_log_err("SPICE: Failed to get system info from MGIR register\n");
        goto out;
    }
...
static int __sx_spice_sensor_count_get(dev_private_data_t *priv_data)
{
    ...
    if (err) {
        sxd_log_err("SPICE: Failed to get system info from MLXSW_MTCAP_ID register\n");
        goto out;
    }
...
static int __sx_spice_pll_group_num_get(dev_private_data_t *priv_data)
{
    ...
    if (err) {
        sxd_log_err("SPICE: Failed to get num of pll groups from PPLL register\n");
        goto out;
    }
...
static int __sx_spice_module_count_get(dev_private_data_t *priv_data)
{
    ...
    err = sx_ACCESS_REG_MGPIR(priv_data->dev, &reg_data);
    if (err || reg_data.op_tlv.status) {
        sxd_log_err("SPICE: Fails to access register MGPIR, err: %d, status: %d.\n", err, reg_data.op_tlv.status);
        err = -EFAULT;
        goto out;
    }
    priv_data->max_module_index = reg_data.mgpir_reg.hw_info.num_of_modules - 1; /* local modules per ASIC */
...
static int __sx_spice_tree_create(dev_private_data_t *dev_priv)
{
    ...
    if (!dev_priv) {
        sxd_log_err("SPICE: in __sx_spice_tree_create dev is NULL\n");
        goto out;
    }

    err = sx_QUERY_RSRC(dev_priv->dev, &query_rsrc);
    if (err) {
        sxd_log_err("SPICE: Failed to retrieve system number of ports\n");
        goto out;
    }
    ...
    if (err) {
        sxd_log_err("SPICE: Failed to get ambient sensor count\n");
        goto out;
    }

    err = __sx_spice_pll_group_num_get(dev_priv);
    if (err) {
        sxd_log_err("SPICE: Failed to get pll group count\n");
        goto out;
    }

    err = __sx_spice_module_count_get(dev_priv);
    if (err) {
        sxd_log_err("SPICE: Failed to get ASIC module count\n");
        goto out;
    }
    ...
    case SXD_MGIR_HW_DEV_ID_QUANTUM2:
        ...
        if (err) {
            sxd_log_err("SPICE: Failed to create IB ber tree\n");
            goto out;
        }
        break;
    ...
    case SXD_MGIR_HW_DEV_ID_SPECTRUM5:
        ...
        if (err) {
            sxd_log_err("SPICE: Failed to create ETH ber tree\n");
            goto out;
        }

        err = sx_spice_tree_asic_counters_eth_create(dev_priv);
        if (err) {
            sxd_log_err("SPICE: Failed to create ETH asic_counters tree\n");
            goto out;
        }
        break;

    case SXD_MGIR_HW_DEV_ID_SPECTRUM:
        sxd_log_err("ERROR: SPICE is not supported in Spectrum.\n");
        err = -EFAULT;
        goto out;

    default:
        sxd_log_err("ERROR: Unresolved device id (%u), SPICE is not supported.\n", dev_priv->hw_device_id);
        return -EFAULT;
    }
